<?php
	...
	if ($p["cmdAddCategory"]!="" ) {
		...
		if ($name!="") {
			...
			$categoryRecIns = $ingredientCategoryObj->addIngredientCategory($name, $descr, $ingMainCategory);

			if ($categoryRecIns) {
				$sessObj->createSession("displayMsg",$msg_succAddIngredientCategory);
				$sessObj->createSession("nextPage",$url_afterAddIngredientCategory.$selection);
			} else {
				$addMode	=	true;
				$err		=	$msg_failAddIngredientCategory;
			}
	...
	if ($p["editId"]!="" ) {
		$editId			=	$p["editId"];
		$editMode		=	true;
		$categoryRec		=	$ingredientCategoryObj->find($editId);
	...
	if ($p["cmdSaveChange"]!="" ) {
		...
		if ($categoryId!="" && $name!="" && $ingMainCategory!="") {
			$categoryRecUptd = $ingredientCategoryObj->updateCategory($categoryId, $name, $descr, $ingMainCategory);
		}
	
		if ($categoryRecUptd) {
			$sessObj->createSession("displayMsg",$msg_succIngredientCategoryUpdate);
			$sessObj->createSession("nextPage",$url_afterUpdateIngredientCategory.$selection);
		} else {
			$editMode	=	true;
			$err		=	$msg_failIngredientCategoryUpdate;
		}
	...
	if ( $p["cmdDelete"]!="") {
		...
		for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($categoryId!="") {
				...
				$moreEntriesExist = $ingredientCategoryObj->checkMoreEntriesExist($categoryId);
				
				if (!$moreEntriesExist) $categoryRecDel = $ingredientCategoryObj->deleteCategory($categoryId);
			}
		}
		if ($categoryRecDel) {
			$sessObj->createSession("displayMsg", $msg_succDelIngredientCategory);
			$sessObj->createSession("nextPage", $url_afterDelIngredientCategory.$selection);
		} else {
			$errDel	=	$msg_failDelIngredientCategory;
		}
	...
	if ($p["btnConfirm"]!="")
	{
		...
		for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($categoryId!="") {
				...
				$categoryRecConfirm = $ingredientCategoryObj->updateCategoryconfirm($categoryId);
			}
...
for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($categoryId!="") {
				#Check any entries exist
					...
					$categoryRecConfirm = $ingredientCategoryObj->updateCategoryReleaseconfirm($categoryId);
				
			}
		}
		if ($categoryPowerRecConfirm) {
	...
	# List all Category ;	
	$categoryRecords	= $ingredientCategoryObj->fetchAllPagingRecords($offset, $limit, $categoryFilterId);
	$categorySize		= sizeof($categoryRecords);

	## -------------- Pagination Settings II -------------------
	$numrows	=  sizeof($ingredientCategoryObj->fetchAllRecords($categoryFilterId));
	...
	# Include JS
	$ON_LOAD_PRINT_JS	= "libjs/IngredientCategory.js";

	if ($editMode)	$heading = $label_editIngredientCategory;
	else 		$heading = $label_addIngredientCategory;
...
?>
	<form name="frmIngredientCategory" action="IngredientCategory.php" method="post">
	<table cellspacing="0"  align="center" cellpadding="0" width="96%" >
		<tr><TD height="10"></TD></tr>
		<!--<tr><td height="10" align="center"><a href="IngredientsMaster.php" class="link1"> Ingredient Master</a></td></tr>-->
		<? if($err!="" ){?>
		<tr>
									...
									<td colspan="2" >
										<table cellpadding="0"  width="65%" cellspacing="0" border="0" align="center">
											...
											<tr>
												...
												<input type="submit" name="cmdDelCancel" class="button" value=" Cancel " onClick="return cancel('IngredientCategory.php');">&nbsp;&nbsp;
												<input type="submit" name="cmdSaveChange" class="button" value=" Save Changes " onClick="return validateIngredientCategory(document.frmIngredientCategory);">												</td>
												
												<?} else{?>

												
												<td  colspan="2" align="center">
												<input type="submit" name="cmdAddCancel" class="button" value=" Cancel " onClick="return cancel('IngredientCategory.php');">&nbsp;&nbsp;
												<input type="submit" name="cmdAddCategory" class="button" value=" Add " onClick="return validateIngredientCategory(document.frmIngredientCategory);">												</td>
...
<tr>
			...
			<select name="ingMainCategory">
				...
				?>
											...
											<tr>
												...
												<td colspan="2" align="center">
												<input type="submit" name="cmdCancel" class="button" value=" Cancel " onClick="return cancel('IngredientCategory.php');">&nbsp;&nbsp;
												<input type="submit" name="cmdSaveChange" class="button" value=" Save Changes " onClick="return validateIngredientCategory(document.frmIngredientCategory);">												</td>
					...
					<tr>
						<td>
							<table cellpadding="0"  width="100%" cellspacing="0" border="0" align="center">
								<tr>
									<td width="1" background="images/heading_bg.gif" class="page_hint"></td>
									<td background="images/heading_bg.gif" class="pageName" >&nbsp;Ingredient Sub-Category </td>
	<td background="images/heading_bg.gif" align="right" nowrap="nowrap">
		...
		for ($page=1; $page<=$maxpage; $page++) {
			if ($page==$pageNo) {
      				...
      				$nav.= " <a href=\"IngredientCategory.php?pageNo=$page&categoryFilter=$categoryFilterId\" class=\"link1\">$page</a> ";
				//echo $nav;
   			}
		}
		if ($pageNo > 1) {
   			$page  = $pageNo - 1;
   			$prev  = " <a href=\"IngredientCategory.php?pageNo=$page&categoryFilter=$categoryFilterId\"  class=\"link1\"><<</a> ";
	 	} else {
		...
		if ($pageNo < $maxpage) {
   			$page = $pageNo + 1;
   			$next = " <a href=\"IngredientCategory.php?pageNo=$page&categoryFilter=$categoryFilterId\"  class=\"link1\">>></a> ";
	 	} else {
	...
	<tr align="center">
		...
		foreach ($categoryRecords as $cr) {
			...
			$ingMainCategoryRec	=	$ingMainCategoryObj->find($ingredientMainCatId);
		...
		<? if($edit==true){?><td class="listing-item" width="60" align="center"><?php if ($active==0){ ?><input type="submit" value=" Edit " name="cmdEdit" onClick="assignValue(this.form,<?=$categoryId;?>,'editId'); this.form.action='IngredientCategory.php';"><? } ?></td>
		<? }?>
...
?>
	...
	//ensureInFrameset(document.frmIngredientCategory);
	//-->
	</script>
<?php 
	}
