...
function getPrice($selProcessCodeId, $selFreezingStageId, $selFrozenCodeId,$selMCPackingId,$gradeId,$fromDate,$tillDate)
	{
		$qry = "select tdfpa.price_per_kg from t_purchaseorder_rm_entry tdfpo left join t_invoice_rm_entry tdfpa on tdfpo.id=tdfpa.po_entry_id left join t_invoice_main tim on tdfpa.main_id=tim.id where tdfpo.processcode_id='$selProcessCodeId' and tdfpo.freezingstage_id='$selFreezingStageId' and tdfpo.frozencode_id='$selFrozenCodeId' and tdfpo.mcpacking_id='$selMCPackingId' and
		tdfpo.grade_id='$gradeId' and  tim.confirmed='Y' order by tim.invoice_date desc limit 0,1";
		//echo $qry;
		//echo "<br>";
		$result	= $this->databaseConnect->getRecords($qry);
...
# get Grade Recs
	function getProductionGradeRecs($fromDate, $tillDate)
	{
		$qry = "select tdfpg.grade_id, mg.code  from 
			...
			where tdfpm.select_date>='".$fromDate."' and tdfpm.select_date<='".$tillDate."' and tdfpg.number_mc!=0 and tdfpg.number_mc is not null group by tdfpg.grade_id order by mg.code asc ";	
