...
{  
	...
	*****************************************************************/
	...
	function addPurchaseOrderRawEntries($lastInsId, $selFish, $selProcessCode, $selEuCode, $selBrand, $selGrade, $selFreezingStage, $selFrozenCode, $selMCPacking, $numMC, $pricePerKg, $valueInUSD, $valueInINR, $brandFrom, $wtType)
	{
		...
		$qry = "insert into t_purchaseorder_rm_entry (main_id, fish_id, processcode_id,brand_id, grade_id, freezingstage_id, frozencode_id, mcpacking_id, number_mc, priceperkg, value_usd, value_inr, brand_from, wt_type,created_on) values('$lastInsId', '$selFish', '$selProcessCode','$selBrand', '$selGrade', '$selFreezingStage', '$selFrozenCode', '$selMCPacking', '$numMC', '$pricePerKg', '$valueInUSD', '$valueInINR', '$brandFrom', '$wtType',now())";
		//echo $qry."<br>";			
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function fetchAllPOItem($poMainId)
	{
		...
		$qry = "select id, fish_id, processcode_id,brand_id, grade_id, freezingstage_id, frozencode_id, mcpacking_id, number_mc, priceperkg, value_usd, value_inr, brand_from, wt_type from t_purchaseorder_rm_entry where main_id='$poMainId' ";
		//echo "--$qry--";
		$result	= $this->databaseConnect->getRecords($qry);
		return $result;
	}
	...
	# Update Raw Entries
	function updatePORawEntries($poEntryId, $selFish, $selProcessCode, $selEuCode, $selBrand, $selGrade, $selFreezingStage, $selFrozenCode, $selMCPacking, $numMC, $pricePerKg, $valueInUSD, $valueInINR, $brandFrom)
	{		
...
# Check Frozen Packing MC Exist
	...
	function genQEL($poMainId, $userId, $lastDate)
	{
		...
		/*$qry = "select a.customer_id, b.eucode_id, b.brand_id, b.brand_from, b.freezingstage_id, b.frozencode_id, b.mcpacking_id, mc.customer_name, mfp.code as frznCode, mfs.rm_stage as frznStage, mmcp.code as mcPkgCode 
			...
			$qry = "select a.customer_id, tm.eucode, b.brand_id, b.brand_from, b.freezingstage_id, b.frozencode_id, b.mcpacking_id, mc.customer_name, mfp.code as frznCode, mfs.rm_stage as frznStage, mmcp.code as mcPkgCode 
			from 
				t_purchaseorder_main a join t_purchaseorder_rm_entry b on a.id=b.main_id 
				left join m_customer mc on mc.id=a.customer_id 
				left join m_frozenpacking mfp on mfp.id=b.frozencode_id 
...
//echo "<br>$qry<br>";
		...
		foreach ($purchaseOrderRecs as $por) {
			...
			$insQELMainRec = $this->addFznPkngQuickEntryList($qelName, $freezingStageId, $selQuality, $eucodeId, $brandId, $customerId, $frozenCodeId, $mcPackingId, $frozenLotId, $exportLotId, $userId, $brandFrom, $lastDate);
			if ($insQELMainRec) {
				$qelUpdated = true;
	...
	# Insert QEL MainRec
	function addFznPkngQuickEntryList($qeName, $freezingStage, $selQuality, $eUCode, $brand, $selCustomerId, $frozenCode, $mCPacking, $frozenLotId, $exportLotId, $userId, $brandFrom, $shipmentLastDate)
	{
...
function getPendingOrders($processCodeId, $freezingStageId, $frozenCodeId, $MCPkgId)
	{
		// Original
	...
	//$qry =	"select pom.id, pom.po_no, pom.po_date, CONCAT(pom.po_no,' (',DATE_FORMAT(pom.po_date,'%d/%m/%Y'),')') as PODate from t_purchaseorder_main pom JOIN t_purchaseorder_rm_entry pore ON pom.id=pore.main_id where (pom.complete <>'C' or pom.complete is null) and pore.processcode_id='$processCodeId' and freezingstage_id='$freezingStageId' and frozencode_id='$frozenCodeId' and mcpacking_id='$MCPkgId' and pore.delivered_status!=1  group by pom.id order by pom.po_date asc";
	$qry =	"select pom.id, pom.po_no, pom.po_date, CONCAT(pom.po_no,' (',DATE_FORMAT(pom.po_date,'%d/%m/%Y'),')') as PODate from t_purchaseorder_main pom JOIN t_purchaseorder_rm_entry pore ON pom.id=pore.main_id where (pom.complete <>'C' or pom.complete is null) and pore.processcode_id='$processCodeId' and freezingstage_id='$freezingStageId' and frozencode_id='$frozenCodeId' and mcpacking_id='$MCPkgId' and pore.delivered_status is null  group by pom.id order by pom.po_date asc";
	...
	{
		$qry = "SELECT tdar.id from t_purchaseorder_rm_entry tpore left join t_dailyfrozenpacking_allocate_rmlotid  tdar on tpore.id=tdar.po_rm_id where tpore.main_id='$id' union all SELECT tdar.id from t_purchaseorder_rm_entry tpore left join t_dailyfrozenpacking_allocate  tdar on tpore.id=tdar.po_rm_id where tpore.main_id='$id'";
		//echo $qry;
		$result	=	array();
		$result	=	$this->databaseConnect->getRecords($qry);
