...
//$xajax->configure('statusMessages', true); // For display status
	...
	function displayFrozenRate($processcodeId,$fishId,$freezingStage,$frozenCode,$selQuality,$rowId,$rateTag,$editGrade)
	{	//$result="";
		$objResponse 			= new xajaxResponse();
		...
		$gradeRecs = $frznPkgRatingObj->getGradeRate($processcodeId,$fishId,$freezingStage,$frozenCode,$selQuality,$rowId,$rateTag,$editGrade);
		if(sizeof($gradeRecs)>0)
		{
			$objResponse->assign("dialog", "innerHTML", $gradeRecs);
		}
