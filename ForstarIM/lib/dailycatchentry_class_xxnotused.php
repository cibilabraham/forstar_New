...
class DailyCatchEntry
{
	...
	*/
	...
	function addTempRecDailyCatchEntry($lastId)
	{
		...
		//$closingTime = date("Y-m-d H:i:s", strtotime($closingTime) + (60*5));
		$qry	= "insert into t_dailycatchentry (main_id,select_date,entry_time) values('$lastId',Now(),'$closingTime ')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
	...
	#Insert One Blan Record in t_dailycatch_Entry Table
	function addTempRecDailyCatchEntryold($lastId)
	{
   ...
   // function addDailyCatch($fish, $processCode, $ice, $count,$countAverage, $entryLocal, $entryWastage, $entrySoft, $reasonAdjust, $entryAdjust, $goodPack, $peeling, $entryRemark, $entryActualWt, $entryEffectiveWt, $entryTotalGrossWt, $entryTotalBasketWt,$entryGrossNetWt, $declWeight, $declCount, $gradeId, $basketWt, $reasonLocal, $reasonWastage, $reasonSoft, $entryOption, $selectDate, $entryId,$catchEntryNewId, $gradeCountAdj, $gradeCountAdjReason, $receivedBy, $noBilling, $rm_lot_id,$supplyDetails,$make_payment,$payment,$count_code)
	...
	{
		$closingTime=date("Y-m-d H:i:s");
		$qry	= "update t_dailycatchentry set fish='$fish', fish_code='$processCode', ice_wt='$ice', count_values='$count', average='$countAverage', local_quantity='$entryLocal', wastage='$entryWastage', soft='$entrySoft', reason='$reasonAdjust', adjust='$entryAdjust', good='$goodPack', peeling='$peeling', remarks='$entryRemark', actual_wt='$entryActualWt', effective_wt='$entryEffectiveWt', gross='$entryTotalGrossWt', total_basket='$entryTotalBasketWt', net_wt='$entryGrossNetWt', decl_wt='$declWeight', decl_count='$declCount', select_date='$selectDate', grade_id='$gradeId', basket_wt='$basketWt', reason_local='$reasonLocal', reason_wastage='$reasonWastage', reason_soft='$reasonSoft', entry_option='$entryOption', grade_count_adj='$gradeCountAdj', grade_count_adj_reason='$gradeCountAdjReason', received_by='$receivedBy', no_billing='$noBilling' , weightment_id_available='$weightId' ,entry_time='$closingTime' where id='$catchEntryNewId' and main_id='$entryId'  ";
		 //echo $qry;
		 //die;
		$result	=	$this->databaseConnect->updateRecord($qry);
	...
	function updateCurrentDailyCatch($unit, $landingCenter, $mainSupplier, $vechicleNo,$supplierGroup,$pondName, $weighChallanNo, $selectDate, $selectTime, $catchEntryId, $paymentBy, $subSupplier, $supplyChallanNo, $billingCompany, $alphaCode,$rm_lot_id,$supplyDetails,$make_payment,$payment,$count_code,$lotIdAvailable)

	{
		...
		$qry	= " update t_dailycatch_main set unit='$unit', entry_date=Now(), vechile_no='$vechicleNo',supplier_group_name='$supplierGroup',pond_name='$pondName', weighment_challan_no='$weighChallanNo', landing_center='$landingCenter', main_supplier='$mainSupplier', select_date='$selectDate', select_time='$selectTime', flag='1', payment_by='$paymentBy', confirm=0, payment_confirm='N', report_confirm='N', sub_supplier='$subSupplier', supplier_challan_no='$supplyChallanNo', billing_company_id='$billingCompany', alpha_code='$alphaCode',rm_lot_id='$rm_lot_id',supplyDetails='$supplyDetails',make_payment='$make_payment',payment='$payment',count_code='$count_code',rm_id_available='$lotIdAvailable' where id='$catchEntryId' ";
		//echo $qry;
		//die;
		$result	= $this->databaseConnect->updateRecord($qry);		
		if ($result)	$this->databaseConnect->commit();
	...
	function filterCatchEntryPagingRecords($fromDate, $tillDate, $selRecord, $offset, $limit, $supplierId, $selFish, $selProcesscode, $fBillingCompany) 
	{
		...
		if ($supplierId=="") $whr .= "";
		else $whr .= " and (a.main_supplier=$supplierId or a.payment=$supplierId)";
	...
	//	$qry		= "select a.id, a.weighment_challan_no, a.entry_date, a.flag, b.count_values, b.grade_id, b.id, b.fish_code, b.effective_wt, a.confirm, b.received_by, b.grade_count_adj, b.grade_count_adj_reason, a.main_supplier, a.select_date, a.payment_confirm, b.paid, b.average, a.alpha_code,a.payment from t_dailycatch_main a left join t_dailycatchentry b on a.id=b.main_id";
		...
		if ($whr!="") $qry   .=" where $whr and (b.fish_code is not null or b.fish_code>0) and (b.fish is not null or b.fish>0)";
	...
	function getSupplierList($fromDate, $tillDate, $billingCompanyId)
	{
		...
		$qry1= "select distinct a.main_supplier, b.id, b.name as supplierName from t_dailycatch_main a, supplier b  where a.main_supplier=b.id and a.select_date>='".$fromDate."' and a.select_date<='".$tillDate."'";

		$qry2= "select distinct a.payment, b.id, b.name as supplierName from t_dailycatch_main a, supplier b  where a.payment=b.id and a.select_date>='".$fromDate."' and a.select_date<='".$tillDate."'";

		
		if ($whr!="") 		$qry1.=$whr; $qry2.=$whr;
		$qry=$qry1." union ".$qry2;
		if ($orderBy!="")	$qry	.= " order by ".$orderBy;
		//echo "<br>$qry<br>";		
		$result	=	$this->databaseConnect->getRecords($qry);
	...
	function getSupplierList_old($fromDate, $tillDate, $billingCompanyId)
	{
		$Date1			=	explode("/",$fromDate);
		$fromDate		=	$Date1[2]."-".$Date1[1]."-".$Date1[0];
	...
	function getFishList($fromDate, $tillDate, $selectSupplier, $billingCompanyId)
	{
		...
		if 	($selectSupplier==0) $whr .="";
		else	$whr .=" and (a.main_supplier='$selectSupplier' or a.payment='$selectSupplier')";

		if ($billingCompanyId!="") $whr .= " and a.billing_company_id='$billingCompanyId' ";
		
		$orderBy	= "c.name asc";
	
		$qry = "select distinct b.fish,c.name from t_dailycatch_main a, t_dailycatchentry b, m_fish c ";
	...
	function getProcessCodeList($fromDate, $tillDate, $selectSupplier, $fishId, $billingCompanyId)
	{
		...
		$qry = "select distinct b.fish_code,c.code from t_dailycatch_main a, t_dailycatchentry b, m_processcode c ";	
		if ($whr!="") $qry .= " where ".$whr;		
		if ($orderBy!="") $qry .= " order by ".$orderBy;			
		//echo $qry;		
		$result	=	$this->databaseConnect->getRecords($qry);
	...
	{
		$closingTime=date("Y-m-d H:i:s");
		$qry = "update t_dailycatchentry set adjust='$entryAdjust', reason='$reasonAdjust', grade_count_adj='$gradeCountAdj', grade_count_adj_reason='$gradeCountAdjReason', actual_wt='$entryActualWt', local_quantity='$entryLocal', reason_local='$reasonLocal', wastage='$entryWastage', reason_wastage='$reasonWastage', soft='$entrySoft', reason_soft='$reasonSoft', modified=NOW(), modified_by='$currentUserId',entry_time='$closingTime' where id='$challanEntryId' and main_id='$challanMainId'";
		//echo $qry;
		$result	=	$this->databaseConnect->updateRecord($qry);		
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	{
		$qry	= "select start_no, end_no from number_gen where billing_company_id='$billingCompany' and date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate' or (end_date is null || end_date=0)) and start_no<='$challanNum' and end_no>='$challanNum' and type='RM'  and unitid='0'";
		if($unitId!="") $qry.=" or  billing_company_id='$billingCompany' and date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate' or (end_date is null || end_date=0)) and start_no<='$challanNum' and end_no>='$challanNum' and type='RM'  and unitid='$unitId'";
		//echo $qry;
		...
		if(sizeof($results) > 0)
		{
			...
			foreach($results as $res)
			{
				if($res[6]== 'procurement')
				{
					...
					e.payment_by,f.payment_by,g.name,h.name,i.name,j.name,b.pond_id ,'procurement' as value,a.pond_id,b.landing_center_id
					from `t_rmreceiptgatepass` a 
				...
				else
				{
					...
					a.supplier_Challan_No,a.supplier_id,e.name,b.company_id,b.unit_id,d.vehicle_number,
					b.supplier_id,f.name,b.challan_no,CONCAT(k.alpha_character,k.rm_lotid) as lot_Id,
					e.payment_by,f.payment_by,g.name,h.name,i.name,j.name,b.pond_id,'receipt' as value,a.pond_id,a.landingcenter_id  
					from `t_rmreceiptgatepass` a 
					left join t_rm_receipt_gatepass_supplier b on a.id = b.receipt_gatepass_id 
					left join t_manage_rmlotid_details c on b.id = c.receipt_id 
					left join t_manage_rm_lotid k on k.id = c.rmlot_main_id 
	...
	function getQualityDetails($supplier_name,$pond_name,$rm_lot_id,$weightmentId)
	{
		$qry = "";
		if($weightmentId!="")
		{
			$qry = "select id,name,billing_include from m_quality where id in
				...
				and id=$weightmentId) and id not in(5,6,7)";
		}
	...
	{
		$qry	=	"delete from t_qualityentry where id=$id";
		$result	=	$this->databaseConnect->delRecord($qry);
			if ($result) $this->databaseConnect->commit();
			else $this->databaseConnect->rollback();
			return $result;
	...
	{
		$qry	=	"SELECT b.id, b.supplier_name,b.pond_name,b.product_species,b.process_code_id,b.quality_id,b.count_code,b.weight,b.soft_per,b.soft_weight,c.name,d.pond_name,e.name,f.code,concat(g.alpha_character,g.rm_lotid) as rmlotid,h.id,h.name,i.id,i.name,c.payment_by,b.procurement_center,lm.name FROM weighment_data_sheet a left join  t_weightment_data_entries b  on a.id= b.weightment_data_sheet_id left join supplier c on c.id=b.supplier_name left join m_pond_master d on d.id=b.pond_name  left join m_fish e on e.id=b.product_species left join m_processcode f on f.id=b.process_code_id left join t_manage_rm_lotid g on g.id=a.rm_lot_id left join m_plant h on h.id=g.unit_id left join m_billing_company i on i.id=g.company_id  left join m_landingcenter lm on lm.id=b.procurement_center
		...
		if(sizeof($result) > 0)
		{
			...
			$returnVal.='<tr height="30" align="center"><td class="listing-head" colspan="8" > '.$result[0][14].' - Suppliers  </td></tr>';
			...
			$returnVal.='<td class="listing-head" style="padding:5px; ">Landing Center</td>';
			...
			{ 
				/*if($i!=0)
				{
					...
					$landingCenterId=$res[20];
					$landingCenter=$res[21];
				$returnVal.='<tr height="30" align="center">';	
					$returnVal.= '<td  class="listing-item" style="padding:5px; "><a class="supplier_name_'.$supplierId.'" href="javascript:void(0);" onclick="assignSupplierDetailsWeightment('.$weightmentId.','.$supplierId.','.$pondId.','.$unitId.','.$companyId.','.$supplierChallanNo.','.$makePayment.','.$speciesId.','.$processcodeId.','.$countcode.','.$landingCenterId.');" style="color:#800000; text-decoration:none;">'.$supplierName.'</a></td>';
					
					$returnVal.= '<td class="listing-item" style="padding:5px; "><a class="supplier_name_'.$supplierId.'" href="javascript:void(0);" onclick="assignSupplierDetailsWeightment('.$weightmentId.','.$supplierId.','.$pondId.','.$unitId.','.$companyId.','.$supplierChallanNo.','.$makePayment.','.$speciesId.','.$processcodeId.','.$countcode.','.$landingCenterId.');" style="color:#800000; text-decoration:none;">'.$pondNm.'</a></td>';
					
					$returnVal.= '<td class="listing-item" style="padding:5px; "><a class="supplier_name_'.$supplierId.'" href="javascript:void(0);" onclick="assignSupplierDetailsWeightment('.$weightmentId.','.$supplierId.','.$pondId.','.$unitId.','.$companyId.','.$supplierChallanNo.','.$makePayment.','.$speciesId.','.$processcodeId.','.$countcode.','.$landingCenterId.');" style="color:#800000; text-decoration:none;">'.$landingCenter.'</a></td>';


					$returnVal.= '<td class="listing-item" style="padding:5px; "><a class="supplier_name_'.$supplierId.'" href="javascript:void(0);" onclick="assignSupplierDetailsWeightment('.$weightmentId.','.$supplierId.','.$pondId.','.$unitId.','.$companyId.','.$supplierChallanNo.','.$makePayment.','.$speciesId.','.$processcodeId.','.$countcode.','.$landingCenterId.');" style="color:#800000; text-decoration:none;">'.$unitName.'</a></td>';
					
					 $returnVal.= '<td class="listing-item" style="padding:5px; "><a class="supplier_name_'.$supplierId.'" href="javascript:void(0);" onclick="assignSupplierDetailsWeightment('.$weightmentId.','.$supplierId.','.$pondId.','.$unitId.','.$companyId.','.$supplierChallanNo.','.$makePayment.','.$speciesId.','.$processcodeId.','.$countcode.','.$landingCenterId.');" style="color:#800000; text-decoration:none;">'.$companyNm.'</a></td>';
					 
					 $returnVal.= '<td class="listing-item" style="padding:5px; "><a class="supplier_name_'.$supplierId.'" href="javascript:void(0);" onclick="assignSupplierDetailsWeightment('.$weightmentId.','.$supplierId.','.$pondId.','.$unitId.','.$companyId.','.$supplierChallanNo.','.$makePayment.','.$speciesId.','.$processcodeId.','.$countcode.','.$landingCenterId.');" style="color:#800000; text-decoration:none;">'.$species.'</a></td>';
					 
					 $returnVal.= '<td class="listing-item" style="padding:5px; "><a class="supplier_name_'.$supplierId.'" href="javascript:void(0);" onclick="assignSupplierDetailsWeightment('.$weightmentId.','.$supplierId.','.$pondId.','.$unitId.','.$companyId.','.$supplierChallanNo.','.$makePayment.','.$speciesId.','.$processcodeId.','.$countcode.','.$landingCenterId.');" style="color:#800000; text-decoration:none;">'.$processcode.'</a></td>';
					 
					$returnVal.= '<td class="listing-item" style="padding:5px; "><a class="supplier_name_'.$supplierId.'" href="javascript:void(0);" onclick="assignSupplierDetailsWeightment('.$weightmentId.','.$supplierId.','.$pondId.','.$unitId.','.$companyId.','.$supplierChallanNo.','.$makePayment.','.$speciesId.','.$processcodeId.','.$countcode.','.$landingCenterId.');" style="color:#800000; text-decoration:none;">'.$countcode.'</a></td>';
	...
	{
		$date=date("Y-m-d");
		$qry	=	"select alpha_code from number_gen where '$date' between start_date and end_date and billing_company_id='$companyId' AND challan_status!='1' and type='RM'  and unitid='0'";
		if ($unitId!="") $qry .= "or '$date' between start_date and end_date and billing_company_id='$companyId' AND challan_status!='1' and type='RM'  and unitid=$unitId";
	...
	function getChallanDetails($weighChallanNo,$chkUnit,$chkBillingCompany)
	{
		$qry="select id  from t_dailycatch_main  where weighment_challan_no='$challanNo' and unit='$chkUnit' and billing_company_id='$chkBillingCompany'";
		//echo $qry;
		///die();
		$result=$this->databaseConnect->getRecord($qry);
		return (sizeof($result)>0)? $result[0]:"";
	...
	function compareChallan($challanNo,$chkUnit,$chkBillingCompany,$chkLandingCenter,$chkSupplier)
	{
		$qry="select id  from t_dailycatch_main  where  weighment_challan_no='$challanNo' and unit='$chkUnit' and billing_company_id='$chkBillingCompany' and landing_center='$chkLandingCenter' and main_supplier='$chkSupplier' or weighment_challan_no='$challanNo' and unit='$chkUnit' and billing_company_id='$chkBillingCompany' and landing_center='$chkLandingCenter' and payment='$chkSupplier'";
		//echo $qry;
		$result= $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)? true:false;
	}

	function delEntryRow($catchEntryNewId)
	{
		$qry	=	" delete from t_dailycatchentry where id=$catchEntryNewId";
		$result	=	$this->databaseConnect->delRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
		return $result;
	...
	function timeupdate($mainid,$entryid)
	{
		$qry="select entry_time  from  t_dailycatchentry  where id='$entryid' and main_id='$mainid'";
		$res=$this->databaseConnect->getRecord($qry);
		$closingTime=$res[0];
		$newclosingTime = date("Y-m-d H:i:s", strtotime($closingTime) + (60*5));
		$up	=	"update t_dailycatchentry set entry_time= '".$newclosingTime."'
					 where id = '".$entryid."' ";
		 //echo $up;die;
		$updateStatus	=	$this->databaseConnect->updateRecord($up);
	...
	### delete the record whose fish and processcode is equal to zero or less than zero or equal to null
	function ininclude()
	{
		$qry="select id,main_id,entry_time from t_dailycatchentry where (fish_code<='0' or fish_code is null) and (fish<='0' or fish is null)";
		//echo $qry;
		...
		if(sizeof($res)>0)
		{
			...
			{	//echo "hhjh";
				$dt=$rs[2];
				$dailyCatchEntryId=$rs[0];
				$catchMainId=$rs[1];
				list($urlFnId, $urlModuleId, $urlSubModuleId) = $this->getFunctionIds('DailyCatchEntry_New.php');	
				$rfrshTimeLimit = $this->getRefreshTimeLimit($urlSubModuleId, $urlFnId);
				$refreshTimeLimit = ($rfrshTimeLimit!=0)?$rfrshTimeLimit:60;
				$newclosingTime = date("Y-m-d H:i:s", strtotime($dt) + ($refreshTimeLimit));
				//echo "hii".$dateTime. "---".$newclosingTime;
				//$newclosingTime = date("Y-m-d H:i:s", strtotime($dt) + (60*10));
				if($dateTime>$newclosingTime)
				{ //echo "hii";
					$dailyCatchEntryRecDel=	$this->deleteDailyCatchEntryQualityWt($dailyCatchEntryId);
				
						#$dailyCatchEntryGrossRecDel		
						$dailyCatchEntryRecDel=	$this->deleteDailyCatchEntryGrossWt($dailyCatchEntryId);	
				
						#$delLastInsertRec			
						$dailyCatchEntryRecDel		=	$this->delLastInsertId($dailyCatchEntryId);
						
						#Check Record Exist
						$exisitingRecords = $this->checkRecordsExist($catchMainId);
						if (sizeof($exisitingRecords)==0) {
							$dailyCatchEntryRecDel = $this->delEntryMainId($catchMainId);
						}
				}
			}
		}
	...
	###check rm lot id available for this record in daily catch main
	function checkRmLotIdAvailable($id)
	{
		$qry="select rm_lot_id,weighment_challan_no from   t_dailycatch_main  where id='$id' and rm_id_available='1'";
		//echo $qry;
		$result=$this->databaseConnect->getRecord($qry);
		//echo sizeof($result);
		return $result;
	...
	function chkChallanNo($supplierChallanNo,$supplier)
	{
		
		//$res=false;
		$res=false;
		$qry="select id as mainid,select_date as seldate from t_dailycatch_main where supplier_challan_no='$supplierChallanNo' and main_supplier='$supplier' or payment='$supplier' and supplier_challan_no='$supplierChallanNo' union all select c.id as mainid,c.supplier_challan_date as seldate from t_dailycatch_main a join t_dailycatchentry b on a.id=b.main_id join t_dailycatch_declared c on b.id=c.entry_id where (a.main_supplier='$supplier' and c.sub_supplier='0' and c.supplier_challan_no='$supplierChallanNo') or (a.payment='$supplier' and c.sub_supplier='0' and c.supplier_challan_no='$supplierChallanNo' ) or (c.sub_supplier='$supplier' and c.supplier_challan_no ='$supplierChallanNo')";
		
		//$qry="select id,select_date from t_dailycatch_main where supplier_challan_no='$supplierChallanNo' and main_supplier='$supplier' or payment='$supplier'  and supplier_challan_no='$supplierChallanNo' union all select c.id,c.supplier_challan_date from t_dailycatch_main a  join t_dailycatchentry b on a.id=b.main_id  join t_dailycatch_declared c on b.id=c.entry_id where (a.main_supplier='$supplier' and c.sub_supplier='0' and c.supplier_challan_no='$supplierChallanNo') or (a.payment='$supplier' and c.sub_supplier='0' and c.supplier_challan_no='$supplierChallanNo' ) or (c.sub_supplier='$supplier' and c.supplier_challan_no='$supplierChallanNo')";
		//$qry="select id,select_date from t_dailycatch_main where supplier_challan_no='$supplierChallanNo' and main_supplier='$supplier' or payment='$supplier'  and supplier_challan_no='$supplierChallanNo' union all select id,supplier_challan_date from t_dailycatch_declared where supplier_challan_no='$supplierChallanNo' and sub_supplier='$supplier'";
		//$qry="select id,select_date from t_dailycatch_main where supplier_challan_no='$supplierChallanNo' and main_supplier='$supplier' or payment='$supplier'  and supplier_challan_no='$supplierChallanNo'";
		...
		if(sizeof($result)>0)
		{
			foreach($result as $dc)
			{
				...
				if($challandt!="")
				{
					$datetime1 = new DateTime($challandt);
