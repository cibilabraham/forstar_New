...
Class Soaking
{
	...
	*****************************************************************/
	...
	{
		$qry	= "select a.id,a.date,a.rm_lot_id,a.fish,a.process_code,a.company_id,a.unit_id from t_soaking a left join t_soaking_entries b on b.soakin_id=a.id  where a.id=$soakingId";
		//echo $qry;		
		$result	=	$this->databaseConnect->getRecord($qry);
		return $result;
	}
	...
	function chkDuplicateEntryExist($rmlotid,$fishId, $selectDate, $soakingId,$processCode)
	{
		...
		if($rmlotid!="") $uptdQry.= " and rm_lot_id='$rmlotid' ";
		if($processCode=="")	
		{
			$qry	= "select id, fish, date from t_soaking where fish='$fishId' and date='$selectDate' $uptdQry";
		}
...
//////////////////////////////////***athi***///////////////////////////////////////////	
	...
	function getAllCompany($entryDate,$rmLotID)
	{
		$qry1="select b.id as id,b.display_name as name from  t_dailypreprocess a left join m_billing_company b on a.company_id=b.id where a.date='$entryDate' and b.display_name is not null ";
		
		$qry2="select b.id as id,b.display_name as name from  t_dailypreprocess_rmlotid a left join m_billing_company b on a.company_id=b.id where a.date='$entryDate' and b.display_name is not null ";
		if($rmLotID!="")
		{
		$qry2.=" and a.rm_lot_id='$rmLotID'";
		}			
		$qry3="select d.id as id,d.display_name as name  from t_rmweightaftergradingdetails a left join t_rmweightaftergrading b on a.weightment_grading_id=b.id  left join t_manage_rm_lotid c on b.rmLotId=c.id  left join  m_billing_company d on c.company_id =d.id   left join m_processcode e on e.id=a.process_code_id left join m_fish f on f.id=e.fish_id where a.created_on='$entryDate'  and d.display_name is not null"; 
		if($rmLotID!="")
		{
		$qry3.=" and b.rmLotId='$rmLotID'";
		}  
		$qry4="select c.id as id,c.display_name as name from t_dailycatchentry a left join t_dailycatch_main b on a.main_id=b.id left join m_billing_company c on b.billing_company_id=c.id where a.select_date='$entryDate' and c.display_name is not null ";
		...
		if($rmLotID=="")
		{
			$qry="select * from (".$qry1." union ".$qry2." union ".$qry3." union ".$qry4.") dum   order by id ";
		}
		else
		{
			$qry="select * from (".$qry2." union ".$qry3." union ".$qry4.") dum   order by id ";
		}
		//echo $qry;
		$result = $this->databaseConnect->getRecords($qry);
		return $result;
	...
	function getAllUnit($entryDate,$rmLotID,$companyId)
	{
		$qry1="select b.id as id,b.name as name from  t_dailypreprocess a left join m_plant b on a.unit_id=b.id where a.date='$entryDate' and b.name is not null ";
		if($companyId!="") $qry1.=" and  a.company_id='$companyId' ";   

		$qry2="select b.id as id,b.name as name from  t_dailypreprocess_rmlotid a left join m_plant b on a.unit_id=b.id where a.date='$entryDate' and b.name is not null ";
		if($companyId!="") $qry2.=" and  a.company_id='$companyId' ";   

		if($rmLotID!="")
		{
		$qry2.=" and a.rm_lot_id='$rmLotID'";
		}			
		$qry3="select d.id as id,d.name as name  from t_rmweightaftergradingdetails a left join t_rmweightaftergrading b on a.weightment_grading_id=b.id  left join t_manage_rm_lotid c on b.rmLotId=c.id  left join  m_plant d on c.unit_id =d.id   left join m_processcode e on e.id=a.process_code_id left join m_fish f on f.id=e.fish_id where a.created_on='$entryDate'  and d.name is not null"; 
		if($companyId!="") $qry3.=" and  c.company_id='$companyId' ";   
		if($rmLotID!="")
		{
		$qry3.=" and b.rmLotId='$rmLotID'";
		}  
		$qry4="select c.id as id,c.name as name from t_dailycatchentry a left join t_dailycatch_main b on a.main_id=b.id left join m_plant c on b.unit=c.id where a.select_date='$entryDate' and c.name is not null ";
		if($companyId!="") $qry4.=" and  b.billing_company_id='$companyId' ";   
	...
	function getAllFishAddedOnDate($entryDate,$rmLotID,$companyId,$unitId)
	{
		...
		if($companyId!="") $qry1.=" and  b.company_id='$companyId' "; 
		if($unitId!="") $qry1.=" and  b.unit_id='$unitId' "; 
		...
		if($companyId!="") $qry2.=" and  b.company_id='$companyId' "; 
		if($unitId!="") $qry2.=" and  b.unit_id='$unitId' "; 
		if($rmLotID!="")
		{
		$qry2.=" and b.rm_lot_id='$rmLotID'";
		}		

		$qry3="select e.id as id,e.name as name from t_rmweightaftergradingdetails a left join t_rmweightaftergrading b on a.weightment_grading_id=b.id  left join m_processcode d on d.id=a.process_code_id left join m_fish e on e.id=d.fish_id left join t_manage_rm_lotid f on b.rmLotId=f.id where b.created_on='$entryDate'"; 
		if($rmLotID!="")
		{
		$qry3.=" and b.rmLotId='$rmLotID'"; 
		} 
		if($companyId!="") $qry3.=" and  f.company_id='$companyId' ";   
		if($unitId!="") $qry3.=" and  f.unit_id='$unitId' ";   
		...
		if($companyId!="") $qry4.=" and  b.billing_company_id='$companyId' ";   
		if($unitId!="") $qry4.=" and  b.unit='$unitId' ";   
	...
	function getAllProcessCodeForFishAddedOnDate($entryDate,$filterId,$rmLotID,$companyId,$unitId)
	{
		...
		$qry3="select d.id as id,d.code as code from t_rmweightaftergradingdetails a left join t_rmweightaftergrading b on a.weightment_grading_id=b.id  left join m_processcode d on d.id=a.process_code_id left join m_fish e on e.id=d.fish_id  left join t_manage_rm_lotid f on b.rmLotId=f.id where b.created_on='$entryDate' and  d.fish_id='$filterId' "; 
		if($rmLotID!="")
		{
		$qry3.=" and b.rmLotId='$rmLotID'";
		}  
	...
	function soakingRecs($entryDate,$selRateList,$rmLotID,$filterId,$processCode,$companyId,$unitId)
	{	
	...
	# add soaking
	function addSoaking($rmLotId,$fishId,$selectDate,$processCode,$company,$unit)
	{
		$qry	= "insert into t_soaking(rm_lot_id,fish,process_code,date,company_id,unit_id) values('$rmLotId','$fishId','$processCode','$selectDate','$company','$unit')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	{
		//echo $processCode;
		$qry="select a.fish,d.name,b.process_code_id,e.code,b.lotid_available,a.id,a.date, a.process_code,b.id, b.soak_type,b.soak_in_count,b.soak_in_grade,b.soak_in_qty,b.soak_in_time,b.soak_out_count, b.soak_out_grade,b.soak_out_qty,b.soak_out_time,b.temperature,b.gain,b.chemical_id, b.chemical_qty,a.rm_lot_id,b.lotid_available,concat(c.alpha_character,c.rm_lotid),f.chemical_name from t_soaking a left join t_soaking_entries b on a.id = b.soakin_id left join t_manage_rm_lotid c on c.id=b.lotid_available left join m_fish d on d.id=a.fish left join m_processcode e on e.id=b.process_code_id left join m_harvesting_chemical_master f on f.id=b.chemical_id where a.date='$entryDate' and  a.fish='$fishId' and b.process_code_id='$processCode' and a.rm_lot_id='$rmLotID'";
		//echo $qry;
		return $this->databaseConnect->getRecord($qry);
