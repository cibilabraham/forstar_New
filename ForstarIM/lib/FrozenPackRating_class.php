...
{
	/****************************************************************
	This class deals with all the operations relating to Frozen Pack Rating
	*****************************************************************/
	var $databaseConnect;
	...
	function FrozenPackRating(&$databaseConnect)
   {
	...
	# Insert Rec
	function addFznPckRate($qeName,$selProcessor,$effectiveDate,$rate,$userId)
	{
		$qry	 = "insert into m_frznpackrating (name,processor_id,rate,effective_date,created_on, created_by) values('$qeName','$selProcessor','$rate','$effectiveDate',NOW(),'$userId')";
		//echo $qry;
	...
	function addSelGradeRec($ratingId,$entryId,$frznId,$gradeId,$rate,$fromDate,$toDate,$userId)
	{
		$qry	 = "insert into m_frzn_pkg_rate_grade (rating_id,pkg_rate_entry_id,frzn_pack_id,grade_id,rate,from_date,to_date,created_by) values('$ratingId','$entryId','$frznId','$gradeId','$rate','$fromDate','$toDate','$userId')";
		//echo $qry.'<br/>';
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	*/
	...
	function getSelGradeRecords($userId, $ratingId,$pkgRateEntryId,$packfrozenId)
	{
		...
		$qry = " select a.grade_id, c.code, a.id,a.rate,from_date,to_date from m_frzn_pkg_rate_grade a, m_grade c";
	...
	{
		$qry = "select a.process_code_id, b.code,a.freezing_stage_id,c.rm_stage,a.id from m_frzn_pkg_rate a left join m_processcode b on a.process_code_id=b.id left join m_freezingstage c on a.freezing_stage_id=c.id where a.rating_id='$fznPkngRateId' order by a.id asc";
	...
	# Update
	function updateFznPkngRateRec($fznPkngRateId,$qeName,$selProcessor,$effectiveDate,$rate)
	{	
		$qry = "update m_frznpackrating set  name='$qeName',processor_id='$selProcessor' ,effective_date='$effectiveDate', rate='$rate' where id='$fznPkngRateId' ";
		//echo $qry;
			...
			if($rateTag!="")
			{
				...
				for($i=0; $i<$rs; $i++)
				{
					...
					//	$exptRate=$gradeTot->Rate;
						$fromDate=$gradeTot->FromDate;
						$toDate=$gradeTot->ToDate;
						$arrayCnt=$i;
						//echo $rateVal;
					}
					//printr($gradeIds); 
			...
			<tr><td colspan="3">&nbsp;</td></tr>
								<tr>
									<td colspan="3" style="padding:5px;">
										<table bgcolor="#ccc" cellpadding="1" cellspacing="1">
											<tr >
												<td style="padding:5px;" class="listing-head" bgcolor="#e8edff">From Date:</td>
												<td style="padding:5px;" class="listing-head"  bgcolor="#e8edff">To Date:</td>
											</tr>
											<tr>
												<td nowrap style="padding:5px;" class="listing-item" bgcolor="#e8edff">
													<input type="text" id="start_date_'.$rowId.'_'.$fieldId.'" name="start_date_'.$rowId.'_'.$fieldId.'" size="12" autocomplete="off" value="'.$fromDate.'" onmouseover="startDate('.$rowId.','.$fieldId.');" onchange="displayGradeStatus('.$rowId.','.$fieldId.');" />
												</td>
												<td nowrap style="padding:5px;" class="listing-item" bgcolor="#e8edff">
													<input type="text" id="end_date_'.$rowId.'_'.$fieldId.'" name="end_date_'.$rowId.'_'.$fieldId.'"  size="12"  autocomplete="off"
													onmouseover="endDate('.$rowId.','.$fieldId.');" onchange="displayGradeStatus('.$rowId.','.$fieldId.');" value="'.$toDate.'" />
												</td>
											</tr>
										</table>
									</td>
													...
													<table  bgcolor="#e8edff">
														<tr >
															...
															$gradeAllChkStatus = "";
																...
																$table.='<input type="checkbox" name="gradeAll" id="gradeAll" value="0" class="chkBox" onclick="chkGradeExist('.$rowId.','.$fieldId.');" '.$gradeAllChk.' '.$gradeAllChkStatus.'>&nbsp;ALL
															</td>
														</tr>
														<TR >';	
														...
														if (sizeof($result)>0) {
															...
															$table.='<td class="listing-item" nowrap>
																...
																<input type="checkbox" name="gradeId_'.$i.'" id="gradeId_'.$i.'" value="'.$gId.'" '.$disabledChk.' '.$checked.' class="chkBox" onclick="return checkDateStatus('.$i.','.$rowId.','.$fieldId.');">
																'.$gName.'
															</td>';
																...
																<table>
																	...
																	<TR>
																		...
																		<tr align="center">
																			...
																			<Th class="listing-head">From Date</Th>
																			<Th class="listing-head">To Date</Th>
																		...
																		$table.='<TR>
																				...
																				<TD class="listing-item">'.$fromDate.'</TD>
																				<TD class="listing-item">'.$toDate.'</TD>
...
*/
	...
	function getGradeRecs($selFish,$selProcessCode,$selFrozenCode,$startDate,$endDate)
	{
		$startDt=mysqlDateFormat($startDate);
		$endDt=mysqlDateFormat($endDate);
		
		$qry="SELECT  a.grade_id FROM `m_frzn_pkg_rate_grade` a join m_frzn_pkg_rate b on a.pkg_rate_entry_id=b.id join m_frzn_pkg_frozen c on a.frzn_pack_id=c.id where ( b.fish_id='$selFish' and b.process_code_id ='$selProcessCode' and c.frozen_id='$selFrozenCode' and a.from_date >= '$startDt' AND a.to_date <= '$endDt') OR ( b.fish_id='$selFish' and b.process_code_id ='$selProcessCode' and c.frozen_id='$selFrozenCode' and a.to_date >= '$startDt' AND a.from_date <= '$endDt')";
		
		//$qry="select  a.grade_id from m_frzn_pkg_rate_grade a left join m_frzn_pkg_rate b on a.pkg_rate_entry_id=b.id left join m_frzn_pkg_frozen c on c.id=a.frzn_pack_id  where b.fish_id='$selFish' and b.process_code_id='$selProcessCode' and c.frozen_id='$selFrozenCode' and (a.from_date between '$startDt' and '$endDt') or  b.fish_id='$selFish' and b.process_code_id='$selProcessCode' and c.frozen_id='$selFrozenCode' and (a.to_date between '$startDt' and '$endDt')";
		//$qry="select a.grade_id from m_frzn_pkg_rate_grade a left join m_frzn_pkg_rate b on a.pkg_rate_entry_id=b.id left join m_frzn_pkg_frozen c on c.id=a.frzn_pack_id where b.fish_id='$selFish' and b.process_code_id='$selProcessCode' and c.frozen_id='$selFrozenCode' and a.from_date>='$startDt' or b.fish_id='$selFish' and b.process_code_id='$selProcessCode' and c.frozen_id='$selFrozenCode' and a.to_date<='$endDt'";
		//echo $qry;
		$result = $this->databaseConnect->getRecords($qry);
		foreach($result as $res)
		{
	...
	function getFrozenData($frznCodeRate,$entryId)
	{
		$qry	= "select a.frozen_id,b.code from m_frzn_pkg_frozen a left join m_frozenpacking b  on a.frozen_id=b.id where a.rating_id='$frznCodeRate' and a.packing_entry_id='$entryId' order by b.id desc";
		//echo $qry;
