...
{  
	...
	*****************************************************************/
	...
	function addProcumentOrder($selCompanyName,$entryDate,$procurmentNo,$schedule_date,$userId,$unitId)
	{
		$qry	= "insert into t_rmprocurmentorder(company,procurement_number,date_of_entry,schedule_date,created_on, created_by,unit_id) values('$selCompanyName','$procurmentNo','$entryDate','$schedule_date', Now(),'$userId','$unitId')";
	...
	function addProcurmentSupplier($lastId, $supplierName,$pondName,$location)
	{
		$qry	= "insert into t_rmprocurmentsupplier(rmProcurmentOrderId,supplier_id,pond_id,procurement_center) values('$lastId','$supplierName','$pondName','$location')";
		$insertStatus	=	$this->databaseConnect->insertRecord($qry);
		if ($insertStatus) {
			$this->databaseConnect->commit();
		} else {
	...
	{
		$qry="SELECT a.supplier,a.id, a.pond_name FROM m_pond_master a JOIN supplier b ON a.supplier = b.id WHERE a.supplier = '$supplierNames' and a.active='1'";
		//echo $qry;
	...
	function supplierName($supplierNameId)
	{
		$qry="select name from supplier where id='$supplierNameId'";
	...
	function fetchAllDateRangeRecords($fromDate, $tillDate)
	{
		...
		//$qry	= "select a.id,a.suppler_group_name,a.procurement_number,a.date_of_entry,a.active,a.generated,b.supplier_group_name,c.vehicle_number,d.name_of_person,a.schedule_date from t_rmprocurmentorder a left join m_supplier_group b on a.suppler_group_name=b.id left join m_vehicle_master c on a.vehicle_number=c.id left join m_driver_master d on a.driver_name=d.id where date_of_entry>='$fromDate' and date_of_entry<='$tillDate' order by date_of_entry desc ";
	...
	{
		$qry	= "select a.id,a.company,a.procurement_number,a.date_of_entry,a.schedule_date,b.name,a.unit_id from t_rmprocurmentorder a left join m_billing_company b on a.company=b.id  where a.id=$orderId";
	...
	{	
		$qry	= "select a.id,a.chemical_id,a.required_quantity,b.chemical_name from t_rmprocurmentchemical a left join m_harvesting_chemical_master b on a.chemical_id=b.id where a.rmProcurmentOrderId='$procurmentId' ";
	...
	$qry	= "select id,vehicle_id,driver_id from  t_rm_procurement_vehicle_details where rmProcurmentOrderId='$procurmentId' ";
	 ...
	 function fetchAllProcurmentVehicleAndDriverName($procurmentId)
	{
		$qry	= "select a.id,a.vehicle_id,a.driver_id,b.vehicle_number,c.name_of_person from  t_rm_procurement_vehicle_details a left join m_vehicle_master b on a.vehicle_id=b.id  left join  m_driver_master c on a.driver_id=c.id where a.rmProcurmentOrderId='$procurmentId' ";
		// $qry	= "select * from t_rmprocurmentchemical where rmProcurmentOrderId='$procurmentId' ";
		//echo $qry;
		$result	=	array();
		$result	=	$this->databaseConnect->getRecords($qry);
	...
	function updateProurmentOrder($procurementId, $selCompanyName,$entryDate,$schedule_date,$procurmentNo,$unitId)
	{	
			$qry	= "update t_rmprocurmentorder set company='$selCompanyName',procurement_number='$procurmentNo',date_of_entry='$entryDate',schedule_date='$schedule_date',unit_id='$unitId' where id='$procurementId'";
	...
	*/
	...
	function updateProcurmentSupplier($rmId,$supplierName,$pondName,$location)
	{	
		$qry = " update t_rmprocurmentsupplier set supplier_id='$supplierName',pond_id='$pondName',procurement_center='$location' where id='$rmId'";
		//echo $qry;
		// die();
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
	...
	{
	 $qry	= "select id,name from supplier where active='Y' and activeconfirm='1' order by name asc"; 
		//echo $qry;
		$result = $this->databaseConnect->getRecords($qry);
		return $result;
	}
	...
	{
		//$qry="select a.id,a.supplier_name,b.name from m_supplier_group_details a join supplier b on a.supplier_name=b.id where supplier_group_name_id='$supplierGroupId' order by supplier_name asc";
		$qry="select a.id,a.name,b.pond_size,b.pond_qty,b.registration_no,b.pond_name from m_landingcenter a inner join  m_pond_master b on a.id=b.location  where b.id='$pondNameId'";
		$result = array();
		$result = $this->databaseConnect->getRecords($qry);
	...
	function fetchAllDriverName($schedule_date,$procurementId)
	{
		$currentdate=date("Y-m-d");
		//echo $schedule_date;
		$qry	= "select id, name_of_person from m_driver_master where active='1' and licence_expiry_date > '$currentdate'";
		//echo $qry;
		$result	=	array();
		$result	=	$this->databaseConnect->getRecords($qry);
		$arrayVal=array();
				...
				$results	=	$this->databaseConnect->getRecords($query);
					...
					if(sizeof($results) >0)
					{
						...
						{	
							...
							{
								$pondMaxs =$this->maximumPondValue();
								$returnDay=$pondMaxs[0];
							}
							$newdate=date('Y-m-d',strtotime($allotDt) - (24*3600*$returnDay));
							$freedate=date('Y-m-d',strtotime($allotDt) - (24*3600*$returnDay));
							$date = date('Y-m-d',strtotime($allotDt) + (24*3600*$returnDay));	
							if(strtotime($allotDt)==strtotime($schedule_date))
							{
								continue;	
							}
							elseif( $allotDt>$schedule_date)
							{
								$dt1=strtotime($allotDt);
								$dt2=strtotime($schedule_date);
								$dtdiffer = $dt1 - $dt2;
								$dtediffer=round($dtdiffer / 86400);
								//echo $dtediffer.','.$returnDay;
								if($dtediffer<=$returnDay)
								{
									if($freedate <$schedule_date)
									{
										continue;
									}
								}
							...
							elseif($allotDt<$schedule_date)
							{
							//echo $returnDay.','.$date.','.$allotDt.','.$driverName.' / ';
								$dat1=strtotime($schedule_date);
								$dat2=strtotime($allotDt);
								$differ = $dat1 - $dat2;
								$datediffer=round($differ / 86400);
								//echo $datediffer;
								if($datediffer<=$returnDay)
								{
						...
						elseif(sizeof($results)>1)
						{
							...
							foreach($results as $driverValues)
							{
								...
								//$newdates=date('Y-m-d',strtotime($dates) - (24*3600*$add_days));
								$freedates=date('Y-m-d',strtotime($allotdts) - (24*3600*$add_days));
								//echo $schedule_date.','.$allotdts.','.$add_days.','.$dates.','.$freedates.','.$driverName.' / ';
									if(strtotime($allotdts)==strtotime($schedule_date))
									{
										$setvalue=0;	
									}
									elseif( strtotime($allotdts)>strtotime($schedule_date))
									{	
									$date1=strtotime($allotdts);
									$date2=strtotime($schedule_date);
									$diff = $date1 - $date2;
									$datediff=round($diff / 86400);
									  // echo	$allotdts.','.$schedule_date.','.$datediff.' / ';
									  	//echo $allotdts.','.$add_days.','.$freedates.','.$schedule_date.','.$driverName;
										//echo $allotdts.','.$schedule_date.','.$driverName.' / ';
										if($datediff<=$add_days)
										{
										$dateCompare =$this->getAllDatesBetweenTwoDates($schedule_date,$allotdts);
										//print_r($dateCompare);
										$maxValsize=sizeof($dateCompare);
											if($maxValsize >0)
											{
												for($i=0; $i<($maxValsize-1); $i++)
												{
												// $dateCompare[$i];
													$scheduleDateExist =$this->findScheduledatedriver($dateCompare[$i],$driverid);
													if(sizeof($scheduleDateExist)>0)
													{
														$setvalue=0;
													}
												}
												...
												if($setvalue!="0")
												{
													$dateArray =$this->getAllDatesBetweenTwoDates($freedates,$allotdts);
												//print_r($dateArray);
													$maxsize=sizeof($dateArray);
													if($maxsize >0)
													{
														for($j=0; $j<$maxsize; $j++)
														{
															$scheduleExist =$this->findScheduledatedriver($dateCompare[$i],$driverid);
															if(sizeof($scheduleExist)>0)
															{
																$setvalue=0;
															}
														}
											...
											/*if(strtotime($freedates) >strtotime($schedule_date))
											{
												$setvalue=1;
											}
											else
									...
									//elseif( strtotime($allotdts)>strtotime($schedule_date))
									//{	
											//echo $allotdts.','.$schedule_date.','.$freedates.','.$driverName.' / ';
											/*if(strtotime($freedates) >strtotime($schedule_date))
											{
												$setvalue=1;
											}
									...
									elseif(strtotime($allotdts)<strtotime($schedule_date))
										{
									//echo $allotdts.','.$schedule_date.','.$add_days.','.$freedates.','.$driverName.' / ';
										//echo $dates.','.$schedule_date.','.$driverName.' / ';
											if(strtotime($dates)>=strtotime($schedule_date))
											{
												$setvalue=0;	
											}
											else
				...
				$results	=	$this->databaseConnect->getRecords($query);
					...
					if(sizeof($results) >0)
					{
						...
						if(sizeof($results)==1)
						{
							...
							elseif( strtotime($allotDt)>strtotime($schedule_date))
							{
								//echo $freedate.','.$driverName.' / ';
								/*if(strtotime($freedate) >strtotime($schedule_date))
								{
									continue;
								}*/
								$dt1=strtotime($allotDt);
							...
							elseif(strtotime($allotDt)<strtotime($schedule_date))
							{
										//	echo $date.','.$schedule_date.','.$driverName.' / ';
								/*if(strtotime($date)>!strtotime($schedule_date))
								{
									continue;
								}*/
									  ...
									  // echo	$allotdts.','.$schedule_date.','.$datediff.' / ';
										...
										$maxValsize=sizeof($dateCompare);
											if($maxValsize >0)
											{
												...
												{
												// $dateCompare[$i];
													$scheduleDateExist =$this->findScheduledate($dateCompare[$i],$vehicleid);
													if(sizeof($scheduleDateExist)>0)
													{
														$setvalue=0;
													}
												...
												//print_r($dateArray);
													...
													if($maxsize >0)
													{
														for($j=0; $j<$maxsize; $j++)
														{
															$scheduleExist =$this->findScheduledate($dateArray[$j],$vehicleid);
															if(sizeof($scheduleExist)>0)
															{
																$setvalue=0;
															}
	...
	function updateProcurementOrderGenerate($procurement_ids)
	{
		$qry	= "update t_rmprocurmentorder set generated='1' where id=$procurement_ids";
 	//echo $qry;
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	function updateProcurementOrderGenerateDelete($procurement_ids)
	{
		$qry	= "update t_rmprocurmentorder set generated='0' where id=$procurement_ids";
 		//echo $qry;
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	function displayGenerate($vehicleid)
	{
		$qry	= "select min(a.schedule_date) from t_rmprocurmentorder a left join t_rm_procurement_vehicle_details b on a.id=b.rmProcurmentOrderId left join m_vehicle_master c on c.id=b.vehicle_id where a.generated=0 and a.active=1 and b.vehicle_id='$vehicleid' and c.allocated='0'";
		$result = array();
		$result = $this->databaseConnect->getRecord($qry);
		return $result;
	}
	function checkActiveExist($vehicleid)
	{
		$qry	= "select b.vehicle_id from t_rmprocurmentorder a left join t_rm_procurement_vehicle_details b on a.id=b.rmProcurmentOrderId left join m_vehicle_master c on c.id=b.vehicle_id where  a.active=1 and b.vehicle_id='$vehicleid' and c.allocated='1'";
		$result = array();
		$result = $this->databaseConnect->getRecord($qry);
		return $result;
	}
	...
	function findScheduledate($scheduledate,$vehicleid)
	{
		$qry	= "select id from t_rm_procurement_vehicle_details where schedule_date='$scheduledate' and vehicle_id='$vehicleid'";
		$result = array();
		$result = $this->databaseConnect->getRecord($qry);
		return $result;
	}
	function findScheduledatedriver($scheduledate,$driverid)
	{
		$qry	= "select id from t_rm_procurement_vehicle_details where schedule_date='$scheduledate' and driver_id='$driverid'";
		$result = array();
		$result = $this->databaseConnect->getRecord($qry);
		return $result;
	}

	function maximumPondValue()
	{
		$qry	= "SELECT MAX(return_days) FROM `m_pond_master` ";
		$result = array();
		$result = $this->databaseConnect->getRecord($qry);
		return $result;
	}
	...
	function getLandingCenterSupplierEdit($supplier)
	{
		$qry	= "SELECT lc.id,lc.name FROM `m_landingcenter` lc left join m_supplier2center s2c on lc.id=s2c.center_id where s2c.supplier_id='$supplier'";
		$result = $this->databaseConnect->getRecords($qry);
		return $result;
