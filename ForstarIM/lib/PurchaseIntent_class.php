...
{  
	...
	*****************************************************************/
	...
	function PurchaseIntent(&$databaseConnect)
	{
	...
	function addIngredientRec($lastId, $ingredientId, $quantity, $fixedQtyChk, $currentStock, $fixedQty, $percentagePerBatch, $ratePerBatch, $ingGmsPerPouch, $percentageWtPerPouch, $ratePerPouch, $percentageCostPerPouch, $cleanedQty)
	{
		$qry = "insert into t_production_plan_entry (production_plan_id, ingredient_id, quantity, fixed_qty_chk, current_stock, fixed_qty, percent_per_btch, rate_per_btch, ing_gms_per_pouch, percent_wt_per_pouch, rate_per_pouch, percent_cost_per_pouch, cleaned_qty) values('$lastId', '$ingredientId', '$quantity', '$fixedQtyChk', '$currentStock', '$fixedQty', '$percentagePerBatch', '$ratePerBatch', '$ingGmsPerPouch', '$percentageWtPerPouch', '$ratePerPouch', '$percentageCostPerPouch', '$cleanedQty')";
		//echo $qry."<br>";
		$insertStatus	=	$this->databaseConnect->insertRecord($qry);
	...
	function fetchAllPagingRecords($cDate, $offset, $limit)
	{
		$qry = "select a.id, a.planned_date, a.product_id, a.created, a.createdby, b.name, a.num_pouch from t_production_plan a, m_productmaster b where a.product_id=b.id and a.planned_date<='$cDate' and status='P' order by a.planned_date desc limit $offset, $limit";
		//echo $qry;
		$result	=	array();
	...
	# Returns all Records
	function fetchDateRangeRecords($cDate)
	{
		$qry = "select a.id, a.planned_date, a.product_id, a.created, a.createdby, b.name, a.num_pouch from t_production_plan a, m_productmaster b where a.product_id=b.id and a.planned_date<='$cDate' and status='P' order by a.planned_date desc";
		//echo $qry;
		$result	=	array();
		$result	=	$this->databaseConnect->getRecords($qry);
		return $result;
