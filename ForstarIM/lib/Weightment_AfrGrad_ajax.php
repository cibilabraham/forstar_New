...
$xajax->configure('statusMessages', true);
	...
	function supplierDetails($rmLotId,$material_type)
	{
		$objResponse 			= new xajaxResponse();		
		$databaseConnect 		= new DatabaseConnect();		
		$weightmentAfterGradingObj 	= new WeightmentAfterGrading($databaseConnect);
		...
		{
			$material_type=1;
			$fishRecords  = $weightmentAfterGradingObj->getAllFishesInDailyPreprocess($rmLotId);
		}
		else
		{
			$material_type=0;
		...
		if(sizeof($weightment)>0)
		{
			...
			{
				$fishes.= '<select name="fish_id_0" id="fish_id_0" onchange="xajax_getProcessCode(this.value,'.$rmLotId.',0,'.$material_type.');"><option value=""> -- Select -- </option>';
	...
	function getProcessCode($fishId,$rmlotid,$inputId,$material_type)
	{
		$objResponse 			= new xajaxResponse();		
		$databaseConnect 		= new DatabaseConnect();		
		$weightmentAfterGradingObj 	= new WeightmentAfterGrading($databaseConnect);
		//$objResponse->alert($material_type);
		if($material_type=="1")
		{
		$result 			= $weightmentAfterGradingObj->getProcessCodeDailyProcess($fishId,$rmlotid);
		}
