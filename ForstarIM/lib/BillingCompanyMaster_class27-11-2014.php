...
{  
	...
	*****************************************************************/
	...
	# Insert A Rec
	function addBillingCompany($companyName, $address, $place, $pinCode, $country, $telNo, $faxNo, $userId, $alphaCode, $displayName,$vatTin,$cstTin,$range,$division,$commissionerate,$exciseNo,$notificationDetails,$panNo,$eicApprovalNo)
	{		
		$qry = "insert into m_billing_company (name, address, place, pin, country, telno, faxno, created, createdby, alpha_code, display_name,vat_tin,cst_tin,notification_details,ti_range,ti_division,commissionerate,excise_no,pan_no,eic_approval_no) values('$companyName', '$address', '$place', '$pinCode', '$country', '$telNo', '$faxNo', NOW(), '$userId', '$alphaCode', '$displayName','$vatTin','$cstTin','$notificationDetails','$range','$division','$commissionerate','$exciseNo','$panNo','$eicApprovalNo') ";
		//echo $qry."<br>";			
	...
	{
		$qry = " select id from t_dailycatch_main where billing_company_id='$billingCompanyId' union select id from m_exporter where billing_company_id='$billingCompanyId'";
		//echo $qry."<br>";
		//$result	= array();
		$result	= $this->databaseConnect->getRecords($qry);
