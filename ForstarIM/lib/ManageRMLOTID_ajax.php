<?php
						...
						<tr bgcolor="#f2f2f2">
																					...
																					<td height="5">
																						<select id="Company_Name" name="Company_Name" required onchange=\'xajax_getUnit(this.value,"","");\'>
																							<option value="">--select--</option>';			$userId		=	$sessObj->getValue("userId");
																							list($companyNames,$unitRecords,$departmentRecords,$defaultCompany)=$manageusersObj->getUserReferenceSet($userId);
																								if(sizeof($companyNames) > 0)
																								{
																									foreach($companyNames as $cmpId=>$cmpNm)
																									{
																										...
																										$result.= '<option '.$sel.' value="'.$companyId.'">'.$companyName.'</option>';
																									}
																								}
											  										    $result.= '</select>										      
																					</td>
											  												...
											  												if(sizeof($units) > 0)
																								{
																									foreach($units as $untId=>$untNm)
																									{
																										...
																										$result.='<option '.$sel.' value="'.$unitId.'">'.$unitName.'</option>';
																									}
																								}
											  										    $result.= '</select>										      
																					</td>
	...
	function getRMlotId($receiptID,$cnt,$procurementAvailable,$company,$unit)
	{
		...
		$billingCompanyRec=$billingCompanyObj->find($company);
		$companyName=$billingCompanyRec[1];
		$unitRec=$plantandunitObj->find($unit);
		$unitName=$unitRec[3];
	...
	### assign button to save the generated lot id
	...
	### check rm lot id for a unit already exist
