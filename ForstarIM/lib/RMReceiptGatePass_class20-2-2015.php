...
{  
	...
	function addReceiptGatepassSupplier($lastId,$supplier_id,$pond_id,$challan_no,$challan_date,$Company_Name,$unit)
	{
		$qry	=	"insert into t_rm_receipt_gatepass_supplier (receipt_gatepass_id, supplier_id, pond_id,challan_no, challan_date, company_id,unit_id) values('".$lastId."','".$supplier_id."','".$pond_id."','".$challan_no."','".$challan_date."','".$Company_Name."','".$unit."')";
	...
	{
		$qry = "SELECT id,name FROM supplier";
		$result	=	array();
		$result	=	$this->databaseConnect->getRecords($qry);
		return $result;
	...
	{
	 $qry="select id,in_seal,number_gen_id,alpha_code FROM m_gate_pass_seal WHERE ( in_seal!='' and in_seal is not null ) and rm_gate_pass_id='$procurement_id'";
	...
	{
		/*$supplierChallanDate=$data['supplier_Challan_Date'];
		$supplierchal=explode("/", $supplierChallanDate);
		$supChal=array($supplierchal[2],$supplierchal[1],$supplierchal[0]);
		$supplierChallanDateval=implode("-",$supChal);
		
		$date_Of_EntryDate=$data['date_Of_Entry'];
		$date_Of_Entrychal=explode("/", $date_Of_EntryDate);
		$date_Entrychal=array($date_Of_Entrychal[2],$date_Of_Entrychal[1],$date_Of_Entrychal[0]);
		$date_Of_EntryDateval=implode("-",$date_Entrychal);*/
	...
	function updateReceiptGatepassSupplier($receipt_id,$supplier_id,$pond_id,$challan_no,$challan_date,$Company_Name,$unit)
	{
		$qry="update  t_rm_receipt_gatepass_supplier set supplier_id='$supplier_id',pond_id='$pond_id',challan_no='$challan_no',challan_date='$challan_date',company_id='$Company_Name',unit_id='$unit' where id='$receipt_id'";
		$updateStatus	= $this->databaseConnect->updateRecord($qry);		
		if ($updateStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
		return $updateStatus;
