		...
		if (sizeof($checkGateNumberSettingsExist)>0){
			...
			if ($checkExist>0){
				...
				if ($nextGatePassId>$validendno){
					...
					$tempStore=$objManageRMLOTID->addLotIdTemporary($slnumber,$numbergen);
					}
			}
			else
			{
...
function getNumPK($mcPackingId,$rowId)
	{
		$objResponse 	 = new NxajaxResponse();
		$databaseConnect = new DatabaseConnect();
		$dailyfrozenpackingObj	= new DailyFrozenPackingNew($databaseConnect);	
		...
		$objResponse->assign("numMcPackCnvrtLs_$rowId", "value", $numPacks);
		$objResponse->script("changeAllMC($numPacks);");		
...
$xajax->register(XAJAX_FUNCTION, 'getNumPK', array('onResponseDelay'=>'showLoading','onComplete'=>'hideLoading'));
