...
function weightmentSupplierName($supplierGroupId,$inputId,$selSupplierGroupId)
	...
	function weightmentSpecies($rm_lot_id,$supplier_id,$pondNameId,$inputId,$pondId)
	{
		...
		//$inputData = ( $data == "") ? 0 : number_format($data,0,"","");
		//$objResponse->assign("supplierAddress_$inputId", "value", "$supplierAddressRecs");
		$landRecs 			= $objWeighmentDataSheet->getLandBasedOnRmLotIdAndSupplier($rm_lot_id,$supplier_id,$pondNameId);
		if (sizeof($landRecs)>0) addDropDownOptions("procurementCenter_$inputId", $landRecs, "", $objResponse);
	...
	function generateDatasheet($company,$unit)
	{
		$selDate=Date('Y-m-d');
		$objResponse 			= new xajaxResponse();
		...
		if(sizeof($checkGateNumberSettingsExist)>0)
		{
			...
			$checkExist=$objWeighmentDataSheet->checkDataSheetDisplayExist($numbergen);
			if ($checkExist>0)
			{
				$getFirstRecord=$objWeighmentDataSheet->getmaxDataSheetId($numbergen);
				...
				$validendno=$objWeighmentDataSheet->getValidendnoDataSheetId($selDate,$company,$unit);	
			...
			else
			{
				...
				$validPassNo=$objWeighmentDataSheet->getValidDataSheetId($selDate,$company,$unit);	
...
function getTableRowBasedRmLotId($rm_lot_id,$edit = '')
{
	...
	if($edit == '')
	{
		...
		$returnVal = '<tr bgcolor="#f2f2f2" align="center">
			...
			if(sizeof($result) > 0)
			{
				...
				foreach($result as $res)
				{
					...
					if($res[2]!='0')
					{
						...
						$landdet = $objWeighmentDataSheet->landingCenter($res[2]);
						$procurementCenter='<select name="procurementCenter[]" id="procurementCenter_'.$i.'" />
										<option value="'.$landdet[0].'">'.$landdet[1].'</option>
										</select>';
					}
					else
					{
					...
					{
						$hiddenFields.= '<input type="hidden" name="procurementAvailable" id="procurementAvailable" value="1">';
					}
					else if($res[6]=='notprocurement')
					{
						$hiddenFields.= '<input type="hidden" name="procurementAvailable" id="procurementAvailable" value="0">';
					}
					...
					$returnVal.='<tr bgcolor="#f2f2f2" align="center">
							...
							<td class="listing-head"> '.$procurementCenter.' </td>
							...
							<td class="listing-head"> <input type="text" name="weight[]" id="weight_'.$i.'" size="10" onkeyup="calculateWeightAndPercent();"  onblur="changePhtTag('.$i.');" / autocomplete="off"> </td>
							<td class="listing-head"> <input type="text" name="soft_precent[]" id="soft_precent_'.$i.'" size="10" onkeyup="calculateWeightAndPercent();" autocomplete="off"/> </td>
			...
			list($company,$unit) = $objWeighmentDataSheet->getCompanyUnit($rm_lot_id);
			$objResponse->script("xajax_generateDatasheet($company,$unit);");
...
function getPondBasedOnRmLotIdAndSupplier($rm_lot_id,$supplier_id,$inputId,$sel)
{
		...
		if (sizeof($ponds)>0) addDropDownOptions("pondName_$inputId", $ponds, $sel, $objResponse);
		$lands = $objWeighmentDataSheet->getLandBasedOnRmLotIdAndSupplier($rm_lot_id,$supplier_id);
		if (sizeof($lands)>0) addDropDownOptions("procurementCenter_$inputId", $lands, $sel, $objResponse);
		return $objResponse;
}
