...
{  
	...
	*****************************************************************/
	...
	function getAvailableSealNosAll()
	{
		...
		if(sizeof($result) > 0)
		{
			$sql = "SELECT out_seal FROM m_rm_gate_pass WHERE number_gen_id = '".$result[0]['id']."' 
					...
					UNION 
					SELECT seal_no FROM t_t_ seal_assigned";
	...
	{
		$qry	= "insert into  seal_assigned(number_gen_id,seal_no,user_id,login_time) values ('$number_gen_id','$seal_no','$user_id','$login_time')";	
		$insertStatus	=	$this->databaseConnect->insertRecord($qry);
		//echo $qry;
		echo $insertStatus;
		if ($insertStatus) {
	...
	/*function addSealAssigned($id, $seal,$userid,$logintime)
	{
		$qry	= "insert into  seal_assigned(number_gen_id,seal_no, user_id,login_time) values('$id','$seal','$userid','$logintime')";
	//echo $qry;
