...
class ManageGatePass
{
	...
	*****************************************************************/
	...
	function getNextGatePassNo($company_id,$unit_id)
	{			
		$selDate = date("Y-m-d");
		$soYear	 = date("Y", strtotime($selDate));
		$gatePassNum = $this->getMaxGatePassNum($soYear,$company_id,$unit_id);
		$validGatePassNum = $this->getValidGatePassNum($gatePassNum, $selDate,$company_id,$unit_id);		
		if ($validGatePassNum) return $gatePassNum+1;
		else return $this->getCurrentGatePassNum($selDate,$company_id,$unit_id);
	}

	function getMaxGatePassNum($soYear,$company_id,$unit_id)
	{
		//old
		//$qry = " select max(gate_pass_no) from t_gate_pass where gate_pass_no!=0 and gp_year='$soYear' ";
		//New 
		$qry = " select max(gate_pass_no) from t_gate_pass where gate_pass_no!=0 and gp_year='$soYear' and company_id='$company_id' and unit_id='$unit_id' ";
	...
	function getValidGatePassNum($gatePassNum, $selDate,$company_id,$unit_id)
	{		
	...
	$qry	= "select start_no, end_no from number_gen where type='GP' and  date_format(start_date,'%Y-%m-%d')<='$selDate' and date_format(end_date,'%Y-%m-%d')>='$selDate' and start_no<='$gatePassNum' and end_no>='$gatePassNum' and billing_company_id='$company_id' and unitid='$unit_id' ";
	...
	function getCurrentGatePassNum($selDate,$company_id,$unit_id)
	{
		...
		$qry	= "select start_no,alpha_code, end_no from number_gen where type='GP' and date_format(start_date,'%Y-%m-%d')<='$selDate' and date_format(end_date,'%Y-%m-%d')>='$selDate' and billing_company_id='$company_id' and unitid='$unit_id' ";

		//echo "<br>$qry<br>";
		$result = $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?$result[0][1].$result[0][0]:"";
	}
	# ----- Gate Pass Ends Here -----------------------------
	...
	function fetchAllRecords($fromDate, $tillDate)
	{
		...
		$qry = "select a.id, a.so_id, b.invoice_type, b.so, b.proforma_no, b.sample_invoice_no, c.name, a.confirm_status, a.gate_pass_no, a.user_id, a.editing_time, TIME_TO_SEC(TIMEDIFF(NOW(), a.editing_time)), b.complete_status,a.gpass_date from t_gate_pass a left join t_salesorder b on a.so_id=b.id left join m_distributor c on b.distributor_id=c.id";
	...
	{
		$qry	="select id,start_no, end_no from number_gen where date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate' or (end_date is null || end_date=0)) and type='GP' and billing_company_id='$compId' and unitid='$invUnit'  or date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate' or (end_date is null || end_date=0)) and type='GP' and billing_company_id='$compId' and unitid='0'";
		//echo $qry;
		$rec = $this->databaseConnect->getRecords($qry);
		return $rec;
	}
	...
	{
		$qry = "select alpha_code from number_gen where type='GP' and id='$id'";
		//echo $qry;
		$rec = $this->databaseConnect->getRecord($qry);
	...
	{
		$qry	= "select end_no from number_gen where date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate') and type='GP' and billing_company_id='$companyId' and unitid='$unitId' OR date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate') and type='GP' and billing_company_id='$companyId' and unitid='0' ";
		//echo $qry;
		$rec = $this->databaseConnect->getRecord($qry);
		return (sizeof($rec)>0)?$rec[0]:0;
	}
	...
	function getValidGatePassId($selDate,$companyId,$unitId)
	{
		...
		$qry	= "select start_no from number_gen where date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate') and type='GP' and billing_company_id='$companyId' and unitid='$unitId' OR date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate') and type='GP' and billing_company_id='$companyId' and unitid='$unitId'";
...
###----------------------------------CODE FOR GENERATE	PO ID ENDS----------------------------------------------------------
	/*KD*/
	function fetchcompanyrecs($gatePassId) 
	{
		$qry = "select company_id from t_gate_pass where id='$gatePassId'";
		return $this->databaseConnect->getRecord($qry);
	}
