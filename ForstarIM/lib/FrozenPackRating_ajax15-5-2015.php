...
//$xajax->configure('statusMessages', true); // For display status
	...
	function displayFrozenRate($processcodeId,$fishId,$freezingStage,$frozenCode,$selQuality,$rowId,$rateTag)
	{	//$result="";
		$objResponse 			= new xajaxResponse();
		// $objResponse->alert($rm_lot_id);
		$databaseConnect 		= new DatabaseConnect();
		...
		$gradeRecs = $frznPkgRatingObj->getGradeRate($processcodeId,$fishId,$freezingStage,$frozenCode,$selQuality,$rowId,$rateTag);
		if(sizeof($gradeRecs)>0)
		{
			$objResponse->assign("dialog", "innerHTML", $gradeRecs);
		}
