...
#--------------------------------------------------------------------------------------------------------
	...
	# Get Record
	function find($rateListId=null , $taxType)
	{
		
		$whr  = "tax_type='$taxType'";
		
		if ($rateListId) $whr = $whr ." and rate_list_id='$rateListId'";
	...
	# Latest rate list	
	function latestRateList($taxType)
	{
		$cDate = date("Y-m-d");
	
		$qry	= "select mcrl.id from m_tax_ratelist mcrl inner join m_tax mt on mcrl.id = mt.id where '$cDate'>=date_format(mcrl.start_date,'%Y--%m-%d') and mt.tax_type='$taxType' order by mcrl.start_date desc";
		//echo "<br><br>".$qry;
		$rec = $this->databaseConnect->getRecord($qry);
		return (sizeof($rec)>0)?$rec[0]:"";
	}

	function fetchAllCSTRateListRecs($taxType)
	{
		//$qry	= "select mcrl.id, mcrl.name, mcrl.start_date from m_tax_ratelist mcrl order by mcrl.start_date desc";
			$qry	= "select mcrl.id, mcrl.name, mcrl.start_date from m_tax_ratelist mcrl inner join m_tax mt on mcrl.id = mt.id where mt.tax_type = '$taxType' order by mcrl.start_date desc";
			//echo("<br><br>");
			//echo($qry);
