...
$xajax = new xajax();	
	...
	# Check Entry Exist
	function chkEntryExist($fishId,$processCode,$selectDate, $dailyPreProcessId, $mode, $filterDate)
	{
		$objResponse 		= new NxajaxResponse();		
		$databaseConnect 	= new DatabaseConnect();
		$dailypreprocessObj	= new DailyPreProcess($databaseConnect);
		...
		if ($fishId!="" && $selPreProcessDate!="") {
			# Check entry exist
			$entryExist	= $dailypreprocessObj->chkDuplicateEntryExist($fishId,$processCode, $selPreProcessDate, $dailyPreProcessId);
	...
	function chkEntryExistLot($lotId,$fishId,$processCode,$selectDate, $dailyPreProcessId, $mode, $filterDate)
	{
		$objResponse 		= new NxajaxResponse();		
		$databaseConnect 	= new DatabaseConnect();
		$dailypreprocessObj	= new DailyPreProcess($databaseConnect);
		$dateS			= explode("/", $selectDate);
		$selPreProcessDate	= $dateS[2]."-".$dateS[1]."-".$dateS[0];
		//$objResponse->alert($processCode);
		$filterSelDate	=	mysqlDateFormat($filterDate);
		if ($fishId!="" && $selPreProcessDate!="") {
			# Check entry exist
			$entryExist	= $dailypreprocessObj->chkDuplicateEntryLotExist($lotId,$fishId,$processCode,$selPreProcessDate, $dailyPreProcessId);
			# Checking date confirmed
			$confirmed	= $dailypreprocessObj->chkDaysEntryConfirmedRmLot($selPreProcessDate);
		}
		if ($entryExist || $confirmed) {
			if ($confirmed) {
				$objResponse->assign("divEntryExistTxt", "innerHTML", " The pre-process entry for the day already confirmed. so kindly select another date.");
	...
	function chkEntryExistInTable($lotId,$fishId,$processCode,$selectDate, $dailyPreProcessId,$selRateListId,$mode, $filterDate)
	{
		$objResponse 		= new NxajaxResponse();		
		$databaseConnect 	= new DatabaseConnect();
		$dailypreprocessObj	= new DailyPreProcess($databaseConnect);
		...
		if ($fishId!="" && $selPreProcessDate!="") {
			# Check entry exist
			$entryExist	= $dailypreprocessObj->chkDuplicateEntryExistInTable($lotId,$fishId,$processCode,$selPreProcessDate, $dailyPreProcessId,$selRateListId);
			# Checking date confirmed
			$confirmed	= $dailypreprocessObj->chkDaysEntryConfirmedRmLot($selPreProcessDate);
		}
		if ($entryExist || $confirmed) {
	...
	function confirmDailyPreProcessEntry($selectDate)
	{
		...
		if ($selectDate!="") {
			...
			$processorEntryExistRMLotId = $dailypreprocessObj->chkDailyPPProcessorEntryRMLotId($selPreProcessDate);
			
			if (sizeof($processorEntryExist)>0) {
				$zeroAvailableQtyExist = $dailypreprocessObj->chkZeroAvailableQtyExist($selPreProcessDate);
				
				if (sizeof($zeroAvailableQtyExist)>0) {
					$confirmed = "Y";
			...
			elseif(sizeof($processorEntryExistRMLotId)>0)
			{
				//$objResponse->alert($selectDate);
				$zeroAvailableQtyExistRMLotId = $dailypreprocessObj->chkZeroAvailableQtyExistRMLotID($selPreProcessDate);
				if (!$zeroAvailableQtyExistRMLotId) {
					$confirmed = "Y";
...
$xajax->register(XAJAX_FUNCTION, 'chkEntryExistLot', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));
