...
{  
	...
	*****************************************************************/
	...
	function addGoodsReceipt($selPoId, $selDepartment, $challanNo, $billNo, $gateEntryNo, $storeEntry, $rejectedEntry, $userId, $grnRemarks)
	{
		$qry	=	"insert into goods_receipt (po_id,department_id, challanno, billno, gate_entry, store_entry,rejected_gate_entry,created, createdby, remarks) values('$selPoId', '$selDepartment',	'$challanNo', '$billNo', '$gateEntryNo', '$storeEntry', '$rejectedEntry', Now(),'$userId', '$grnRemarks')";
		//echo $qry;
	...
	# Returns all Purchase Order
	function fetchAllPORecords()
	{
		$qry = "select id, po, po_number, supplier_id, created, createdby, status from m_purchaseorder where confirmed='1' order by po desc";
		//echo $qry;
		$result	=	array();
		$result	=	$this->databaseConnect->getRecords($qry);
		return $result;
