...
Class Soaking
{
	...
	*****************************************************************/
	...
	function addSoaking($fishId,$selectDate)
	{
	...
	function addPreProcess($lotID,$preProcessId, $openingBalQty, $todayArrivalQty, $totalQty, $totalPreProcessQty, $actualYield, $idealYield, $diffYield, $lastInsertedId, $lanCenterId, $availableQty, $autoGenCalc)
	{
	...
	{
		$qry	= "select a.id, a.fish_id, a.date,b.lot_id from t_dailypreprocess a 
...
# Filter m_process table using fish id 	 
	...
	{	
		...
		/*$qry1="select d.id,d.code,e.id,e.name,SUBSTRING_INDEX(c.processes,',',1),a.process, sum(a.total_preprocess_qty) as sum from  t_dailypreprocess_entries a left join t_dailypreprocess b on a.dailypreprocess_main_id=b.id left join m_process c  on c.id=a.process left join  m_processcode d on d.id=SUBSTRING_INDEX(c.processes,',',1) left join m_fish e on e.id=d.fish_id  where b.date='$entryDate' and b.fish_id='$filterId' GROUP BY d.id";
		...
		{
			$qry2="SELECT a.id,a.code,b.id,b.name FROM m_processcode a left join m_fish b on b.id=a.fish_id WHERE a.fish_id ='".$filterId."' and a.active='1' ";
...
###commented on 25-09-2014
# Filter m_process table using fish id 	 
/*	function preProcessRecs($entryDate,$selRateList,$rmLotID,$filterId,$processCode)
	{	
		$mPreProcessArr = array();
		$qry1 = "select a.id, a.code, SUBSTRING_INDEX(a.processes,',',1) as pfrom, a.processes, a.fish_id, b.name,'notrm' as rmlot,'0' as lot,frs.process_criteria as process,frs.sort_id as sort,tos.process_criteria as tosprocess,tos.sort_id as tossort from m_process a left join m_fish b on a.fish_id=b.id join pre_process_sequence frs on frs.processcode_id = substring(a.processes,1,instr(a.processes,',')-1) join pre_process_sequence tos on tos.processcode_id = substring(a.processes,length(a.processes)-instr(reverse(a.processes),',')+2) left join t_dailypreprocess_entries e on e.process= a.id left join t_dailypreprocess f on f.id=e.dailypreprocess_main_id
