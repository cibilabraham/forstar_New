...
class SupplierRateList
{
	...
	*****************************************************************/
	...
	function addSupplierRateList($rateListName, $startDate, $copyRateList, $selSupplierId, $supplierCurrentRateListId)
	{
		$qry = "insert into m_supplier_ratelist (name, start_date, supplier_id) values('".$rateListName."', '".$startDate."', '$selSupplierId')";
	...
	function fetchAllPagingRecords($offset, $limit, $supplierFilterId)
	{
		...
		$qry	= "select a.id, a.name, a.start_date, b.name from m_supplier_ratelist a, supplier b";
	...
	function latestRateList($supplierId)
	{
		$cDate = date("Y-m-d");
