<?php
Class RMFreezingCalendarReport
{
	...
	*****************************************************************/
	...
	function RMFreezingCalendarReport(&$databaseConnect)
	{
		$this->databaseConnect =&$databaseConnect;
	}

	# Get Records For a Selected Date 
	function getDFPForADate($fromDate, $toDate, $details, $summary, $selProcessor) 
	{
		...
		if ($selectDate) {
			...
			if ($summary) $orderBy	= "fishName asc, processCode asc, freezingStage asc, euCode asc,brandName asc, frozenCode asc, mcPkg asc";
			else $orderBy	= "fishName asc,processCode asc, freezingStage asc, euCode asc, brandName asc, frozenCode asc, mcPkg asc";
			...
			if ($summary) $orderBy	= "fishName asc, processCode asc, freezingStage asc, euCode asc, brandName asc,frozenCode asc, mcPkg asc";
			//else $orderBy	= "dfp.select_date asc, fishName asc, dfpe.frozen_lot_id asc";
			else $orderBy	= "selDate asc, fishName asc,frozenLot asc";
		} else $orderBy	= "fishName asc, processCode asc, freezingStage asc, euCode asc,brandName asc, frozenCode asc, mcPkg asc";
		...
		*/
		...
		if ($summary) {
			...
			$groupBy = "fishId,processId, freezingStageId,eucodeId,brandId,frozenCodeId,mcPkgId";
		...
		$qry1 = "select 
				dfp.id as mainid, dfp.select_date as selDate, dfp.unit, dfpe.frozen_lot_id as frozenLot, dfpe.id as subid, dfpe.fish_id as fishId, dfpe.processcode_id as processId, dfpe.freezing_stage_id as freezingStageId, dfpe.eucode_id as eucodeId, dfpe.brand_id as brandId, dfpe.frozencode_id as frozenCodeId, dfpe.mcpacking_id as mcPkgId, dfpe.export_lot_id, dfp.processor_id, mpp.name as processorName, mf.name as fishName, pc.code as processCode, fs.rm_stage as freezingStage, ec.code as euCode, mb.brand as brandName, fpc.code as frozenCode, mmcp.code as mcPkg, $uptdQryF, dfppo.po_id, CONCAT(pom.po_no,' (',DATE_FORMAT(pom.po_date,'%d/%m/%Y'),')') as PO_Date,'0' as lotStatus,'0' as rmlotid from 
			...
			$qry2 = "select 
				dfp.id as mainid, dfp.select_date as selDate, dfp.unit, dfpe.frozen_lot_id as frozenLot, dfpe.id as subid, dfpe.fish_id as fishId, dfpe.processcode_id  as processId, dfpe.freezing_stage_id  as freezingStageId, dfpe.eucode_id  as eucodeId, dfpe.brand_id  as brandId, dfpe.frozencode_id as frozenCodeId, dfpe.mcpacking_id  as mcPkgId, dfpe.export_lot_id, dfp.processor_id, mpp.name as processorName, mf.name as fishName, pc.code as processCode, fs.rm_stage as freezingStage, ec.code as euCode, mb.brand as brandName, fpc.code as frozenCode, mmcp.code as mcPkg, $uptdQryF, dfppo.po_id, CONCAT(pom.po_no,' (',DATE_FORMAT(pom.po_date,'%d/%m/%Y'),')') as PO_Date,'1' as lotStatus,dfp.rm_lot_id as rmlotid from 
				t_dailyfrozenpacking_main_rmlotid dfp left join m_preprocessor mpp on mpp.id=dfp.processor_id 
				left join t_dailyfrozenpacking_entry_rmlotid dfpe on dfp.id=dfpe.main_id 
				left join m_fish mf on dfpe.fish_id=mf.id
				left join m_processcode pc on pc.fish_id=mf.id and pc.id=dfpe.processcode_id
				left join m_freezingstage fs on fs.id=dfpe.freezing_stage_id
				left join m_eucode ec on ec.id=dfpe.eucode_id
				...
				left join m_frozenpacking fpc on fpc.id=dfpe.frozencode_id
				left join m_mcpacking mmcp on mmcp.id=dfpe.mcpacking_id
				left join t_dailyfrozenpacking_po_rmlotid dfppo on dfppo.entry_id=dfpe.id
				left join t_purchaseorder_main pom on pom.id=dfppo.po_id
			";
		...
		if ($groupBy!="")	
		{ 
			$qry1   .= " group by ".$groupBy; 	$qry2.= " group by ".$groupBy;
		}
