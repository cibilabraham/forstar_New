<?php
	...
	function getRMLotIDResult($selDate,$rmLotOD)
	{
		//$sel = ''; 
		//$date=mysqlDateFormat($selDate);
		$result='';
		...
		$totalResultVal = $objManageRMLOTID->getLotIdTotalvalue($rmLotOD);
						...
						<td>
							...
							foreach($totalResultVal as $res)
							{
								...
								if($i == 0)
								{
									...
									$result.= '<td class="listing-item"  '.$style.'>'.$res[0][5].'</td>';
					...
					<table  width="100%" border="0" bgcolor="#999999" align="center" cellspacing="1" cellpadding="2">
						<tr bgcolor="#f2f2f2">
																				...
																				<tr bgcolor="#f2f2f2">
																					...
																					<td><input type="text" name="alphavalue" id="alphavalue" readonly="readonly" size="2"/>
																					<input type="text" name="generateNewLotId" id="generateNewLotId" size="8"/>
	...
	function getRMlotId($receiptID,$cnt)
	{
	
		$selDate=Date('Y-m-d'); $supplyDetail=''; $supplyDetailId=''; $receiptIdVal='';  $companyIdVal='';  $unitIdVal='';
		$objResponse 			= new xajaxResponse();
		...
		$checkGateNumberSettingsExist=$objManageRMLOTID->chkValidGatePassId($selDate);
		 ...
		 $alphaCode=$objManageRMLOTID->getAlphaCode();
