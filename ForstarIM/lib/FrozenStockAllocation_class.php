...
class FrozenStockAllocation
{
	...
	*****************************************************************/
	...
	function getPagingDFPRecs($fromDate, $tillDate, $offset, $limit, $filterProcessCode)
	{
		...
		$whr1		= " a.select_date>='".$fromDate."'  and a.select_date<='".$tillDate."' and mpc.code is not null" ;
		if ($filterProcessCode>0) $whr .= " and tdfpe.processcode_id='$filterProcessCode' ";
		
		$whr11		= " a.select_date>='".$fromDate."'  and a.select_date<='".$tillDate."' and mpc.name is not null" ;
		if ($filterProcessCode>0) $whr .= " and tdfpe.processcode_id='$filterProcessCode' ";
		...
		$rsLimit 		= " $offset, $limit";
				...
				$qry2 	= "select a.id as id, a.select_date as selDate, mpc.fish_id as fishId, tdfpe.processcode_id as processcodeId, tdfpe.freezing_stage_id as freezingStageId, tdfpe.frozencode_id as frozenCodeId, mpc.code as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, a.report_confirm as confirmed, dfppo.po_id as poId, tdfpe.mcpacking_id as mcPkgId, mcp.code as mcPkgCode, mcp.number_packs as numPack,
						((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty, sum(tdfpg.number_mc) as numMcs, ((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as frozenQty, ((sum(tdfpg.number_mc)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt)*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt))) as actualQty, ((sum(tdfpg.number_mc)*mcp.number_packs)+sum(tdfpg.number_loose_slab)) as totSlabs, 0 as allocatedMC, sum(tdfpg.number_loose_slab) as numLSPack, 0 as allocatedLS,mfp.filled_wt as filledwt,a.repack_frozen_id as repackfrozenid,sum(flag) as flag,repacked_from as repackedfrom,sum(repkdQty) as repkdQty,sum(rgzQty) as rgzQty,reglazed_from as reglazedfrom,concat(mg.alpha_character,mg.rm_lotid) as new_lot_IdNm,a.rm_lot_id as new_lot_Id,tdfpe.id as entryid,mg.unit_id as unit,mg.company_id as company,'primary' as processtype
			    from 
				...
				$qry11 	= "select a.id as id, a.select_date as selDate, mpc.fish_id as fishId, tdfpe.processcode_id as processcodeId, tdfpe.freezing_stage_id as freezingStageId, tdfpe.frozencode_id as frozenCodeId, mpc.name as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, a.report_confirm as confirmed, dfppo.po_id as poId, tdfpe.mcpacking_id as mcPkgId, mcp.code as mcPkgCode, mcp.number_packs as numPack,
						((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty, sum(tdfpg.number_mc) as numMcs, ((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as frozenQty, ((sum(tdfpg.number_mc)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt)*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt))) as actualQty, ((sum(tdfpg.number_mc)*mcp.number_packs)+sum(tdfpg.number_loose_slab)) as totSlabs, 0 as allocatedMC, sum(tdfpg.number_loose_slab) as numLSPack, 0 as allocatedLS,mfp.filled_wt as filledwt,a.repack_frozen_id as repackfrozenid,sum(flag) as flag,repacked_from as repackedfrom,sum(repkdQty) as repkdQty,sum(rgzQty) as rgzQty,reglazed_from as reglazedfrom,'0' as new_lot_IdNm,'0' as new_lot_Id,tdfpe.id as entryid,a.unit as unit, a.company as company,'secondary' as processtype
			    from 
				t_dailyfrozenpacking_main a left join t_dailyfrozenpacking_entry tdfpe on a.id=tdfpe.main_id 
				join t_dailyfrozenpacking_grade tdfpg on tdfpe.id=tdfpg.entry_id
				left join m_secondary_processcode mpc on mpc.id=tdfpe.processcode_id
				left join m_freezingstage mfs on mfs.id=tdfpe.freezing_stage_id
				left join m_frozenpacking mfp on mfp.id=tdfpe.frozencode_id
				left join m_mcpacking mcp on tdfpe.mcpacking_id=mcp.id
				left join t_dailyfrozenpacking_po dfppo on dfppo.entry_id";
				...
				$qry12 	= "select a.id as id, a.select_date as selDate, mpc.fish_id as fishId, tdfpe.processcode_id as processcodeId, tdfpe.freezing_stage_id as freezingStageId, tdfpe.frozencode_id as frozenCodeId, mpc.name as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, a.report_confirm as confirmed, dfppo.po_id as poId, tdfpe.mcpacking_id as mcPkgId, mcp.code as mcPkgCode, mcp.number_packs as numPack,
						((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty, sum(tdfpg.number_mc) as numMcs, ((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as frozenQty, ((sum(tdfpg.number_mc)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt)*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt))) as actualQty, ((sum(tdfpg.number_mc)*mcp.number_packs)+sum(tdfpg.number_loose_slab)) as totSlabs, 0 as allocatedMC, sum(tdfpg.number_loose_slab) as numLSPack, 0 as allocatedLS,mfp.filled_wt as filledwt,a.repack_frozen_id as repackfrozenid,sum(flag) as flag,repacked_from as repackedfrom,sum(repkdQty) as repkdQty,sum(rgzQty) as rgzQty,reglazed_from as reglazedfrom,concat(mg.alpha_character,mg.rm_lotid) as new_lot_IdNm,a.rm_lot_id as new_lot_Id,tdfpe.id as entryid,mg.unit_id as unit,mg.company_id as company ,'secondary' as processtype
			    from 
				t_dailyfrozenpacking_main_rmlotid a left join t_dailyfrozenpacking_entry_rmlotid tdfpe on a.id=tdfpe.main_id 
				join t_dailyfrozenpacking_grade_rmlotid tdfpg on tdfpe.id=tdfpg.entry_id
				left join m_secondary_processcode mpc on mpc.id=tdfpe.processcode_id
		...
		if ($whr1!="") $qry1 .= " where ".$whr1; $qry2 .= " where ".$whr1;
		if ($groupBy) $qry1 .= " group by ".$groupBy; $qry2 .= " group by ".$groupBy;
		//
		//if ($limit) $qry .= " limit ".$limit;
		$qrys=$qry1." union all ".$qry2; 
		if ($orderBy!="") $qry .= " order by ".$orderBy;	

		if ($whr11!="") $qry11 .= " where ".$whr11; $qry12 .= " where ".$whr11;
		if ($groupBy) $qry11 .= " group by ".$groupBy; $qry12 .= " group by ".$groupBy;
		$qryss=$qry11." union all ".$qry12; 
		$qry=$qrys." union all ".$qryss; 
		//
		//if ($limit) $qry .= " limit ".$limit;
		// echo $qry;
		
		$fsWhr1="dfpa.created_on>='".$fromDate."' and  dfpa.created_on<='".$tillDate."'  and mpc.code is not null";
		...
		$fsWhr11="dfpa.created_on>='".$fromDate."' and  dfpa.created_on<='".$tillDate."'  and mpc.name is not null";
		if ($filterProcessCode>0) $fsWhr1 .= " and dfppo.processcode_id='$filterProcessCode' ";

		$fsGroupBy11	= " dfppo.processcode_id, dfppo.freezing_stage_id, dfppo.frozencode_id, dfppo.mcpacking_id";
		$fsOrderBy11	= " mpc.code asc, mfs.rm_stage asc, mfp.code asc, mcp.code asc";


		$fsWhr2="dfpa.created_on>='".$fromDate."' and  dfpa.created_on<='".$tillDate."' and mpc.code is not null";
		...
		$fsLimit 		= " $offset, $limit";
			...
			$fsQry2 	= " select 
						0 as id,0 as selDate,mpc.fish_id as fishId, dfppo.processcode_id as processcodeId, dfppo.freezing_stage_id as freezingStageId, dfppo.frozencode_id as frozenCodeId, mpc.code as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, 0 as confirmed, dfppo.po_id as poId, dfppo.mcpacking_id as mcPkgId, mcp.code as mcPkgCode, mcp.number_packs as numPack, 0 as pkdQty, 0 as numMcs, 0 as frozenQty, 0 as actualQty, 0 as totSlabs, sum(dfpa.number_mc) as allocatedMC, 0 as numLSPack, sum(dfpa.number_loose_slab) as allocatedLS,mfp.filled_wt as filledwt,0 as repackfrozenid,0 as flag,0 as repackedfrom,0 as repkdQty,0 as rgzQty,0 as reglazedfrom,concat(mg.alpha_character,mg.rm_lotid) as new_lot_IdNm,dfppo.rm_lot_id as new_lot_Id,dfppo.id as entryid,mg.unit_id as unit,mg.company_id as company ,'primary' as processtype
			...
			$fsQry11 	= " select 
						0 as id,0 as selDate,mpc.fish_id as fishId, dfppo.processcode_id as processcodeId, dfppo.freezing_stage_id as freezingStageId, dfppo.frozencode_id as frozenCodeId, mpc.name as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, 0 as confirmed, dfppo.po_id as poId, dfppo.mcpacking_id as mcPkgId, mcp.code as mcPkgCode, mcp.number_packs as numPack, 0 as pkdQty, 0 as numMcs, 0 as frozenQty, 0 as actualQty, 0 as totSlabs, sum(dfpa.number_mc) as allocatedMC, 0 as numLSPack, sum(dfpa.number_loose_slab) as allocatedLS,mfp.filled_wt as filledwt,0 as repackfrozenid,0 as flag,0 as repackedfrom,0 as repkdQty,0 as rgzQty,0 as reglazedfrom,'0' as new_lot_IdNm,'0' as new_lot_Id,'0' as entryid,dfppo.unit as unit, dfppo.company as company,'secondary' as processtype
			    from  t_dailyfrozenpacking_po dfppo   join t_dailyfrozenpacking_allocate dfpa on dfppo.id=dfpa.po_entry_id
				left join m_secondary_processcode mpc on mpc.id=dfppo.processcode_id 
				left join m_freezingstage mfs on mfs.id=dfppo.freezing_stage_id
				left join m_frozenpacking mfp on mfp.id=dfppo.frozencode_id
				left join m_mcpacking mcp on dfppo.mcpacking_id=mcp.id ";
		
			$fsQry21	= " select 
						0 as id,0 as selDate,mpc.fish_id as fishId, dfppo.processcode_id as processcodeId, dfppo.freezing_stage_id as freezingStageId, dfppo.frozencode_id as frozenCodeId, mpc.name as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, 0 as confirmed, dfppo.po_id as poId, dfppo.mcpacking_id as mcPkgId, mcp.code as mcPkgCode, mcp.number_packs as numPack, 0 as pkdQty, 0 as numMcs, 0 as frozenQty, 0 as actualQty, 0 as totSlabs, sum(dfpa.number_mc) as allocatedMC, 0 as numLSPack, sum(dfpa.number_loose_slab) as allocatedLS,mfp.filled_wt as filledwt,0 as repackfrozenid,0 as flag,0 as repackedfrom,0 as repkdQty,0 as rgzQty,0 as reglazedfrom,concat(mg.alpha_character,mg.rm_lotid) as new_lot_IdNm,dfppo.rm_lot_id as new_lot_Id,dfppo.id as entryid,mg.unit_id as unit,mg.company_id as company ,'secondary' as processtype
			    from  t_dailyfrozenpacking_po_rmlotid dfppo join t_dailyfrozenpacking_allocate_rmlotid dfpa on dfppo.id=dfpa.po_entry_id
				left join m_secondary_processcode mpc on mpc.id=dfppo.processcode_id 
				left join m_freezingstage mfs on mfs.id=dfppo.freezing_stage_id
				left join m_frozenpacking mfp on mfp.id=dfppo.frozencode_id
		...
		if ($fsWhr1!="") $fsQry1 .= " where ".$fsWhr1;		if ($fsWhr11!="") $fsQry11 .= " where ".$fsWhr11;	
		if ($fsWhr2!="") $fsQry2.= " where ".$fsWhr2;			if ($fsWhr21!="") $fsQry21.= " where ".$fsWhr21;
		if ($fsGroupBy1) $fsQry1 .= " group by ".$fsGroupBy1;	if ($fsGroupBy11) $fsQry11 .= " group by ".$fsGroupBy1;		
		if ($fsGroupBy2) $fsQry2.= " group by ".$fsGroupBy2; if ($fsGroupBy21) $fsQry21.= " group by ".$fsGroupBy21;
		
		
		//
		//if ($limit) $qry .= " limit ".$limit;
		$fsQrys1=$fsQry1." union all ".$fsQry11; 
		//if ($orderBy!="") $fsQrys .= " order by ".$orderBy;	

		$fsQrys2=$fsQry2." union all ".$fsQry21; 
		$fsQry=$fsQrys1." union all ".$fsQrys2; 

		if($filterProcessCode!=""){$WPCF = "where processcodeId='$filterProcessCode'";}else{$WPCF="";}//filter for process code
		$qryfsQry=$qry." union all ".$fsQry; 
		//echo $qryfsQry;
		//$result11	= $this->databaseConnect->getRecords($qryfsQry);
		//print_r($result11);
		//die("X");
		//echo $fsQry;
		$nQry = "select id, selDate, fishId, processcodeId, freezingStageId, frozenCodeId, processCode, freezingStage, frznPkgCode, confirmed, poId, mcPkgId, mcPkgCode, numPack, pkdQty, numMcs, frozenQty, actualQty, totSlabs, sum(allocatedMC), (sum(numMcs)-sum(allocatedMC)) as balMC, sum(numLSPack), sum(allocatedLS), (sum(numLSPack)-sum(allocatedLS)) as balLS,'0' as invConfirm,filledwt,repackfrozenid,flag,repackedfrom,repkdQty,rgzQty,reglazedfrom,new_lot_IdNm,new_lot_Id,entryid,unit,company,processtype from (
			$qryfsQry
				) as x $WPCF group by processcodeId, freezingStageId, frozenCodeId, mcPkgId ,new_lot_Id order by new_lot_IdNm desc,processCode asc, freezingStage asc, frznPkgCode asc, mcPkgCode asc  limit $fsLimit
				";
			//echo "--------bbb<br>-------$nQry";
	...
	//echo $fsQry;
		$nQry = "select id, selDate, fishId, processcodeId, freezingStageId, frozenCodeId, processCode, freezingStage, frznPkgCode, confirmed, poId, mcPkgId, mcPkgCode, numPack, pkdQty, numMcs, frozenQty, actualQty, totSlabs, sum(allocatedMC), (sum(numMcs)-sum(allocatedMC)) as balMC, sum(numLSPack), sum(allocatedLS), (sum(numLSPack)-sum(allocatedLS)) as balLS,sum(invConfirm),filledwt,repackfrozenid,flag,repackedfrom,repkdQty,rgzQty,reglazedfrom,new_lot_IdNm,new_lot_Id,entryid,unit,company,processtype from (
				$qry
				union
				$fsQry
...
#Get Records For Selected Date Range
	function getPagingDFPRecs_old($fromDate, $tillDate, $offset, $limit, $filterProcessCode)
	{
...
//echo "--------bbb<br>-------$nQry";
	...
	function getDFPForDateRange_old($fromDate, $tillDate, $filterProcessCode)
	{		
		$whr		= " a.select_date>='".$fromDate."'  and a.select_date<='".$tillDate."'" ;
		if ($filterProcessCode>0) $whr .= " and tdfpe.processcode_id='$filterProcessCode' ";
...
###############################################################################
	...
	function insertDFPPORecs($dfpEntryID, $POId, $totalSlabs, $totalQty, $processId, $freezingStageId, $frozenCodeId, $MCPkgId, $userId,$hidCompanyId,$hidUnitId)
	{
		$qry	= "insert into t_dailyfrozenpacking_po (entry_id, po_id, total_slabs, total_qty, created_on, processcode_id, freezing_stage_id, frozencode_id, mcpacking_id, created_by,company,unit) values('$dfpEntryID', '$POId', '$totalSlabs', '$totalQty', NOW(), '$processId', '$freezingStageId', '$frozenCodeId', '$MCPkgId', '$userId','$hidCompanyId','$hidUnitId')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function insertDFPPORMLotRecs($dfpEntryID, $POId, $totalSlabs, $totalQty, $processId, $freezingStageId, $frozenCodeId, $MCPkgId, $userId,$rmLotID,$hidCompanyId,$hidUnitId)
	{
		$qry	= "insert into t_dailyfrozenpacking_po_rmlotid (entry_id, po_id, rm_lot_id, total_slabs, total_qty, created_on, processcode_id, freezing_stage_id, frozencode_id, mcpacking_id, created_by,company,unit) values('$dfpEntryID', '$POId','$rmLotID', '$totalSlabs', '$totalQty', NOW(), '$processId', '$freezingStageId', '$frozenCodeId', '$MCPkgId', '$userId','$hidCompanyId','$hidUnitId')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
...
function getRmentryid($POId,$processId,$freezingStageId,$frozenCodeId,$MCPkgId,$gradeId)
	...
	function insertDailyThawing($selectDate,$processId,$freezingStageId,$frozenCodeId,$MCPkgId,$fishIdth,$company,$unit,$hidProcessType)
	{
		$qry	= "insert into t_dailythawing(select_date,processcode_id,freezing_stage_id,frozencode_id,mcpacking_id,fish_id,company_id,unit_id,process_type) values ('$selectDate',$processId,$freezingStageId,$frozenCodeId,$MCPkgId,$fishIdth,'$company','$unit','$hidProcessType')";
		//echo $qry;
		//die();
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();
	...
	function insertDailyThawingRMLot($selectDate,$processId,$freezingStageId,$frozenCodeId,$MCPkgId,$fishIdth,$rmlotid,$company,$unit,$hidProcessType)
	{
		$qry	= "insert into t_dailythawing_rmlotid(select_date,processcode_id,freezing_stage_id,frozencode_id,mcpacking_id,fish_id,rm_lot_id,company_id,unit_id,process_type) values ('$selectDate','$processId','$freezingStageId','$frozenCodeId','$MCPkgId','$fishIdth','$rmlotid','$company','$unit','$hidProcessType')";
		//echo $qry;
		//die();
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();
	...
	function getAllocateProductionRecs($fromDate, $tillDate, $processCodeId, $freezingStageId, $frozenCodeId, $mcPkgId)
	{
		...
		$qry = "select tdfpe.id, tdfpe.frozen_lot_id, tdfpe.mcpacking_id, mcp.code as mcPkgCode, tdfpm.id as MainId, dfppo.id as POEntryId, dfppo.po_id as POID,tdfpe.processcode_id,tdfpe.freezing_stage_id,tdfpe.frozencode_id,tdfpe.mcpacking_id,' ' as lot,tdfpm.company as company,tdfpm.unit as unit from t_dailyfrozenpacking_main tdfpm join t_dailyfrozenpacking_entry tdfpe on tdfpm.id=tdfpe.main_id left join m_mcpacking mcp on tdfpe.mcpacking_id=mcp.id left join t_dailyfrozenpacking_po dfppo ON dfppo.entry_id=tdfpe.id  where tdfpe.processcode_id='$processCodeId' and tdfpe.freezing_stage_id='$freezingStageId' and tdfpe.frozencode_id='$frozenCodeId' and tdfpe.mcpacking_id='$mcPkgId'
	...
	{
		//tdfpm.select_date>='".$fromDate."' and  and tdfpe.id not in (select entry_id from t_dailyfrozenpacking_po)
		$qry = "select tdfpe.id, tdfpe.frozen_lot_id, tdfpe.mcpacking_id, mcp.code as mcPkgCode, tdfpm.id as MainId, dfppo.id as POEntryId, dfppo.po_id as POID,tdfpe.processcode_id,tdfpe.freezing_stage_id,tdfpe.frozencode_id,tdfpe.mcpacking_id ,tdfpm.rm_lot_id as lot,tdfpm.company as company,tdfpm.unit as unit
	...
	function updateDFPPORecs($dfpEntryID, $POId, $totalSlabs, $totalQty, $POEntryId,$hidCompanyId,$hidUnitId)
	{

		$qry	= "update t_dailyfrozenpacking_po SET entry_id='$dfpEntryID', po_id='$POId', total_slabs='$totalSlabs', total_qty='$totalQty',company='$hidCompanyId',unit='$hidUnitId' WHERE id='$POEntryId'";
		//echo $qry;		
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function updateDFPPORMLotRecs($dfpEntryID, $POId, $totalSlabs, $totalQty, $POEntryId,$rmLotID,$hidCompanyId,$hidUnitId)
	{

		$qry	= "update t_dailyfrozenpacking_po_rmlotid SET entry_id='$dfpEntryID', po_id='$POId', rm_lot_id='$rmLotID',company='$hidCompanyId',unit='$hidUnitId',total_slabs='$totalSlabs', total_qty='$totalQty' WHERE id='$POEntryId'";
		//echo $qry;		
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function getAvailablePacks($processCodeId, $freezingStageId, $frozenCodeId, $gradeId, $mcPkgId,$fromDate,$tillDate)
	{
		...
		$qry = "select sum(tdfpg.number_mc) as numMc, sum(tdfpg.number_loose_slab) as numLS, 0 as allocatedMC , 0 as allocatedLS, tdfpe.processcode_id as processCodeId, tdfpe.freezing_stage_id as freezingStageId, tdfpe.frozencode_id as frozenCodeId, tdfpe.mcpacking_id as mcPackingId,tdfpm.company as company,tdfpm.unit as unit  
			from 
				t_dailyfrozenpacking_main tdfpm join t_dailyfrozenpacking_entry tdfpe on tdfpm.id=tdfpe.main_id 
				join t_dailyfrozenpacking_grade tdfpg on tdfpe.id=tdfpg.entry_id 
			...
			group by tdfpe.processcode_id, tdfpe.freezing_stage_id, tdfpe.frozencode_id, tdfpe.mcpacking_id";	
			 ...
			 $fsQry = "select 0 as numMc, 0 as numLS, sum(dfpa.number_mc) as allocatedMC , sum(dfpa.number_loose_slab) as allocatedLS, dfppo.processcode_id as processCodeId, dfppo.freezing_stage_id as freezingStageId, dfppo.frozencode_id as frozenCodeId, dfppo.mcpacking_id as mcPackingId,dfppo.company as company,dfppo.unit as unit   
			from 
			...
			group by dfppo.processcode_id, dfppo.freezing_stage_id, dfppo.frozencode_id, dfppo.mcpacking_id
			 ";
	//echo "<br>$fsQry";
		$uQry = " select sum(numMc), sum(numLS), sum(allocatedMC), sum(allocatedLS), processCodeId, freezingStageId, frozenCodeId, mcPackingId, (sum(numMc)-sum(allocatedMC)) as balMC, (sum(numLS)-sum(allocatedLS)) as balLS,company,unit from (
					$qry
					union
					$fsQry
...
//	echo $qry;
	...
	###upadte thawing with new rm lot 
	function updatelotidInThawingMain($newRMCompanyId,$newRMUnitId,$rm_lot_id, $mainId)
	{
		$qry	= "update t_dailythawing  set linked_newrm='$rm_lot_id',company_id='$newRMCompanyId',unit_id='$newRMUnitId' where id='$mainId'";
		//echo $qry;
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
