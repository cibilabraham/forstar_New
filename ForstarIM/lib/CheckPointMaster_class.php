...
class CheckPointMaster
{
	...
	*****************************************************************/
	...
	{
		$qry	= "select id, name, description,active,(select count(id) from stk_subcategory_chkpoint where check_point_id=mcp.id) as tot from m_check_point mcp order by name asc";
	...
	{
	$qry	= "select id, name, description,active from m_check_point mcp where active=1 order by name asc";
	...
	{
		$qry	= "select id, name, description,active,(select count(id) from stk_subcategory_chkpoint where check_point_id=mcp.id) as tot from m_check_point mcp order by name asc limit $offset, $limit";
	...
	function updateCheckPointconfirm($checkPointId){
		$qry	= "update m_check_point set active='1' where id=$checkPointId";
 		//echo $qry;
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	function updateCheckPointReleaseconfirm($checkPointId){
	$qry	= "update m_check_point set active='0' where id=$checkPointId";
 		//echo $qry;
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
