...
class FrozenPackRating
{
	...
	*****************************************************************/
	...
	# Insert Rec
	function addFznPckRate($qeName,$freezingStage,$frozenCode,$selQuality,$selProcessor,$ratelistid,$userId)
	{
		$qry	 = "insert into m_frznpackrating (name,freezing_id,frozen_id,quality_id,processor_id,ratelistid,created_on, created_by) values('$qeName','$freezingStage','$frozenCode', '$selQuality','$selProcessor,','$ratelistid',NOW(),'$userId')";
		//echo $qry;
	...
	{
		$qry	 = "insert into m_frzn_pkg_rate (rating_id, fish_id, process_code_id) values('$qelId', '$selFish', '$selProcessCode')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function addSelGradeRec($ratingId,$entryId, $gradeId, $displayOrder,$rate,$userId,$processorId)
	{
		$qry	 = "insert into m_frzn_pkg_rate_grade (rating_id,pkg_rate_entry_id, grade_id, display_order,rate,created_by,pre_processor_id) values('$ratingId','$entryId', '$gradeId', '$displayOrder','$rate','$userId','$processorId')";
		//echo $qry.'<br/>';
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function getSelGradeRecords($userId, $qelEntryId)
	{
		...
		if ($qelEntryId=="") $whr .= " and a.created_by='$userId' and (a.pkg_rate_entry_id is null or a.pkg_rate_entry_id=0) ";
		else $whr .= " and a.pkg_rate_entry_id='$qelEntryId' ";*/
	...
	{
		$qry = " select id, fish_id, process_code_id from m_frzn_pkg_rate where rating_id='$fznPkngRateId' order by id asc ";
	...
	# Update
	function updateFznPkngRateRec($fznPkngRateId, $qeName, $freezingStage,$frozenCode,$selQuality,$selProcessor,$ratelistid)
	{	
		$qry = "update m_frznpackrating set  name='$qeName', freezing_id='$freezingStage',frozen_id='$frozenCode',quality_id='$selQuality',ratelistid='$ratelistid' ,processor_id='$selProcessor' where id='$fznPkngRateId' ";
		//echo $qry;
	...
	*/
	...
	# get Grade Recs
	function getGrades($processCodeId,$fishId, $freezingStageId, $frozenCodeId, $qualityId,$rowId)
	{
		$table="";
															...
															<TD valign="top">
																<table cellpadding="0" cellspacing="0">
																	<tr>
																		...
																		<TD align="center">
																			<input type="text" name="exptRate" id="exptRate" size="3" value="'.$exptRate.'" style="text-align:right; font-size:11px;" autocomplete="off" />
																		</TD>
																		<td nowrap style="padding-left:10px;">
																			<input type="button" name="cmdAddExptRate" id="cmdAddExptRate" value="Save" onclick="addGrade('.$rowId.','.$processCodeId.','.$fishId.', '.$freezingStageId.','.$frozenCodeId.','.$qualityId.','.$rowId.');" style=" font-size:11px;"/>
...
# get Grade Recs
	function getGradeRate($processCodeId,$fishId, $freezingStageId, $frozenCodeId, $qualityId,$rowId,$rateTag)
	{
		$table="";
		$whr = " qee.processcode_id='$processCodeId' and qem.freezing_stage_id='$freezingStageId' and qem.quality_id='$qualityId' and qem.frozencode_id='$frozenCodeId' and qem.freezing_stage_id!=0 ";	
														...
														$table.='<tr>
															<TD valign="top">
																<table>
																	<tr>
																		<TD align="right">
																		<input type="button" name="cmdDelete" value="Delete" style="text-align:right; font-size:11px;" onclick="getGrades('.$processCodeId.','.$fishId.','.$freezingStageId.', '.$frozenCodeId.','.$qualityId.','.$rowId.');" />
																		</TD>
																	</tr>
																	<TR>
																		<TD>
																		...
																		$table.='<TR>
																				<td width="20" align="center"><input type="checkbox" name="remove_'.$j.'" id="remove_'.$j.'" value="'.$grades.'" class="chkBox" ></td>	
																				<TD class="listing-item">'.$gradeName.'</TD>
																				<TD class="listing-item" align="right">'.$rateValue.'</TD>	
																				<td>
																					<input type="submit" name="cmdEdit" value="Edit" onClick="assignValue(this.form,'.$editVal.',"editId");this.form.action="FrznPkgGradeException.php";" style="text-align:right; font-size:11px;"/>
																				</td>
																			</TR>';
																		}
																	$table.='</tbody>
