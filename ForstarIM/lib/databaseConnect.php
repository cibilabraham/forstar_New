...
class DatabaseConnect
{
    ...
    var $pwd      = "" ;			//Mysql DB User Password   ais2008
	var $db       = "Forstar_latest_im";
	var $conn     = "";
	
	function DatabaseConnect()
	{
		$this->conn = mysql_connect($this->server,$this->user, $this->pwd) or die("Could not connect ".mysql_errno().": ".mysql_error());
	...
	function getRecords($query,$limit=0)
 	{
		...
		$stmt = mysql_unbuffered_query($query, $this->conn);
   		if ($stmt == false) {
   			return mysql_errno().": ".mysql_error()."<br>"; 			
   		}
