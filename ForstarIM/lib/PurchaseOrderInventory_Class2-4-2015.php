...
{  
	...
	*****************************************************************/
	...
	#Insert PO Rec
	function addPurchaseOrder($purchaseOrderNo, $poNumber, $selSupplierId, $userId, $hidSupplierRateListId,$compId,$remarks,$totalQuantity,$delivarydate,$deliveredto,$vat,$transport,$excise,$factory,$bearer,$unitpo)
	{
		$qry = "insert into m_purchaseorder (po , po_number, supplier_id, created, createdby, status, used_rate_list_id,billing_company_id,remarks,delivarydate,deliveredtounit,transport,excise,vat,delivaryatfact,bearer,unitInv) values('$purchaseOrderNo','$poNumber','$selSupplierId', Now(),'$userId','P', '$hidSupplierRateListId','$compId','$remarks','$delivarydate','$deliveredto','$vat','$transport','$excise','$factory','$bearer','$unitpo')";
		//echo $qry."<br>";
	...
	{
		$qry = "select id, po, po_number, supplier_id, created, createdby, status, used_rate_list_id,remarks,nettotal,delivarydate,deliveredtounit,transport,excise,vat,delivaryatfact,bearer,unitInv from m_purchaseorder where id=$orderId";
		//echo $qry;
		return $this->databaseConnect->getRecord($qry);
	}
	...
	function updatePurchaseOrder($purchaseOrderId, $poNumber, $selSupplierId,$remarks,$totalQuantity,$delivarydate,$deliveredto,$vat,$transport,$excise,$cmdConfirmPurchaseOrderStatus,$factory,$bearer,$unitpo)
	{
		$qry	=	"update m_purchaseorder set supplier_id='$selSupplierId', po_number='$poNumber',remarks='$remarks',nettotal='$totalQuantity',delivarydate='$delivarydate',deliveredtounit='$deliveredto',vat='$vat',transport='$transport',excise='$excise',confirmed='$cmdConfirmPurchaseOrderStatus',delivaryatfact='$factory',bearer='$bearer',unitInv='$unitpo' where id='$purchaseOrderId'";		//echo $qry;
		$result	=	$this->databaseConnect->updateRecord($qry);
		if ($result) {
			$this->databaseConnect->commit();
		} else {
