...
{  
	...
	*****************************************************************/
	...
	function addSalesOrderEntries($lastId, $selProductId, $unitPrice, $quantity, $totalAmt, $selMcPkgId, $mcPack, $loosePack, $distMgnStateEntryId, $taxPercent, $pGrossWt, $pMCPkgGrossWt, $freePkts, $basicRate, $exDutyPercent, $exDutyAmt, $exDutyMasterId, $eduCessAmt, $secEduCessAmt, $taxAmt, $mcPkgWtId,$gstPercent, $cgstPercent,$sgstPercent,$gstAmt,$cgstAmt,$sgstAmt,$gstEntryId, $igstPercent, $igstAmt, $igstEntryId )
	{
		$qry =	"insert into t_salesorder_entry (salesorder_id, product_id, rate, quantity, total_amount, mc_pkg_id, mc_pack, loose_pack, dist_mgn_state_id, tax_percent, p_gross_wt, p_mc_wt, free_pkts, basic_rate, ex_duty_percent, ex_duty_amt, ex_duty_id,  edu_cess_amt, sec_edu_cess_amt, tax_amt, mc_pkg_wt_id, gst_percent,cgst_percent,sgst_percent, gst_amt,cgst_amt,sgst_amt, gst_entry_id, igst_percent, igst_amt, igst_entry_id) values('$lastId', '$selProductId', '$unitPrice', '$quantity', '$totalAmt', '$selMcPkgId', '$mcPack', '$loosePack','$distMgnStateEntryId', '$taxPercent', '$pGrossWt', '$pMCPkgGrossWt', '$freePkts', '$basicRate', '$exDutyPercent', '$exDutyAmt', '$exDutyMasterId', '$eduCessAmt', '$secEduCessAmt', '$taxAmt', '$mcPkgWtId','$gstPercent','$cgstPercent','$sgstPercent','$gstAmt','$cgstAmt','$sgstAmt','$gstEntryId','$igstPercent','$igstAmt','$igstEntryId')";
	...
	function fetchAllPagingRecords($fromDate, $tillDate, $offset, $limit, $invoiceTypeFilter, $distributorFilter)
	{
		...
		//a.invoice_date
		$qry = " select a.id, a.so, a.distributor_id, if (a.so!=0 and a.invoice_type='T', a.invoice_date, entry_date), a.createdby, b.name, a.last_date, a.extended, a.logstatus, a.logstatus_descr, a.payment_status, a.dispatch_date, a.status_id, a.complete_status, a.grand_total_amt, a.invoice_type, a.settled, a.paid, a.area_id, a.city_id, ROUND((a.grand_total_amt+a.round_value),2) as tSOAmt, a.modified_by, a.modified_time, TIME_TO_SEC(TIMEDIFF(NOW(), a.modified_time)) as diffTinS, a.proforma_no, a.sample_invoice_no, a.pkng_gen, a.pkng_confirm, a.gpass_gen, a.gpass_confirm,a.company_id,a.unit_id,a.number_gen_id,a.tax_type,a.state_id, a.ewaybill_no, a.eway_file_name,a.eway_file_name from t_salesorder a, m_distributor b ";
	...
	*/
	...
	{		
		...
		else if($invoiceType=='T') $whr .= " and so_invoice_type='PF'";   //rekha updated code 
	...
	function getDistributorRec($distributorId, $stateId, $cityId, $areaId, $exportEnabled='N')
	{
		...
		$qry = " select  a.id, a.code, a.name, a.contact_person, a.address, a.contact_no, a.opening_bal, a.credit_limit, b.vat_no, b.tin_no, b.cst_no, b.tel_no, b.billing_address, b.delivery_address, b.same_billing_adr, b.pin_code, b.ecc_no, b.gstint_no from m_distributor a, m_distributor_state b, m_distributor_city c, m_distributor_area d, m_area e ";
	...
	function  distWiseTaxInvoiceCalc($distributorId, $stateId)
	{
		...
		if (sizeof($result)>0) {
			...
			//rekha modifiy the code
		
			$qCstTax = " select base_cst from m_tax where active='Y' and tax_type='$taxType'";	
			...
			//Rekha modify code 
			if ($taxType=='GST')	return array($taxType,$cstRate);			
			if ($taxType=='IGST')	return array($taxType,$cstRate);			
...
//echo $qry;
	...
	function  getDistributorWiseTax($distributorId, $stateId, $productId, $selDate)
	{
		...
		$qCstTax = "select mt.base_cst from m_tax mt join m_tax_ratelist mtrl on mtrl.id=mt.rate_list_id where mt.active='Y' and tax_type='$taxType' and date_format(mtrl.start_date,'%Y-%m-%d')<='$selDate' and  (date_format(mtrl.end_date,'%Y-%m-%d')>='$selDate' or (mtrl.end_date is null || mtrl.end_date=0)) order by mtrl.start_date desc";
		$cstResult = $this->databaseConnect->getRecord($qCstTax);
		...
		if ($taxType=='GST')	return $cstRate;
		if ($taxType=='IGST')	return $cstRate;
	}
	...
	*/
	...
	function getNextProformaInvoiceNo($compId,$comp_unit)
	{	
		$selDate = date("Y-m-d");

		$soYear	 = date("Y", strtotime($selDate));
		list($soNum, $invoiceDate) = $this->getMaxProformaNum($soYear,$compId,$comp_unit);
		$validSONum = $this->getValidProformaNum($soNum, $invoiceDate,$compId,$comp_unit);		
		 //echo $validSONum;
		if ($validSONum) return $soNum+1;
		else return $this->getCurrentProformaNum($selDate,$compId,$comp_unit);
	}

	function getMaxProformaNum($soYear,$compId,$comp_unit)
	{
		//old
		//echo($compId);
		//echo("<br><br>");
		/*
		if($compId!=""){
			$qry = $qry." and billing_company_id = '$compId'" ;
		}
		if($comp_unit!=""){
			$qry = $qry." and unitid = '$comp_unit'" ;
		}
		*/
		...
		$qry = " select max(proforma_no), proforma_date from t_salesorder where proforma_no!=0 and so_year='$soYear' and company_id = '$compId' and unit_id='$comp_unit' group by id order by id desc, proforma_date desc";
		//echo $qry;
	...
	function getValidProformaNum($soNum, $selDate, $compId,$comp_unit)
	{		
	...
	function getCurrentProformaNum($selDate,$compId,$comp_unit)
	{
		//old
		...
		$qry	= "select start_no, end_no from number_gen where type='SO' and so_invoice_type='PF' and date_format(start_date,'%Y-%m-%d')<='$selDate' and date_format(end_date,'%Y-%m-%d')>='$selDate' and type='SO' and billing_company_id='$compId' and unitid='$comp_unit' ";
	...
	function getGst($selDate, $pCategoryId, $pStateId, $pGroupId)	
	{
		$qry = "select mt.id as edId, mtrl.id as rlId, mt.gst_per from m_gst mt join m_gst_ratelist mtrl on mtrl.id=mt.gst_rate_list_id where mt.active='Y' and date_format(mtrl.start_date,'%Y-%m-%d')<='$selDate' and  (date_format(mtrl.end_date,'%Y-%m-%d')>='$selDate' or (mtrl.end_date is null || mtrl.end_date=0)) and product_category_id='$pCategoryId' and product_state_id='$pStateId' and product_group_id='$pGroupId' and mt.product_id=0 order by mtrl.start_date desc";
	...
	function getIGST($selDate, $pCategoryId, $pStateId, $pGroupId)	
	{
		$qry = "select mt.id as edId, mtrl.id as rlId, mt.igst_per from m_igst mt join m_igst_ratelist mtrl on mtrl.id=mt.igst_rate_list_id where mt.active='Y' and date_format(mtrl.start_date,'%Y-%m-%d')<='$selDate' and  (date_format(mtrl.end_date,'%Y-%m-%d')>='$selDate' or (mtrl.end_date is null || mtrl.end_date=0)) and product_category_id='$pCategoryId' and product_state_id='$pStateId' and product_group_id='$pGroupId' and mt.product_id=0 order by mtrl.start_date desc";
...
*/
	...
	function chkGstActive($selDate)
	{		
		$qry = "select mtrl.id from m_gst_ratelist mtrl where date_format(mtrl.start_date,'%Y-%m-%d')<='$selDate' and  (date_format(mtrl.end_date,'%Y-%m-%d')>='$selDate' or (mtrl.end_date is null || mtrl.end_date=0)) and mtrl.ex_duty_active='Y' order by mtrl.start_date desc";
		$result = $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?true:false;
	}

	function chkIGstActive($selDate)
	{		
		$qry = "select mtrl.id from m_igst_ratelist mtrl where date_format(mtrl.start_date,'%Y-%m-%d')<='$selDate' and  (date_format(mtrl.end_date,'%Y-%m-%d')>='$selDate' or (mtrl.end_date is null || mtrl.end_date=0)) and mtrl.ex_duty_active='Y' order by mtrl.start_date desc";
		$result = $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?true:false;
	}
...
###----------------------------------CODE FOR GENERATE	PO ID ENDS----------------------------------------------------------
	...
	/*KD Added*/
	function fetchInstaPvtRecs($selSOId)
	{
		$qry = "select company_id from t_salesorder where id=$selSOId";	
		//echo $qry;	
		return $this->databaseConnect->getRecord($qry);		
	}
	/*KD Added*/
