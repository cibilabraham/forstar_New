...
{  
	...
	*****************************************************************/
	...
	function getCertificateQuantity($monitoringId)
	{
		$qry 	= "select tphtmq.pht_quantity,tphtmq.setoff_quantity,tphtmq.balance_quantity,concat(alpha_character,rm_lotid),tphtmq.rmlotid FROM t_phtmonitoring_quantity tphtmq left join t_manage_rm_lotid tmgrm on tmgrm.id=tphtmq.rmlotid where tphtmq.pht_monitoring_id='$monitoringId' order by tphtmq.pht_quantity desc";
		// $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return $result;
	}
	...
	{
		$qry	= "select tphtm.id, tphtm.date_on, tphtm.lot_id, tphtm.supplier,tphtm.supplier_group,tphtm.species,tphtm.supply_qty,s.name,msg.supplier_group_name,mf.name,tphtc.PHTCertificateNo from t_phtmonitoring tphtm left join supplier s on s.id=tphtm.supplier left join m_supplier_group msg on msg.id=tphtm.supplier_group left join m_fish mf on mf.id=tphtm.species left join t_phtcertificate tphtc on tphtc.id=tphtm.certificate_id where tphtm.created_on>='$fromDate' and tphtm.created_on<='$tillDate' order by tphtm.created_on desc";
		//$qry	= "select tphtm.id, tphtm.date_on, tphtm.lot_id, tphtm.supplier,tphtm.supplier_group,tphtm.species,tphtm.supply_qty,s.name,msg.supplier_group_name,mf.name,tphtc.PHTCertificateNo from t_phtmonitoring tphtm left join supplier s on s.id=tphtm.supplier left join m_supplier_group msg on msg.id=tphtm.supplier_group left join m_fish mf on mf.id=tphtm.species left join t_phtcertificate tphtc on tphtc.id=tphtm.certificate_id where tphtm.created_on>='$fromDate' and tphtm.created_on<='$tillDate' order by tphtm.created_on desc";
	...
	{
		$qry	= "select tphtm.id, tphtm.date_on, tphtm.lot_id, tphtm.supplier,tphtm.supplier_group,tphtm.species,tphtm.supply_qty,s.name,msg.supplier_group_name,mf.name,tphtc.PHTCertificateNo from t_phtmonitoring tphtm left join supplier s on s.id=tphtm.supplier left join m_supplier_group msg on msg.id=tphtm.supplier_group left join m_fish mf on mf.id=tphtm.species left join t_phtcertificate tphtc on tphtc.id=tphtm.certificate_id";
	...
	{
		$qry	= "select tphtm.id, tphtm.date_on, tphtm.lot_id, tphtm.supplier,tphtm.supplier_group,tphtm.species,tphtm.supply_qty,s.name,msg.supplier_group_name,mf.name,tphtc.PHTCertificateNo from t_phtmonitoring tphtm left join supplier s on s.id=tphtm.supplier left join m_supplier_group msg on msg.id=tphtm.supplier_group left join m_fish mf on mf.id=tphtm.species left join t_phtcertificate tphtc on tphtc.id=tphtm.certificate_id where tphtm.id='$phtMonitoringDataId' ";
	...
	function getCertificate($certificateNo)
	{
		...
		tphtmq.balance_quantity,tphtc.supplier,tphtc.pond_Name,tphtc.species FROM t_phtmonitoring_quantity tphtmq  left join t_phtcertificate tphtc on tphtmq.pht_certificate_number=tphtc.id left join m_fish mf on tphtc.species=mf.id  WHERE  tphtmq.pht_certificate_number='$certificateNo' order by  tphtmq.balance_quantity asc limit 1";
		$result= $this->databaseConnect->getRecord($qry);
		if(sizeof($result)>0)
		{
	...
	function getWeightmentData($rmlotid,$phtCerificate)
	{
		...
		if(sizeof($res)>0)
		{
			...
			{	
				//$adjustQty='0';
				//$adjustQty=$this->getPhtMonitoringDataSupplier($weighment[0]);
				$adjustQty=$this->getPhtMonitoringData($weighment[0],$phtCerificate);
				if(sizeof($adjustQty)>0)
				{
					foreach($adjustQty as $adjustVal)
					{
						($adjustVal[12]!='0')? $chkdisabled="disabled" : $chkdisabled="";
					$result.="<tr bgcolor='#e8edff'><td><input type='checkbox' class='chkBox' value='".$adjustVal[0]."' id='weightmentId_".$i."' name='weightmentId_".$i."' onclick='alloteData(".$i.");' $chkdisabled>
					</td><td class='listing-item' nowrap>$adjustVal[1]</td><td class='listing-item' nowrap>$adjustVal[8]</td>
					<td class='listing-item' nowrap>$adjustVal[9]</td>
					<td class='listing-item' nowrap>$adjustVal[11]</td>
					<td class='listing-item' nowrap>$adjustVal[6]</td>
					<td class='listing-item' nowrap id='supplyQty_".$i."'>$adjustVal[7]<input type='hidden' name='hideSupplyQty_".$i."' id='hideSupplyQty_".$i."' size='15' value='".$adjustVal[7]."'></td>
					<td class='listing-item' nowrap id='adjustedQty_".$i."'>$adjustVal[12]</td>
					<td class='listing-item' nowrap id='balanceQty_".$i."'>$adjustVal[13]</td><input type='hidden' name='hideAdjustedQty_".$i."' id='hideAdjustedQty_".$i."' size='15'><input  type='hidden' name='hideBalanceQty_".$i."' id='hideBalanceQty_".$i."' size='15'><input type='hidden' name='hidePhtCertifyQty_".$i."' id='hidePhtCertifyQty_".$i."' size='15'><input type='hidden' name='hidePhtMonitoringId_".$i."' id='hidePhtMonitoringId_".$i."' size='15' value='".$adjustVal[14]."'></tr>";
					$i++;
					}
				}
				else
				{
					...
					<td class='listing-item' nowrap id='supplyQty_".$i."'>$weighment[7]<input type='hidden' name='hideSupplyQty_".$i."' id='hideSupplyQty_".$i."' size='15' value='".$weighment[7]."'></td>
					<td class='listing-item' nowrap id='adjustedQty_".$i."'>0</td>
					<td class='listing-item' nowrap id='balanceQty_".$i."'>$weighment[7]</td><input type='hidden' name='hideAdjustedQty_".$i."' id='hideAdjustedQty_".$i."' size='15'><input  type='hidden' name='hideBalanceQty_".$i."' id='hideBalanceQty_".$i."' size='15'><input type='hidden' name='hidePhtCertifyQty_".$i."' id='hidePhtCertifyQty_".$i."' size='15'><input type='hidden' name='hidePhtMonitoringId_".$i."' id='hidePhtMonitoringId_".$i."' size='15' ></tr>";
					$i++;
				}
			}
			$result.="</table></td></tr>";
	...
	{
		$qry	= "SELECT twde.id,concat(tmngrm.alpha_character,tmngrm.rm_lotid),twde.supplier_name,twde.pond_name,twde.product_species,twde.process_code_id,twde.count_code,tphtmq.supply_qty,s.name,mpm.pond_name,mf.name,mp.code,tphtmq.pht_certificate_number,tphtmq.pht_quantity,tphtmq.setoff_quantity,tphtmq.id from t_phtmonitoring_quantity tphtmq left join t_weightment_data_entries twde on tphtmq.weightment_id= twde.id left join supplier s on twde.supplier_name=s.id left join m_pond_master mpm on twde.pond_name=mpm.id left join m_fish mf on twde.product_species=mf.id left join t_manage_rm_lotid tmngrm on tmngrm.id=tphtmq.rmlotid left join m_processcode mp on mp.id=twde.process_code_id  where tphtmq.weightment_id='$weightmentId'";
		//echo $qry;
		$result=$this->databaseConnect->getRecords($qry);
		return $result;
	}
	...
	function getPhtMonitoringData($weightmentId,$phtcertificate)
	{
		$qry	= "SELECT twde.id,concat(tmngrm.alpha_character,tmngrm.rm_lotid),twde.supplier_name,twde.pond_name,twde.product_species,twde.process_code_id,twde.count_code,tphtmq.supply_qty,s.name,mpm.pond_name,mf.name,mp.code,tphtmq.adjusted_qty,tphtmq.supply_balance_qty,tphtmq.id from t_phtmonitoring_quantity tphtmq left join t_weightment_data_entries twde on tphtmq.weightment_id= twde.id left join supplier s on twde.supplier_name=s.id left join m_pond_master mpm on twde.pond_name=mpm.id left join m_fish mf on twde.product_species=mf.id left join t_manage_rm_lotid tmngrm on tmngrm.id=tphtmq.rmlotid left join m_processcode mp on mp.id=twde.process_code_id where twde.id='$weightmentId' and tphtmq.pht_certificate_number='$phtcertificate' or twde.id='$weightmentId' and tphtmq.pht_certificate_number='' or  tphtmq.weightment_id='$weightmentId' and tphtmq.pht_certificate_number!=''";
		//$qry	= "SELECT twde.id,concat(tmngrm.alpha_character,tmngrm.rm_lotid),twde.supplier_name,twde.pond_name,twde.product_species,twde.process_code_id,twde.count_code,tphtmq.supply_qty,s.name,mpm.pond_name,mf.name,mp.code,tphtmq.adjusted_qty,tphtmq.supply_balance_qty,tphtmq.id from t_phtmonitoring_quantity tphtmq left join t_weightment_data_entries twde on tphtmq.weightment_id= twde.id left join supplier s on twde.supplier_name=s.id left join m_pond_master mpm on twde.pond_name=mpm.id left join m_fish mf on twde.product_species=mf.id left join t_manage_rm_lotid tmngrm on tmngrm.id=tphtmq.rmlotid left join m_processcode mp on mp.id=twde.process_code_id where twde.id='$weightmentId' and tphtmq.pht_certificate_number='$phtcertificate' or twde.id='$weightmentId' and tphtmq.pht_certificate_number=''";
		//echo $qry;
		$result=$this->databaseConnect->getRecords($qry);
		return $result;
	}	
	...
	function getWeightmentDataRMLotID($rmlotid)
	{
		...
		if(sizeof($res)>0)
		{
			...
			foreach($res as $weighment)
			{
				...
				$exists=$this->getPhtMonitoringDataSupplier($weighment[0]);
				if(sizeof($exists)>0) 
				{
					...
					*/
					foreach($exists as $existVal)
					{
					$certificate=$existVal[12];
					if($existVal[14]=='0')
					{
						$balanceQty='';
						$availableQty='';
					...
					{
						$balanceQty=$existVal[14];
						$availableQty=$existVal[13];
					}
					//($existVal[13]=='0')? $availableQty='': $availableQty=$existVal[13];
					//($existVal[14]=='0')? $balanceQty='': $balanceQty=$existVal[14];
					($certificate=='')? $disabled='': $disabled="disabled";
					if($certificate!='')
					{
					...
					{
						$query	= "SELECT tphtc.id,tphtc.PHTCertificateNo FROM t_phtmonitoring tphtm join t_phtcertificate tphtc on tphtc.id =tphtm.certificate_id where tphtc.supplier='$supplierId' and tphtc.pond_Name='$pondId' and tphtc.species='$speciesId'and (select balance_quantity from t_phtmonitoring_quantity where pht_certificate_number=tphtc.id and tphtc.supplier='$supplierId' and tphtc.pond_Name='$pondId' and tphtc.species='$speciesId' order by pht_certificate_number desc limit 1) is null or (select balance_quantity from t_phtmonitoring_quantity where pht_certificate_number=tphtc.id and tphtc.supplier='$supplierId' and tphtc.pond_Name='$pondId' and tphtc.species='$speciesId' order by pht_certificate_number desc limit 1) !=0";	
					}
				//echo $query;
					$rest = $this->databaseConnect->getRecords($query);

					$result.="<tr bgcolor='#e8edff'><td class='listing-item' nowrap>$existVal[1]</td><td class='listing-item' nowrap>$existVal[8]</td><td class='listing-item' nowrap>$existVal[9]</td><td class='listing-item' nowrap>$existVal[11]</td><td class='listing-item' nowrap>$existVal[6]</td><td class='listing-item' nowrap id='supplyQty_".$i."'>$existVal[7]</td><td class='listing-item' nowrap><select name='certificateNo_".$i."' id='certificateNo_".$i."' onchange='xajax_certificateAvailableQty(this.value,".$i.",".$existVal[0].");' ".$disabled.">";
					...
					$result.= "</select>";
					$result.="</td><td class='listing-item' nowrap><input name='availableQtySupplier_".$i."' id='availableQtySupplier_".$i."' size='15' value='".$availableQty."' / readonly></td><td class='listing-item' nowrap><input name='adjustedQtySupplier_".$i."' id='adjustedQtySupplier_".$i."' size='15' value='".$balanceQty."' readonly/><input type='hidden' name='balanceQtySupplier_".$i."' id='balanceQtySupplier_".$i."' size='15' /><input type='hidden' name='weightmentSupplier_".$i."' id='weightmentSupplier_".$i."' size='15' value='$existVal[0]'/><input type='hidden' name='phtMonitoringEntryId_".$i."' id='phtMonitoringEntryId_".$i."' size='15' value='$existVal[15]'/>
					<input type='hidden' name='supplyQtyVal_".$i."' id='supplyQtyVal_".$i."' size='15' value='$existVal[7]'/></td></tr>";
					$i++;
					}
				}
				else
	...
	{
		$qry	= "delete from t_monitoring_supplier_temporary where user_id='$userid'";
		$result	= $this->databaseConnect->delRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
		return $result;
