...
class DailyPreProcessMain
{
	...
	*****************************************************************/
	...
	{
		$whr = " a.date='".$recordsDate."' and mp.active='1'" ;
...
//echo $qry.'</br/>';		
	...
	{				
		...
		if ($filterId!=0)  $whr .= " and a.fish_id='".$filterId."' and mp.active='1'" ;
	...
	{	
		...
		//modify here dated 18 june 2019
		$qry.= " and a.active='1' order by frs.process_criteria desc, frs.sort_id asc, tos.process_criteria desc, tos.sort_id asc";
		//echo $qry;
		$result = $this->databaseConnect->getRecords($qry);
		foreach ($result as $ppr) {
			$preProcessId 	= $ppr[0];
	...
	function getPkgQty($processCodeId, $fromDate,$company,$unit)
	{
		...
		//$qry = " select * from(select a.id as id, a.select_date as select_date, a.unit as unit, b.freezing_stage_id as freezing_stage, b.eucode_id as eucode, b.brand_id as brand, b.frozencode_id as frozencode, b.mcpacking_id as mcpacking, sum(c.number_mc) as totalMc, sum(c.number_loose_slab) as totalLc, mf.filled_wt as filled_wt, mcp.number_packs as number_packs, mf.code as frznCode, mf.actual_filled_wt as actual_filled_wt from t_dailyfrozenpacking_main a left join t_dailyfrozenpacking_entry b on a.id=b.main_id left join t_dailyfrozenpacking_grade c on b.id=c.entry_id left join m_frozenpacking mf on b.frozencode_id=mf.id left join m_mcpacking mcp on b.mcpacking_id=mcp.id where a.select_date='$fromDate' and b.processcode_id='$processCodeId' and a.company='$company' and a.unit='$unit' union all  select a.id as id, a.select_date as select_date, a.unit as unit, b.freezing_stage_id as freezing_stage, b.eucode_id as eucode, b.brand_id as brand, b.frozencode_id as frozencode, b.mcpacking_id as mcpacking, sum(c.number_mc) as totalMc, sum(c.number_loose_slab) as totalLc, mf.filled_wt as filled_wt, mcp.number_packs as number_packs, mf.code as frznCode, mf.actual_filled_wt as actual_filled_wt from t_dailyfrozenpacking_main_rmlotid a left join t_dailyfrozenpacking_entry_rmlotid b on a.id=b.main_id left join t_dailyfrozenpacking_grade_rmlotid c on b.id=c.entry_id left join m_frozenpacking mf on b.frozencode_id=mf.id left join m_mcpacking mcp on b.mcpacking_id=mcp.id where a.select_date='$fromDate' and b.processcode_id='$processCodeId' and a.company='$company' and a.unit='$unit' ) dum group by frozencode";
		
		
		# Daily Frzn Packing records
		$qry = " select * from(select a.id as id, a.select_date as select_date, a.unit as unit, b.freezing_stage_id as freezing_stage, b.eucode_id as eucode, b.brand_id as brand, b.frozencode_id as frozencode, b.mcpacking_id as mcpacking, sum(c.number_mc) as totalMc, sum(c.number_loose_slab) as totalLc, mf.filled_wt as filled_wt, mcp.number_packs as number_packs, mf.code as frznCode, mf.actual_filled_wt as actual_filled_wt from t_dailyfrozenpacking_main a left join t_dailyfrozenpacking_entry b on a.id=b.main_id left join t_dailyfrozenpacking_grade c on b.id=c.entry_id left join m_frozenpacking mf on b.frozencode_id=mf.id left join m_mcpacking mcp on b.mcpacking_id=mcp.id where a.select_date='$fromDate' and b.processcode_id='$processCodeId' and a.company='$company' and a.unit='$unit' and b.processcode_type='primary' union all  select a.id as id, a.select_date as select_date, a.unit as unit, b.freezing_stage_id as freezing_stage, b.eucode_id as eucode, b.brand_id as brand, b.frozencode_id as frozencode, b.mcpacking_id as mcpacking, sum(c.number_mc) as totalMc, sum(c.number_loose_slab) as totalLc, mf.filled_wt as filled_wt, mcp.number_packs as number_packs, mf.code as frznCode, mf.actual_filled_wt as actual_filled_wt from t_dailyfrozenpacking_main_rmlotid a left join t_dailyfrozenpacking_entry_rmlotid b on a.id=b.main_id left join t_dailyfrozenpacking_grade_rmlotid c on b.id=c.entry_id left join m_frozenpacking mf on b.frozencode_id=mf.id left join m_mcpacking mcp on b.mcpacking_id=mcp.id where a.select_date='$fromDate' and b.processcode_id='$processCodeId' and a.company='$company' and a.unit='$unit' and b.processcode_type='primary') dum group by frozencode";
		//echo "<br>$qry<br>"; 
		...
		$qrys= "select * from(select a.id as id, a.select_date as select_date, a.unit as unit, b.freezing_stage_id as freezing_stage, b.eucode_id as eucode, b.brand_id as brand, b.frozencode_id as frozencode, b.mcpacking_id as mcpacking, sum(c.number_mc) as totalMc, sum(c.number_loose_slab) as totalLc, mf.filled_wt as filled_wt, mcp.number_packs as number_packs, mf.code as frznCode, mf.actual_filled_wt as actual_filled_wt,b.processcode_id as processcode from t_dailyfrozenpacking_main a left join t_dailyfrozenpacking_entry b on a.id=b.main_id left join t_dailyfrozenpacking_grade c on b.id=c.entry_id left join m_frozenpacking mf on b.frozencode_id=mf.id left join m_mcpacking mcp on b.mcpacking_id=mcp.id where a.select_date='$fromDate' and a.company='$company' and a.unit='$unit' and b.processcode_type='secondary' union all  select a.id as id, a.select_date as select_date, a.unit as unit, b.freezing_stage_id as freezing_stage, b.eucode_id as eucode, b.brand_id as brand, b.frozencode_id as frozencode, b.mcpacking_id as mcpacking, sum(c.number_mc) as totalMc, sum(c.number_loose_slab) as totalLc, mf.filled_wt as filled_wt, mcp.number_packs as number_packs, mf.code as frznCode, mf.actual_filled_wt as actual_filled_wt,b.processcode_id as processcode from t_dailyfrozenpacking_main_rmlotid a left join t_dailyfrozenpacking_entry_rmlotid b on a.id=b.main_id left join t_dailyfrozenpacking_grade_rmlotid c on b.id=c.entry_id left join m_frozenpacking mf on b.frozencode_id=mf.id left join m_mcpacking mcp on b.mcpacking_id=mcp.id where a.select_date='$fromDate' and a.company='$company' and a.unit='$unit' and b.processcode_type='secondary') dum  where id is not null group by frozencode";
		//echo "<br>$qrys<br>"; 
		$dailyFrznPkgRecss = $this->databaseConnect->getRecords($qrys);
		$rt=""; $effWt=0;

		foreach($dailyThawedRecss as $dailyRec)
		{
			$dailyFrznPkgRecss=$dailyRec[2];
			$totalWt=$dailyRec[9];
			$res=$this->getProcessCodeFromSecondary($processCode,$processCodeId);
			if(sizeof($res)>0)
			{
				$rt=$res[0];
				$eff=($totalWt*$rt)/100;
		...
		$totalAvalWt=$effWt+$totalGrossWt;
		//echo $totalAvalWt;
		return $totalAvalWt;
	}
	...
	function getRPMQty($processCodeId, $fromDate,$company,$unit)
	{
		...
		where tdt.select_date='$fromDate' and tdt.processcode_id='$processCodeId' and tdt.flag=1 and tdt.company_id='$company' and tdt.unit_id='$unit'  and tdt.process_type='primary' union all select tdt.id as id, tdt.select_date as select_date, tdt.frozencode_id as frozencode_id, tdt.mcpacking_id as mcpacking_id, sum(tdtg.number_mc_thawing) as number_mc_thawing, sum(tdtg.number_loose_slab_thawing) as number_loose_slab_thawing, mf.filled_wt as filled_wt, mf.actual_filled_wt as actual_filled_wt from t_dailythawing_rmlotid tdt left join t_dailythawing_grade_rmlotid tdtg on tdt.id=tdtg.main_id left join m_frozenpacking mf on tdt.frozencode_id=mf.id
		where tdt.select_date='$fromDate' and tdt.processcode_id='$processCodeId' and tdt.flag=1 and tdt.company_id='$company' and tdt.unit_id='$unit'  and tdt.process_type='primary')  dum  where id is not null group by frozencode_id";
		
		//echo "<br>$qry<br>"; 
		$dailyThawedRecs = $this->databaseConnect->getRecords($qry);
		$totalGrossWt = 0; $totalAvalWt=0;
		...
		//($totalGrossWt!=0)?number_format($totalGrossWt,2,'.',''):0;
		

		$qrys = "select * from (select tdt.id as id, tdt.select_date as select_date,tdt.processcode_id as processcode, tdt.frozencode_id as frozencode_id, tdt.mcpacking_id as mcpacking_id, sum(tdtg.number_mc_thawing) as number_mc_thawing, sum(tdtg.number_loose_slab_thawing) as number_loose_slab_thawing, mf.filled_wt as filled_wt, mf.actual_filled_wt as actual_filled_wt,sum(tdtg.number_mc_stock) as number_mc_stock from t_dailythawing tdt left join t_dailythawing_grade tdtg on tdt.id=tdtg.main_id left join m_frozenpacking mf on tdt.frozencode_id=mf.id
		where tdt.select_date='$fromDate'  and tdt.flag=1 and tdt.company_id='$company' and tdt.unit_id='$unit'  and tdt.process_type='secondary' union all select tdt.id as id, tdt.select_date as select_date,tdt.processcode_id as processcode, tdt.frozencode_id as frozencode_id, tdt.mcpacking_id as mcpacking_id, sum(tdtg.number_mc_thawing) as number_mc_thawing, sum(tdtg.number_loose_slab_thawing) as number_loose_slab_thawing, mf.filled_wt as filled_wt, mf.actual_filled_wt as actual_filled_wt,sum(tdtg.number_mc_stock) as number_mc_stock from t_dailythawing_rmlotid tdt left join t_dailythawing_grade_rmlotid tdtg on tdt.id=tdtg.main_id left join m_frozenpacking mf on tdt.frozencode_id=mf.id
		where tdt.select_date='$fromDate' and tdt.flag=1 and tdt.company_id='$company' and tdt.unit_id='$unit'  and tdt.process_type='secondary')  dum  where id is not null group by frozencode_id";
		$dailyThawedRecss = $this->databaseConnect->getRecords($qrys);
		//echo $qrys."<br/>" ;
		//printr($dailyThawedRecss);
		$rt=""; $effWt=0;
		foreach($dailyThawedRecss as $dailyRec)
		{
			$processCode=$dailyRec[2];
			$totalWt=$dailyRec[9];
			$res=$this->getProcessCodeFromSecondary($processCode,$processCodeId);
			if(sizeof($res)>0)
			{
	...
	function getProcessCodeFromSecondary($processCode,$processCodeId)
	{
		$qry="select percentage from m_secondary_processcode_entry where secondary_id='$processCode' and processcode_id='$processCodeId'";
		//echo $qry;
		$result = $this->databaseConnect->getRecord($qry);
		return $result;
	}
