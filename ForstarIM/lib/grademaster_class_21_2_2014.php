...
{  
	...
	*****************************************************************/
	...
	# Insert
	function addGrade($gradeCode, $min, $max)
	{
		$qry	= "insert into m_grade (code,max,min) values('".$gradeCode."',".$max.",".$min.")";
		//echo $qry;
	...
	{
		$qry	= "select id, code, max, min from m_grade order by code asc";		
	...
	{
		$qry	= "select id, code,max,min from m_grade order by code asc limit $offset, $limit";	
		$result	= $this->databaseConnect->getRecords($qry);
		return $result;
	}
	...
	{
		$qry	= "select id, code,max,min from m_grade where id=$gradeId";
		//echo $qry;
		return $this->databaseConnect->getRecord($qry);
	}
	...
	# Delete Fish
	function updateGrade($gradeCode, $min, $max, $gradeId)
	{
		$qry	=	" update m_grade set code='$gradeCode', min='$min' , max='$max' where id=$gradeId"; 
