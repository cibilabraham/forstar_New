...
$xajax = new xajax();	
	...
	function getIngredientRecords($selectId)
	{
		...
		$data = $productMasterObj->fetchAllIngredientRecords($selRateList);
	...
	function getIngRate($ingId, $rowId, $selIngRateListId)
	{
		...
		if ($selIngType=='ING') {	# Ingredient
			...
			list($lastPrice,$declYield) = $productMasterObj->getIngredientRate($ingId, $selRateList);	
		} else if ($selIngType=='SFP') {  # Semi Finished Product
			list($lastPrice,$declYield) = $productMasterObj->getSemiFinishRate($ingId);	
		}
	...
	function chkPCodeExist($pCode, $cId)
	{
		...
		$databaseConnect 	= new DatabaseConnect();
		$productMasterObj	 = new ProductMaster($databaseConnect);	
		if ($pCode) $pCodeExist = $productMasterObj->chkProductCodeExist(trim($pCode), $cId);
		if ($pCodeExist) {
