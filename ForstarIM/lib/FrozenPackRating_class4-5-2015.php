...
class FrozenPackRating
{
	...
	*****************************************************************/
	...
	{
		$qry	 = "insert into t_frznpackrate_grade (frznrate_id, grade_id, display_order,rate,created_by, active) values('$entryId', '$gradeId', '$displayOrder','$rate','$userId','$gradeStatus')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function getSelGradeRecords($userId, $qelEntryId)
	{
		...
		$qry = " select a.grade_id, c.code, a.id, a.display_order, a.frznrate_id, a.active,a.rate from t_frznpackrate_grade a, m_grade c";
	...
	function addGradeRec($qelEntryId, $gradeId, $g, $userId, $gradeStatus='N',$rate)
	{
		$qry	 = "insert into t_frznpackrate_grade (frznrate_id, grade_id, display_order, created_by, active,rate) values('$qelEntryId','$gradeId', '$g', '$userId', '$gradeStatus','$rate')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
