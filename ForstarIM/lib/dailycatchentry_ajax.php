...
$xajax->configure('statusMessages', true); // For display status
	...
	class NxajaxResponse extends xajaxResponse
	{
		...
		function addDropDown($sSelectId, $options, $cId)
		{
			...
			//document.getElementById('mainSupplier').length=0;
			//addOption('','mainSupplier','','--Select--');
			//$this->script("alert('rekha')");
			//$this->script("alert(document.getElementById('".$sSelectId."'))");
	...
	function chkValidCNum($billingCompanyId, $selChallanNumber, $seltDate, $cId, $unitId, $mode)
	{


		
		$selDate = mysqlDateFormat($seltDate);
		$objResponse = new xajaxResponse();
	    $databaseConnect = new DatabaseConnect();
	    $dailyCatchEntryObj = new DailyCatchEntry($databaseConnect);
		$manageChallanObj   = new ManageChallan($databaseConnect);
		//$objResponse->alert($billingCompanyId.'--,--'.$selChallanNumber.'--,--'.$selDate.'--,--'.$cId.'--,--'.$unitId.'--,--'.$mode);
	
		if ($selDate && $selChallanNumber && $billingCompanyId && $unitId) {

			$validChallanNum = $dailyCatchEntryObj->chkValidChallanNum($selDate, $selChallanNumber, $billingCompanyId,$unitId);
			
				
			if ($validChallanNum) {
				# Check challan number exist
				$challanExist = $dailyCatchEntryObj->chkChallanExist($selChallanNumber, $billingCompanyId, $cId,$unitId);
				if ($challanExist) {
	...
	function getBasketwt($processCodeId)
	{
		$objResponse = new NxajaxResponse();
	    $databaseConnect = new DatabaseConnect();
	    $dailyCatchEntryObj = new DailyCatchEntry($databaseConnect);
	    $data = $dailyCatchEntryObj->getBasketWtProcess($processCodeId);
		if($data>0) 
		{
			$objResponse->script("setGrossWt($data);"); 
		}
		return $objResponse;
	}
	...
	function filterSubSupplier($supplierId, $landingCenterId, $selSubSupplierId)
	{
		...
		$objResponse->script("reloadDeclared($supplierId, $landingCenterId);");
		//$objResponse->script("reloadDeclared();");
		//$objResponse->script("reloadDeclared2();");
	        return $objResponse;
	}
...
/*		$result = $dailycatchentryObj->getQualityDetails($supplier,$pondName,$rmLotId);
		...
		if(sizeof($result)>0)
		{
			...
			$rw = 0;
			$returnVal.= '<td colspan="3"><table>';
			foreach($result as $res)
			{
			//$objResponse->alert($k);
			//$objResponse->alert("hii");
			 $returnVal.= '<tr>
							  <td nowrap="" class="fieldName1" width="25%">'.$res[1].'</td>
							  <td nowrap="" class="listing-item" width="16%">
						     <input type="text" readonly="" style="text-align:right" size="3" value="0.00" id="qualityPercent_'.$rw.'" name="qualityPercent_'.$rw.'">&nbsp;%
							 <input type="hidden" name="quality_new_'.$rw.'" id="quality_new_'.$rw.'" value="'.ucfirst($res[1]).'" />
							 <input type="hidden" name="qualityId_'.$rw.'" id="qualityId_'.$rw.'" value="'.$res[0].'" />
							  <input type="hidden" name="weightmentStatus_'.$rw.'" id="weightmentStatus_'.$rw.'" value="1" />
							  <input type="hidden" name="Status_'.$rw.'" id="Status_'.$rw.'" value="" /> 
							  <input type="checkbox" name="billing_'.$rw.'" id="billing_'.$rw.'" value=""   />
							 
							</td>
							<td nowrap="" class="listing-item">
									<input type="text" autocomplete="off" style="text-align: right" value="0" onkeyup="return effectiveWtNew();" size="5" id="qualityWeight_'.$rw.'" name="qualityWeight_'.$rw.'">&nbsp;Kg
								</td>
								<td nowrap="" class="fieldName1">Reason</td>
								<td class="listing-item"><input type="text" autocomplete="off" value="" tabindex="26" size="20" id="qualityReason_'.$rw.'" name="qualityReason_'.$rw.'"></td>
	...
	function getCountCodeweightment($weightmentId,$pondName,$rmLotId,$supplier,$selPond)
	{
		...
		$fish=array(); $process=array();
		$dailycatchentryObj 	= new DailyCatchEntry($databaseConnect);		
		$countRecs 			= $dailycatchentryObj->getCodeWeightment($pondName,$rmLotId,$weightmentId);
		...
		$result = $dailycatchentryObj->getQualityDetails($supplier,$pondName,$rmLotId,$weightmentId);
		//$returnVal = '<input type="hidden" name="total_new_entry" id="total_new_entry" value="'.sizeof($result).'" />';
		$resSize=sizeof($result);
		$objResponse->assign("total_new_entry", "value", $resSize);
		$returnVal="";
			...
			else
			{
			 ...
			 $returnVal.= '<tr>
							 ...
							 <input type="hidden" name="qualityId_'.$rw.'" id="qualityId_'.$rw.'" value="'.$res[0].'" />
							  <input type="hidden" name="weightmentStatus_'.$rw.'" id="weightmentStatus_'.$rw.'" value="1" />
							  <input type="hidden" name="Status_'.$rw.'" id="Status_'.$rw.'" value="" /> 
							  <input type="checkbox" name="billing_'.$rw.'" id="billing_'.$rw.'" value="'.$billingStatus.'" '.$checkBill.' onclick="checkStatus('.$rw.');"  />
		...
		if(sizeof($result) > 0)
		{
			...
			{ 
				...
				if($res[22] == 'receipt')
				{
					...
					$landingCenterId=$res[24];
					if($landingCenterId=="0" && $pondId!='')
					{
						$landingCenterId=$dailycatchentryObj->landingCenter($pondId);
					}
				}
				else
				{
				 ...
				 $returnVal.= '<td class="listing-head"><a class="supplier_name_'.$supplierId.'" href="javascript:void(0);" onclick="assignSupplierDetails('.$supplierId.','.$pondId.','.$unitId.','.$companyId.','.$supplierChallanNo.','.$makePayment.','.$landingCenterId.');">'.$supplierName.'</a></td>';
	...
	$objResponse->script("resetField('$rowcnt');");
		return $objResponse;
	}
	function getSuppierDetailsWeightment($rmLotId)
	{
		$objResponse 			= new xajaxResponse();		
		$databaseConnect 		= new DatabaseConnect();
		$dailycatchentryObj 	= new DailyCatchEntry($databaseConnect);
		//$objResponse->alert("hii");	
		$result  = $dailycatchentryObj->getAllDetailsWeighment($rmLotId);
		//print_r($result );
		if(sizeof($result)>0)
		{
			$objResponse->assign("popupcontent", "innerHTML", $result);
		}		
		return $objResponse;
	...
	function checkProcurementStatus($rmLotId)
	{
		...
		//$objResponse->alert($rmLotId);		
		$dailycatchentryObj 	= new DailyCatchEntry($databaseConnect);
		$results  				= $dailycatchentryObj->checkLotIdInWeightmentDatasheet($rmLotId);
			if(sizeof($results)>0)
			{
				if($results[0][1]=='1')
				{
					$objResponse->script("getSupplierDetail('$rmLotId');");
				}
				else
				{
					$objResponse->script("confirmInDatasheet('$rmLotId');");
				}
			}
			else
			{
				$objResponse->script("requireInDatasheet('$rmLotId');");
			}
...
####commented on 16-4-2014

/*	function checkProcurementStatus_old($rmLotId)
	{
		$objResponse 			= new xajaxResponse();		
		$databaseConnect 		= new DatabaseConnect();
		//$objResponse->alert($rmLotId);		
		$dailycatchentryObj 	= new DailyCatchEntry($databaseConnect);
		$result  				= $dailycatchentryObj->checkProcurementStatus($rmLotId);
		if(sizeof($result)>0)
		{ 
			$results  				= $dailycatchentryObj->checkLotIdInWeightmentDatasheet($rmLotId);
			if(sizeof($results)>0)
		...
		{
			$objResponse->script("getDetail('$rmLotId');");
				//$objResponse->xajax_getSuppierDetails($rmLotId);
		}
		//$objResponse->alert($qualityName);
		return $objResponse;
	}
*/
	function getChallanDetails($unitId,$companyId)
	{
		$objResponse 			= new xajaxResponse();		
		$databaseConnect 		= new DatabaseConnect();
		$dailycatchentryObj 	= new DailyCatchEntry($databaseConnect);
		...
		//$objResponse->alert($companyId);
		$result  				= $dailycatchentryObj->getAlphaCode($unitId,$companyId);
		if(sizeof($result)>0) 
		{ 
			$objResponse->assign("alphaCode", "value", $result); 
		}
		else
		{
			$alphaCode	= $billingCompanyObj->getBillingCompanyAlphaCode($companyId);
			if ($companyId=="") {
				$alphaCode	= $billingCompanyObj->getDefaultBillingCompany();
			}	
			if ($alphaCode=="") $objResponse->alert("Please define an Alpha code.");
	...
	function checkChallanStatus($weighChallanNo,$chkUnit,$chkBillingCompany,$chkLandingCenter,$chkSupplier)
	{	
		$objResponse 			= new xajaxResponse();		
		$databaseConnect 		= new DatabaseConnect();
		$dailycatchentryObj 	= new DailyCatchEntry($databaseConnect);
		$challanId=$dailycatchentryObj->getChallanDetails($weighChallanNo,$chkUnit,$chkBillingCompany);
		$checkStatus="";
		if($challanId!='')
		{	
			$checkStatus=$dailycatchentryObj->compareChallan($weighChallanNo,$chkUnit,$chkBillingCompany,$chkLandingCenter,$chkSupplier);
		//die();
		}
		else
		{
		...
		$objResponse->setReturnValue($checkStatus);
		//$objResponse->alert($checkStatus);
		//return $checkStatus;
		return $objResponse;
	}
	...
	function timeupdate($mainid,$entryid)
	{
		...
		$databaseConnect 		= new DatabaseConnect();
		$dailycatchentryObj 	= new DailyCatchEntry($databaseConnect);
		//$objResponse->alert($entryid);	
		$update=$dailycatchentryObj->timeupdate($mainid,$entryid);
		$objResponse->setReturnValue($update);
		return $objResponse;
	}
	...
	function getUnitRmLot($companyId,$row,$cel)
	{
		$objResponse 	= new NxajaxResponse();	
		$databaseConnect= new DatabaseConnect();
		$manageusersObj			=	new ManageUsers($databaseConnect);
		...
		$unit=$unitRecords[$companyId];
		$unit = array('0' => '--Select--') + $unit;
		$objResponse->addDropDownOptions("lotUnit",$unit,$cel);
		return $objResponse;	
	}


	function allQualityhide()
	{
		$objResponse 	= new NxajaxResponse();	
		$databaseConnect= new DatabaseConnect();
		$dailycatchentryObj 	= new DailyCatchEntry($databaseConnect);
		...
		$objResponse->assign("allQualityhide", "value", $allQualityhide);	
		return $objResponse;
	}
	...
	function chkChallanNo($supplierChallanNo,$supplier)
	{
		...
		$databaseConnect= new DatabaseConnect();
		$dailycatchentryObj 	= new DailyCatchEntry($databaseConnect);
		if($supplierChallanNo!="" && $supplier!="")
		{
			$duplicateChallan = $dailycatchentryObj->chkChallanNo($supplierChallanNo,$supplier);
			if($duplicateChallan)
			{
				$msg="Challan no already exist";
			}
			else
			{
				$msg="";
		...
		$objResponse->assign("supChallan1", "innerHTML", $msg);			
		return $objResponse;

	}

	
/*	function insertRow($catchEntryNewId)
	{
		$objResponse 			= new xajaxResponse();		
		$databaseConnect 		= new DatabaseConnect();
		$dailycatchentryObj 	= new DailyCatchEntry($databaseConnect);
		$sessObj				=	new Session($databaseConnect);
		$del=$dailycatchentryObj->delEntryRow($catchEntryNewId);
		$userId		=	$sessObj->getValue("userId");
		$tempdataRecIns=$dailycatchentryObj->addTempMaster($userId);
		if ($tempdataRecIns!="") {				
			$entryId = $databaseConnect->getLastInsertedId();
	...
	/*$xajax->register(XAJAX_FUNCTION,'insertRow', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading','mode' => "'synchronous'"));*/
	
 

	$xajax->register(XAJAX_FUNCTION,'chkChallanNo', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));
	$xajax->register(XAJAX_FUNCTION,'allQualityhide', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));
	$xajax->register(XAJAX_FUNCTION,'getUnitRmLot', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading','mode' => "'synchronous'"));
	$xajax->register(XAJAX_FUNCTION,'getUnit', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading','mode' => "'synchronous'"));
	$xajax->register(XAJAX_FUNCTION,'timeupdate',array('mode' => "'synchronous'"));
	$xajax->register(XAJAX_FUNCTION,'checkChallanStatus', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading','mode' => "'synchronous'"));
	$xajax->register(XAJAX_FUNCTION,'getChallanDetails', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));
	$xajax->register(XAJAX_FUNCTION,'getBasketwt', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));
	$xajax->register(XAJAX_FUNCTION,'getCountCodeweightment', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));
	$xajax->register(XAJAX_FUNCTION,'getSuppierDetailsWeightment', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));
	$xajax->register(XAJAX_FUNCTION,'checkProcurementStatus', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));
