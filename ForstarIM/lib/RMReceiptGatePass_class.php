...
{  
	...
	{	
		...
		if(sizeof($checkStatus)>0){
			...
			if($letters == 'F'){
				$qry="select id,billing_company_id,unitid from number_gen where alpha_code=0 and '$currentdate' between start_date and end_date  and '$numbers' between start_no and end_no and type='RG'";
				$result = array();
				$result = $this->databaseConnect->getRecord($qry);
				return $result;
			}else{
