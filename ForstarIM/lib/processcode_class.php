...
{  
	...
	*****************************************************************/
	...
	function ProcessCode(&$databaseConnect)
	{
	...
	function find($processCodeId)
	{
		$qry	= "select a.id, a.fish_id, a.code, a.descr, a.b_weight, a.grade_unit_raw, a.grade_unit_frozen, a.available_option,b.id, b.name, a.frozen_available, a.count_unit_raw, a.count_unit_frozen from m_processcode a, m_fish b where a.fish_id = b.id and  a.id=$processCodeId";
		//echo $qry;		
		return $this->databaseConnect->getRecord($qry);
	}
	...
	{	
		...
		/*
		$qry	= " delete from m_processcode2grade where processcode_id='$processCodeId'";
		$result	= $this->databaseConnect->delRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
		return $result;
	...
	# Returns all Process Code (paging)
	function fetchAllPagingRecords($offset, $limit, $fishId)
	{		
		if ($fishId!=0) $whr = " a.fish_id='$fishId' ";

		$orderBy = "b.name asc, a.code asc";

		$limit = " $offset, $limit ";

		$qry = "select a.id, a.fish_id, a.code, a.descr, a.b_weight, a.grade_unit_raw, a.count_unit_frozen, a.available_option, b.id, b.name,a.frozen_available, a.grade_unit_frozen, a.count_unit_raw,a.active,((select COUNT(a1.id) from m_process a1 where (a1.processes like a.id or a1.processes like 'a.id,%' or a1.processes like '%,a.id,%' or a1.processes like '%,a.id'))+(select COUNT(a2.id) from  t_dailycatchentry a2 where a2.fish_code=a.id)) as tot from m_processcode a left join m_fish b on a.fish_id = b.id";
	...
	function fetchAllRecords($fishId=null)
	{
		if ($fishId!=0) $whr = " a.fish_id='$fishId' ";

		$orderBy = "b.name asc, a.code asc";

		$qry = "select a.id, a.fish_id, a.code, a.descr, a.b_weight, a.grade_unit_raw, a.count_unit_frozen, a.available_option, b.id, b.name,a.frozen_available, a.grade_unit_frozen, a.count_unit_raw,a.active from m_processcode a left join m_fish b on a.fish_id = b.id";
