...
function getTableRowBasedRmLotId($rm_lot_id,$edit = '')
{
	...
	if($edit == '')
	{
			...
			if(sizeof($result) > 0)
			{
				...
				foreach($result as $res)
				{
					...
					$returnVal.='<tr bgcolor="#f2f2f2" align="center">
							...
							<td class="listing-head"> <input type="text" name="weight[]" id="weight_'.$i.'" size="10" onkeyup="calculateWeightAndPercent();" /> </td>
				...
				/*$returnVal.='<tr bgcolor="#f2f2f2" align="right">
								...
								<td class="listing-item"><input type="text" name="total_quantity" id="total_quantity" value="" readonly="readonly" size="10"> </td>
								<td class="listing-item"><input type="text" name="total_soft" id="total_soft" value="" readonly="readonly" size="10"></td>
								<td class="listing-item"> </td>
								<td class="listing-item"> </td>								
							</tr>';*/
		...
		/*$chemicalRecords = $objWeighmentDataSheet->getChemicalValues($rm_lot_id);
		$equipmentRecords = $objWeighmentDataSheet->getEquipmentValues($rm_lot_id);
		
		$chemenicalTable = '';$equipmentTable = '';
		if(sizeof($chemicalRecords) > 0)
		{
			$chemenicalTable.= '<table width="100%" bgcolor="#999999" cellspacing="1" cellpadding="3" id="tblHarvestingChemical">';
			$chemenicalTable.= '<tbody>';
			$chemenicalTable.= '<tr bgcolor="#f2f2f2" align="center">
									<td class="listing-head">Chemical name</td>
									<td class="listing-head">Required quantity(Kgs)</td>
									<td class="listing-head">Issued quantity</td>
									<td class="listing-head">Difference</td> 
								</tr>';
			foreach($chemicalRecords as $chemical)
			{
				$chemenicalTable.= '<tr bgcolor="#f2f2f2" align="center">
									<td class="listing-item">'.$chemical[0].'</td>
									<td class="listing-item">'.$chemical[1].' (Kgs)</td>
									<td class="listing-item">'.$chemical[2].'</td>
									<td class="listing-item">'.$chemical[3].'</td> 
								</tr>';
			}
			$chemenicalTable.= '</tbody></table>';
		}
		if(sizeof($equipmentRecords) > 0)
		{
			$equipmentTable.= '<table width="100%" bgcolor="#999999" cellspacing="1" cellpadding="3" id="tblHarvestingEquipment">';
			$equipmentTable.= '<tbody>';
			$equipmentTable.= '<tr bgcolor="#f2f2f2" align="center">
									<td class="listing-head">Equipment name</td>
			...
			{
				$equipmentTable.= '<tr bgcolor="#f2f2f2" align="center">
									<td class="listing-item">'.$equipment[0].'</td>
									<td class="listing-item">'.$equipment[1].' (Kgs)</td>
									<td class="listing-item">'.$equipment[2].'</td>
									<td class="listing-item">'.$equipment[3].'</td> 
								</tr>';
			}
			$equipmentTable.= '</tbody></table>';
		}
		// $objResponse->alert($chemenicalTable);
		// $objResponse->alert($equipmentTable);
		$objResponse->assign("tblWeighmentChemiclas", "innerHTML", $chemenicalTable);
		$objResponse->assign("tblWeighmentEquipments", "innerHTML", $equipmentTable);*/
		return $objResponse;
}
