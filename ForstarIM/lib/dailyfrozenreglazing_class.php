...
class DailyFrozenReGlazing
{
	/****************************************************************
	This class deals with all the operations relating to Daily Frozen Re-Glazing
	*****************************************************************/
	var $databaseConnect;

	//Constructor, which will create a db instance for this class
	function DailyFrozenReGlazing(&$databaseConnect)
	{
...
# Delete a Daily Frozen RePacking Grade Rec
	...
	function deleteFrozenReGlazingGradeRec($dailyFrozenRePackingMainId)
	{
		$qry	=	" delete from t_dailyfrozenreglazing_grade where main_id=$dailyFrozenRePackingMainId";
		//echo $qry;
	...
	function deleteFrozenReGlazingGradeRecRMLotID($dailyFrozenRePackingMainId)
	{
		$qry	=	"delete from t_dailyfrozenreglazing_grade_rmlotid where main_id=$dailyFrozenRePackingMainId";
		//echo $qry;
		$result	=	$this->databaseConnect->delRecord($qry);
		if ($result) {
			$this->databaseConnect->commit();
	...
	function deleteFrozenReGlazingEntryRecMain($entryId)
	{
	...
	function deleteFrozenReGlazingEntryRecMainRMLotID($entryId)
	{
	...
	function deleteFrozenReGlazingGradeRecMain($gradeEntryId)
	{
	...
	function deleteFrozenReGlazingGradeRecMainRMLotID($gradeEntryId)
	{
	...
	function deleteDailyFrozenReGlazingMainRecMain($dailyFrozenRePackingMainId)
	{
	...
	function deleteDailyFrozenReGlazingMainRecMainRMLotID($dailyFrozenRePackingMainId)
	{
...
/*	###olddata
	...
	{
		$qry="select id from t_dailyfrozenpacking_entry_rmlotid where main_id='$mainId'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?array($result[0][0]):array();
	}*/
	function getGlzEntryId($mainId)
	{
		$qry="select id from t_dailyfrozenreglazing_grade where main_id='$mainId'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?array($result[0][0]):array();
	}
...
/*	function getGradeEntryId($entryId)
	...
	{
		$qry="select id from t_dailyfrozenpacking_grade_rmlotid	 where entry_id='$entryId'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?array($result[0][0]):array();
	}
...
/*function checkRecordsExist($dailyFrozenPackingMainId){
	...
	function deleteDailyFrozenReGlazingMainRec($dailyFrozenRePackingMainId)
	{
		$qry	=	" delete from t_dailyfrozenreglazing where id=$dailyFrozenRePackingMainId";
		//echo $qry;
		$result	=	$this->databaseConnect->delRecord($qry);
		if ($result) {
			$this->databaseConnect->commit();
	...
	function deleteDailyFrozenReGlazingMainRecRMLotID($dailyFrozenRePackingMainId)
	{
		$qry	=	" delete from t_dailyfrozenreglazing_rmlotid where id=$dailyFrozenRePackingMainId";
			//echo $qry;
		$result	=	$this->databaseConnect->delRecord($qry);
		if ($result) {
			$this->databaseConnect->commit();
...
function getDFPForDateRange($fromDate, $tillDate, $offset, $limit)
	{
		$whr		= " a.select_date>='".$fromDate."' and a.select_date<='".$tillDate."' and reglaze_main_id is not null and tdfpg.number_mc>0";
		//$groupBy	= " b.processcode_id, b.freezing_stage_id, b.frozencode_id, b.mcpacking_id";
		$groupBy	= " b.processcode_id, b.freezing_stage_id, b.frozencode_id, b.mcpacking_id,a.flag";
		...
		$limit 		= " $offset, $limit";		
		$qry1 	= " select 
				a.id, a.select_date, b.fish_id, b.processcode_id, b.freezing_stage_id, b.frozencode_id, mpc.code as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, a.report_confirm, (select count(*) from t_dailyfrozenpacking_allocated_entry dfpae where b.id=dfpae.entry_id group by dfpae.entry_id) as allocatedCount, b.mcpacking_id, mcp.code as mcPkgCode,((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty, sum(tdfpg.number_mc) as numMcs, ((sum(tdfpg.number_mc)*mfp.decl_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.decl_wt)) as frozenQty,((sum(tdfpg.number_mc)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt)*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt))) as availableQty,a.physical_stock_main_id,sum(a.physical_stock_main_id),flag,a.processor_id,a.unit,sum(tdfpg.number_loose_slab) as numLSs,reglaze_main_id as reglazemainid,'0' as rmlotid, '0' as rmlotName  
			    from 
				t_dailyfrozenpacking_main a left join t_dailyfrozenpacking_entry b on a.id=b.main_id left join t_dailyfrozenpacking_grade tdfpg on b.id=tdfpg.entry_id 
				left join m_processcode mpc on mpc.id=b.processcode_id
				left join m_freezingstage mfs on mfs.id=b.freezing_stage_id
		...
		$qry2 	= " select 
				a.id, a.select_date, b.fish_id, b.processcode_id, b.freezing_stage_id, b.frozencode_id, mpc.code as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, a.report_confirm, (select count(*) from t_dailyfrozenpacking_allocated_entry dfpae where b.id=dfpae.entry_id group by dfpae.entry_id) as allocatedCount, b.mcpacking_id, mcp.code as mcPkgCode,((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty, sum(tdfpg.number_mc) as numMcs, ((sum(tdfpg.number_mc)*mfp.decl_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.decl_wt)) as frozenQty,((sum(tdfpg.number_mc)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt)*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt))) as availableQty,a.physical_stock_main_id,sum(a.physical_stock_main_id),flag,a.processor_id,a.unit,sum(tdfpg.number_loose_slab) as numLSs,reglaze_main_id as reglazemainid ,a.rm_lot_id as rmlotid ,concat(tmg.alpha_character,tmg.rm_lotid) as rmlotName  
			    from 
	...
	###commented on 7-11-2014
	/*function getDFPForDateRange($fromDate, $tillDate, $offset, $limit)
	{
		$whr1		= " a.select_date>='".$fromDate."' and a.select_date<='".$tillDate."' and reglaze_main_id is not null and tdfpg.number_mc>0";
		//$groupBy	= " b.processcode_id, b.freezing_stage_id, b.frozencode_id, b.mcpacking_id";
		$groupBy1	= " b.processcode_id, b.freezing_stage_id, b.frozencode_id, b.mcpacking_id,a.flag";
		
		$whr2		= " a.select_date>='".$fromDate."' and a.select_date<='".$tillDate."' and reglaze_main_id is not null and tdfpg.number_mc>0";
		//$groupBy	= " b.processcode_id, b.freezing_stage_id, b.frozencode_id, b.mcpacking_id";
		$groupBy2	= " b.processcode_id, b.freezing_stage_id, b.frozencode_id, b.mcpacking_id,a.flag,a.rm_lot_id";
		


		$orderBy	= "mcPkgCode asc, freezingStage asc, frznPkgCode asc";
		
		$limit 		= " $offset, $limit";		
		$qry1 	= " select 
				a.id, a.select_date, b.fish_id, b.processcode_id, b.freezing_stage_id, b.frozencode_id, mpc.code as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, a.report_confirm, (select count(*) from t_dailyfrozenpacking_allocated_entry dfpae where b.id=dfpae.entry_id group by dfpae.entry_id) as allocatedCount, b.mcpacking_id, mcp.code as mcPkgCode,((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty, sum(tdfpg.number_mc) as numMcs, ((sum(tdfpg.number_mc)*mfp.decl_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.decl_wt)) as frozenQty,((sum(tdfpg.number_mc)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt)*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt))) as availableQty,a.physical_stock_main_id,sum(a.physical_stock_main_id),flag,a.processor_id,a.unit,sum(tdfpg.number_loose_slab) as numLSs,reglaze_main_id as reglazemainid,'0' as rmlotId,'0' as rmlotIdNm
			    from 
				t_dailyfrozenpacking_main a left join t_dailyfrozenpacking_entry b on a.id=b.main_id left join t_dailyfrozenpacking_grade tdfpg on b.id=tdfpg.entry_id 
				left join m_processcode mpc on mpc.id=b.processcode_id
				left join m_freezingstage mfs on mfs.id=b.freezing_stage_id
		...
		$qry2 	= " select 
				a.id, a.select_date, b.fish_id, b.processcode_id, b.freezing_stage_id, b.frozencode_id, mpc.code as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, a.report_confirm, (select count(*) from t_dailyfrozenpacking_allocated_entry_rmlotid dfpae where b.id=dfpae.entry_id group by dfpae.entry_id) as allocatedCount, b.mcpacking_id, mcp.code as mcPkgCode,((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty, sum(tdfpg.number_mc) as numMcs, ((sum(tdfpg.number_mc)*mfp.decl_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.decl_wt)) as frozenQty,((sum(tdfpg.number_mc)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt)*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt))) as availableQty,a.physical_stock_main_id,sum(a.physical_stock_main_id),flag,a.processor_id,a.unit,sum(tdfpg.number_loose_slab) as numLSs,reglaze_main_id as reglazemainid,a.rm_lot_id as rmlotId,concat(tmgrm.alpha_character,tmgrm.rm_lotid) as rmlotIdNm  
			    from 
				t_dailyfrozenpacking_main_rmlotid a left join t_dailyfrozenpacking_entry_rmlotid b on a.id=b.main_id left join t_dailyfrozenpacking_grade_rmlotid tdfpg on b.id=tdfpg.entry_id 
				left join m_processcode mpc on mpc.id=b.processcode_id
				left join m_freezingstage mfs on mfs.id=b.freezing_stage_id
				left join m_frozenpacking mfp on mfp.id=b.frozencode_id
				left join m_mcpacking mcp on b.mcpacking_id=mcp.id
				left join t_manage_rm_lotid tmgrm on tmgrm.id=a.rm_lot_id
			";


		if ($whr!="") $qry1 .= " where ".$whr1;  $qry2 .= " where ".$whr2;
		if ($groupBy) $qry1 .= " group by ".$groupBy1;  $qry2 .= " group by ".$groupBy2;
...
/*function getDFPReForDateRange($fromDate, $tillDate)
	{
		...
		//$limit 		= " $offset, $limit";		
		$qry 	= " select 
				a.id, a.select_date, b.fish_id, b.processcode_id, b.freezing_stage_id, b.frozencode_id, mpc.code as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, a.report_confirm, (select count(*) from t_dailyfrozenpacking_allocated_entry dfpae where b.id=dfpae.entry_id group by dfpae.entry_id) as allocatedCount, b.mcpacking_id, mcp.code as mcPkgCode,((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty, sum(tdfpg.number_mc) as numMcs, ((sum(tdfpg.number_mc)*mfp.decl_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.decl_wt)) as frozenQty,((sum(tdfpg.number_mc)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt)*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt))) as availableQty,a.physical_stock_main_id,sum(a.physical_stock_main_id),flag,a.processor_id,a.unit,sum(tdfpg.number_loose_slab) as numLSs,reglaze_main_id as reglazemainid     
			    from 
				t_dailyfrozenpacking_main a left join t_dailyfrozenpacking_entry b on a.id=b.main_id left join t_dailyfrozenpacking_grade tdfpg on b.id=tdfpg.entry_id 
				left join m_processcode mpc on mpc.id=b.processcode_id
				left join m_freezingstage mfs on mfs.id=b.freezing_stage_id
...
*/
	function getDFPReForDateRange($fromDate, $tillDate)
	{
		...
		$whr2		= " a.select_date>='".$fromDate."' and a.select_date<='".$tillDate."' and reglaze_main_id is not null and tdfpg.number_mc>0 and a.rm_lot_id";
		//$groupBy	= " b.processcode_id, b.freezing_stage_id, b.frozencode_id, b.mcpacking_id";
		$groupBy2	= " b.processcode_id, b.freezing_stage_id, b.frozencode_id, b.mcpacking_id,a.flag,a.rm_lot_id";
	...
	function updateDailyFrozenPackingGrade($gradeUpid,$thawGrdTotal,$thawGrdLsTotal,$repkdQty)
	{
	...
	function updateDailyFrozenPackingEntry($mCPacking,$frozencodeid,$entryId)
	{
		
		//$qry = "update t_dailyfrozenpacking_entry  set  mcpacking_id='$mCPacking' where id='$entryId'";
		$qry = "update t_dailyfrozenpacking_entry  set  mcpacking_id='$mCPacking',frozencode_id='$frozencodeid' where id='$entryId'";
		//echo $qry;
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function updateDailyFrozenGlazingEntry($mCPacking,$frozencodeid,$entryId)
	{
		
		//$qry = "update t_dailyfrozenpacking_entry  set  mcpacking_id='$mCPacking' where id='$entryId'";
		$qry = "update t_dailyfrozenreglazing  set mcpacking_id='$mCPacking',frozencode_id='$frozencodeid' where id='$entryId'";
		//echo $qry;
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function updateDailyFrozenReglazingGradeEdit($numMC,$numLS,$gradeEntryId,$gradeId)
	{
	...
	function updateDailyFrozenReglazingGrade($numMC,$numLS,$entryId,$gradeEntryId)
	{
	...
	function deleteFrozenReGlazingGradeWithEntryId($entryId)
	{
	...
	function deleteFrozenReGlazingGradeRMLotIDWithEntryId($entryId)
	{
	...
	{
		$qry="select reglaze_frozen_id from t_dailyfrozenpacking_main where id='$mainId'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?array($result[0][0]):array();
	}
	...
	{
		$qry="select reglaze_frozen_id from t_dailyfrozenpacking_main_rmlotid where id='$mainId'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?array($result[0][0]):array();
	}
	function getEntryId($mainId)
	{
		$qry="select id,mcpacking_id,frozencode_id from t_dailyfrozenpacking_entry where main_id='$mainId'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return $result;
	}
	function getEntryIdRMLotID($mainId)
	{
		$qry="select id,mcpacking_id,frozencode_id from t_dailyfrozenpacking_entry_rmlotid where main_id='$mainId'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return $result;
	}
	function getGradeEntryId($entryId)
	{
		$qry="select id,grade_id,number_mc,number_loose_slab,rgzQty from t_dailyfrozenpacking_grade where entry_id='$entryId'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return $result;
	}
	function getGradeEntryIdRMLotID($entryId)
	{
		$qry="select id,grade_id,number_mc,number_loose_slab,rgzQty from t_dailyfrozenpacking_grade_rmlotid where entry_id='$entryId'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return $result;
	}
	...
	function getFrozenFilledWt($id)
	{
		$qry="select filled_wt from m_frozenpacking where id='$id'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?array($result[0][0]):array();
	...
	function updateFrozenPackingGradeReEnter($id, $gradeId, $numMC, $reglzNew)
	{
		$qry	=	" update t_dailyfrozenpacking_grade set number_mc='$numMC', rgzQty='$reglzNew' where id='$id' and grade_id='$gradeId'";
		//echo $qry;
		$result	=	$this->databaseConnect->updateRecord($qry);
		if ($result) {
			$this->databaseConnect->commit();
	...
	#update Frozen Re Packing Grades
	function updateFrozenPackingGradeReEnterRMLotID($id, $gradeId, $numMC,$reglzNew)
	{
		$qry	=	" update t_dailyfrozenpacking_grade_rmlotid set number_mc='$numMC',rgzQty='$reglzNew' where id='$id' and grade_id='$gradeId'";
		//echo $qry;
		$result	=	$this->databaseConnect->updateRecord($qry);
		if ($result) {
			$this->databaseConnect->commit();
