...
class IngredientRateMaster
{
	...
	*****************************************************************/
	...
	# Add a Record
	function addIngredientRate($selIngredient, $ingRatePerKg, $ingYield, $ingHighPrice, $ingLowPrice, $ingLastPrice, $ingRateList,$cleanedCost,$materialType, $userId)
	{
		$qry	= "insert into m_ingredient_rate (ingredient_id, rate_per_kg, yield, highest_price, lowest_price, last_price, rate_list_id,cleaned_cost,	material_type, created, createdby) values('$selIngredient', '$ingRatePerKg', '$ingYield', '$ingHighPrice', '$ingLowPrice', '$ingLastPrice', '$ingRateList','$cleanedCost','$materialType', Now(), '$userId')";
		//echo $qry;
	...
	function fetchAllPagingRecords($selRateList, $offset, $limit, $categoryFilterId, $mainCategoryFilterId)
	{
		...
		$qry = " select a.id, a.ingredient_id, a.rate_per_kg, a.yield, a.highest_price, a.lowest_price, a.last_price, a.rate_list_id, b.name,a.active,a.cleaned_cost,a.material_type from (m_ingredient_rate a, m_ingredient b, ing_category c) left join ing_main_category d on c.main_category_id=d.id ";
	...
	{
		$qry = "select id, ingredient_id, rate_per_kg, yield, highest_price, lowest_price, last_price, rate_list_id,cleaned_cost,material_type from m_ingredient_rate where id=$ingredientRateId";
		//echo $qry;
	...
	# Update  a  Record
	function updateIngredientRate($ingredientRateId, $selIngredient, $ingRatePerKg, $ingYield, $ingHighPrice, $ingLowPrice, $ingLastPrice, $ingRateList,$cleanedCost,$materialType)
	{
		$qry	= " update m_ingredient_rate set ingredient_id='$selIngredient', rate_per_kg='$ingRatePerKg', yield='$ingYield', highest_price='$ingHighPrice', lowest_price='$ingLowPrice', last_price='$ingLastPrice', rate_list_id='$ingRateList',cleaned_cost='$cleanedCost',material_type='$materialType' where id=$ingredientRateId ";
		//echo $qry;
