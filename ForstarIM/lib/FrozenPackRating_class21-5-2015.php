...
class FrozenPackRating
{
	...
	*****************************************************************/
	...
	function addSelGradeRec($ratingId,$entryId, $gradeId,$rate,$userId)
	{
		$qry	 = "insert into m_frzn_pkg_rate_grade (rating_id,pkg_rate_entry_id, grade_id, rate,created_by) values('$ratingId','$entryId', '$gradeId', '$rate','$userId')";
		//echo $qry.'<br/>';
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function getSelGradeRecords($userId, $ratingId,$pkgRateEntryId)
	{
	...
	*/
	...
	# get Grade Recs
	function getGradeRate($processCodeId,$fishId, $freezingStageId, $frozenCodeId, $qualityId,$rowId,$rateTag,$editGradeValue)
	{
		$table="";
															...
															<TD valign="top">
																<table cellpadding="0" cellspacing="0">
																	<tr>
																		...
																		<TD align="center">
																			<input type="text" name="exptRate" id="exptRate" size="3" value="'.$exptRate.'" style="text-align:right; font-size:11px;" autocomplete="off" '.$rateField.' />
																		</TD>
																		<td nowrap style="padding-left:10px;">
																			<input type="button" name="cmdAddExptRate" id="cmdAddExptRate" value="Save" onclick="addGrade('.$rowId.','.$processCodeId.','.$fishId.', '.$freezingStageId.','.$frozenCodeId.','.$qualityId.');" style=" font-size:11px;"/>
																...
																<table>
																	...
																	<TR>
																		...
																		$table.='<TR>
																				...
																				<td>
																					<input type="submit" name="cmdEdit" value="Edit" onClick="editGrade('.$processCodeId.','.$fishId.','.$freezingStageId.', '.$frozenCodeId.','.$qualityId.','.$rowId.','.$editgrades.');" style="text-align:right; font-size:11px;"/>
																				</td>
																			</TR>';
																		}
																	$table.='</tbody>
