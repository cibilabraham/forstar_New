...
Class DailyCatchReport
{
	...
	*****************************************************************/
	...
	{
		$qry	= "select * from (select distinct a.main_supplier as supplier, b.id, b.name as name from t_dailycatch_main a, supplier b where a.main_supplier=b.id and a.select_date>='".$fromDate."' and a.select_date<='".$tillDate."' and a.weighment_challan_no is not null and  a.main_supplier!='0' union all select distinct a.payment as supplier, b.id, b.name as name from t_dailycatch_main a, supplier b where a.payment=b.id and a.select_date>='".$fromDate."' and a.select_date<='".$tillDate."' and a.weighment_challan_no is not null and  a.payment!='') dum group by supplier  order by name asc";
		 //order by b.name asc
	...
	{
		$qry = " select id,weighment_challan_no, alpha_code from t_dailycatch_main  where select_date>='".$fromDate."' and select_date<='".$tillDate."' and weighment_challan_no is not null and billing_company_id='".$billingCompanyId."' and main_supplier='".$selectSupplier."'  or select_date>='".$fromDate."' and select_date<='".$tillDate."' and weighment_challan_no is not null and billing_company_id='".$billingCompanyId."' and payment='".$selectSupplier."' group by weighment_challan_no order by weighment_challan_no asc";
	...
	function fetchAlldailyCatchReportRecords($selWeighmentNo, $fromDate, $tillDate,$viewType)
	{
	...
	function fetchAllCatchReportRecords($challanMainId,$selViewType)
	{	
		
		$qry	= "select a.id, a.unit, a.entry_date, a.select_date, a.vechile_no, a.supplier_challan_no, a.weighment_challan_no, a.landing_center, a.main_supplier, b.ice_wt, a.sub_supplier, b.fish, b.fish_code, b.count_values, b.average, b.basket_wt, b.local_quantity, b.wastage, b.soft, b.reason, b.adjust, b.good, b.peeling, b.remarks, b.gross, b.total_basket, b.net_wt, b.actual_wt, b.effective_wt, b.decl_wt, b.decl_count, a.flag, b.select_weight, b.select_rate, b.actual_amount, b.paid, b.settlement_date, b.grade_id, b.reason_local, b.reason_wastage, b.reason_soft, b.entry_option, b.id,a.select_time,a.payment_by,a.confirm, b.grade_count_adj, b.grade_count_adj_reason, b.received_by, a.alpha_code, a.rmp_memo_print_count,a.payment,a.rm_lot_id from t_dailycatch_main a join t_dailycatchentry b on a.id=b.main_id join m_fish c on b.fish=c.id join m_processcode d on b.fish_code=d.id left join m_grade e on b.grade_id=e.id where a.id in ($challanMainId)";
		if($selViewType== "lot") $qry.=" and (a.rm_lot_id<>0  and a.rm_lot_id is not null)";
		$qry.= " order by c.name asc, d.code asc, b.count_values asc, e.code asc, b.effective_wt asc";			
		//echo "<br>$qry<br>";
		$result	=	$this->databaseConnect->getRecords($qry);
		return $result;
	}		
	...
	function filterDailyCatchEntryRecords($selectUnit, $landingCenterId, $selectSupplier, $fromDate, $tillDate, $fishId, $processId, $weighNumber, $challanMainId,$selViewType)
	{
		
		$criteriaSelection = "";

		if ($challanMainId!="") {
			$criteriaSelection = "a.id in ($challanMainId)"; //Single Date
		} else {
		...
		if($selViewType=="lot") $whr.=" and (a.rm_lot_id<>0  and a.rm_lot_id is not null)";
		$orderBy	= "a.weighment_challan_no asc, c.name asc, d.code asc, b.count_values asc, e.code asc, b.effective_wt asc";
		//echo $selViewType;
		$qry		= "select a.id, a.unit, a.entry_date, a.select_date, a.vechile_no, a.supplier_challan_no, a.weighment_challan_no, a.landing_center, a.main_supplier, b.ice_wt, a.sub_supplier, b.fish, b.fish_code, b.count_values, b.average, b.basket_wt, b.local_quantity, b.wastage, b.soft, b.reason, b.adjust, b.good, b.peeling, b.remarks, b.gross, b.total_basket, b.net_wt, b.actual_wt, b.effective_wt, b.decl_wt, b.decl_count, a.flag, b.select_weight, b.select_rate, b.actual_amount, b.paid, b.settlement_date, b.grade_id, b.reason_local, b.reason_wastage, b.reason_soft, b.entry_option, b.id, a.select_time, a.payment_by, a.confirm, b.grade_count_adj, b.grade_count_adj_reason, b.received_by, a.payment_confirm, a.payment_date, a.report_confirm, a.alpha_code,a.rm_lot_id from t_dailycatch_main a, t_dailycatchentry b, m_fish c, m_processcode d, m_grade e";
		//if($selViewType== "lot") $qry.=" and (a.rm_lot_id<>0  and a.rm_lot_id is not null)";
		if ($whr!="")		$qry   .=" where ".$whr;
		if ($orderBy!="") 	$qry   .=" order by ".$orderBy;
		//echo $qry;
	...
	#For Supplier Memo 
	function getSupplierDeclaredWtRecords($challanMainId, $selSuppChallanNo,$viewType)
	{	
		...
		$qry		= "select a.id, b.fish, b.fish_code, b.count_values, b.grade_id, b.effective_wt, b.received_by, e.supplier_challan_no, e.supplier_challan_date, e.decl_wt, e.decl_count, c.name, d.code, sum(e.decl_wt), b.remarks,a.rm_lot_id from t_dailycatch_main a, t_dailycatchentry b, m_fish c, m_processcode d, t_dailycatch_declared e";
		if ($whr!="") $qry   .=" where ".$whr;
		if($viewType=="lot") $qry.=" and (a.rm_lot_id<>0  and a.rm_lot_id is not null)";
	...
	function fetchBillingCompanyRecords($selectSupplier, $fromDate, $tillDate)
	{
		...
		if ($selectSupplier)
		{	$whr1= "$whr  and a.main_supplier='".$selectSupplier."'";
			$whr2= "$whr  and a.payment='".$selectSupplier."'";
		}
		$orderBy	= " a.weighment_challan_no asc";
			
		$qry = " select distinct b.id, b.display_name from t_dailycatch_main a, m_billing_company b  ";
			
		if ($whr!="" && $selectSupplier=="") $qry .= " where ".$whr;
		if ($whr!="" && $selectSupplier!="") $qry .= " where $whr1 or $whr2";;
		if ($orderBy!="")	$qry .= " order by ".$orderBy;
		//echo $qry;	
		$result	= $this->databaseConnect->getRecords($qry);
		return $result;
	...
	{
		$qry = "select b.id, b.display_name from t_dailycatch_main a, m_billing_company b where a.billing_company_id=b.id and a.weighment_challan_no='$weighNumber' group by b.name order by b.name asc";
		//echo $qry;
		$result = $this->databaseConnect->getRecords($qry);		
		return $result;
	}
	...
	function getSupplierLandCenter($supplierId)
	{
		$qry	= "SELECT ml.id as landid,ml.name as landName FROM `m_landingcenter` ml inner join m_supplier2center msc on ml.id=msc.center_id inner join supplier s on s.id=msc.supplier_id where msc.supplier_id='$supplierId'";
		//echo $qry;
		$result = $this->databaseConnect->getRecords($qry);		
		return $result;	
	}

	function getLotIDInReceipt($rmlotIDName)
	{
		$qry	= "SELECT vehicle_number_other FROM `t_rmreceiptgatepass` WHERE `lot_Id` REGEXP '(^|,)$rmlotIDName($|,)'";
		$result=  $this->databaseConnect->getRecords($qry);
		//echo $qry; 
		return $result;
	}
