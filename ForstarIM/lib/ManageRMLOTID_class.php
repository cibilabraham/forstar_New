...
Class ManageRMLOTID
{
	...
	###get rm lot id name(alpha& number)
	function getRMLotName($editId)
	{
		$qry="select concat(alpha_character,rm_lotid) from t_manage_rm_lotid where id='$editId' ";
		$result=$this->databaseConnect->getRecord($qry);
		//echo $qry;
		return $result[0];
	}
	...
	function chkValidGatePassId($selDate,$company,$unit)
	{
		//$selDate=Date('Y-m-d');
		//$selDate=mysqlDateFormat($selDate);
		$qry	="select id,start_no, end_no from number_gen where  date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate' or (end_date is null || end_date=0)) and auto_generate=1 AND TYPE = 'LF' and billing_company_id='$company' and ( unitid='$unit' || unitid='0') ";
	...
	{
		$qry = "select alpha_code from number_gen where date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate' or (end_date is null || end_date=0)) and auto_generate=1 AND TYPE = 'LF' and billing_company_id='$company' and ( unitid='$unit' || unitid='0') ";
		//echo $qry;
	...
	function getValidendnoGatePassId($selDate,$company,$unit)
	{
		
		//$selDate=Date('Y-m-d');
		//$selDate=mysqlDateFormat($selDate);
		$qry	= "select end_no from number_gen where date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate') and type='LF'  and billing_company_id='$company' and ( unitid='$unit' || unitid='0') ";
	...
	function getValidGatePassId($selDate,$company,$unit)
	{
		//$billingCompany=0;
		//$selDate=Date('Y-m-d');
		//$selDate=mysqlDateFormat($selDate);
		 $qry	= "select start_no from number_gen where date_format(start_date,'%Y-%m-%d')<='$selDate' and (date_format(end_date,'%Y-%m-%d')>='$selDate') and type='LF' and billing_company_id='$company' and ( unitid='$unit' || unitid='0')";
	...
	function getAvailableLotIdNos($company,$unit)
	{
	 	$qry = "SELECT id,start_no,end_no,alpha_code,current_no FROM number_gen WHERE type='LF' AND end_date >= '".date('Y-m-d')."' and billing_company_id='$company' and (unitid='$unit' || unitid='0' ) ORDER BY id ASC LIMIT 0,1 ";
		$result = $this->databaseConnect->fetch_array($qry);
		//echo $qry ;
	...
	function getAllRmlotIdOfSameCompanyUnit($companyId,$unitId,$editId)
	{
		...
		$qry	= "select id,CONCAT(alpha_character,rm_lotid) from  t_manage_rm_lotid where company_id='$companyId' and unit_id='$unitId' and id!='$editId'  and id not in (SELECT rm_lot_id  FROM `weighment_data_sheet` where active='1' )  and id not in (SELECT rmLotId  FROM `t_rmweightaftergrading` ) and id not in (SELECT rm_lot_id FROM `t_dailycatch_main`) and id not in (select lot_id_origin from t_manage_rm_lotid) and status='0' ";
		//echo $qry;
		return  $this->databaseConnect->getRecords($qry);
	}
