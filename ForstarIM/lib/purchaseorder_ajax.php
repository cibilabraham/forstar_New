...
//$objResponse->setReturnValue($chkRecExist); // Forretrun a value from ajax function
	...
	function getfishCodes($processId,$rowId, $cPCId)
	{
		$objResponse 	 = new NxajaxResponse();		
		$databaseConnect = new DatabaseConnect();
		$purchaseorderObj = new PurchaseOrder($databaseConnect);
		$processcodeObj = new ProcessCode($databaseConnect);
		//$objResponse->alert($processId);
		if($processId=="S")
		{
			$pcRecs		= $purchaseorderObj->getFishRecs();
		}
		else if($processId=="C")
		{
			$pcRecs		= $purchaseorderObj->getAssorted();
		}
		if (sizeof($pcRecs)>0) $objResponse->addDropDownOptions("selFish_$rowId", $pcRecs, $cPCId);
		return $objResponse;
	}

	# Get Process Codes	
	function getProcessCodes($fishId,$processId,$rowId, $cPCId)
	{
		$objResponse 	 = new NxajaxResponse();		
		$databaseConnect = new DatabaseConnect();
		$purchaseorderObj = new PurchaseOrder($databaseConnect);
	...
	# Get Grade Recs
	function getGradeRecs($processId,$processCodeId, $rowId, $selGradeId)
	{
		$objResponse 	 = new NxajaxResponse();
		$databaseConnect = new DatabaseConnect();
		$purchaseorderObj = new PurchaseOrder($databaseConnect);
		...
		{
			$gradeRecs		= $purchaseorderObj->getSecondaryFrozenGradeRecs($processCodeId);		
		}
	...
	function getRmLotId($fishId,$processCodeId,$gradeId,$rowId,$cel)
	{
		$objResponse 	 = new NxajaxResponse();
		$databaseConnect = new DatabaseConnect();
		...
		$objResponse->addDropDownOptions("selRMLotID_$rowId", $rmLotRecs, $cel);
		return $objResponse;
	}

	function getUnits($company)
	{
		...
		if($company!='0')
		{
			...
			if (sizeof($unitRecs)>0) addDropDownOptions("unit", $unitRecs,$cel,$objResponse);	
		}
		return $objResponse;
	}
	...
	function getPONumber($selDate,$compId,$invUnit)
	{
		...
		$databaseConnect 		= new DatabaseConnect();
		$purchaseorderObj = new PurchaseOrder($databaseConnect);
		$checkGateNumberSettingsExist=$purchaseorderObj->chkValidGatePassId($selDate,$compId,$invUnit);
		if (sizeof($checkGateNumberSettingsExist)>0){
		$alpId=$checkGateNumberSettingsExist[0][0];
		$alphaCode=$purchaseorderObj->getAlphaCode($alpId);
		...
		$checkExist=$purchaseorderObj->checkGatePassDisplayExist($compId,$invUnit);
		if ($checkExist>0){
		$getFirstRecord=$purchaseorderObj->getmaxGatePassId($compId,$invUnit);
		...
		$validendno=$purchaseorderObj->getValidendnoGatePassId($selDate,$compId,$invUnit);
		...
		$validPassNo=$purchaseorderObj->getValidGatePassId($selDate,$compId,$invUnit);	
		$checkPassId=$purchaseorderObj->chkValidGatePassId($selDate,$compId,$invUnit);
...
$xajax->register(XAJAX_FUNCTION, 'getfishCodes', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));
$xajax->register(XAJAX_FUNCTION, 'getPONumber', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));
$xajax->register(XAJAX_FUNCTION, 'getUnits', array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));
