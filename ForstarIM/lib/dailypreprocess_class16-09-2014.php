...
Class DailyPreProcess
{
	...
	*****************************************************************/
	...
	function addDailyPreProcess($fishId,$selectDate,$processCode)
	{
		$qry	= " insert into t_dailypreprocess (fish_id,process_id,date) values('$fishId','$processCode','$selectDate')";
		//echo $qry;
	...
	function getProcessCodeFish($fishId,$recordSelectDate,$selRateListId)
	{	
	
		$qry1="SELECT id,code FROM m_processcode  WHERE fish_id ='".$fishId."' and active='1' and id not in (select SUBSTRING_INDEX(a.processes,',',1) as pfrom from m_process a left join m_fish b on a.fish_id=b.id join pre_process_sequence frs on frs.processcode_id = substring(a.processes,1,instr(a.processes,',')-1) join pre_process_sequence tos on tos.processcode_id = substring(a.processes,length(a.processes)-instr(reverse(a.processes),',')+2) left join  t_dailypreprocess_entries c on c.process=a.id  left join t_dailypreprocess d on d.id=c.dailypreprocess_main_id where a.fish_id='$fishId' and a.rate_list_id='$selRateListId' and d.date='$recordSelectDate' )";
		$array1 = $this->databaseConnect->getRecords($qry1);
	
	
		$qry2=" SELECT id,code FROM m_processcode WHERE fish_id ='$fishId' and active='1' and id not in (select SUBSTRING_INDEX(a.processes,',',1) as pfrom from m_process a left join m_fish b on a.fish_id=b.id join pre_process_sequence frs on frs.processcode_id = substring(a.processes,1,instr(a.processes,',')-1) join pre_process_sequence tos on tos.processcode_id = substring(a.processes,length(a.processes)-instr(reverse(a.processes),',')+2) left join t_dailypreprocess_entries_rmlotid c on c.process=a.id left join t_dailypreprocess_rmlotid d on d.id=c.dailypreprocess_main_id where a.fish_id='$fishId' and a.rate_list_id='$selRateListId' and d.date='$recordSelectDate')";
		$array2 = $this->databaseConnect->getRecords($qry2);
		$result_array =array_merge_recursive($array1, $array2);
		$result=array_diff_assoc( $result_array, array_map('unserialize', array_unique(array_map('serialize', $result_array))));
		//$resultArr=array_map("unserialize", array_unique(array_map("serialize", $result)));
		return $result;
		
	//$res=array_diff_assoc( $result_array, array_unique($result_array) );	
	//printr(array_map('unserialize', array_unique(array_map('serialize', $result_array))));
	//printr($value);
	
	
	//if exist in both table save value to an array
	
	//$qry=$qry1." union ".$qry2;
	
		//$qry="SELECT id,code FROM m_processcode  WHERE fish_id ='".$fishId."' and active='1'";
	//echo	$qry="SELECT id,code FROM m_processcode  WHERE fish_id ='".$fishId."' and active='1' and id not in (SELECT a.process from t_dailypreprocess_entries a left join t_dailypreprocess b on a.dailypreprocess_main_id=b.id where b.date='$recordSelectDate' union SELECT a.process from t_dailypreprocess_entries_rmlotid a left join t_dailypreprocess_rmlotid b on a.dailypreprocess_main_id=b.id where b.date='$recordSelectDate') ";
		//echo $qry;
		//$results = array();
		//$results = $this->databaseConnect->getRecords($qry);
		//return $results;
	}
	...
	function findPreProcessorRecInTable($dailyPreProcessEntryId, $masterPreProcessorId)
	{
		...
		union 
		select id, dailypreprocess_entry_id, preprocessor_id, preprocess_qty, paid, payment_confirm  from t_dailypreprocess_processor_qty_rmlotid where  dailypreprocess_entry_id=$dailyPreProcessEntryId and preprocessor_id='$masterPreProcessorId' 
		";
		//echo $qry;
	...
	{
		$qry	= "select a.id, a.fish_id,a.process_id,a.date from t_dailypreprocess a 
	...
	{
		$qry	= "select a.id,a.rm_lot_id,a.fish_id,a.process_id,a.date from t_dailypreprocess_rmlotid a left join t_dailypreprocess_entries_rmlotid b on b.dailypreprocess_main_id=a.id 
				   where a.id=$dailyPreProcessId";
		// echo $qry;		
		$result	=	$this->databaseConnect->getRecord($qry);
		return $result;
	...
	function chkDuplicateEntryExist($fishId,$processCode,$selectDate, $dailyPreProcessId)
	{
		if ($dailyPreProcessId!="") $uptdQry = " and id!='$dailyPreProcessId' ";
		else $uptdQry = "";
		if($processCode!="") $uptdQry.= " and process_id='$processCode'  or process_id='0'";
	...
	*/
	...
	function chkDuplicateEntryExistInTable($lotId,$fishId,$processCode,$selectDate, $dailyPreProcessId,$selRateListId)
	{
		$uptdQry1="";
		...
		$qry1	= "select a.id, a.rm_lot_id as lot, a.date, a.confirmed from t_dailypreprocess_rmlotid a left join t_dailypreprocess_entries_rmlotid b on a.id=b.dailypreprocess_main_id where  fish_id='$fishId' and date='$selectDate' $uptdQry1";			
		
		$qry2	= "select a.id, 'not' as lot, date, confirmed from t_dailypreprocess
		a left join t_dailypreprocess_entries b on a.id=b.dailypreprocess_main_id	where  fish_id='$fishId' and date='$selectDate' $uptdQry";			
	...
	###commented on 15-09-2014
	//select a.id, a.rm_lot_id, a.date, a.confirmed,b.process from t_dailypreprocess_rmlotid a left join t_dailypreprocess_entries_rmlotid b on a.id=b.dailypreprocess_main_id where a.date='2014-09-15' and b.process in (select a.id from m_process a left join m_fish b on a.fish_id=b.id join pre_process_sequence frs on frs.processcode_id = substring(a.processes,1,instr(a.processes,',')-1) join pre_process_sequence tos on tos.processcode_id = substring(a.processes,length(a.processes)-instr(reverse(a.processes),',')+2) where a.fish_id='36' )
	
	
	/*function chkDuplicateEntryExistInTable($lotId,$fishId,$processCode,$selectDate, $dailyPreProcessId)
	{
		$uptdQry1="";
		//select id, rm_lot_id, date, confirmed from t_dailypreprocess_rmlotid where rm_lot_id='10' and fish_id='36' and date='2014-09-13' union select id,"not" as rm_lot_id, date, confirmed from t_dailypreprocess where  fish_id='36' and date='2014-09-13'
		if ($dailyPreProcessId!="") $uptdQry = " and id!='$dailyPreProcessId' ";
		...
		if($lotId!="") $uptdQry1.= " and rm_lot_id='$lotId' ";
		$uptdQry1=$uptdQry.$uptdQry1;
		
		$qry1	= "select id, rm_lot_id, date, confirmed from t_dailypreprocess_rmlotid where  fish_id='$fishId' and date='$selectDate' $uptdQry1";			
		
		$qry2	= "select id, 'not' as rm_lot_id, date, confirmed from t_dailypreprocess where  fish_id='$fishId' and date='$selectDate' $uptdQry";			
	...
	function getDPPExistRecInTable($selDate, $fishId, $processId, $lanCenterId,$rmlotid,$processFrom)
	{	$whr1="";
		...
		$qry1 = "select dpp.id, dppe.id, dppe.opening_bal_qty, dppe.arrival_qty, dppe.total_qty, dppe.total_preprocess_qty, dppe.actual_yield, dppe.ideal_yield, dppe.diff_yield, dppe.center_id, dppe.available_qty from 
		t_dailypreprocess dpp left join t_dailypreprocess_entries dppe on dpp.id =dppe.dailypreprocess_main_id";
		
		if ($whr) $qry1 .= " where ".$whr;
	
		if($rmlotid!="")
		{
		$whr2=" and dpp.rm_lot_id='$rmlotid'";
		$whr1.=$whr.$whr2;
		}
		
		$qry2 = "select dpp.id, dppe.id, dppe.opening_bal_qty, dppe.arrival_qty, dppe.total_qty, dppe.total_preprocess_qty, dppe.actual_yield, dppe.ideal_yield, dppe.diff_yield, dppe.center_id, dppe.available_qty from 
			t_dailypreprocess_rmlotid dpp left join t_dailypreprocess_entries_rmlotid dppe on dpp.id =dppe.dailypreprocess_main_id";
		if($whr1) $qry2.= " where ".$whr1;
		elseif ($whr) $qry2.= " where ".$whr;
	...
	# Get all Pre-Process rec
	function getDPPExistRecLotID($selDate, $fishId, $processId, $lanCenterId,$rmlotid,$processFrom)
	{
	...
	function preProcessRecs($filterId,$processCode,$selRateList,$rmLotId)
	{	
	...
	function chkDaysEntryConfirmedRmLot($selectDate)
	{
		$qry	= "select id, confirmed from t_dailypreprocess_rmlotid  where date='$selectDate' and confirmed='Y'";
		//echo $qry;		
		$result	= $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?true:false;
	}
	...
	function addDailyPreProcessRmLOtID($rmLotId,$fishId,$selectDate,$processCodeLotid)
	{
		$qry	= "insert into t_dailypreprocess_rmlotid(rm_lot_id,fish_id,process_id,date) values('$rmLotId','$fishId','$processCodeLotid','$selectDate')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	function getProcessCodeRMLot($fishId,$rmLotId,$entryDate,$selRateListId)
	{
		$qry	= "SELECT id ,code FROM `m_processcode` where id in (SELECT process_code_id FROM `t_rmweightaftergradingdetails` a left join t_rmweightaftergrading b on a.weightment_grading_id=b.id left join t_dailypreprocess_entries_rmlotid c on c.process=a.id left join t_dailypreprocess_rmlotid d on d.id=c.dailypreprocess_main_id where a.fish_id='$fishId' and a.rate_list_id='$selRateListId' and d.date='$entryDate' and b.rmLotId='$rmLotId' )";
