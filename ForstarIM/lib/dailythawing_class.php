...
class DailyThawing
{
	...
	*****************************************************************/
	...
	function getEditDateRMLotid($mainId)
	{
	$qry = "select select_date from t_dailythawing_rmlotid where id='$mainId'";
		//echo $qry;
	return $this->databaseConnect->getRecord($qry);
	...
	function insertDailyThawing($selectDate,$processId,$freezingStageId,$frozenCodeId,$MCPkgId,$fish_id)
	{
		$qry	= "insert into t_dailythawing(select_date,processcode_id,freezing_stage_id,frozencode_id,mcpacking_id,fish_id) values ('$selectDate',$processId,$freezingStageId,$frozenCodeId,$MCPkgId,$fish_id)";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	function insertDailyThawingRMLotID($selectDate,$processId,$freezingStageId,$frozenCodeId,$MCPkgId,$rmlotid,$fish_id)
	{
		$qry	= "insert into t_dailythawing_rmlotid(rm_lot_id,select_date,processcode_id,freezing_stage_id,frozencode_id,mcpacking_id,fish_id) values ('$rmlotid','$selectDate','$processId','$freezingStageId','$frozenCodeId','$MCPkgId','$fish_id')";
		echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	function insertDFPPOGradeForThawing($dfpPOEntryId, $gradeId, $numMC,$netWt)
	{
		$qry	= "insert into t_dailythawing_grade(main_id,grade_id,number_mc_thawing,number_mc_stock) values('$dfpPOEntryId', '$gradeId', '$numMC', '$netWt')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	function insertDFPPOGradeForThawingRMLotID($dfpPOEntryId, $gradeId, $numMC, $netWt)
	{
		$qry	= "insert into t_dailythawing_grade_rmlotid(main_id,grade_id,number_mc_thawing,number_mc_stock) values('$dfpPOEntryId', '$gradeId', '$numMC', '$netWt')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	function getThaGradeQtyRMLot($processId,$freezingStage,$frozenCode,$stkAllocateMCPkgId,$selectDate,$sGradeId,$editId,$rmlotID)
	{
	//$qry="select sum(number_mc_thawing) from t_dailythawing td left join t_dailythawing_grade tdg on td.id=tdg.main_id where processcode_id='$processId' and freezing_stage_id='$freezingStage' and frozencode_id='$frozenCode' and select_date='$selectDate' and mcpacking_id='$stkAllocateMCPkgId' and grade_id='$sGradeId' and td.id='$editId'";

	$qry="select sum(number_mc_thawing) from t_dailythawing_rmlotid td left join t_dailythawing_grade_rmlotid tdg on td.id=tdg.main_id where processcode_id='$processId' and freezing_stage_id='$freezingStage' and frozencode_id='$frozenCode' and select_date='$selectDate' and mcpacking_id='$stkAllocateMCPkgId' and grade_id='$sGradeId' and rm_lot_id='$rmlotID'";
		//echo "^^^^".$qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?array($result[0][0]):array();	
	
	}
	function getThaFreeGradeQty($processId, $freezingStage, $frozenCode,$stkAllocateMCPkgId,$selectDate,$thaweditDate,$sGradeId,$editId)
	...
	function getThaFreeGradeQtyRMLotID($processId, $freezingStage, $frozenCode,$stkAllocateMCPkgId,$selectDate,$thaweditDate,$sGradeId,$editId,$rmlotID)
	{	//echo $rmlotID;
		$qry="select sum(tdg.number_mc_thawing) from t_dailythawing_rmlotid td left join t_dailythawing_grade_rmlotid tdg on td.id=tdg.main_id where td.processcode_id='$processId' and td.freezing_stage_id='$freezingStage' and td.frozencode_id='$frozenCode' and td.select_date>='$selectDate' and   td.mcpacking_id='$stkAllocateMCPkgId' and tdg.grade_id='$sGradeId' and td.rm_lot_id='$rmlotID'";
	...
	function getThadtGradeQtyRMLotID($processId, $freezingStage, $frozenCode,$mCPacking,$selectDate,$thaweditDate,$sGradeId,$rmlotID)
	{
		$qry="select sum(tdg.number_mc_thawing) from t_dailythawing_rmlotid td left join t_dailythawing_grade_rmlotid tdg on td.id=tdg.main_id where td.processcode_id='$processId' and td.freezing_stage_id='$freezingStage' and td.frozencode_id='$frozenCode' and td.select_date='$thaweditDate' and td.mcpacking_id='$mCPacking' and tdg.grade_id='$sGradeId' and td.rm_lot_id='$rmlotID'";
		//echo "^^^^".$qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?array($result[0][0]):array();
	}
	...
	function getThadelEntryId($selProcessCodeId,$selFreezingStageId,$selFrozenCodeId,$frznStkMCPkgId,$delDate)
	{
		...
		$qry		=	"select id,linked_newrm from t_dailythawing where processcode_id='$selProcessCodeId' and freezing_stage_id='$selFreezingStageId' and frozencode_id='$selFrozenCodeId' and mcpacking_id='$frznStkMCPkgId' and select_date='$delDate'";
	...
	function getThadelEntryIdRMLotid($selProcessCodeId,$selFreezingStageId,$selFrozenCodeId,$frznStkMCPkgId,$delDate,$hidrmlotID)
	{
			
		$qry		=	"select id from t_dailythawing_rmlotid where processcode_id='$selProcessCodeId' and freezing_stage_id='$selFreezingStageId' and frozencode_id='$selFrozenCodeId' and mcpacking_id='$frznStkMCPkgId' and select_date='$delDate' and rm_lot_id='$hidrmlotID'";
	...
	function getThadelGradeIdRMLotID($Id)
	{
			
		$qry		=	"select id,main_id from t_dailythawing_grade_rmlotid where main_id='$Id'";
	...
	{
		//$whr		= "select_date>='".$fromDate."' and select_date<='".$tillDate."' and flag=1" ;
		$whr1		= "select_date>='".$fromDate."' and select_date<='".$tillDate."' and flag=1" ;
		$groupBy1	= "td.processcode_id, td.freezing_stage_id, td.frozencode_id, td.mcpacking_id,td.select_date";

		$whr2		= "select_date>='".$fromDate."' and select_date<='".$tillDate."' and flag=1 " ;
		$groupBy2	= "td.processcode_id, td.freezing_stage_id, td.frozencode_id, td.mcpacking_id,td.select_date, td.rm_lot_id";

		
		$orderBy	= "select_date asc";
		

		$qry1		= "select td.id, td.fish_id, td.processcode_id, td.freezing_stage_id, td.eucode_id, td.brand_id, td.frozencode_id, td.mcpacking_id, td.select_date as select_date,sum(tdg.number_mc_thawing) as thawStock,tdg.grade_id,tdg.id,sum(tdg.number_mc_stock) as stocksum ,'0' as rmlotid,'0' as  rmlotName from t_dailythawing td left join t_dailythawing_grade tdg on td.id=tdg.main_id";
		
		$qry2		= "select td.id, td.fish_id, td.processcode_id, td.freezing_stage_id, td.eucode_id, td.brand_id, td.frozencode_id, td.mcpacking_id, td.select_date as select_date,sum(tdg.number_mc_thawing) as thawStock,tdg.grade_id,tdg.id,sum(tdg.number_mc_stock) as stocksum ,td.rm_lot_id as rmlotid,concat(tmr.alpha_character,tmr.rm_lotid) from t_dailythawing_rmlotid td left join t_dailythawing_grade_rmlotid tdg on td.id=tdg.main_id left join t_manage_rm_lotid tmr on tmr.id=td.rm_lot_id ";
		
		
		if ($whr1!="")		$qry1   .=" where ".$whr1; if ($whr2!="") $qry2   .=" where ".$whr2;
		if ($groupBy1) $qry1 .= " group by ".$groupBy1; if ($groupBy2) $qry2 .= " group by ".$groupBy2;
	...
	function fetchLogRecordsRMLot($mainId)
	{
		//$whr		= "select_date>='".$fromDate."' and select_date<='".$tillDate."' and flag=1" ;
		$whr		= "td.id='$mainId'" ;		

		$qry		= "select td.id, fish_id, processcode_id, freezing_stage_id, eucode_id, brand_id, frozencode_id, mcpacking_id, select_date,number_mc_thawing,grade_id from t_dailythawing_rmlotid td left join t_dailythawing_grade_rmlotid tdg on td.id=tdg.main_id";
	...
	# Get Thawing Record  based on id
	function findRMLot($dailyThawingId)
	{
		//$qry	= "select id, fish_id, processcode_id, freezing_stage_id, eucode_id, brand_id, frozencode_id, mcpacking_id, select_date, customer_id, brand_from from t_dailythawing where id='$dailyThawingId'";
		$qry	= "select tdtr.id,tdtr.fish_id, tdtr.processcode_id, tdtr.freezing_stage_id, tdtr.eucode_id, tdtr.brand_id, tdtr.frozencode_id, tdtr.mcpacking_id, tdtr.select_date,concat(mg.alpha_character,mg.rm_lotid) as rm_lot_id from t_dailythawing_rmlotid tdtr left join t_manage_rm_lotid mg on tdtr.rm_lot_id= mg.id  where tdtr.id='$dailyThawingId'";
	//echo $qry;
		return $this->databaseConnect->getRecord($qry);
	}
	...
	function getAllocateProductionRMLotIDRecs($fromDate, $tillDate, $processCodeId, $freezingStageId, $frozenCodeId, $mcPkgId,$rmlotID)
	{
		...
		left join m_mcpacking mcp on tdfpe.mcpacking_id=mcp.id 
		left join t_dailyfrozenpacking_po_rmlotid dfppo ON dfppo.entry_id=tdfpe.id  
		where tdfpe.processcode_id='$processCodeId' and tdfpe.freezing_stage_id='$freezingStageId' and tdfpe.frozencode_id='$frozenCodeId' and tdfpe.mcpacking_id='$mcPkgId' and dfppo.rm_lot_id='$rmlotID' and tdfpm.rm_lot_id='$rmlotID'
		 limit 0,1";
	...
	function getAvailablePacksRMLotID($processCodeId, $freezingStageId, $frozenCodeId, $gradeId, $mcPkgId,$fromDate,$tillDate,$rmlotID)
	{
...
# Delete a Daily Thawing Grade Rec
	...
	function insDailyfrozenpackingAllocRMLotId($processcodeid,$freestid,$frozid,$mcpid,$gradeid,$numc,$purchaseOrderId,$id,$type,$invoiceId,$userId,$rmlotID)
	{
		$date=Date(Y-m-d);
		$qry="insert into t_frozenpacking_log(processcode_id,freezingstage_id,frozencode_id,mcpacking_id,purchase_order_id,grade_id,num_mc,type,invoice_id,date_allocated,modified_by,rm_lot_id) values ('$processcodeid','$freestid','$frozid','$mcpid','$purchaseOrderId','$gradeid','$numc','$type','$invoiceId',now(),'$userId','$rmlotID')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function deleteDailyThawingGradeRMLotIDRec($dailyThawingMainId)
	{
		$qry	=	" delete from t_dailythawing_grade_rmlotid where id=$dailyThawingMainId";
		//echo $qry;
	...
	# Delete a Daily Thawing Main Rec
	function deleteDailyThawingMainRMLotIDRec($dailyThawingMainId)
	{
		$qry	= " delete from t_dailythawing_rmlotid where id=$dailyThawingMainId";
		//echo $qry;
		$result	= $this->databaseConnect->delRecord($qry);
		if ($result)  $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
...
//------------------------ Delete End------------------------------
	...
	function fetchFrozenGradeRMLotIDRecords($codeId, $mainId)
 	{
		$qry	=	"select a.grade_id, c.code, b.id, b.main_id, b.number_mc_thawing, b.number_loose_slab_thawing, b.number_mc_stock, b.number_loose_slab_stock from m_processcode2grade a left join t_dailythawing_grade_rmlotid b on a.grade_id=b.grade_id and b.main_id='$mainId', m_grade c where a.grade_id = c.id and a.processcode_id='$codeId' and a.unit_select='f' order by c.code asc";
		//echo $qry;		
		$result	= $this->databaseConnect->getRecords($qry);
		return $result;
	}
	...
	function getNewRm($processCode,$freezingStage,$frozenCode,$mCPackingCode,$entryDate)
	{
		$qry="select concat(b.alpha_character,b.rm_lotid) FROM `t_dailythawing` a left join t_manage_rm_lotid b on a.linked_newrm=b.id where processcode_id='$processCode' and freezing_stage_id='$freezingStage' and frozencode_id='$frozenCode' and mcpacking_id='$mCPackingCode'  and linked_newrm >0 and select_date='$entryDate'";
		//echo $qry;
		//die();
		$result	=	array();
		$result	=	$this->databaseConnect->getRecords($qry);
	...
	function deleteRmLot($id)
	{
		$qry	=	" delete from t_manage_rm_lotid where id='$id'";
		//echo $qry;
		$result	=	$this->databaseConnect->delRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
