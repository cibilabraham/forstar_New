...
{  
	...
	*****************************************************************/
	...
	function addUserDetail($lastId,$company,$unit,$department)
	{
		$qry	= "insert into user_details (user_id,company_id,unit_id,department_id) values('".$lastId."','".$company."','".$unit."','".$department."')";
	...
	{
		$qry	= "select * from user order by username";
	...
	###UPDATE USER DETAILS
	function updateUserDetail($userEditId,$company,$unit,$department,$editUserDetail)
	{
		$qry	= " update user_details set user_id='$userEditId',company_id='$company',unit_id='$unit',department_id='$department' where id=$editUserDetail";
 		//echo $qry;
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
