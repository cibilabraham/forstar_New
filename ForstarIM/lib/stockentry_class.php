	...
	*****************************************************************/
	...
	function addStock($code, $name, $selCategory, $selSubCategory, $reOrder, $descr, $stockType, $brand, $unit, $quantity, $size, $dimension, $weight, $color, $made, $layer, $carton, $packingBrand, $packingColor, $packingWeight, $packing, $numColors, $packingDimension, $cartonWeight, $active, $userId, $selFrozenCode, $packingKg, $reorderRequired, $basicUnitQty, $minOrderUnit, $minOrderQtyPerUnit, $brandType, $modelNo, $dimensionLength,  $dimensionBreadth, $dimensionHeight, $dimensionDiameter, $dimensionRadius, $particularsDescription, $additionalHoldingPercent, $stockingPeriod,$tolerancelevel,$plantunit,$frozen,$rte)
	{
		...
		if ($stockType=='P') {
			...
			$qry="insert into m_stock (code, name, category_id, subcategory_id, reorder, description, quantity, stock_type, layer, carton, packing_brand, packing_color, packing,packing_dimension, carton_weight, active, created, createdby, actual_quantity, packing_kg, additional_holding_percent, stocking_period, reorder_required,tolerance_level,plant_unit,frozen,rte) values('$code', '$name', '$selCategory', '$selSubCategory', '$reOrder', '$descr', '$quantity', '$stockType', '$layer', '$carton', '$packingBrand', '$packingColor', '$packing','$packingDimension', '$cartonWeight', '$active', Now(), '$userId', '$quantity', '$packingKg', '$additionalHoldingPercent', '$stockingPeriod', '$reorderRequired','$tolerancelevel','$plantunit','$frozen','$rte')";
		} else { # Ordinary section	
			$qry	= "insert into m_stock (code, name, category_id, subcategory_id, reorder, description, stock_type, brand, unit, quantity, size, dimension, weight, color, made, active, created, createdby, actual_quantity, reorder_required, basic_unit_qty, min_order_unit, min_order_qty_per_unit, brand_type, model_no, dim_length, dim_breadth, dime_height, dim_diameter, dim_radius, particulars_description, additional_holding_percent, stocking_period,plant_unit,frozen,rte) values('$code', '$name', '$selCategory', '$selSubCategory', '$reOrder', '$descr', '$stockType', '$brand', '$unit', '$quantity', '$size', '$dimension', '$weight', '$color', '$made', '$active', Now(), '$userId', '$quantity', '$reorderRequired', '$basicUnitQty', '$minOrderUnit', '$minOrderQtyPerUnit', '$brandType', '$modelNo', '$dimensionLength',  '$dimensionBreadth', '$dimensionHeight', '$dimensionDiameter', '$dimensionRadius', '$particularsDescription', '$additionalHoldingPercent', '$stockingPeriod','$plantunit','$frozen','$rte')";
		}
		//echo $qry;
...
$qry = " select a.id,a.company_id, a.plant_unit,a.actual_quantity from m_stock_plantunit a where a.stock_id=$stockId"; 
		//echo $qry;
		$result = $this->databaseConnect->getRecords($qry);
		return $result;
...
function updateStockFromUnit($stockIdFrom,$qty,$unitFrom,$supplier)
{
	$cDate = date("Y-m-d");
	$query="select id from supplier_stock where stock_id='$stockIdFrom' and supplier_id='$supplier' and  (('$cDate'>=start_date && (end_date is null || end_date=0)) or ('$cDate'>=start_date and '$cDate'<=end_date))";
	$res	=	$this->databaseConnect->getRecord($query);
	if(sizeof($res)>0)
	{
		$supplierStockId=$res[0];
		$query1="select id,stock_quantity from supplier_stock_quantity where supplierstock_id='$supplierStockId' and supplier_id='$supplier' and stock_id='$stockIdFrom'";
		$supplierDetail	=	$this->databaseConnect->getRecords($query1);
...
$result	=	array();
	...
	{
		$qry="select st.id,st.date,ms.name,mbc.display_name,mp.name,mbcp.display_name,mpt.name,st.quantity from stock_transfer st left join m_stock ms on st.item=ms.id left join m_billing_company mbc on mbc.id=st.from_company  left join m_plant mp on mp.id=st.fromunit  left join m_billing_company mbcp on mbcp.id=st.to_company left join m_plant mpt on mpt.id=st.tounit";
		//echo $qry;
		//$qry="select mfp.id,tfqe.name from t_fznpakng_quick_entry tfqe left join m_frozenpacking mfp on  tfqe.frozencode_id=mfp.id left join m_stock2frozencode ms on mfp.id=ms.frozencode_id where expiry_date is null and ms.stock_id='$stockid'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
	...
	function addStockTransfer($fromCompany,$unitFrom,$toCompany,$unitTo,$item,$supplier,$quantity,$date)
	{
		//$qry="insert into stock_transfer(fromunit,tounit,item,quantity,date) values ('$unitFrom','$unitTo','$item','$quantity','$date')";
		$qry="insert into stock_transfer(from_company,fromunit,to_company,tounit,item,quantity,date,supplier) values ('$fromCompany','$unitFrom','$toCompany','$unitTo','$item','$quantity','$date','$supplier')";
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();			
		else $this->databaseConnect->rollback();		
		return $insertStatus;
	...
	function addUnitStock($companyId,$unitId,$stkIdVal)
	{
		$qry="insert into m_stock_plantunit(stock_id,company_id,plant_unit) values('$stkIdVal','$companyId','$unitId')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();			
		else $this->databaseConnect->rollback();		
	...
	function updateUnitStock($companyId,$punitId,$stockqtyid)
	{
		$qry	= "update m_stock_plantunit set company_id='$companyId',plant_unit='$punitId' where id='$stockqtyid'";
		//echo $qry;
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
...
*/
	...
	###getting the total quantity for a stock in supplier stock
	function getCurrentStockQty($stockId)
	{
		$qry = "select sum(stock_quantity) from supplier_stock_quantity where stock_id='$stockId'";
		//echo $qry;		
	...
	{
		$qry = "select id from m_stock where name='$name'";
		//echo $qry;		
		$result	= $this->databaseConnect->getRecord($qry);
	...
	###get company and unit alloted for this item exist
	function getCompanyUnit($stockid)
	{
		$qry = "select mbc.display_name,mp.name,spu.company_id,spu.plant_unit from m_stock_plantunit spu left join m_billing_company mbc on spu.company_id=mbc.id left join m_plant mp on spu.plant_unit=mp.id where stock_id='$stockid' order by mbc.display_name,mp.name";
		//echo $qry;		
		$result	= $this->databaseConnect->getRecords($qry);
		return $result;
	}
	...
	function getAllSupplierInPo($company,$unit,$item)
	{
		$qry="select s.id,s.name from m_purchaseorder mp left join supplier s on mp.supplier_id=s.id  left join purchaseorder_entry poe on poe.po_id=mp.id where mp.billing_company_id='$company' and mp.unitInv='$unit' and poe.stock_id='$item' group by s.id";
		//echo $qry;
		$result = array();
		$result = $this->databaseConnect->getRecords($qry);
	...
	function getTotalUnitStockQty($item,$company,$unit,$supplier)
	{
		$qry = "select id from supplier_stock_company_unit where company_id='$company' and unit_id='$unit' and supplier_id='$supplier' and stock_id='$item'";
		//$qry = "select sum(qty_received) from goods_receipt_entries gre left join goods_receipt gr on gre.goods_receipt_id=gr.id left join m_purchaseorder mpo on mpo.id=gr.po_id where gre.stock_id='$item' and gr.company_id='$company' and gr.unit_id='$unit' and mpo.supplier_id='$supplier' order by gre.id";
		//echo $qry;		
		$result	= $this->databaseConnect->getRecords($qry);
		foreach($result as $res)
		{
		...
		$query="select sum(stock_quantity) from supplier_stock_quantity where companyunitId in ($companyunitId)";
		//echo $query;
		$ret= $this->databaseConnect->getRecord($query);
		$retArr= (sizeof($ret)>0)?$ret[0]:"0";
		return array($companyunitId,$retArr);
	}

	function getAllStockQtyCmpUtId($companyunitId)
	{
		$qry="select id,stock_quantity from supplier_stock_quantity where companyunitId='$companyunitId'";
		$result= $this->databaseConnect->getRecords($qry);
		//echo $qry;
		//die();
		return $result;
	...
	function getCompanyInGRN($item,$supplier)
	{
		$qry = "select mpo.billing_company_id,mbc.display_name from goods_receipt_entries gre left join goods_receipt gr on gre.goods_receipt_id=gr.id left join m_purchaseorder mpo on mpo.id=gr.po_id left join m_billing_company mbc on mbc.id=mpo.billing_company_id where gre.stock_id='$item' and mpo.supplier_id='$supplier' group by mpo.billing_company_id order by gre.id ";
		//echo $qry;		
		$result = array();
		$result = $this->databaseConnect->getRecords($qry);
		if (sizeof($result)>=1) $resultArr = array(''=>'-- Select --');
	...
	{
		$qry = "select mpo.unitInv,mp.name from goods_receipt_entries gre left join goods_receipt gr on gre.goods_receipt_id=gr.id left join m_purchaseorder mpo on mpo.id=gr.po_id left join m_plant mp on mp.id=mpo.unitInv where gre.stock_id='$item' and mpo.supplier_id='$supplier' and mpo.billing_company_id='$company'  group by mpo.unitInv order by gre.id ";
		//echo $qry;		
		$result = array();
		$result = $this->databaseConnect->getRecords($qry);
		if (sizeof($result)>=1) $resultArr = array(''=>'-- Select --');
	...
	function getItems($company,$unit)
	{
		$qry = "select ms.id,ms.name from m_stock_plantunit spu left join m_stock ms on spu.stock_id=ms.id where spu.company_id='$company' and spu.plant_unit='$unit' and ms.name is not null group by ms.name order by ms.name";
		//echo $qry;		
		$result = array();
		$result = $this->databaseConnect->getRecords($qry);
		if (sizeof($result)>=1) $resultArr = array(''=>'-- Select --');
	...
	function getStockQuantity($stockIdFrom,$qty,$unitTo)
	{
		$qry="select * from  m_stock_plantunit where stock_id='$stockIdFrom' and plant_unit='$unitTo'";
		//echo $qry;
		$result	=	array();
	...
	function updateStockQuantity($stkQty,$stockqtyid)
	{
		$qry	= "update supplier_stock_quantity set stock_quantity='$stkQty' where id='$stockqtyid'";
		//echo $qry;
		$result	= $this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	function getCompanyUnitId($toCompany,$unitTo,$stockIdFrom)
	{
		$selDate=date("Y-m-d");
		$qry	= "select a.id,b.id  from supplier_stock a left join supplier_stock_company_unit b on a.id=b.supplierstock_id where date_format(a.start_date,'%Y-%m-%d')<='$selDate' and b.company_id='$toCompany' and b.unit_id='$unitTo' and b.stock_id='$stockIdFrom' order by b.id desc";
		//echo $qry; 
		//echo die();
		$result	=	$this->databaseConnect->getRecord($qry);
	...
	function addStockTransferQuantity($supplierstockid,$supplierId,$stockId,$stockQuantity,$companyunitId)
	{
		$stockDate=date("Y-m-d");
		
		$qry	=	"insert into supplier_stock_quantity (supplierstock_id, supplier_id, stock_id,stock_quantity, stock_date, companyunitId) values('$supplierstockid','$supplierId','$stockId','$stockQuantity','$stockDate','$companyunitId')";
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
		return $insertStatus;
