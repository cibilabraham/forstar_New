...
class DailyFrozenPackingNew
{
	...
	*****************************************************************/
	...
	function DailyFrozenPackingNew(&$databaseConnect)
    	{
	...
	#Indert blank record
	function addTempDataMainTableRmlotid($userId,$rm_lot_id,$company,$unit)
	{
		$qry	= "insert into t_dailyfrozenpacking_main_rmlotid (select_date, user_id,rm_lot_id,company,unit) 
				   values(Now(), '$userId', '$rm_lot_id','$company','$unit')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function updatelotidInPackingMain($newRMCompanyId,$newRMUnitId,$rm_lot_id, $mainId)
	{
...
function getPagingDFPRecs($fromDate, $tillDate, $offset, $limit,$filterProcessCode)
	{
		...
		$limit 		= " $offset, $limit";		
		$qry1 	= " select a.id, a.select_date, b.fish_id, b.processcode_id, b.freezing_stage_id, b.frozencode_id, 
				...
				";
				//echo $qry1;
		$qry2 	= " select a.id, a.select_date, b.fish_id, b.processcode_id, b.freezing_stage_id, b.frozencode_id, 
				mpc.code as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, a.report_confirm, 
				(select count(*) from t_dailyfrozenpacking_allocated_entry dfpae where b.id=dfpae.entry_id 
				group by dfpae.entry_id) as allocatedCount, b.mcpacking_id, mcp.code as mcPkgCode,
				((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty,
				sum(tdfpg.number_mc) as numMcs, 
				((sum(tdfpg.number_mc)*mfp.decl_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.decl_wt)) as frozenQty,
				((sum(tdfpg.number_mc)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt)*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt))) as availableQty,a.physical_stock_main_id,sum(a.physical_stock_main_id),repack_main_id,sum(tdfpg.number_loose_slab) as numLS, concat(mg.alpha_character,mg.rm_lotid) as new_lot_IdNm,a.rm_lot_id as new_lot_Id,b.id,mg.unit_id as unit,mg.company_id as company  from 
			...
			$qry11 	= " select a.id, a.select_date, b.fish_id, b.processcode_id, b.freezing_stage_id, b.frozencode_id, 
				mpc.name as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, a.report_confirm, 
				(select count(*) from t_dailyfrozenpacking_allocated_entry dfpae where b.id=dfpae.entry_id 
				group by dfpae.entry_id) as allocatedCount, b.mcpacking_id, mcp.code as mcPkgCode,
				((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty,
				sum(tdfpg.number_mc) as numMcs, 
			    ...
			    '0' as new_lot_IdNm,'0' as new_lot_Id,b.id,'0' as unit, '0' as company from 
				t_dailyfrozenpacking_main a left join t_dailyfrozenpacking_entry b on a.id=b.main_id left join t_dailyfrozenpacking_grade tdfpg on b.id=tdfpg.entry_id 
				left join m_secondary_processcode mpc on mpc.id=b.processcode_id
				left join m_freezingstage mfs on mfs.id=b.freezing_stage_id
				left join m_frozenpacking mfp on mfp.id=b.frozencode_id
				left join m_mcpacking mcp on b.mcpacking_id=mcp.id 
				";
				//echo $qry1;
		$qry12 	= " select a.id, a.select_date, b.fish_id, b.processcode_id, b.freezing_stage_id, b.frozencode_id, 
				mpc.name as processCode, mfs.rm_stage as freezingStage, mfp.code as frznPkgCode, a.report_confirm, 
				(select count(*) from t_dailyfrozenpacking_allocated_entry dfpae where b.id=dfpae.entry_id 
				group by dfpae.entry_id) as allocatedCount, b.mcpacking_id, mcp.code as mcPkgCode,
				((sum(tdfpg.number_mc)*mfp.filled_wt*mcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty,
...
###############################################################################
	#Add Grade Records
	function addFrozenPackingGrade($entryId, $gradeId, $numMC, $numLooseSlab, $LSToMCConversionType,$netWt)
	{
		$qry	= " insert into t_dailyfrozenpacking_grade (entry_id, grade_id, number_mc, number_loose_slab, convert_type,net_wt) values($entryId, $gradeId, $numMC, $numLooseSlab, '$LSToMCConversionType','$netWt')";				
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function addFrozenPackingGradeRmlotid($entryId, $gradeId, $numMC, $numLooseSlab, $LSToMCConversionType,$netWt)
	{
		$qry	= " insert into t_dailyfrozenpacking_grade_rmlotid (entry_id, grade_id, number_mc, number_loose_slab, convert_type,net_wt) values($entryId, $gradeId, $numMC, $numLooseSlab, '$LSToMCConversionType','$netWt')";				
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	{
 		$qry1="select a.grade_id, c.code as code, b.id, b.entry_id, b.number_mc, b.number_loose_slab from m_processcode2grade a left join t_dailyfrozenpacking_grade b on a.grade_id=b.grade_id and b.entry_id='$entryId', m_grade c where a.grade_id = c.id and a.processcode_id='$codeId' and a.unit_select='f' ";
		$qry2	="select a.secondary_grade, c.code  as code, b.id, b.entry_id, b.number_mc, b.number_loose_slab from m_secondary_processcode a left join t_dailyfrozenpacking_grade b on a.secondary_grade=b.grade_id and b.entry_id='$entryId', m_grade c where a.secondary_grade = c.id and a.id='$codeId'";
		$qry=$qry1." union ".$qry2;
	...
	{
 		$qry1	= "select a.grade_id, c.code as code , b.id, b.entry_id, b.number_mc, b.number_loose_slab from m_processcode2grade a left join t_dailyfrozenpacking_grade_rmlotid b on a.grade_id=b.grade_id and b.entry_id='$entryId', m_grade c where a.grade_id = c.id and a.processcode_id='$codeId' and a.unit_select='f'";
		$qry2	="select a.secondary_grade, c.code  as code, b.id, b.entry_id, b.number_mc, b.number_loose_slab from m_secondary_processcode a left join t_dailyfrozenpacking_grade b on a.secondary_grade=b.grade_id and b.entry_id='$entryId', m_grade c where a.secondary_grade = c.id and a.id='$codeId'";
		$qry=$qry1." union ".$qry2;
		$qry.=" order by code asc";
		//echo $qry;		
	...
	#Update daily Frozen Packing Entry Table Rec
	function addDailyFrozenPackingEntry($mainId, $hidFishId, $hidProcesscodeId, $qeFreezingStageId, $qeEUCodeId, $qeBrandId, $qeFrozenCodeId, $qeMCPackingId, $qeFrozenLotId, $qeExportLotId, $qeQualityId, $qeCustomerId, $qeBrandFrom, $selQuickEntryList,$processType)
	{
		$qry	= "insert into t_dailyfrozenpacking_entry (main_id, fish_id, processcode_id, freezing_stage_id, eucode_id, brand_id, frozencode_id, mcpacking_id, frozen_lot_id, export_lot_id, quality_id, customer_id, brand_from, quick_entry_list_id,processcode_type) values('$mainId', '$hidFishId', '$hidProcesscodeId', '$qeFreezingStageId', '$qeEUCodeId', '$qeBrandId', '$qeFrozenCodeId', '$qeMCPackingId', '$qeFrozenLotId', '$qeExportLotId', '$qeQualityId', '$qeCustomerId', '$qeBrandFrom', '$selQuickEntryList','$processType')";
		//echo $qry;
		$insertStatus	= $this->databaseConnect->insertRecord($qry);		
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();		
	...
	function addDailyFrozenPackingEntryRmlotid($mainId, $hidFishId, $hidProcesscodeId, $qeFreezingStageId, $qeEUCodeId, $qeBrandId, $qeFrozenCodeId, $qeMCPackingId, $qeFrozenLotId, $qeExportLotId, $qeQualityId, $qeCustomerId, $qeBrandFrom, $selQuickEntryList,$processType)
	{
	...
	{	
		$qry = "select * from (select a.processcode_id as processid, a.fish_id as fish_id, b.code as pc, c.code as fc,'primary' as processtype,a.id as id from t_fznpakng_qel_entry a, m_processcode b, m_fish c where a.fish_id=c.id and a.processcode_id=b.id and a.qe_entry_id='$selQuickEntryList'   union select a.processcode_id as processid, a.fish_id as fish_id,b.name , c.code as fc,'secondary' as processtype,a.id as id from t_fznpakng_qel_entry a, m_secondary_processcode b, m_fish c where a.fish_id=c.id and a.processcode_id=b.id and a.qe_entry_id='$selQuickEntryList' ) dum order by id asc";
	...
	# Update grade Rec
	function updateDFPGradeEntry($gradeEntryId, $numMC, $numLS, $oldMC, $oldLS, $conversionDate,$netWt)
	{
		$uptdQry = "";
		if ($conversionDate!="") $uptdQry = " , converted_date='$conversionDate' ";
		$qry	= " update t_dailyfrozenpacking_grade set number_mc='$numMC', number_loose_slab='$numLS', mc_old='$oldMC', loose_slab_old = '$oldLS',net_wt='$netWt' $uptdQry where id='$gradeEntryId'";
		//echo "<br>$qry";
		$result	=	$this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	# Update grade Rec
	function updateDFPGradeEntryRMlotId($gradeEntryId, $numMC, $numLS, $oldMC, $oldLS, $conversionDate,$netWt)
	{
		$uptdQry = "";
		if ($conversionDate!="") $uptdQry = " , converted_date='$conversionDate' ";
		$qry	= " update t_dailyfrozenpacking_grade_rmlotid set number_mc='$numMC', number_loose_slab='$numLS', mc_old='$oldMC', loose_slab_old = '$oldLS',net_wt='$netWt'  $uptdQry where id='$gradeEntryId'";
		//echo "<br>$qry";
		$result	=	$this->databaseConnect->updateRecord($qry);
		if ($result) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
	...
	function getPkdQty($fromDate, $tillDate, $processCodeId, $freezingStageId, $frozenCodeId, $mcPackingId)
	{
		...
		$qry = "select ((sum(tdfpg.number_mc)*mfp.filled_wt*mmcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.filled_wt)) as pkdQty, sum(tdfpg.number_mc) as numMcs, ((sum(tdfpg.number_mc)*mfp.decl_wt*mmcp.number_packs)+(sum(tdfpg.number_loose_slab)*mfp.decl_wt)) as frozenQty, ((sum(tdfpg.number_mc)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt)*mmcp.number_packs)+(sum(tdfpg.number_loose_slab)*if(mfp.actual_filled_wt!=0,mfp.actual_filled_wt,mfp.filled_wt))) as availableQty 
			from t_dailyfrozenpacking_main tdfpm join t_dailyfrozenpacking_entry tdfpe on tdfpm.id=tdfpe.main_id join t_dailyfrozenpacking_grade tdfpg on tdfpe.id=tdfpg.entry_id left join m_frozenpacking mfp on tdfpe.frozencode_id=mfp.id
			left join m_mcpacking mmcp on mmcp.id=tdfpe.mcpacking_id where tdfpe.processcode_id='$processCodeId' and tdfpe.freezing_stage_id='$freezingStageId' and tdfpe.frozencode_id='$frozenCodeId' and tdfpe.mcpacking_id='$mcPackingId'  and tdfpm.select_date>='".$fromDate."' and tdfpm.select_date<='".$tillDate."' 
	...
	{
		$qry = "SELECT id,concat(alpha_character,rm_lotid) FROM t_manage_rm_lotid where created_on='$entryDate' and company_id='$company' and unit_id='$unit' and active='1'";
	...
	{
		$qry = "select tdfpe.id, tdfpe.frozen_lot_id, tdfpe.mcpacking_id, mcp.code as mcPkgCode, tdfpm.id,tdfpm.physical_stock_main_id,tdfpm.rm_lot_id as rm_lot_id,tdfpe.fish_id,tdfpe.processcode_id,tdfpe.freezing_stage_id,tdfpe.frozencode_id,concat(mg.alpha_character,mg.rm_lotid) as rmName,mcp.number_packs as numberpack,tdfpm.company,tdfpm.unit from t_dailyfrozenpacking_main_rmlotid tdfpm join t_dailyfrozenpacking_entry_rmlotid tdfpe on tdfpm.id=tdfpe.main_id left join m_mcpacking mcp on tdfpe.mcpacking_id=mcp.id left join t_manage_rm_lotid mg on mg.id=tdfpm.rm_lot_id  where tdfpm.rm_lot_id in ($rmlotIds) and tdfpe.processcode_id='$processId' and tdfpe.freezing_stage_id='$freezingId' and tdfpe.frozencode_id='$frozenCode' order by tdfpm.rm_lot_id , tdfpe.id";
	...
	function getNewlot($mcPackingIdCvt,$fishIdCvt,$processIdCvt,$freezingIdCvt,$frozenCodeIdCvt,$companyId,$unitId,$lsPkgArr,$gradeId,$filledWt,$EntryId,$alphacode,$slnumber,$numbergen)
	{
		...
		$result.="<td class='listing-head' style='padding-left:2px;padding-right:2px; '>SL No</td>";
		$result.="<td class='listing-head' style='padding-left:2px;padding-right:2px; '>RM Lot Id</td>";
		...
		$result.="<td class='listing-item' style='padding-left:2px;padding-right:2px; '>1";
	...
	function secondaryProcessCodeHasGrade($processCodeId, $gradeId)
	{
		$qry = "select a.secondary_grade from m_secondary_processcode a where a.id='$processCodeId' and a.secondary_grade='$gradeId'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecords($qry);
		return (sizeof($result)>0)?true:false;
	}

	function getFrozenCode($selQuickEntryList)
	{
		$qry = "select a.frozencode_id from t_fznpakng_quick_entry a where a.id='$selQuickEntryList'";
		//echo $qry;
		$result	= $this->databaseConnect->getRecord($qry);
