...
$xajax = new xajax();	
	...
	# get Stock Details
	function getStockBalanceQty($rowId,$stockId,$supplierId,$company,$unit)
	{
		$objResponse = new NxajaxResponse();
		$databaseConnect = new DatabaseConnect();		
		$purchaseOrderInventoryObj 	= new PurchaseOrderInventory($databaseConnect);			
		$balanceStockQty = $purchaseOrderInventoryObj->getBalanceQty($stockId,$supplierId,$company,$unit);
...
function getAllRecords($rowId,$supplierId,$stockId,$plantUnitId,$poItem,$supplierRateListId,$company,$mode)
{
	...
	$minimumOrderQty = $purchaseOrderInventoryObj->getStockMinOrderQty($stockId);	
	$balanceStockQty = $purchaseOrderInventoryObj->getBalanceQty($stockId,$supplierId,$company,$plantUnitId);
	$getSupplierStockRecs = $purchaseOrderInventoryObj->getSupplierRec($stockId, $supplierId, $poItem);
	$otherSupplier = "";		
	if (sizeof($getSupplierStockRecs)>0) 
	{		
...
#Generate GatePass
	...
	function getUnit($supplierId,$companyId,$row,$cel)
	{
		$objResponse 	= new NxajaxResponse();	
		$databaseConnect= new DatabaseConnect();
		$purchaseOrderInventoryObj = new PurchaseOrderInventory($databaseConnect);
		//$manageusersObj			=	new ManageUsers($databaseConnect);
		$unit=$purchaseOrderInventoryObj->getUnitRecs($supplierId,$companyId);
	...
	*/
	...
	function getAllStock($company,$unitpo,$selSupplier)
	{
		$objResponse 	= new NxajaxResponse();	
		$databaseConnect= new DatabaseConnect();
		$purchaseOrderInventoryObj = new PurchaseOrderInventory($databaseConnect);
		$stock=$purchaseOrderInventoryObj->getStockRecs($company,$unitpo,$selSupplier);
		$objResponse->addDropDownOptions("selStock_0",$stock,$cel);
		return $objResponse;
	}
	...
	function getStock($selStockId)
	{
		$objResponse 	= new NxajaxResponse();	
		$databaseConnect= new DatabaseConnect();
		$purchaseOrderInventoryObj = new PurchaseOrderInventory($databaseConnect);
		$stock=$purchaseOrderInventoryObj->getStock($selStockId);
		$objResponse->addDropDownOptions("selStock_0",$stock,$cel);
		return $objResponse;
	}


$xajax->register(XAJAX_FUNCTION,'getStock',array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));	
$xajax->register(XAJAX_FUNCTION,'getAllStock',array('onResponseDelay'=>'showFnLoading','onComplete'=>'hideFnLoading'));	
