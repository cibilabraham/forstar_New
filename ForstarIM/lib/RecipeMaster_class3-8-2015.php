...
{  
	...
	*****************************************************************/
	...
	{
		$qry = "insert into m_recipemaster_entry (recipe_id,ingredient_id,quantity,fixed_qty_chk,fixed_qty, percent_per_btch, rate_per_btch, ing_gms_per_pouch, percent_wt_per_pouch, rate_per_pouch, percent_cost_per_pouch, cleaned_qty, sel_ing_type, rate_per_kg) values('$lastId', '$ingredientId', '$quantity', '$fixedQtyChk', '$fixedQty', '$percentagePerBatch', '$ratePerBatch', '$ingGmsPerPouch', '$percentageWtPerPouch', '$ratePerPouch', '$percentageCostPerPouch', '$idealQty', '$selIngType', '$ratePerKg')";
		//echo $qry."<br>";			
		$insertStatus = $this->databaseConnect->insertRecord($qry);
		if ($insertStatus) $this->databaseConnect->commit();
		else $this->databaseConnect->rollback();
