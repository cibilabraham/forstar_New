...
*/
		...
		if ($purchaseOrderNo!="" ) {
			if ($selSupplierId!="") {
				$purchaseOrderRecIns = $ingredientPurchaseorderObj->addPurchaseOrder($purchaseOrderNo, $selSupplierId,$company,$unit, $userId);
			...
			for ($i=0; $i<$itemCount; $i++) {
				$status = $p["status_".$i];
			  	if ($status!='N') {
					...
					$totalQty	=	$p["total_".$i];
					$hidSupplierIng=$p["hidSupplierIng_".$i];

					if ($lastId!="" && $ingredientId!="" && $unitPrice!="" && $quantity!="") {
						$purchaseItemsIns = $ingredientPurchaseorderObj->addPurchaseEntries($lastId, $ingredientId, $unitPrice, $quantity, $totalQty,$hidSupplierIng);
					}
			  	}
			}
		}
	...
	if ($p["editId"]!="" ) {
		...
		$selCompanyName			=	$purchaseOrderRec[7];
		$unit		=	$purchaseOrderRec[8];
	...
	if ($p["cmdSaveChange"]!="" ) {
		...
		if ($purchaseOrderId!="" && $selSupplierId!="") {
			$purchaseOrderRecUptd = $ingredientPurchaseorderObj->updatePurchaseOrder($purchaseOrderId, $selSupplierId,$company,$unit, $ingredientRateListId);
...
## ----------------- Pagination Settings I End ------------	
	...
	if($_REQUEST["action"]=="displayMsg")
	{
		...
		//ECHO 	$company.$unit;
		//DIE();
		$selDate=Date('Y-m-d'); $response_array[]="";
		$ing_POYear	 = date("Y", strtotime($selDate));
		
		
		
		//code modify by rekha verma
		list($ing_PONum, $ing_PODate) = $ingredientPurchaseorderObj->getMaxIng_PONum($ing_POYear,$company,$unitID);
		$valid_ingPONum = $ingredientPurchaseorderObj->getValid_IngPONum($ing_PONum, $ing_PODate,$company,$unitID);	

		if ($valid_ingPONum){
			$disPONo=$ing_PONum+1;
			//$alphaCode=$ingredientPurchaseorderObj->getAlphaCode($selDate,$company,$unitID);
			//$alphaCodePrefix= $alphaCode[0];
			
			//$disPONo = $alphaCode[0]."-".$disPONo ;	
			$response_array =array("",$disPONo,"");
		
		}
		else{
			$disPONo = $ingredientPurchaseorderObj->getCurrent_ingPONum($selDate,$company,$unitID);
			
			if($disPONo==""){
				$POMsg="Please set the Lot Id in Settings";
				$response_array =array($POMsg,"","");
			}
			else{
		...
		/*
		$checkGateNumberSettingsExist=$ingredientPurchaseorderObj->chkValidPOId($selDate,$company,$unitID);
		if (sizeof($checkGateNumberSettingsExist)>0){
		$alphaCode=$ingredientPurchaseorderObj->getAlphaCode($selDate,$company,$unitID);
		$alphaCodePrefix= $alphaCode[0];
		$numbergen=$checkGateNumberSettingsExist[0][0];
		$unitVal=$checkGateNumberSettingsExist[0][3];
		($unitVal==0)?$unit="0":$unit=$unit;
		//echo $unit; die();
		$checkExist=$ingredientPurchaseorderObj->checkPOIDDisplayExist($company,$unitID);
		if ($checkExist>0){
		$getFirstRecord=$ingredientPurchaseorderObj->getmaxPOId($company,$unitID);
		$getFirstRec= $getFirstRecord[0];
		$getFirstRecEx=explode($alphaCodePrefix,$getFirstRec);
		$nextGatePassId=$getFirstRecEx[1]+1;
		//$nextGatePassId;
		
		$validendno=$ingredientPurchaseorderObj->getValidendnoPOId($selDate,$company,$unitID);
		if ($nextGatePassId>$validendno){
		$POMsg="Please set the Gate Pass number in Settings,since it reached the end no";
		$response_array =array($POMsg,"","");
		}
		else{
		
		$disPONo="$alphaCodePrefix$nextGatePassId";
		$response_array =array("",$disPONo,$numbergen);
		}
		
		}
		else
		{
			$validPassNo=$ingredientPurchaseorderObj->getValidPOId($selDate,$company,$unit);	
			$checkPassId=$ingredientPurchaseorderObj->chkValidPOId($selDate,$company,$unit);
			$disPOId="$alphaCodePrefix$validPassNo";
			$response_array =array("",$disPOId,$numbergen);
		}
		}
		else{
			$POMsg="Please set the Lot Id in Settings";
									...
									<td colspan="2" >
										<table cellpadding="0"  width="65%" cellspacing="0" border="0" align="center">
											...
											<tr>
												...
												<td  colspan="2" align="center">
												<input type="submit" name="cmdCancel" class="button" value=" Cancel " onClick="return cancel('IngredientPO.php');">&nbsp;&nbsp;
												<input type="submit" name="cmdAdd" id="cmdAdd" class="button" value=" Add " onClick="return validateIngredientPurchaseOrder(document.frmIngredientPO);"> &nbsp;&nbsp;												
												</td>
												<?}?>
											</tr>
												...
												<td colspan="2" nowrap>
													...
													<tr>
															...
															<td class="listing-item">
																<select id="company" name="company"  onchange="xajax_getUnit(document.getElementById('company').value,'','');" <? if($editMode) { ?> disabled <? } ?>>
															...
															<td class="listing-item">
																<select id="unit" name="unit" required onchange="getPOInvId(document.getElementById('company').value,this.value);" <? if($editMode) { ?> disabled <? } ?>>
																	<option value="">--select--</option>
											  							<?php
														...
														<tr>
																<td class="fieldName" nowrap>PO ID : </td>
																<td class="listing-item">
																...
																<input type="hidden" name="number_gen_id" id="number_gen_id" value=""/>
																<b id="message" style="font-size:11px; color:red;"></b>
																</td>
																<td>&nbsp;</td>
																<td class="fieldName" nowrap>*Supplier</td>
																<td class="listing-item">
																	<select name="selSupplier" id="selSupplier" onchange="xajax_supplierIngRecords(document.getElementById('selSupplier').value,document.getElementById('hidTableRowCount').value,'<?=$mode?>');" <? if($editMode) { ?> disabled <? } ?>>
																		<option value="">--select--</option>
																		<?
															...
															<tr><td nowrap><div id="divPOIdExistTxt" style='line-height:normal; font-size:10px; color:red;'></div>
															</td></tr>
														</table>
													</td>
												</tr>
													...
													<TD>
														<table  cellspacing="1" bgcolor="#999999" cellpadding="3" id="tblAddPOIngItem">
															<tr bgcolor="#f2f2f2" align="center">
																...
																<td class="listing-head">Last Purchase Order Supplier,Qty,Price</td>
															...
															foreach ($purchaseRecs as $pr) {
																...
																$balanceQty = $ingredientPurchaseorderObj->getBalanceQty($selSupplierId,$selIngId);
																$otherSuppliers= $ingredientPurchaseorderObj->OtherSuppliers($selSupplierId,$selIngId);
																$lastPO= $ingredientPurchaseorderObj->getLastPurchaseOrder($selIngId);
															?>
															<tr align="center" class="whiteRow" id="row_<?=$k?>">
																...
																<td align="center" class="listing-item">
																	<select onchange="xajax_getIngRate(document.getElementById('selSupplier').value,document.getElementById('selIngredient_<?=$k?>').value, '<?=$k?>');" id="selIngredient_<?=$k?>" name="selIngredient_<?=$k?>">
																	<?php
																...
																<td align="center" class="listing-item">
																	...
																	<input name="hidSupplierIng_<?=$k?>" type="hidden" id="hidSupplierIng_<?=$k?>"  value="<?=$supIngId?>">	
																</td>
																<td align="center" class="listing-item">
																	<input type="text" value="<?=$totalAmt?>" style="text-align: right;" readonly="" size="6" id="total_<?=$k?>" name="total_<?=$k?>"/>
																</td>
																<td class="listing-item" align="center" nowrap="">
																	<div id="otherSupplierDiv_<?=$k?>"><?=$otherSuppliers?></div>
																</td>
																<td class="listing-item" align="center">
																	<div id="LastPurchaseDiv_<?=$k?>"><?=$lastPO?></div>
																</td>
