<?php
	...
	//begin 
		# Update
	if ($p["cmdGSTSaveChange"] != "") {
		//echo("update GST Rate ");
		//exit;
		$taxRecId = $p["hidTaxMasterRecId_gst"];
		$baseGST = $p["baseGST"];
		$gstActive = ($p["gstActive"] == "") ? N : $p["gstActive"];

		$hidBaseGST = $p["hidBaseGST"];
		$hidGSTActive = $p["hidGSTActive"];
		$gstRateListId = $p["gstRateListId"];
		//startDate_gst
		//hidStartDate_gst
		if ($baseGST != "" && $taxRecId != "") {
			$taxRecUptd = $taxMasterObj->updateTaxMasterRec($taxRecId, $baseGST, $gstActive, $gstRateListId);
		} else {
			//$taxRecUptd = $taxMasterObj->addTaxMasterRec($baseGST, $gstActive, $gstRateListId);
		}
	...
	# Delete GST rate list
	// Delete GST 
	if ($p["cmdDelGSTRateList"] != "") {
		
		//echo("delete rec");
		//exit;
		$selGSTRateList = $p["selGSTRateList"];

		$tmr = $taxMasterObj->taxRLRec($selGSTRateList);
		$srlStartDate = $tmr[2];
		
		# chk rate list in use
		//$chkRateListInUse = $taxMasterObj->chkRateListInUse($srlStartDate);
		//if (!$chkRateListInUse) {
			$delRateListRec = $taxMasterObj->delRateListRec($selGSTRateList);
		//}
		...
		if ($delRateListRec) {
			...
			$sessObj->createSession("nextPage",$url_afterUpdateTax.$selBaseGST);
		} else {
	...
	# Get Latest GST rate list id
	if ($g["selGSTRateList"] != "") $GSTRateListId = $g["selGSTRateList"];
	else if ($p["selGSTRateList"] != "") $GSTRateListId = $p["selGSTRateList"];
	else $GSTRateListId = $taxMasterObj->latestRateList('GST');

		
	# Edit
	$taxMasterRec = $taxMasterObj->find($GSTRateListId,'GST');
	$editTaxMasterRecId = $taxMasterRec[0];
	$baseGST = $taxMasterRec[1];
	$GSTActive = $taxMasterRec[2];
	$gst_active = "";
	if ($GSTActive == 'Y') $gst_active = "checked";
	$selTmr = $taxMasterObj->taxRLRec($GSTRateListId);
	$selStartDate_gst = $selTmr[2];
	$readOnly = ($selTmr[3] != '0000-00-00' && $selTmr[3] != "") ? "readonly" : "";
	$disabled = ($selTmr[3] != '0000-00-00' && $selTmr[3] != "") ? "disabled='true'" : "";
	
	# Get All Rate list
	$gstRateListRecs = $taxMasterObj->fetchAllCSTRateListRecs('GST');
	...
	//begin 
		# Update
	if ($p["cmdIGSTSaveChange"] != "") {
		//echo("update GST Rate ");
		//exit;
		$taxRecId = $p["hidTaxMasterRecId_igst"];
		$baseIGST = $p["baseIGST"];
		$igstActive = ($p["igstActive"] == "") ? N : $p["igstActive"];

		$hidBaseIGST = $p["hidBaseIGST"];
		$hidIGSTActive = $p["hidIGSTActive"];
		$igstRateListId = $p["igstRateListId"];
		//startDate_gst
		//hidStartDate_gst
		if ($baseIGST != "" && $taxRecId != "") {
			$taxRecUptd = $taxMasterObj->updateTaxMasterRec($taxRecId, $baseIGST, $igstActive, $igstRateListId);
		} else {
			//$taxRecUptd = $taxMasterObj->addTaxMasterRec($baseGST, $gstActive, $gstRateListId);
		}
	...
	# Delete GST rate list
	// Delete GST 
	if ($p["cmdDelIGSTRateList"] != "") {
		$selIGSTRateList = $p["selIGSTRateList"];
		$tmr = $taxMasterObj->taxRLRec($selIGSTRateList);
		$srlStartDate = $tmr[2];
		
		# chk rate list in use
		//$chkRateListInUse = $taxMasterObj->chkRateListInUse($srlStartDate);
		//if (!$chkRateListInUse) {
			$delRateListRec = $taxMasterObj->delRateListRec($selIGSTRateList);
		//}
		...
		if ($delRateListRec) {
			...
			$p["selIGSTRateList"] = "";
			$sessObj->createSession("displayMsg", $msg_succDelTaxMaster);
			$sessObj->createSession("nextPage",$url_afterUpdateTax.$selBaseIGT);
		} else {
			$err = $msg_failDelTaxMaster;
		}
	}
	...
	# Get Latest IGST rate list id
	if ($g["selIGSTRateList"] != "") $IGSTRateListId = $g["selIGSTRateList"];
	else if ($p["selIGSTRateList"] != "") $IGSTRateListId = $p["selIGSTRateList"];
	else $IGSTRateListId = $taxMasterObj->latestRateList('IGST');

		
	# Edit
	$taxMasterRec = $taxMasterObj->find($IGSTRateListId,'IGST');
	$editTaxMasterRecId = $taxMasterRec[0];
	$baseIGST = $taxMasterRec[1];
	$IGSTActive = $taxMasterRec[2];
	$igst_active = "";
	if ($IGSTActive == 'Y') $igst_active = "checked";
	$selTmr = $taxMasterObj->taxRLRec($IGSTRateListId);
	$selStartDate_igst = $selTmr[2];
	$readOnly = ($selTmr[3] != '0000-00-00' && $selTmr[3] != "") ? "readonly" : "";
	$disabled = ($selTmr[3] != '0000-00-00' && $selTmr[3] != "") ? "disabled='true'" : "";
	
	# Get All Rate list
	$igstRateListRecs = $taxMasterObj->fetchAllCSTRateListRecs('IGST');
	...
	else $cstRateListId = $taxMasterObj->latestRateList('CST');
	
	
	# Edit
	$taxMasterRec = $taxMasterObj->find($cstRateListId,'CST');
	$editTaxMasterRecId = $taxMasterRec[0];
	...
	# Get All Rate list
	$cstRateListRecs = $taxMasterObj->fetchAllCSTRateListRecs('CST');
							...
							<TD>
								...
								<tr>
									<td>
										...
										<!--rekha added code dated on 24 sept 2018 -->
												<TR>
												<TD class="fieldName" nowrap="true">GST:</TD>
												<td class="listing-item" nowrap="true">
													<strong><?=$baseGST;?>%</strong>&nbsp;(Start date: <?= ($selStartDate_gst) ? dateFormat($selStartDate_gst) : ""; ?>)
												</td>
												<td>
													<?php
													if ($gst_active!="") {
												...
												<td>
												<a href="###" onclick="showTax('GST');" class="link1">Manage</a>
												</td>
											</TR>	
										<!-- end code -->
										...
										<!--rekha added code dated on 24 sept 2018 -->
												<TR>
												<TD class="fieldName" nowrap="true">IGST:</TD>
												<td class="listing-item" nowrap="true">
													<strong><?=$baseIGST;?>%</strong>&nbsp;(Start date: <?= ($selStartDate_igst) ? dateFormat($selStartDate_igst) : ""; ?>)
												</td>
												<td>
													<?php
													if ($igst_active!="") {
												...
												<td>
												<a href="###" onclick="showTax('IGST');" class="link1">Manage</a>
												</td>
											</TR>	
										<!-- end code -->
...
?>
											<td colspan="3" align="center">
												...
												<input type="submit" name="cmdSaveChange" class="button" value=" Save Changes " onclick=" return validateTaxMaster(document.frmTaxMaster);"><? } ?>
...
<!-- rekha added code -->
						...
						<div id="GSTDiv" style="display:none;">
								<table width="35%" align="center" cellpadding="0" cellspacing="0">
							<TR>
							<TD>
										<?php
								...
								<table width="200" align="center">
								<tr>
									<td background="images/heading_bg.gif" class="pageName" align="left" width="100%" colspan="3">&nbsp;GST</td>
								</tr>
								<tr>
									<td class="fieldName" nowrap>Rate List </td>
									<td>
									<select name="selGSTRateList" id="selGSTRateList" onchange="this.form.submit();">
										<option value="">--Select--</option>
<?php
										foreach ($gstRateListRecs as $crl) {
										$rateListId = $crl[0];
										$rateListName = stripSlash($crl[1]);
										$rlStartDate = dateFormat($crl[2]);
										$displayRateList = $rateListName . "&nbsp;(" . $rlStartDate . ")";
										$selected = ($GSTRateListId == $rateListId) ? "Selected" : "";
?>
											<option value="<?= $rateListId ?>" <?= $selected ?>><?= $displayRateList ?></option>
<? } ?>
										</select>
										</td>
<? if ($del == true && sizeof($gstRateListRecs) > 1) { ?>
										<td>
											<input name="cmdDelGSTRateList" type="submit" class="button" id="cmdDelGSTRateList" value="Delete Rate List" title="click here to delete the selected rate list " onclick="return cfmDel();" />
										</td>
<? } ?>
									</tr>
									<tr>
										<td class="fieldName" nowrap>*GST</td>
										<td class="listing-item" align="left">
										<INPUT NAME="baseGST" TYPE="text" id="baseGST" value="<?= $baseGST; ?>" size="4" style="text-align:right;" onblur="chkCSTChange();" autocomplete="off">&nbsp;%
										<INPUT NAME="hidBaseGST" TYPE="hidden" id="hidBaseGST" value="<?= $baseGST; ?>" size="4" style="text-align:right;" readonly="true">
										<input type="hidden" name="hidTaxMasterRecId_gst" value="<?= $GSTRateListId; ?>">
										<input type="hidden" name="gstRateListId" value="<?= $GSTRateListId; ?>">
									...
									<tr>
										<td class="fieldName" nowrap="nowrap">Active</td>
										<td nowrap="true" align="left">
										<input name="gstActive" type="checkbox" id="gstActive" value="Y" <?= $gst_active ?> class="chkBox" onclick="chkCSTChange();">&nbsp;&nbsp;<span style="vertical-align:middle; line-height:normal" class="fieldName"><font size="1">(If Yes, please give tick mark)</font></span>
										<input name="hidGSTActive" type="hidden" id="hidGSTActive" value="<?= $gst_active ?>" readonly="true">
										</td>
									</tr>
									<tr>
										<TD colspan="2">
										<fieldset>
											...
											<tr>
												<td class="fieldName" nowrap title="Rate list start date" >*Start Date </td>
												<td>
												<INPUT NAME="startDate_gst" TYPE="text" id="startDate_gst" value="<?= ($selStartDate_gst) ? dateFormat($selStartDate_gst) : ""; ?>" size="8" autocomplete="off" <?= $readOnly ?>>
												<INPUT NAME="hidStartDate_gst" TYPE="hidden" id="hidStartDate_gst" value="<?= ($selStartDate_gst) ? dateFormat($selStartDate_gst) : ""; ?>" size="8" autocomplete="off" readonly="true">
												</td>
											</tr>
											</table>
										</fieldset>
...
?>
											<td colspan="3" align="center">
												<? if ($edit == true && $isAdmin == true) { ?>&nbsp;&nbsp;
												<input type="submit" name="cmdGSTSaveChange" class="button" value=" Save GST Changes " onclick=" return validateTaxMaster_gst(document.frmTaxMaster);" ><? } ?>
												&nbsp;<input type="button" name="cmdCancelTax" id="cmdCancelTax" class="button" value=" cancel " onclick="cancelTax();" />
												</td>
<? } else { ?>
											<td align="center">&nbsp;&nbsp;</td>
...
<!-- rekha added code -->
						...
						<div id="IGSTDiv" style="display:none;">
														<table width="35%" align="center" cellpadding="0" cellspacing="0">
							<TR>
							<TD>
										<?php
...
<? } ?>
									...
									<tr>
										...
										<input type="hidden" name="hidTaxMasterRecId_igst" value="<?= $IGSTRateListId; ?>">
										<input type="hidden" name="igstRateListId" value="<?= $IGSTRateListId; ?>">
...
?>
											<td colspan="3" align="center">
												<? if ($edit == true && $isAdmin == true) { ?>&nbsp;&nbsp;
												<input type="submit" name="cmdIGSTSaveChange" class="button" value=" Save IGST Changes " onclick=" return validateTaxMaster_gst(document.frmTaxMaster);" ><? } ?>
												&nbsp;<input type="button" name="cmdCancelTax" id="cmdCancelTax" class="button" value=" cancel " onclick="cancelTax();" />
												</td>
<? } else { ?>
											<td align="center">&nbsp;&nbsp;</td>
