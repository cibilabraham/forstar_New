<?php
	...
	if (($p["cmdSearch"]!="") || ($dateFrom!='' &&  $dateTill!='')) {
		...
		if ($dateTill) $toDate  = mysqlDateFormat($dateTill);
			...
			//echo "hii".$unitId;
			$units = $yearwiseProductionReportObj->getAllUnit($fromDate,$toDate);
			$fishRec = $yearwiseProductionReportObj->getAllFish($fromDate,$toDate,$unitId);
	...
	# Display heading
	$heading = $label_YearwiseProductionReport;

	$ON_LOAD_PRINT_JS	= "libjs/yearwiseproductionreport.js";

	# Include Template [topLeftNav.php]
	require("template/topLeftNav.php");
?>
<form name="frmYearwiseProductionReport" action="YearwiseProductionReport.php" method="Post">
																										...
																										<td>
																											<table width="100" border="0">
																												<tr> 
																													...
																													<td nowrap="true">
																							<!--   -->					<input type="text" id="supplyFrom" name="supplyFrom" size="8" value="<?=$dateFrom?>" autocomplete="off"   onchange="submitForm('supplyFrom','supplyTill', frmYearwiseProductionReport);"/>
																													</td>
																													<td class="fieldName">Till:</td>
																													<td nowrap="true">
																														<input type="text" id="supplyTill" name="supplyTill" size="8"  value="<?=$dateTill?>" autocomplete="off"  onchange="submitForm('supplyFrom','supplyTill', frmYearwiseProductionReport);"/>
																													</td>
																												</tr>	
																											</table>
																										</td>
																		...
																		?>
																			...
																			<tr>
																				...
																				<? if(sizeof($displayMnth)>0)   
																				{
																					...
																					$fromDate=$disp[1].'-'.'01';
																					$toDate=$disp[1].'-'.'31';
																					$production = $yearwiseProductionReportObj->getDFPForAFish($fromDate, $toDate,$fishId,$unitId);
																					$prdDetail=$production[2];
																					$prdDetailTotal+=$prdDetail;
																				?>
																					<td bgcolor="#fff" class="listing-head" style="padding-left:5px; padding-right:5px;" align="right"><?=$prdDetail?></td>
																				<?
																					}	
																				}
																				?>
																			...
																			<tr>
																				...
																				<? if(sizeof($displayMnth)>0)   
																				{
																					foreach($displayMnth as $disp) 
																					{
																						$fromDate=$disp[1].'-'.'01';
																						$toDate=$disp[1].'-'.'31';
																						$purchaseTotal = $yearwiseProductionReportObj->getDFPForADate($fromDate, $toDate,$unitId);
																						$purchaseDetailTot=$purchaseTotal[2];
																			...
																			<tr><td bgcolor="#fff" class="listing-head" style="padding-left:5px; padding-right:5px; ">Grand Total</td><td bgcolor="#fff" colspan="<?=sizeof($displayMnth)?>" class="listing-head" style="padding-left:5px; padding-right:5px; " align="right"><?=number_format($prdDetailTotal, 2, '.', '')?></td></tr>
																	<?php
																		} 
