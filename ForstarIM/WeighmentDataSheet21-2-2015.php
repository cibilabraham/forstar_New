...
*/
	...
	{  
		...
		{	
			...
			{
				$objWeighmentDataSheet->addWeightmentProcurementValue($rm_lot_id,$data_sheet_slno,$data_sheet_date,$receiving_supervisor,$total_quantity,$procurementAvailable,$userId);
		...
		if(sizeof($countCode) > 0)
		{
			...
			//echo $weightmentId[$i].'---'.$mstatus[$i].'<br/>';
				...
				{
					//echo "hii";
					$weightmentIns=$objWeighmentDataSheet->addWeightmentSupplierProcurementValue($lastId, $supplierName[$i],$pondName[$i],$product_species[$i],$process_code[$i],$quality[$i],$countCode[$i],$weight[$i],$soft_precent[$i],$soft_weight[$i],$newData[$i]);
				...
				{
					$weightmentUpdate=$objWeighmentDataSheet->updateWeightmentSupplierProcurementValue($weightmentId[$i],$lastId, $supplierName[$i],$pondName[$i],$product_species[$i],$process_code[$i],$quality[$i],$countCode[$i],$weight[$i],$soft_precent[$i],$soft_weight[$i],$newData[$i]);
																					...
																					<td width="40%" valign="top" align="center" colspan="3">
																						<table width="10%" bgcolor="#999999" cellspacing="1" cellpadding="3" name="tblWeighmentMultiple" id="tblWeighmentMultiple">
																							<tbody>
																												...
																												if(sizeof($speciesArray) > 0)
																													{
																														...
																														{	
																															...
																															$sel = '';
																															if($data[5] == $species[0]) $sel = 'selected';
																															
																															$speciesdropDown.= '<option '.$sel.' value="'.$species[0].'">'.$species[1].'</option>';
																														}
																													...
																													$speciesdropDown = '<select name="product_species[]" id="product_species_'.$i.'" />
																																			<option value="'.$data[5].'">'.$data[6].'</option>
																																		</select>';
																								...
																								?>
																								<tr>
																									<td  class="err1"  nowrap align="center" colspan="11"> No records found </td>
																								</tr>
																								<?php
																									}
																								?>
