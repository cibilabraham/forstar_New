<?php
	...
	*/
	if ($p["cmdAddSoaking"]!="" || $p["cmdSaveAdd"]!="") {
		...
		if ($selectDate!="")
		{
			...
			for ($i=0; $i<$processRowCount; $i++) {
				...
				//echo "hii";
					$soakingIns	=	$soakingObj->addSoaking($rmlotid,$selFishId, $selectDate,$processCode,$company,$unit);
					$lastInsertedValue=$soakingObj->lastIdInSoaking();
				...
				if ($lastInsertedId!=0 &&  $gradeName!="0" && $soak_inQty!="") {
					$soakingEntryRecIns = $soakingObj->addSoakingEntry($lastInsertedId,$processFrom, $gradeName, $soak_inCount, $soak_inGrade,$soak_outCount, $soak_outGrade, $soak_inQty, $soak_inTime ,$soak_outQty, $soak_outTime, $temperature, $gain, $chemicalName, $chemicalQty,$lotidAvailable);
			...
			# Reverse calc ends here 
				if($prevSelFish!=$selFishId && $fishId!="" && $selectDate!="" && $soakingId==""		) {
					$soakingIns	=	$soakingObj->addSoaking($rmlotid,$selFishId, $selectDate,$processCode,$company,$unit);
						...
						$soakId = $soakingObj->getSoakingMainId($selectDate, $selFishId);
					
				
				if ($soakingEntryId=="" &&  $gradeName!="0" && $soak_inQty!="" ) {
					$soakingEntryRecInsUptd = $soakingObj->addSoakingEntry($soakId,$processFrom, $gradeName, $soak_inCount, $soak_inGrade,$soak_outCount, $soak_outGrade, $soak_inQty, $soak_inTime ,$soak_outQty, $soak_outTime, $temperature, $gain, $chemicalName, $chemicalQty,$lotidAvailable);
				}
				else
				{
					$soakingEntryRecInsUptd = $soakingObj->updateSoakingEntry($processFrom, $gradeName, $soak_inCount, $soak_inGrade,$soak_outCount, $soak_outGrade, $soak_inQty, $soak_inTime ,$soak_outQty, $soak_outTime, $temperature, $gain, $chemicalName, $chemicalQty,$lotidAvailable,$soakingEntryId);
				}				
	...
	if ($p["cmdDelete"]!="") {
		...
		for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($soakingMainId!="") {
				...
				//$processAcConfirmed = $soakingObj->chkPreProcessAcConfirmed($dailyPreProcessEntryId);		
				//if (!$processAcConfirmed) {					
					# Delete Pre-Processor qty rec
					//$dailyPreProcessRecDel	=	$soakingObj->delDailyPreProcessorQty($dailyPreProcessEntryId);
			
					# Deleting Process Entry Rec
					$soakingRecDel	=	$soakingObj->delSoakingEntryQty($soakingEntryId);
			
					# Checking Records Existing for the selected Main Id
					$exisitingRecords = $soakingObj->checkRecordsExist($soakingMainId);

					# Delete Main Rec		
					if (sizeof($exisitingRecords)==0) {					
						$soakingRecDel =	$soakingObj->deleteSoaking($soakingMainId);
					}					
	...
	if ($recordsFilterId!="" || $selDate!="" || $p["cmdSearch"]!="") {	
		...
		$soakingRecords	= $soakingObj->soakingRecPagingFilter($recordsFilterId, $recordsDate, $offset, $limit);
		$soakingRecordsSize	= sizeof($soakingRecords);
		// echo '<pre>';
			// print_r($soakingRecords);
		// echo '</pre>';
		$numrows	=  sizeof($soakingObj->soakingRecFilter($recordsFilterId, $recordsDate));
		
		# Checking Entry Confirmed
		//$entryConfirmed	= $soakingObj->chkEntryConfirmed($recordsDate);
						//18/10/08
		$prevDate = date("Y-m-d",mktime(0, 0, 0,$Date[1],$Date[0]-1,$Date[2]));
		
		# Check Prev date Entry
		//$prevDateEntryConfirmed = $soakingObj->chkPrevDateEntryConfirmed($prevDate);
		$displayPrevdate = date("jS M Y", mktime(0, 0, 0, $Date[1], $Date[0]-1, $Date[2]));
		//$dppConfirmEnabled = $manageconfirmObj->isDPPConfirmEnabled();

		# Check RM CB Exist
		$ppEntryExist = $soakingObj->chkRecExist('', $recordsDate);
	}
...
#Select all Pre Processing Master Records based on fishId
	...
	if ($addMode || $editMode) {
		...
		if ($recordSelectDate) {
			...
			if ($validDPPEnabled) {
				# get Valid Fish Recs
				$getFishRecords = $soakingObj->getFishRecords($entryDate,'');
				$rmLotIds  = $soakingObj->getLotRecords($entryDate,'');
				# Days Active PC Recs
				$daysActivePCRecs = $soakingObj->getDaysActivePreProcessCodes($entryDate, $fishId, $selRateListId);
				//$daysActiveLotPCRecs = $soakingObj->getDaysActivePreProcessLotCodes($entryDate, $lotId, $selRateListId);
			} 
			else 
			{
				...
				if($entryDate!="")
				{
					...
					$rmLotIds  = $soakingObj->getLotIdAfterGradingLoad($entryDate);
				}
				...
				$companyRecords= $soakingObj->getAllCompany($entryDate,$rmLotID);
				//echo $companyId;
				$unitRecords= $soakingObj->getAllUnit($entryDate,$rmLotID,$companyId);
				
				# get Valid Fish Recs
				$getFishRecords = $soakingObj->getAllFishAddedOnDate($entryDate,$rmLotID,$companyId,$unitId);
				

				$process_codes= $soakingObj->getAllProcessCodeForFishAddedOnDate($entryDate,$fishId,$rmLotID,$companyId,$unitId);
				...
				if ($addMode || $editMode)
				{
					...
					{
						$daysActivePCRecs = $soakingObj->soakingRecs($entryDate,$selRateListId,$rmLotID,$fishId,$processCode,$companyId,$unitId);
					}
	...
	$harvestingchemicalNameRecs = $soakingObj->WeightmentHarvestingChemical();
	...
	$ON_LOAD_PRINT_JS	= "libjs/Soaking.js";
	//echo "hii";
...
<form name="frmSoaking" id="frmSoaking" action="Soaking.php" method="Post">
	<table cellspacing="0"  align="center" cellpadding="0" width="99%">
		<? if($err!="" ){?>
		<tr>
													...
													<table width="100%" cellpadding="0" cellspacing="1" bgcolor="#999999">
														<tr bgcolor="#f2f2f2" align="center">
															...
															<td class="listing-head" style="padding-left:5px; padding-right:5px;"> </td>
															 <td class="listing-head" style="padding-left:2px; padding-right:2px;">COUNT/GRADE</td>
															...
															<td class="listing-head" style="padding-left:5px; padding-right:5px;">COUNT/GRADE </td>
														...
														$processFrom = $dps[2];
														$rmlotid= $dps[4];
														$dapr=$soakingObj->soakingRecEditsingle($entryDate,$fishId,$rmLotID,$processFrom);
														...
														if ($ProcessCodes!="") {
															...
															$pcCountArr[$processFrom] = $j;
																...
																$grades = $soakingObj->getProcessGradeEdit($processFrom);		
														?>
														<tr bgcolor="#FFFFFF">
										...
										<tr>
											<td>
												...
												<? } else if ($edit && $ppEntryExist) {?>
													<input type="submit" value=" Edit " name="cmdEdit" class="button" <? if (sizeof($soakingRecords)<=0) {?> disabled="true" <? }?>>
												<? }?>
												<? if($add==true){?>
								...
								<td colspan="2" >
									...
									<? if($maxpage>1){?>
										...
										<tr  bgcolor="#f2f2f2" align="center" > 
											...
											<td class="listing-head" nowrap style="padding-left:5px; padding-right:5px;">RM Lot ID</td>
											<td class="listing-head" nowrap style="padding-left:5px; padding-right:5px;">Fish</td>
											...
											<td class="listing-head" style="padding-left:5px; padding-right:5px;">COUNT<br/>/GRADE</td>
											<td class="listing-head" style="padding-left:5px; padding-right:5px;">SOAK-IN<br>Count
											</td>
											<td class="listing-head" style="padding-left:5px; padding-right:5px;">SOAK-IN<br>Grade
											</td>
											 <td class="listing-head" style="padding-left:5px; padding-right:5px;">SOAK-IN<br>
											Qty</td>
											 <td class="listing-head" style="padding-left:5px; padding-right:5px;">SOAK-IN <br>
											Time</td> 
											<td class="listing-head" style="padding-left:5px; padding-right:5px;">SOAK-OUT<br>
											Count</td>
											 <td class="listing-head" style="padding-left:5px; padding-right:5px;">SOAK-OUT<br>Grade
											</td>
											 <td class="listing-head" style="padding-left:5px; padding-right:5px;">SOAK-OUT<br>
											Qty</td>
											 <td class="listing-head" style="padding-left:5px; padding-right:5px;">SOAK-OUT<br>
											Time</td>
											<td class="listing-head" style="padding-left:5px; padding-right:5px;">Temperature<br></td>
											<td class="listing-head" style="padding-left:5px; padding-right:5px;">Gain(%)<br></td>
											<td class="listing-head" style="padding-left:5px; padding-right:5px;">Chemical
											<br>Used</td>
											<td class="listing-head" style="padding-left:5px; padding-right:5px;">Chemical<br>
											QtY</td>
										...
										foreach ($soakingRecords as $pr) 
										{
											...
											//$processRate		=	$soakingObj->findProcessRate($preProcessId);		
											$openingBalQty		=	$pr[5];
											$arrivalQty		=	$pr[6];
											...
											*/
											...
											foreach ($preProcessorRecords as $ppr) {
												$mPrePId = $ppr[0];
												$ppRec = $soakingObj->findPreProcessorRec($soakingEntryId,$mPrePId);
												$ppQty = $ppRec[3];
												$preProcessorAmt = 0;
												if ($ppQty!=0) {
													list($ppeRate, $ppeCommission, $ppeCriteria, $ppYieldTolerance) = $soakingObj->getPProcessorExpt($preProcessId, $mPrePId);
													/*
											...
											*/
											...
											if (!isset($pFromArr[$selProcessFromId])) {
												# Display Calc
												$displayAvailableCalc = $soakingObj->disAvailableQtyCalc($selProcessFromId, $recordsDate);
										...
										<tr <?=$listRowMouseOverStyle?>> 
											...
											<td class="listing-item" nowrap style="padding-left:4px; padding-right:4px;"><?=$rmlotid?></td>
											<td class="listing-item" style="padding-left:4px; padding-right:4px;"><?=$fishName;?></td>
											<td class="listing-item" nowrap style="padding-left:4px; padding-right:4px;"><?=$ProcessCode?>
									
											<!--<td class="listing-item" align="right" style="padding-left:2px; padding-right:2px;"><?//=$openingBalQty?></td>-->
											<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px;">
											<?php 
											if($pr[7]=="1")
											{
												echo "Count";
										...
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px;" ><?=$pr[8]?></td>
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px;"><?=$grade_in?></td>
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px;"><?=$pr[10]?></td>
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px;"><?=$pr[11]?> </td>
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px;"><?=$pr[12]?></td>
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px;"><?=$grade_out?></td>
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px;"><?=$pr[14]?> </td>
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px;"><?=$pr[15]?></td>
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px;"><?=$pr[16]?></td>
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px; " ><?=$pr[17]?></td>
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px; " ><?=$pr[24]?></td>
										<td class="listing-item" align="right" style="padding-left:4px; padding-right:4px;" ><?=$pr[19]?></td>
										<? if($edit==true && !$validDPPEnabled){?>
...
?>	
	<script language="JavaScript">
		xajax_chkEntryExist('<?=$rmlotid?>','<?=$fishId?>','<?=$recordSelectDate?>', '<?=$soakingMainId?>',  '<?=$mode?>', '','<?=$processCode?>');
	</script>
<?php
	}
?>
...
<SCRIPT>
	...
	var out=parseInt(soakout) ;
		...
		document.getElementById("gain_"+id).value=parseInt(gainvalue);
