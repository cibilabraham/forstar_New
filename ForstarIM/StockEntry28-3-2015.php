...
for ($i=0; $i<$rowCountQty; $i++) {
				...
				$hidId=$p["hidid_".$i];
								...
								$stkId=$stkIdVal[0];								
								$StockEntryRecInsqty = $stockObj->addUnitStock($unitId,$stockQty,$stkId);
	...
	//echo "j value is $j--tc---$rowCount2";
				...
				if ($statusUnit!='N' && $stockqtyid!="") {
					...
					$stkQty=$p["stockQty_".$j];	
						...
						$StockEntryRecInsst = $stockObj->addUnitStock($punitId,$stkQty,$stockId);
					}else if ($statusUnit=='N' && $stockqtyid!="") {
						$punitId=$p["punitId_".$j];
					$stkQty=$p["stockQty_".$j];	
...
<td></td><td><table><tr>
			...
			<td nowrap  nowrap class="fieldName">	Quantity in Stock</td></tr></table></td></tr>
			</tr>
		  <!--<tr>
			<td nowrap class="fieldName">*Unit</td>
			<td nowrap>			
...
<? if($maxpage>1){?>
	...
	<tr title="<?=$displayTitle?>" <?php if ($active==0){?> bgcolor="#afddf8"  onMouseOver="ShowTip('<?=$disMsgInactive?>');" onMouseOut="UnTip();" <?php }?>>
		...
		<?//=$displayActualQty?><?php }?><?=trim($qStA,',');?></td>
...
<script language="JavaScript" type="text/javascript">	
	...
	?>	
		addNewPOItem2('tblPOItem2', '<?=$unitId?>', '<?=$quantity?>','<?=$Id?>','','','','','','<?=$mode?>');	
