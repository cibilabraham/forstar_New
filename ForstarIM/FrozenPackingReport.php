<?php
	...
	if ($purchaseOrderId>0) {
		$enableSearch = true;
		// Get All Invoices
		$invoiceRecs = $frozenpackingreportObj->getAllInvoices($purchaseOrderId);
	}
	...
	if ($selDate || ($dateFrom && $dateTill)) {
		...
		# Get Processors
		$processorsRecs = $frozenpackingreportObj->getProcessors($fDate, $tDate);
	}
	...
	if ($p["cmdStatusUpdate"]!="") {
		...
		if ($changeStatus=="CRA") {
			if ($csPurchaseOrderId>0) {
				$relAllocationRec=$frozenpackingreportObj->releaseAllocation($csPurchaseOrderId, $csInvoiceId, $userId);
				if ($relAllocationRec) $releaseAllocation = true;				
			}
		}
		else if ($changeStatus=="INVR"){
				if ($csPurchaseOrderId>0) {
					$relInvoiceRec = $frozenpackingreportObj->releaseInvoice($csPurchaseOrderId, $csInvoiceId, $userId);				
	...
	if (($p["cmdSearch"]!="" && ($details!="" || $summary!="")) || (($statusChangeMode || $enableSearch) && ($details!="" || $summary!="")) ) {
		...
		if ($dateSelection=='SD' || $dateSelection=="") {
			$fromDate = "";
			$toDate = "";
		} else if ($dateSelection=='DR') $selectDate = "";		

		$dailyFrozenPackingRecords = $frozenpackingreportObj->getDFPForADate($selectDate, $fromDate, $toDate, $details, $summary, $selProcessor);
		//printr($dailyFrozenPackingRecords);

		if (sizeof($dailyFrozenPackingRecords)>0)  $confirmed = $frozenpackingreportObj->isConfirmed($selectDate);

		// Allocated PO Recs
		$allocatedPORecs = $frozenpackingreportObj->getAllocatedPurchaseOrders($selectDate, $fromDate, $toDate);
	...
	if ($p["cmdConfirm"]!="") {
		$selDate	= $p["selDate"];
 		if ($selDate)  {
			...
			$updateDailyFrozenPackingRecords = $frozenpackingreportObj->updateDailyFrozenPackingRecords($selectDate);
  		}
	}
	

	# Display heading
	$heading = $label_FrozenPackingReport;

	$ON_LOAD_PRINT_JS	= "libjs/frozenpackingreport.js";

	# Include Template [topLeftNav.php]
	require("template/topLeftNav.php");
?>
	<form name="frmFrozenPackingReport" action="FrozenPackingReport.php" method="Post">
				...
				<fieldset>
				 <legend class="listing-item">Date Selection</legend>
					...
					<table width="60" border="0">
                                          <tr>
                                            <td>
						<input name="dateSelection" id="dateSelectionSD" type="radio" value="SD" onclick="this.form.submit();" <?=$singleDay?> class="chkBox">
					</td>
                                        ...
                                        <td><table width="100" border="0">
                                          <tr>
                                            <td>
						<input name="dateSelection" id="dateSelectionDR" type="radio" value="DR" onClick="this.form.submit();" <?=$dateRange?> class="chkBox">
					</td>
			...
			<TR><TD>
				...
				<? if($singleDay!=""){?>
					<tr> 
						...
						<td nowrap="true">
<!--   -->
							<input name="selDate" type="text" id="selDate" size="8" value="<?=$selDate?>" autocomplete="off" onchange="this.form.submit();" />
						</td>
						</tr>
						<? }?>
						<? if($dateRange!="") {?>
						...
						<td nowrap="true">
<!--   -->
							<input type="text" id="supplyFrom" name="supplyFrom" size="8" value="<?=$dateFrom?>" autocomplete="off" onchange="submitForm('supplyFrom','supplyTill',document.frmFrozenPackingReport);" />
						</td>
						<td class="fieldName">Till:</td>
						<td nowrap="true">
							<input type="text" id="supplyTill" name="supplyTill" size="8"  value="<?=$dateTill?>" autocomplete="off" onchange="submitForm('supplyFrom','supplyTill',document.frmFrozenPackingReport);" />
						</td>
						</tr>						
						<? }?>
						...
						<legend class="listing-item">Change Status</legend>
							<table>
								<TR>
									...
									<td>									
										<select name="csPurchaseOrder" id="csPurchaseOrder" style="width:120px;" onchange="this.form.submit();">
											<option value="">-- Select PO --</option>
											<?php
											foreach ($allocatedPORecs as $apor) {
												$poId	= $apor[0];
												$poNum	= $apor[1];
												$selected = ($purchaseOrderId==$poId)?"selected":"";
											?>
											<option value="<?=$poId?>" <?=$selected?>><?=$poNum?></option>
											<?php
												}
											?>
										</select>
									...
									<td>									
										<select name="csInvoice" id="csInvoice" style="width:120px;" onchange="disabPckStk(this.value)">
											<option value="-1">-- All Invoice --</option>
											<?php
											foreach ($invoiceRecs as $apor) {
												$invId	= $apor[0];
												$invNum	= $apor[1];
											?>
											<option value="<?=$invId?>"><?=$invNum?></option>
											<?php
												}
											?>
										</select>
									...
									<!--<TD>
										<table>
										<TR>
										<TD><INPUT type="radio" name="changeStatus" value="CRA" <?=$allocationReleaseChk?> class="chkBox" id="chkCRA"></TD>
										<TD class="listing-item" nowrap="true">Allocation</TD>
										<TD><INPUT type="radio" name="changeStatus" value="INVR" <?=$allocationReleaseChk?> class="chkBox" id="chkCRA"></TD>
										<TD class="listing-item" nowrap="true">Invoice Release</TD>
										</TR>
										</table>
									</TD>-->									
									<td>
										<!--<input type="submit" name="cmdStatusUpdate" value=" Update " class="button" onclick="return validateStatusUpdate(document.frmFrozenPackingReport);" />-->
...
<TR>
									...
									<td>									
										<INPUT type="radio" name="changeStatus" value="CRA" <?=$allocationReleaseChk?> class="chkBox" id="chkCRA" ></TD>
										<TD class="listing-item" nowrap="true"><label id="chkCRAL" >Allocation</label>
									</td>
									<td >									
										<INPUT type="radio" name="changeStatus" value="INVR" <?=$allocationReleaseChk?> class="chkBox" id="chkINVR" style="display:none"></TD>
										<TD   class="listing-item" nowrap="true"><label id="chkINVRL" style="display:none">Invoice Release</label>
									</td>
									<!--<TD>
										<table>
										<TR>
...
<!--  Change Status End here   -->
	...
	<?php
		...
		foreach ($dailyFrozenPackingRecords as $dfpr) {
			...
			if ($summary && $rmlot=='0') {
				$gFPEntryId = $dfpr[22];
				$gradeMasterRecords	= $frozenpackingreportObj->getGroupedFPGradeRecs($gFPEntryId);
			} 
			else if($summary=="" && $rmlot=='0')
			{
				$gradeMasterRecords	= $frozenpackingreportObj->getDetailedFPGradeRecs($dailyFrozenPackingEntryId);
			}
			else if ($summary && $rmlot=='1') {
				$gFPEntryId = $dfpr[22];
				$gradeMasterRecords	= $frozenpackingreportObj->getGroupedFPGradeRecsLot($gFPEntryId);
			} 
			else if($summary=="" && $rmlot=='1')
			{
				$gradeMasterRecords	= $frozenpackingreportObj->getDetailedFPGradeRecsLot($dailyFrozenPackingEntryId);
			}
