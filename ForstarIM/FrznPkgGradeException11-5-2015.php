<?php
	...
	if ($frznPkgRateId) {
		...
		$frznGradeRecs = $frozenPackingRateObj->getFrznPkgGrade($frznPkgRateId);

		# Sel Grade recs
		$gradeArr = $frozenPackingRateObj->getSelGrade($frznPkgRateId);	
		$gradeAllChk = "";
		//if (in_array(0, $gradeArr)) $gradeAllChk = "disabled";
	}
	
	# Grade Recs
	$gradeRecs = $frozenPackingRateObj->getGrades($processCodeId, $freezingStageId, $qualityId, $frozenCodeId);
	...
	if ($p["cmdDelete"]!="") {
		...
		for ($i=1; $i<=$tblRowCount; $i++) {
			...
			if ($delId!="") {
				...
				if($recExistFrozen)
				{
					$delSelGradeCombination = $frozenPackingRateObj->deleteFrznPkgRateGrade($groupEntryId);	
				}			
