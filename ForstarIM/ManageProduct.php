...
for ($i=1; $i<=$rowCount; $i++) {
	...
	#List all Records
	$productRecords = $manageProductObj->fetchAllPagingRecords($offset, $limit, $selProductCategoryId, $selProductStateId, $selProductGroupId, $srchName);
	$productRecordSize    = sizeof($productRecords);

	## -------------- Pagination Settings II -------------------
	$fetchProductRecords = $manageProductObj->fetchAllRecords($selProductCategoryId, $selProductStateId, $selProductGroupId, $srchName);	// fetch All Records
	...
	<? if($maxpage>1){ ?>
		...
		for ($page=1; $page<=$maxpage; $page++) {
			if ($page==$pageNo) {
      				...
      				$nav.= " <a href=\"ManageProduct.php?pageNo=$page&selProductCategory=$selProductCategoryId&selProductState=$selProductStateId&selProductGroup=$selProductGroupId&srchName=$srchName\" class=\"link1\">$page</a> ";				
   			}
		}
		if ($pageNo > 1) {
   			$page  = $pageNo - 1;
   			$prev  = " <a href=\"ManageProduct.php?pageNo=$page&selProductCategory=$selProductCategoryId&selProductState=$selProductStateId&selProductGroup=$selProductGroupId&srchName=$srchName\"  class=\"link1\"><<</a> ";
	 	} else {
		...
		if ($pageNo < $maxpage) {
   			$page = $pageNo + 1;
   			$next = " <a href=\"ManageProduct.php?pageNo=$page&selProductCategory=$selProductCategoryId&selProductState=$selProductStateId&selProductGroup=$selProductGroupId&srchName=$srchName\"  class=\"link1\">>></a> ";
	 	} else {
