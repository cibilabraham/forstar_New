<?php
	require("include/include_taskmgmt.php");
	//require_once("lib/DistMarginStructure_ajax.php");
	//require("db_taskmgmt.php");
	//echo("gkgkg");
	...
	if ($p["cmdAdd"]!="" || $p["cmdSaveAddNew"]!="") {
		...
		if ($marginSelection=="C") {
			...
			//Rekha updated code dated on 7 sept 2018
...
*/
	...
	$distWiseProuctWiseMarginRecords = $taskMasterObj->getTaskPagingRecords($offset, $limit);
	
	//print_r($distWiseProuctWiseMarginRecords);
	...
	#heading Section
	if ($editMode) $heading	=	'Edit New Task';
	else	       $heading	=	'Add New Task';
...
?>
	...
	<?php 
		require("template/sidemenuleft_taskmgmt.php");
	?>
	</td>
	<td width="85%" valign="top" align="left">
	<form name="frmDistMarginStructure" id="frmDistMarginStructure" action="tasklist.php" method="post">
	<? 
		if (!$editSelected) {
	?>
...
<tr>
	<td align="center">
		...
		<td colspan="2" align="center">
			<input type="submit" name="cmdCancel" class="button" value=" Cancel " onclick="return cancel('tasklist.php');">&nbsp;&nbsp;
...
?>
	...
	<tr><TD height="10"></TD></tr>
					...
					<table width="200">
					<tr>
					<td nowrap class="fieldName">*Task</td>
					<td nowrap>
					...
					<tr>
					<td nowrap class="fieldName">Assign To</td>
					<td nowrap>
						<input type='text' name='task'>
					</td>
					</tr>					
				<tr>
					<td nowrap class="fieldName">Completed Date</td>
					<td nowrap>
						<input type='text' name='task'>
					</td>
					</tr>
		<tr>
					<td nowrap class="fieldName">Status</td>
					<td nowrap>
						<select name=''>
					...
					<tr>
					<td nowrap class="fieldName">Attachment</td>
					<td nowrap>
						<input type='text' name='task'>
					</td>
					</tr>					
...
?>		
		...
		?>
			...
			<tr>
				<td colspan="3" align="center">
                	...
                	<tr>
		<td class="listing-item" nowrap="true">Assign To:&nbsp;</td>
                <td>
	   ...
	   <td class="listing-item" nowrap="true">Assign By:&nbsp;</td>
	<td>
						...
						<table>
							<TR>
								<TD nowrap="true">
									<?
									if($del==true){?>
									<input type="submit" value=" Delete **" style="background-color:#ff0000;color: white;"  name="cmdDelete" onClick="return confirmDelete(this.form,'delId_',<?=$distMarginRecordSize;?>);" ><? }?>&nbsp;<? if($add==true){?><input type="submit" value=" Add New Task" name="cmdAddNew" class="button">
									<? 
									}?>
								</TD>
							</TR>
...
<? if($maxpage>1){?>
		...
		for ($page=1; $page<=$maxpage; $page++) {
			if ($page==$pageNo) {
      				...
      				$nav.= " <a href=\"tasklist.php?pageNo=$page&distributorFilter=$distributorFilterId&distributorRateListFilter=$distributorRateListFilterId\" class=\"link1\">$page</a> ";
				//echo $nav;
   			}
		}
		if ($pageNo > 1) {
   			$page  = $pageNo - 1;
   			$prev  = " <a href=\"tasklist.php?pageNo=$page&distributorFilter=$distributorFilterId&distributorRateListFilter=$distributorRateListFilterId\"  class=\"link1\"><<</a> ";
	 	} else {
		...
		if ($pageNo < $maxpage) {
   			$page = $pageNo + 1;
   			$next = " <a href=\"tasklist.php?pageNo=$page&distributorFilter=$distributorFilterId&distributorRateListFilter=$distributorRateListFilterId\"  class=\"link1\">>></a> ";
	 	} else {
	...
	<tr  align="center">
		...
		?>
		<th class="listing-head" style="padding-left:5px; padding-right:5px;">Task</th>
		<th class="listing-head" style="padding-left:5px; padding-right:5px;">Description</th>
		<th class="listing-head" style="padding-left:5px; padding-right:5px;">Assign to</th>
		<th class="listing-head" style="padding-left:5px; padding-right:5px;" onMouseover="ShowTip('Final Margin');" onMouseout="UnTip();">Completed Date</th>	
		<th class="listing-head" style="padding-left:5px; padding-right:5px;">Status</th>	
		<? if($edit==true){?>
			<th class="listing-head">Action</th>
		<? }?>
	</tr>
	</thead>
	<tbody>
	...
	<? if($edit==true){?>
		<td class="listing-item" width="60" align="center">
			<input type="submit" value=" Edit " name="cmdEdit" onClick="assignValue(this.form,'<?=$selCriteria?>','editSelItem');assignValue(this.form,'1','editSelectionChange');this.form.action='tasklist.php';" >
		</td>
	<? }?>
	</tr>
	<?
								...
								<tr>	
									<td colspan="3">
										<table cellpadding="0" cellspacing="0" align="center">
											<tr>
												<td><? if($del==true){?>
												<input type="submit" value=" Delete **" style="background-color:#ff0000;color: white;" name="cmdDelete" onClick="return confirmDelete(this.form,'delId_',<?=$distMarginRecordSize;?>);" ><? }?>&nbsp;<? if($add==true){?><input type="submit" value=" Add New " name="cmdAddNew" class="button"><? }?></td>
											</tr>
										</table>									</td>
								</tr>
								<tr>
	...
	<tr>
		<td  colspan="3" align="center">
			<input type="submit" name="cmdBack" onClick="this.form.action='tasklist.php';" class="button" value=" Go Back " >&nbsp;&nbsp;<input type="submit" name="cmdContinue" class="button" value=" Continue " onClick="return validateEditSelection();">
		</td>
	</tr>			
								<tr>
									<td colspan="3" height="5" ></td>
