...
//echo "itemSelect".$itemSelect;
	...
	if($p["supplierSelect"]!="" && $p["searchMode"]=="S")
	{
		$addMode=true;
	...
	else if($p["itemSelect"]!="" && $p["searchMode"]=="I")
	{	
		...
		$selSupplierId="";
		$selectSupplierId="";
		//echo "--1--".$editSupplierId.")--2--".$selectSupplierId; 
		$supplierStockRecords	= $purchaseOrderInventoryObj->fetchAllSuppliers($supplierFilterId, $supplierRateListFilterId,$stockid);
	}

	//echo "searchMode".'--'.$p["searchMode"];
	echo "supplierSelect".$p["supplierSelect"].'----'."itemSelect".$p["itemSelect"];
...
<form name="frmPurchaseOrderInventory" action="PurchaseOrderInventory.php" method="post">
	<table cellspacing="0"  align="center" cellpadding="0" width="90%">	
		...
		<?
			if (($editMode && $p["searchMode"]=="S") || ($addMode && $p["searchMode"]=="S")  ) {
		?>
		<tr>
			<td>
							...
							<tr class="listing-head" >
								<td colspan="5">
									<table cellpadding="0" cellspacing="0" align="center" border=0>
										<tr class="listing-head">
											<td align="center" colspan=3><input name="searchMode" id="supplierMode" type="radio" value="S"   class="chkBox" onclick="showSupplierList(this);" <?=$chkSupplier?>>Supplier<input name="searchMode" id="itemMode" type="radio" value="I"  <?=$chkItem?> class="chkBox"   onclick="showItemList(this)" >Item
											</td>
										</tr>
										<tr class="listing-head" width="60" align="center" nowrap style="padding-left:10px; padding-right:10px;">
											<td align="center">&nbsp;
												<div id="showSp" style="display:none" nowrap>Supplier 
													<select name="supplierSelect" id="supplierSelect" onChange="supplierLoad(this)" >
														<option value="">--Select--</option>
														 <?
														foreach($supplierRecords as $sr)
														{
												...
												<div id="showIt"  style="display:block"  ><?php } else {?>
													...
													Select Item <select name="itemSelect" onchange="itemLoad(this);"  id="itemSelect"><option value="">Select</option>
