...
## ----------------- Pagination Settings I End ------------	
	...
	if($_REQUEST["action"]=="displayMsg")
	{
		...
		$checkGateNumberSettingsExist=$ingredientPurchaseorderObj->chkValidPOId($selDate,$company,$unit);
		if (sizeof($checkGateNumberSettingsExist)>0){
		$alphaCode=$ingredientPurchaseorderObj->getAlphaCode($selDate,$company,$unit);
		...
		//echo $unit; die();
		$checkExist=$ingredientPurchaseorderObj->checkPOIDDisplayExist($company,$unit);
		if ($checkExist>0){
		$getFirstRecord=$ingredientPurchaseorderObj->getmaxPOId($company,$unit);
		$getFirstRec= $getFirstRecord[0];
		$getFirstRecEx=explode($alphaCodePrefix,$getFirstRec);
		$nextGatePassId=$getFirstRecEx[1]+1;
		//$nextGatePassId;
		$validendno=$ingredientPurchaseorderObj->getValidendnoPOId($selDate,$company,$unit);
