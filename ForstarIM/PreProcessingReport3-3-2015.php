<?php
	...
	require_once("lib/preprocessingreport_ajax.php");
	$err		= "";
	$errDel		= "";
	$editMode	= false;
	$addMode	= true;
		...
		<tr>
			<td>
				...
				<select name="fish" onchange="functionLoad(this)">
					...
					<select name="preProcessCode" id="preProcessCode" onchange="functionLoad(this)">
                                         <option value="">-- Select --</option>
                                        <?
...
?>
		...
		<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#999999">
                	<tr align="left" bgcolor="#FFFFFF">
			      <td colspan="<?=(3+(2*sizeof($preProcessorSummaryRecords)))?>" class="fieldName">&nbsp;PRE-PROCESS SUMMARY<br />&nbsp;FOR THE PERIOD FROM <?=$dateFrom?> TO <?=$dateTo?> </td>
                        </tr>
                        <tr bgcolor="#f2f2f2" align="center">
				...
				?>
                                  <tr bgcolor="#FFFFFF"> 
                                    ...
                                    <?
										...
										$preProcessorHOQty	=	$preprocessingreportObj->findPreProcessorHOQty($fishId,$preProcessId,$fromDate,$tillDate,$masterPreProcessorId);
										//echo $preProcessorHOQty[0];
										
										//commented by athira on 28-1-2015
										//$preProcessQty 	=  number_format(($preProcessorHOQty[0]),'','.','');
									...
									<? }?>
								    <td class="listing-item" nowrap align="right" style="padding-left:5px; padding-right:5px;"><b><? echo number_format($totalPreProcessQty,'2','.','');?><? /*echo number_format($totalPreProcessQty,'','.','');*/?></b></td>
								    <?php
									$preProcessorQty = "";
									$totalPeeledQty = 0;	
									foreach ($preProcessorSummaryRecords as $pr) {
									...
									?>
									<td class="listing-item" nowrap align="right" style="padding-left:5px; padding-right:5px;"><? echo number_format($preProcessorQty,'2','.','');?><? /*echo number_format($preProcessorQty,'','.','');*/?></td>
									<? }?>
									
									<td class="listing-item" nowrap align="right" style="padding-left:5px; padding-right:5px;">
										<b><? echo number_format($totalPeeledQty,'2','.','');?><?/* echo number_format($totalPeeledQty,'','.','');*/?></b>
									</td>
								  <?								  
								  $prevFishId 		= 	$fishId;
								  }?>
								  ...
								  $eachRecordYield = "";
									...
									?>
								  <td class="listing-head" align="right" style="padding-left:5px; padding-right:5px;"><? echo number_format($grandTotalPreProcessQty,'2','.','');?></td>
								  <? }?>
								  <td align="right" class="listing-head" style="padding-left:5px; padding-right:5px;"><? echo number_format($grandTotalSummaryQty,'2','.','');?></td>
								  <?
								  $actualYield = "";
								  $preProcessedQty = "";
								  $preProcessQty = "";
								  ...
								  $grandTotalPreProcessedQty = "";
									...
									?>
								  <td align="right" class="listing-head" style="padding-left:5px; padding-right:5px;"><? echo number_format($grandTotalPreProcessedQty,'2','.','');?></td>
								  <? }?>
								  <td align="right" class="listing-head" style="padding-left:5px; padding-right:5px;"><? echo number_format($grandTotalSummaryQty,'2','.','');?></td>								 
								  </tr>
								</table></td>
