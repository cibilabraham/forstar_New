<?php
	...
	require_once("lib/IgstMaster_ajax.php");
	...
	if ($p["cmdAdd"]!="") {
		...
		$edRateListId =$igstMasterObj->latestRateList();


		# Creating a New Rate List
		if ($gstRateListId=="" && $edRateListId=="") {			
			$rateListName = "Igst"."-".date("dMy");
			$startDate    = date("Y-m-d");
			$gstRateListRecIns = $igstMasterObj->addGstRateList($rateListName, $startDate, '', $userId, '');
			if ($gstRateListRecIns) {
				$gstRateListId =$igstMasterObj->latestRateList();
			}
		...
		if ($gstRateListId!=0) {
					...
					if ($status!='N') {
						...
						if ($selPCategory!="" && $selPState==0) {							
							...
							foreach ($prodStateRecords as $cr) {
								...
								$prodGroupExist = $igstMasterObj->checkProductGroupExist($prodStateId);
								...
								if ($prodGroupExist) {
									...
									foreach ($prodGroupRecs as $pgr) {
										...
										$recExist = $igstMasterObj->checkGstExist($selPCategory, $prodStateId, $prodGroupId, $gstRateListId);
										if (!$recExist) {
											$gst_perIns = $igstMasterObj->addGst($selPCategory, $prodStateId, $prodGroupId, $gstPercent,$gstRateListId, $chapterSubheading, $goodsType);
										}
						...
						if ($selPCategory!="" && $selPState!=0 &&  $selPGroup==0) {
							...
							# Chk Prod Group Exist
							$prodGroupExist = $igstMasterObj->checkProductGroupExist($selPState);
							if ($prodGroupExist) {					
								# Prod Group Recs
								//$prodGroupRecs = $productGroupObj->fetchAllRecords();
								$prodGroupRecs = $productGroupObj->fetchAllRecordsActiveGroup();
								...
								foreach ($prodGroupRecs as $pgr) {
									$prodGroupId	= $pgr[0];
									$recExist = $igstMasterObj->checkGstExist($selPCategory, $selPState, $prodGroupId, $gstRateListId);
									if (!$recExist) {
										$gst_perIns = $igstMasterObj->addGst($selPCategory, $selPState, $prodGroupId, $gstPercent, $gstRateListId, $chapterSubheading, $goodsType);
									}
						...
						if ($selPCategory!="" && $selPState!=0 && $selPGroup!=0 && $selPGroup!='N') {	
							$recExist = $igstMasterObj->checkGstExist($selPCategory, $selPState, $selPGroup, $gstRateListId);
							
							if (!$recExist) {			
								$gst_perIns = $igstMasterObj->addGst($selPCategory, $selPState, $selPGroup, $gstPercent, $gstRateListId, $chapterSubheading, $goodsType);
							}
						}					
			...
			if ($gst_perIns) {
				$addMode = false;
				$sessObj->createSession("displayMsg",$msg_succAddIGstMaster);
				$sessObj->createSession("nextPage",$url_afterAddIGstMaster.$selection);
			} else {
				$addMode = true;
				$err	 = $msg_failAddIGstMaster;
			}
			$stateVatRecIns = false;
		} else {
			$addMode = true;
			if ($entryExist) $err = $msg_failAddIGstMaster."<br>".$msgFailAddStateVatExistRec;
			else $err = $msg_failAddIGstMaster;
		}
	}
	...
	if ($p["cmdSaveChange"]!="") {
			...
			for ($i=0; $i<$tableRowCount; $i++) {
				...
				if ($status!='N') {
					...
					if ($edEntryId!="" && $gstPercent!=0) {
						...
						$updateGstRec = $igstMasterObj->updateGst($edEntryId, $gstPercent, $chapterSubheading, $goodsType);
					} /*else if ($gstId!="" && $selPCategory!="" && $selPState!="" && $edEntryId=="") {
						$gst_perIns = $igstMasterObj->addGst($gstId, $selPCategory, $selPState, $selPGroup, $gstPercent);
					}*/
					if ($gstPercent!=$hidGstPercent) $gstPercentChanged = true;
				} // Status Checking End

				if ($status=='N' && $edEntryId!="") {
					//$delVatEntryRec = $igstMasterObj->delVatEntryRec($edEntryId);
				}
		...
		if ($newRateList>0) {
			...
			$rateListName = "Igstst"."-".date("dMy");
			$startDate    = date("Y-m-d");
			$gstRateListRecIns = $igstMasterObj->addGstRateList($rateListName, $startDate, $gstRateListId, $userId, $gstRateListId);
			if ($gstRateListRecIns) {
				//$gstRateListId =$igstMasterObj->latestRateList();
				for ($i=0; $i<$tableRowCount; $i++) {
					$status   = $p["status_".$i];
					...
					if ($status!='N') {
						...
						$goodsType 	= $p["goodsType_".$i];
						if ($edEntryId!="" && $gstPercent!=0) {
							$updateGstRec = $igstMasterObj->updateGstByBaseId($edEntryId, $gstPercent, $chapterSubheading, $goodsType);
						}
					}
				}
		...
		if ($updateGstRec) {
			$sessObj->createSession("displayMsg",$msg_succigstUpdate);
			if ($newRateList>0) $sessObj->createSession("nextPage",$url_afterUpdateIGstMaster);
			else $sessObj->createSession("nextPage",$url_afterUpdateIGstMaster.$selection);
		} else {
			$editMode	=	true;			
			if ($entryExist) $err = $msg_failIgstMasterUpdate."<br>".$msgFailAddStateVatExistRec;
			else $err = $msg_failIgstMasterUpdate.$newErr;
		}
	...
	if ($p["editId"]!="" && $p["cmdCancel"]=="") {
		$editId		= $p["editId"];
		$editMode	= true;
		$stateVatRec	= $igstMasterObj->find($editId);
		$editStateVatId = $stateVatRec[0];		
		$stateWiseVatRateListId = $stateVatRec[2];
		
		# Get Entry Records
		$gstEntryRecords = $igstMasterObj->getGstEntryRecords($editStateVatId);			
	}
	...
	if ( $p["cmdDelete"]!="") {
		...
		for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($gstId!="") {
				...
				$recInUse  = $igstMasterObj->checkGstRecInUse($gstId);
				# Delete
				if (!$recInUse) $gstRecDel = $igstMasterObj->deleteGstRec($gstId);
						
			} // State vat id ends here
		} // Loop ends here
		if ($gstRecDel) {
			$sessObj->createSession("displayMsg",$msg_succDelIGstMaster);
			$sessObj->createSession("nextPage",$url_afterDelIGstMaster.$selection);
		} else {
			if ($recInUse) $errDel = $msg_failDelIgstMaster."<br>The selected record is alreay in use. ";
			else $errDel	=	$msg_failDelIgstMaster;
		}
		$gstRecDel	=	false;
	}	
...
if ($p["btnConfirm"]!="")
	{
		...
		for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($gstId!="") {
				...
				$gstRecConfirm = $igstMasterObj->updateGstconfirm($gstId);
			}
...
for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($gstId!="") {
				#Check any entries exist
					...
					$gstRecConfirm = $igstMasterObj->updateGstReleaseconfirm($gstId);
	...
	$gst_perResultSetObj = $igstMasterObj->fetchAllPagingRecords($offset, $limit, $gstRateListFilterId);
	$gst_perRecordSize = $gst_perResultSetObj->getNumRows();

	## -------------- Pagination Settings II -------------------
	$fetchGstMasterResultSetObj = $igstMasterObj->fetchAllRecords($gstRateListFilterId);
	...
	$gstRLFilterRecs = $igstMasterObj->filterGstRateListRecs();

	$curgstRateListId = $igstMasterObj->latestRateList();	

	if ($gstRateListFilterId!="") $gstRateListId =$gstRateListFilterId;
	else $gstRateListId = $curgstRateListId;	

	if ($gstRateListId) {		 
		$selTmr = $igstMasterObj->getGSTRateListRec($gstRateListId);
	...
	if ($p["cmdEDActive"]!="") {
		...
		$updateGSTFlag = $igstMasterObj->updateGSTFlag($edActive);
	}

	$edFlag = $igstMasterObj->findGst($curgstRateListId);
	...
	#heading Section
	if ($editMode) $heading	=	$label_editIgstaster;
	else	       $heading	=	$label_addIGstMaster;

	
	$ON_LOAD_SAJAX = "Y"; // This screen is integrated with SAJAX, settings for TopLeftNav	
	# Include JS
	$ON_LOAD_PRINT_JS	= "libjs/IgstMaster.js"; 

	# Include Template [topLeftNav.php]
	require("template/topLeftNav.php");
?>
	<form name="frmGstMaster" action="IgstMaster.php" method="post">
...
<tr>
										...
										<table cellpadding="0"  width="65%" cellspacing="0" border="0" align="center">
											...
											<tr>
											  ...
											  <td colspan="2" align="center">
												<input type="submit" name="cmdCancel" class="button" value=" Cancel " onclick="return cancel('IgstMaster.php');" />&nbsp;&nbsp;
	...
	<tr id="catRow1">
		<td colspan="2" style="padding-left:5px;padding-right:5px;">
			...
			<tr align="center">
				...
				<th class="listing-head" nowrap style="padding-left:5px;padding-right:5px;text-align:center;">*Igst Percent</th>
	...
	if (sizeof($gstEntryRecords)>0) {
		...
		foreach ($gstEntryRecords as $ver) {			
			...
			$productGroupExist = $igstMasterObj->checkProductGroupExist($selProdState);
			# Product Group Records
			$productGroupRecords = $igstMasterObj->filterProductGroupList($productGroupExist);	
								...
								<tr>	
									<td colspan="3">
										<table cellpadding="0" cellspacing="0" align="center">
											<tr>
												<td><? if($del==true){?><input type="submit" value=" Delete **" style="background-color:#ff0000;color: white;"  name="cmdDelete" onClick="return confirmDelete(this.form,'delId_',<?=$gst_perRecordSize;?>);" ><? }?>&nbsp;<? if($add==true){?>
												<input type="submit" value=" Add New " name="cmdAddNew" class="button"><? }?>&nbsp;<? if($print==true){?>
												<!--
												<input type="button" value=" Print " name="btnPrint" class="button" onClick="javascript:printWindow('PrintIgstMaster.php',700,600);">
												-->
																								<? }?></td>
											</tr>
										</table>									</td>
...
<? if($maxpage>1){?>
		...
		for ($page=1; $page<=$maxpage; $page++) {
			if ($page==$pageNo) {
      				...
      				$nav.= " <a href=\"IgstMaster.php?pageNo=$page&gstRateListFilter=$gstRateListFilterId\" class=\"link1\">$page</a> ";
				//echo $nav;
   			}
		}
		if ($pageNo > 1) {
   			$page  = $pageNo - 1;
   			$prev  = " <a href=\"IgstMaster.php?pageNo=$page&gstRateListFilter=$gstRateListFilterId\"  class=\"link1\"><<</a> ";
	 	} else {
		...
		if ($pageNo < $maxpage) {
   			$page = $pageNo + 1;
   			$next = " <a href=\"IgstMaster.php?pageNo=$page&gstRateListFilter=$gstRateListFilterId\"  class=\"link1\">>></a> ";
	 	} else {
	...
	<tr align="center">
		...
		<th class="listing-head" style="padding-left:10px; padding-right:10px;">IGST (%)</th>
...
<? if($edit==true){?>
		<td class="listing-item" width="60" align="center"><?php if ($active==0){ ?><input type="submit" value=" Edit " name="cmdEdit" onClick="assignValue(this.form,<?=$gstId;?>,'editId');assignValue(this.form,'1','editSelectionChange');this.form.action='IgstMaster.php';" ><? }?></td>
<? }?>
