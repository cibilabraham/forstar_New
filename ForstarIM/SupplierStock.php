<?php
	...
	if ($p["cmdAdd"]!="" || $p["cmdAddAnother"]!="") {
		...
		$hidTableRowCount	= $p["hidTableRowCount2"];						
		$vaildDateEntry	=$supplierstockObj->chkValidDateEntry($startdate,"",$selSupplierId,$selStockId); 
		...
		if($vaildDateEntry){
			...
			//$dateStart=mysqlDateformat
				...
				//echo "Supplier".$selSupplierId."Stock".$selStockId."stock".$stockType;	
				//die();
			if ($selSupplierId!="" && $selStockId!="" && $stockType!="") 
			{
				...
				if($supplierStockRecIns) 
				{
					...
					$lastRateListId =$supplierstockObj->getSupplierRateList($endDate,$selSupplierId,$selStockId);
					//echo $lastRateListId;
					//die();
					if ($lastRateListId!="" && $lastRateListId!="0") 
					{
						$updateRateListEndDate = $supplierstockObj->updateRateListRec($lastRateListId, $endDate);
					}	
				}
				...
				if ($stockType=='P') {
					...
					for ($i=0; $i<$hidLayerCount; $i++) {
						...
						if($statusLayer!='N')
						{
							...
							$newLayer	=	$p["newLayer_".$i];
							if ($paperQuality!="" && $layerBrand!="") {
								$layerRecIns	=	$supplierstockObj->addLayer($lastId,$paperQuality,$layerBrand,$layerGsm,$layerBf,$layerCobb,$layerNo,$newLayer);
							}
						}
					}
				}
				...
				if(sizeof($hidTableRowCount)>0)
				{
					for($i=0; $i<$hidTableRowCount; $i++)
					{
						...
						if($statusUnit!='N')
						{
							...
							$punitId	=	$p["punitId_".$i];
							$unitRecIns	=	$supplierstockObj->addCompanyUnit($lastId,$selSupplierId,$selStockId,$companyId,$punitId);
						}
					}
				}
			}
	...
	if ($p["editId"]!="") {
		...
		if(sizeof($layerRecs)>0)
		{
			$layer	=sizeof($layerRecs);
		}
		$companyUnitRecs		=	$supplierstockObj->fetchCompanyUnitRecords($editId);
		$companyRecs		=	$supplierstockObj->getCompany($editStockId);
		//$layer			=	sizeof($layerRecs);
		//echo "The value of layer is $layer";
	}
	...
	if ($p["cmdSaveChange"]!="")
	{
		...
		$hidenewstartDate=mysqlDateFormat($p["hidenewstartDate"]);
		$sDate		= explode("-",$newstartDate);
		$endDate  	= date("Y-m-d",mktime(0, 0, 0,$sDate[1],$sDate[2]-1,$sDate[0]));
		$hidTableRowCount	= $p["hidTableRowCount2"];
		//echo $newstartDate;
		//die();
		...
		if (!$supplierStockInPO) {
			if ($supplierStockId!="" && $selSupplierId!="" && $stockType!="" && $newstartDate!="") {
				$supplierStockRecUptd	=	$supplierstockObj->updateSupplierStock($supplierStockId, $selSupplierId, $selStockId, $quotePrice, $negoPrice, $exciseRate, $cstRate, $schedule, $remarks, $stockType, $layerKgRate, $layerConverRate, $supplierRateList, $unitPricePer, $unitPricePerOneItem,$startdate,$endDate);
				...
				if ($stockType=='P') {							
					...
					for ($i=0; $i<$hidLayerCount; $i++) {
						...
						if($statusLayer!='N')
						{
							...
							$layerNo	=	$p["layerNo_".$i];
							$newLayer	=	$p["newLayer_".$i];
							$hidLayerId	=	$p["hidLayerId".$i];
							if ($paperQuality!="" && $layerBrand!="" && $hidLayerId=="") {
								$layerRecIns	=	$supplierstockObj->addLayer($supplierStockId, $paperQuality, $layerBrand, $layerGsm, $layerBf, $layerCobb, $layerNo,$newLayer);
							}
							else if ($paperQuality!="" && $layerBrand!="" && $hidLayerId!="") {
								$layerRecIns	=	$supplierstockObj->updateLayer($supplierStockId, $paperQuality, $layerBrand, $layerGsm, $layerBf, $layerCobb, $layerNo,$newLayer,$hidLayerId);
							}
						}
						if ($statusLayer=='N' && $hidLayerId!="") 
						{
							$delCompanyUnitRec = $supplierstockObj->delLayerRec($hidLayerId);
						}
					}
				}
				...
				# ----------------------------COMPANY AND UNIT
					...
					for ($j=0; $j<$hidTableRowCount; $j++) {
						...
						$stockCmpUnitid  		= $p["stockCmpUnitid_".$j];
						if ($statusUnit!='N') {
							$companyId	=	$p["companyId_".$j];
							$punitId	=	$p["punitId_".$j];
							
							if ($companyId!="" && $punitId!="" && $stockCmpUnitid!="") {
							//echo 'hi';
								$updateUnitRecIns = $supplierstockObj->updateCompanyUnit($supplierStockId, $selSupplierId,$selStockId,$companyId,$punitId,$stockCmpUnitid);
								
							} else if ($companyId!="" && $punitId!=""  && $stockCmpUnitid=="") {	
								//echo 'test';
								$unitRecIns	=	$supplierstockObj->addCompanyUnit($supplierStockId,$selSupplierId, $selStockId,$companyId,$punitId);
							}
							//die;
						} // Status Checking End

						if ($statusUnit=='N' && $stockCmpUnitid!="") 
						{
							$delCompanyUnitRec = $supplierstockObj->delCompanyUnitRec($stockCmpUnitid);
						}
					} 
			}	
		}

		if ($newstartDate!="" && $hidenewstartDate=="")
		{
	...
	if ($p["cmdDelete"]!="") {
		...
		for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($supplierStockId!="") {
				$deleteLayerRecs	 =	$supplierstockObj->deleteLayerRecs($supplierStockId);
				$deleteCompanyUnitRecs	 =	$supplierstockObj->deleteStockCompanyUnit($supplierStockId);
	...
	*/
	...
	if($p["cmdUpdate"]!="") 
	{
		...
		if($priceModified!="" || $scheduleModified!="")
		{
			for ($i=1; $i<=$rowCount; $i++) 
			{
				$supplierStockId	= $p["hidSupplierStockId_".$i];
				$getPORecords = $supplierstockObj->getPurchaseOrderRec($supplierStockId);
				//die();
				if (sizeof($getPORecords)>0) 
				{
					$err = "The selected Supplier have some pending PO's. So price change will Create a new Price List";
				}
				else
				{
					//$supplierStockId	= $p["hidSupplierStockId_".$i];
	...
	//echo "--&&&$supplierRateList";
?>
<form name="frmSupplierStock" id="frmSupplierStock" action="SupplierStock.php"  method="post">
																	...
																	<td colspan="2" >
																		<table cellpadding="0"  width="65%" cellspacing="0" border="0" align="center">
																				...
																				<td colspan="2" nowrap style="padding-left:10px; padding-right:10px;" >
																					<table width="200" align="center">
																						<tr>
																							<td colspan="2" valign="top">
																								<table>
																									...
																									<tr>
																										...
																										<td nowrap="true">			
																											<!--<select name="selStock" id="selStock" <!--onchange=" <? if ($addMode==true) { ?>this.form.submit();<? } else { ?> this.form.editId.value=<?=$editId?>;this.form.submit();<? }?>  xajax_getCompanyUnit(this.value,'','');" style="width:150px;">		-->
																											<select name="selStock" id="selStock" onchange="xajax_getCompanyUnit(this.value);" style="width:150px;">
																												<option value="">--select--</option>
																												<?php
																									...
																									<tr>
																										...
																										<td>
																											<table cellpadding="1" cellspacing="1" bgcolor="#cccccc">
																												<tr align="">
																													<td nowrap class="fieldName" style="text-align:left" width="45%">*Company</td>
																													<td nowrap  nowrap class="fieldName" style="text-align:left">*Unit</td>
																												</tr>
																												<tr >
																													<td colspan="2">
																														<table id="tblPOItem2">
																														<? 
																														if(sizeof($companyUnitRecs)>0)
																														{
																															$i=0;
																															foreach($companyUnitRecs as $cUR)
																															{
																																...
																																$companyId=$cUR[1];
																																$unitId=$cUR[2];
																																$unitRecs=$supplierstockObj->getUnit($editStockId,$companyId);
																														?>
																														<tr id="row_<?=$i?>">
																															<td nowrap class="listing-item" align="center">
																																<select id="companyId_<?=$i?>" onchange="xajax_getUnit(document.getElementById('selStock').value,document.getElementById('companyId_<?=$i?>').value,'<?=$i?>','');" name="companyId_<?=$i?>">
																																<? 
																																foreach($companyRecs as $com=>$comName)
																																{	
																																	$selected = ($companyId==$com)?"Selected":"";	
																																?>
																																	<option value="<?=$com?>" <?=$selected?>><?=$comName?></option>
																																<? 
																																} 
																																?>
																																</select>
																															...
																															<td nowrap  nowrap class="listing-item" align="center">
																																<select id="punitId_<?=$i?>" onchange="getPackValue('<?=$i?>');" name="punitId_<?=$i?>">
																																<? 
																																foreach($unitRecs as $ut=>$utName)
																																{	
																																	$selected = ($unitId==$ut)?"Selected":"";	
																																?>
																																	<option value="<?=$ut?>" <?=$selected?>><?=$utName?></option>
																																<? 
																																} 
																																?>
																																</select>
																															...
																															<td class="listing-item" align="center">
																																<a onclick="setPOItemStatusUnit('<?=$i?>');" href="###">
																																<img border="0" style="border:none;" src="images/delIcon.gif" title="Click here to remove this item">
																																</a>
																																<input id="statusUnit_<?=$i?>" type="hidden" value="" name="statusUnit_<?=$i?>">
																																<input id="IsFromDB_<?=$i?>" type="hidden" value="N" name="IsFromDB_<?=$i?>">
																																<input id="poEntryId_<?=$i?>" type="hidden" value="" name="poEntryId_<?=$i?>">
																																<input id="stockCmpUnitid_<?=$i?>" type="hidden" value="<?=$companyUnitId?>" name="stockCmpUnitid_<?=$i?>">
																															</td>
																														</tr>
																														<?
																														$i++;	
																									...
																									<tr>
																										<td>&nbsp;<input type='hidden' name="hidTableRowCount2" id="hidTableRowCount2" value="<?=$k1+1;?>"></td>
																										<td>&nbsp;<a href="###" id='addRow' onclick="javascript:addNewItem2();"  class="link1" title="Click here to add new item."><img  SRC='images/addIcon.gif' BORDER='0' style='border:none;padding-right:0px;vertical-align:middle;' >Add Item</a></td>
																									</tr>
																									<tr>
																											...
																											<table cellpadding="0" cellspacing="0">
																												<TR>
																													<TD>
																														...
																														<input name="hidenewstartDate" type="hidden" id="hidenewstartDate" size="8" value="<?=$enddate?>" style="text-align:right;"  <? if($enddate) {?> readonly <? } ?>>
																													</TD>
																													<td class="listing-item"><div id="unitPricePerTxt"></div></td>
																												</TR>
																											</table>
																							...
																							<td valign="top" id="layerDisp">
																							<? if($stockType=='P' && $editMode){?>
																								<table>
																									<tr>
																										...
																										?>
																											<table>
																														...
																														<table width="200" border="0" cellpadding="1" cellspacing="1" class="newspaperType" id="tblNewLayer">
																															...
																															*/
																															...
																															<tr align="center" id='rows_<?=$i?>'>
																																...
																																<td nowrap="true"><? if($newLayer=='1') { ?><a href='###' onClick="setPOItemStatusLayer(<?=$i?>);" ><img title="Click here to remove this item" SRC='images/delIcon.gif' BORDER='0' style='border:none;'></a><input id="statusLayer_<?=$i?>" type="hidden" value="" name="statusLayer_<?=$i?>"><input id="IsFromDB_<?=$i?>" type="hidden" value="N" name="IsFromDB_<?=$i?>"><input id="newLayer_<?=$i?>" type="hidden" value="<?=$newLayer?>" name="newLayer_<?=$i?>"><? } ?><input id="hidLayerId_<?=$i?>" type="hidden" value="<?=$hidLayerId?>" name="hidLayerId_<?=$i?>"></td>
																															</tr>
																															<? 
																															}
																															?>
																												...
																												<tr>
																													<td class="fieldName">
																													<a href="javascript:newLayer(<?=$mode?>)">Add Another Layer </a>
																													<? /* 
																													if($addMode==true){?>
																													<a href="javascript:newLayer(<?=$mode?>)">Add Another Layer </a><? } else {?><a href="javascript:newLayer(<?=$mode?>)" onclick="document.frmSupplierStock.editId.value=<?=$editId?>;">Add Another Layer </a><? } */?>
																													</td>
																												</tr>
																											</table>
																											<!--</fieldset>-->
																										...
																										<td colspan="2" class="fieldName">
																											<table width="200">
																												<tr>
																													...
																													<td class="listing-item" nowrap="nowrap"><input name="layerKgRate" type="text" id="layerKgRate" size="3" value="<?=$layerRate?>" style="text-align:right;">(Per Kg) </td>
																												</tr>
																												<tr>
																			...
																			<tr>
																				...
																				<?}?>
																					<input type="hidden" name="stockType" value="<?=$stockType?>" id="stockType"/>
																			</tr>
																			<tr>
																				<td colspan="2"  height="10" ></td>
																			</tr>
										...
										?>
											<thead>
												...
												<tr align="center">
													...
													<th class="listing-head" style="padding-left:10px; padding-right:10px;">End Date</th>
													<? if($edit==true){?>
											...
											foreach ($supplierStockRecords as $ssr) {
												...
												$plantUnit=$ssr[17];
												$startdate=dateFormat($ssr[18]);
												($ssr[19]!='0000-00-00')?$enddate=dateFormat($ssr[19]):$enddate="";
												//$startdate=$ssr[19];
											?>
	...
	?>
	<script>
	fieldIdStock = <?=sizeof($companyUnitRecs)?>;
	</script>
		<?php }		
	?>
<?
