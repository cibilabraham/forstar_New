		...
		if ($purchaseOrderNo!="")
		{
			...
			for ($i=0; $i<$itemCount; $i++) 
			{
				...
				if( $status != 'N' )
				{
					...
					$excisePercentage=""; $vatPercentage="";
					if($p["exciseS_".$i]!="" && $p["exciseN_".$i]=="" && $p["exciseNA_".$i]=="")
					{
						$excise=$p["exciseS_".$i];
						$excisePercentage=$p["excisePercentage_".$i];
					}
					else if($p["exciseS_".$i]=="" && $p["exciseN_".$i]!="" && $p["exciseNA_".$i]=="")
					{
						$excise=$p["exciseN_".$i];
					}
					else if($p["exciseS_".$i]=="" && $p["exciseN_".$i]=="" && $p["exciseNA_".$i]!="")
					{
						$excise=$p["exciseNA_".$i];
					}


					if($p["vatS_".$i]!="" && $p["vatN_".$i]=="" && $p["vatNA_".$i]=="")
					{
						$vat=$p["vatS_".$i];
						$vatPercentage=$p["vatPercentage_".$i];
					}
					else if($p["vatS_".$i]=="" && $p["vatN_".$i]!="" && $p["vatNA_".$i]=="")
					{
						$vat=$p["vatN_".$i];
					}
					else if($p["vatS_".$i]=="" && $p["vatN_".$i]=="" && $p["vatNA_".$i]!="")
					...
					if ($lastId!="" && $stockId!="" && $unitPrice!="" && $quantity!="" && $poItem=="") {
						$purchaseItemsIns	=	$purchaseOrderInventoryObj->addPurchaseEntries($lastId, $stockId, $unitPrice, $quantity, $totalQty,$proddesc,$printoutdesc,$notover,$plantUnitId,$excise,$excisePercentage,$vat,$vatPercentage);
					}
			   }
			}
				...
				for ($i=0; $i<$itemCount; $i++) 
				{
					...
					if( $status != 'N' )
					{
						...
						if ($purchaseOrderId!="" && $stockId!="" && $unitPrice!="" && $quantity!="") {
							$purchaseItemsIns	=	$purchaseOrderInventoryObj->addPurchaseEntries($purchaseOrderId, $stockId, $unitPrice, $quantity, $totalQty,$proddesc,$printoutdesc,$notover,$plantUnitId,$excise,$excisePercentage,$vat,$vatPercentage);
						}
					}
				}
			}
...
?>
	<SCRIPT LANGUAGE="JavaScript">
		...
		function addNewStockItem() 
		{
			...
			if (hidcount=="")
			{
				...
				{
				addNewStockItemRow('tblAddStockItem', '', '', '', '', '<?=$supplierRateListId?>', 1,'<?=$selPlantId?>','','<?=$proddesc?>','','','','','','','');
			}
	...
	*/
	...
	?>
	<script language="JavaScript">	
		addNewStockItemRow('tblAddStockItem', '<?=$poItem?>', '<?=$selStockId?>', '<?=$quantity?>', '<?=$totalAmt?>', '<?=$supplierRateListId?>', '<?=$mode?>','<?=$unitInv?>','<?=$notover?>','<?=$printoutdescrip?>','<?=$unitprice?>','<?=$prindesc?>','<?=$stkQty?>','<?=$excise?>','<?=$exciseRate?>','<?=$vat?>','<?=$vatRate?>');	
		//xajax_showFunction();
