<?php
	...
	if ($p["cmdAdd"]!="" ) {
		...
		if ($name!="") {
			...
			$categoryRecIns = $productGroupObj->addProductGroup($name, $descr);

			if ($categoryRecIns) {
				$sessObj->createSession("displayMsg",$msg_succAddProductGroup);
				$sessObj->createSession("nextPage",$url_afterAddProductGroup.$selection);
			} else {
				$addMode	=	true;
				$err		=	$msg_failAddProductGroup;
			}
	...
	if ($p["editId"]!="" ) {
		$editId		=	$p["editId"];
		$editMode	=	true;
		$categoryRec	=	$productGroupObj->find($editId);
	...
	if ($p["cmdSaveChange"]!="" ) {
		...
		if ($categoryId!="" && $name!="") {
			$categoryRecUptd = $productGroupObj->updateCategory($categoryId, $name, $descr);
		}
	
		if ($categoryRecUptd) {
			$sessObj->createSession("displayMsg",$msg_succProductGroupUpdate);
			$sessObj->createSession("nextPage",$url_afterUpdateProductGroup.$selection);
		} else {
			$editMode	=	true;
			$err		=	$msg_failProductGroupUpdate;
		}
	...
	if ( $p["cmdDelete"]!="") {
		...
		for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($categoryId!="") {
				...
				//$moreEntriesExist = $productGroupObj->checkMoreEntriesExist($categoryId);
				//if (!$moreEntriesExist)
					$categoryRecDel = $productGroupObj->deleteCategory($categoryId);
			}
		}
		if ($categoryRecDel) {
			$sessObj->createSession("displayMsg", $msg_succDelProductGroup);
			$sessObj->createSession("nextPage", $url_afterDelProductGroup.$selection);
		} else {
			$errDel	=	$msg_failDelProductGroup;
		}
...
if ($p["btnConfirm"]!="")
	{
		...
		for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($categoryId!="") {
				...
				$categoryRecConfirm = $productGroupObj->updateProductGroupconfirm($categoryId);
			}

		}
		if ($categoryRecConfirm) {
			$sessObj->createSession("displayMsg",$msg_succProductGroup);
			$sessObj->createSession("nextPage",$url_afterDelProductGroup.$selection);
		} else {
...
for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($categoryId!="") {
				#Check any entries exist
					...
					$categoryRecConfirm = $productGroupObj->updateProductGroupReleaseconfirm($categoryId);
				
			}
		}
		if ($categoryRecConfirm) {
			$sessObj->createSession("displayMsg",$msg_succRelProductGroup);
			$sessObj->createSession("nextPage",$url_afterDelProductGroup.$selection);
		} else {
	...
	# List all Category 
	$productGroupRecords	=	$productGroupObj->fetchAllPagingRecords($offset, $limit);
	$categorySize		=	sizeof($productGroupRecords);

	## -------------- Pagination Settings II -------------------
	$numrows	=  sizeof($productGroupObj->fetchAllRecords());
	$maxpage	=  ceil($numrows/$limit);
	## ----------------- Pagination Settings II End ------------


	if ($editMode)	$heading = $label_editProductGroup;
	else 		$heading = $label_addProductGroup;
	
	# Include JS
	$ON_LOAD_PRINT_JS	= "libjs/ProductGroup.js";
...
?>
	<form name="frmProductGroup" action="ProductGroup.php" method="post">
									...
									<td colspan="2" >
										<table cellpadding="0"  width="65%" cellspacing="0" border="0" align="center">
											...
											<tr>
												...
												<input type="submit" name="cmdDelCancel" class="button" value=" Cancel " onClick="return cancel('ProductGroup.php');">&nbsp;&nbsp;
												<input type="submit" name="cmdSaveChange" class="button" value=" Save Changes " onClick="return validateProductGroup(document.frmProductGroup);">												</td>
												
												<?} else{?>

												
												<td  colspan="2" align="center">
												<input type="submit" name="cmdAddCancel" class="button" value=" Cancel " onClick="return cancel('ProductGroup.php');">&nbsp;&nbsp;
												<input type="submit" name="cmdAdd" class="button" value=" Add " onClick="return validateProductGroup(document.frmProductGroup);">												</td>
			...
			<TR><TD>
				<table align="center">
											...
											<tr>
												...
												<td colspan="2" align="center">
												<input type="submit" name="cmdCancel" class="button" value=" Cancel " onClick="return cancel('ProductGroup.php');">&nbsp;&nbsp;
												<input type="submit" name="cmdSaveChange" class="button" value=" Save Changes " onClick="return validateProductGroup(document.frmProductGroup);">												</td>
				...
				<table cellpadding="0"  cellspacing="1" border="0" align="center"  width="100%">
					<tr>
						<td   bgcolor="white">
							<!-- Form fields start -->
							<?php	
								$bxHeader="Product Group";
								include "template/boxTL.php";
							?>
							<table cellpadding="0"  width="100%" cellspacing="0" border="0" align="center">
								<!--<tr>
									<td width="1" background="images/heading_bg.gif" class="page_hint"></td>
									<td  colspan="2" background="images/heading_bg.gif" class="pageName" >&nbsp;Product Group </td>
								</tr>-->
								<tr>
...
<tr>
	...
	<td colspan="2" align="center">
		...
		for ($page=1; $page<=$maxpage; $page++) {
			if ($page==$pageNo) {
      				...
      				$nav.= " <a href=\"ProductGroup.php?pageNo=$page\" class=\"link1\">$page</a> ";
				//echo $nav;
   			}
		}
		if ($pageNo > 1) {
   			$page  = $pageNo - 1;
   			$prev  = " <a href=\"ProductGroup.php?pageNo=$page\"  class=\"link1\"><<</a> ";
	 	} else {
		...
		if ($pageNo < $maxpage) {
   			$page = $pageNo + 1;
   			$next = " <a href=\"ProductGroup.php?pageNo=$page\"  class=\"link1\">>></a> ";
	 	} else {
	...
	<? }?>
		...
		<? if($edit==true){?><td class="listing-item" width="60" align="center"><?php if ($active==0){ ?><input type="submit" value=" Edit " name="cmdEdit" onClick="assignValue(this.form,<?=$categoryId;?>,'editId'); this.form.action='ProductGroup.php';"><? } ?></td>
		<? }?>
...
?>
	...
	ensureInFrameset(document.frmProductGroup);
	//-->
	</script>
<?php 
	}
