...
<?php
	...
	<tr <?php if ($active==0){?> bgcolor="#afddf8" onMouseOver="ShowTip('<?=$disMsgInactive?>');" onMouseOut="UnTip();"  <?php }?>>
		...
		<input type="checkbox" name="delId_<?=$i;?>" id="delId_<?=$i;?>" value="<?=$frozenPackingId;?>" class="chkBox"></td>
		<?php 
		}
		?>
		...
		<?php if ($active!=1) {?>
		<input type="submit" value=" Edit " name="cmdEdit" onClick="assignValue(this.form,<?=$frozenPackingId;?>,'editId'); this.form.action='FrozenPacking.php';">
		<? } ?>
		</td>
		  <? }?>
