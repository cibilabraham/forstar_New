	...
	if ($p["cmdAdd"]!="" || $p["cmdAddAnother"]!="") {
		...
		if (sizeof($uniqueRecords)==0) {
			...
			if ( $selSupplierId!="" && $selStockId!="" && $stockType!="") {
				//echo "Haii";
				$supplierStockRecIns	=	$supplierstockObj->addSupplierStock($selSupplierId, $selStockId, $quotePrice,  $negoPrice, $exciseRate, $cstRate, $schedule, $remarks, $stockType, $layerKgRate, $layerConverRate, $userId, $supplierRateList, $unitPricePer, $unitPricePerOneItem,$plantUnitid);
	...
	if ($p["cmdSaveChange"]!="") {
		...
		if (!$supplierStockInPO) {
			if ($supplierStockId!="" && $selSupplierId!="" && $stockType!="" && $newstartDate!="") {
				$supplierStockRecUptd	=	$supplierstockObj->updateSupplierStock($supplierStockId, $selSupplierId, $selStockId, $quotePrice, $negoPrice, $exciseRate, $cstRate, $schedule, $remarks, $stockType, $layerKgRate, $layerConverRate, $supplierRateList, $unitPricePer, $unitPricePerOneItem,$startdate);
...
<?php if ($editMode){?>
	<tr>
		...
		<table cellpadding="0" cellspacing="0">
			<TR>
			<TD>
				<input name="newstartDate" type="text" id="newstartDate" size="8" value="<?//=$startdate?>" style="text-align:right;" onchange="displaySupplierStockUnitPrice();">
			</TD>
			<td class="listing-item"><div id="unitPricePerTxt"></div></td>
			</TR>
		</table>
