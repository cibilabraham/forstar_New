<?php
	...
	$selection = "?pageNo=".$p["pageNo"]."&distributorFilter=".$p["hidDistributorFilterId"]."&distributorRateListFilter=".$p["distributorRateListFilter"]."&srchName=".$p["srchName"];
...
*/
	...
	//Rekha verma modified dated on 27 nov. 2018
	
	$distWiseProuctWiseMarginRecords = $distMarginStructureObj->getDistMarginPagingRecords($offset, $limit, $distributorFilterId, $distributorRateListFilterId,$srchName);
	$distMarginRecordSize = sizeof($distWiseProuctWiseMarginRecords);	

	## -------------- Pagination Settings II -------------------
	$allDistMarginStructureResultSetObj = $distMarginStructureObj->fetchAllRecords($distributorFilterId, $distributorRateListFilterId,$srchName);
...
?>		
	...
	<tr>
		...
		?>
		<thead>
		<!-- Rekha added search box dated on 27 nov 2018-->
	<tr>
...
<? if($maxpage>1){?>
		...
		for ($page=1; $page<=$maxpage; $page++) {
			if ($page==$pageNo) {
      				...
      				$nav.= " <a href=\"DistMarginStructure.php?pageNo=$page&distributorFilter=$distributorFilterId&distributorRateListFilter=$distributorRateListFilterId&srchName=$srchName\" class=\"link1\">$page</a> ";
				//echo $nav;
   			}
		}
		if ($pageNo > 1) {
   			$page  = $pageNo - 1;
   			$prev  = " <a href=\"DistMarginStructure.php?pageNo=$page&distributorFilter=$distributorFilterId&distributorRateListFilter=$distributorRateListFilterId&srchName=$srchName\"  class=\"link1\"><<</a> ";
	 	} else {
		...
		if ($pageNo < $maxpage) {
   			$page = $pageNo + 1;
   			$next = " <a href=\"DistMarginStructure.php?pageNo=$page&distributorFilter=$distributorFilterId&distributorRateListFilter=$distributorRateListFilterId&srchName=$srchName\"  class=\"link1\">>></a> ";
	 	} else {
