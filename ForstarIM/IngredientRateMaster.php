<?php
	...
	{		
		...
		if($hiddenDate!="")
		{
			$existReceipeMaster = $ingredientRateMasterObj->checkRecipeIngredient($ingredientRateId);
			if($existReceipeMaster == "")
			{
		...
		else
		{
			if($ingredientRateId!="" && $selIngredient!="" && $ingRatePerKg!="" && $newEffectDate!="")
			{
				...
				if ($newIngredientRec)
				{
					$existRecipeEntry = $ingredientRateMasterObj->checkRecipeEntry($selIngredient);
					
					if(sizeof($existRecipeEntry)>0)
					{
						foreach($existRecipeEntry as $recipe)
						{
							$recipeId     = $recipe[1];
							$recipeRecord = $ingredientRateMasterObj->getRecipeRecord($recipeId);
							
							if($recipeRecord!="")
							{
								$productGmsperPouch		=	$recipeRecord[0];
								$fishGmsperPouch		=	$recipeRecord[1]; 
								$gravyGmsperPouch		=	$recipeRecord[2];
								$pouchPerBatch			=	$recipeRecord[3];
								$pductKgPouchperBatch	=	$recipeRecord[4];
								$fishKgPouchperBatch	=	$recipeRecord[5];
								$gravyKgPouchperBatch	=	$recipeRecord[6];
							}
							$recipeIngredients = $ingredientRateMasterObj->getRecipeIngredient($recipeId);
							
							if(sizeof($recipeIngredients)>0)
							{
								$fishRsperBatch = 0;
								$gravyRsperBatch = 0;
							    foreach($recipeIngredients as $ing)
								{
									...
									$ingQuantity		= $ing[3];
									$fixedQntyCheck		= $ing[4];
									
									$ingredientRsperBatch = 0;
									$ingredientRsperPouch = 0;
									
									$latestIngredientRate = $ingredientRateMasterObj->getLatestIngredientRate($recipeIngredientId);
									
									if($latestIngredientRate !="")
									{
										$ingRateId = $latestIngredientRate[0];
										$ingRate   = $latestIngredientRate[1];
									}
									
									$ingredientRsperBatch = $ingRate*$ingQuantity;
									$ingredientRsperPouch = number_format((float)$ingredientRsperBatch/$pouchPerBatch, 2, '.', '');
									
									if($fixedQntyCheck == 'Y')
									{
										$fishRsperBatch+=$ingredientRsperBatch;
									}
									else if($fixedQntyCheck == 'N')
									{
										$gravyRsperBatch+=$ingredientRsperBatch;
									}
									
									if($ingredientRsperBatch!="" || $ingredientRsperPouch!="")
									{
										$ingredientRateMasterObj->updateRecipeIngredients($ingRate,$ingredientRsperBatch,$ingredientRsperPouch,$ingRateId,$recipeIngredientId,$recipeId);
									}
								}
								
								$productRsperBatch 		=	$fishRsperBatch + $gravyRsperBatch;
								
								$fishRsperKgperBatch 	=	$fishRsperBatch/$fishKgPouchperBatch;
								$gravyRsperKgperBatch	=	$gravyRsperBatch/$gravyKgPouchperBatch;
								$productRsperKgperBatch	=	$fishRsperKgperBatch + $gravyRsperKgperBatch;
								
								$fishRsPerPouch			=	$fishRsperKgperBatch * $fishGmsperPouch; 
								$gravyRsPerPouch		=	$gravyRsperKgperBatch * $gravyGmsperPouch;
								$productRsPerPouch		=	$fishRsPerPouch	+ $gravyRsPerPouch;
								
								foreach($recipeIngredients as $ing)
								{
									$recipeIngredientId = $ing[2];
									
									$ingredientRsperPouch	=	$ingredientRateMasterObj->getIngredientRsperPouch($recipeIngredientId,$recipeId);
									
									if($ingredientRsperPouch!="")
									{
										$ingRsperPouch		 =	$ingredientRsperPouch[0];
										$percentCostperPouch =	$ingRsperPouch/$productRsPerPouch*100;	
									}
									
									$ingCostperPouch = $ingredientRateMasterObj->updateIngredientCostperPouch($percentCostperPouch,$recipeIngredientId,$recipeId);
									
								}
								
							}
							
							$recipeRec = $ingredientRateMasterObj->updateRecipeRecord($fishRsperBatch,$gravyRsperBatch,$productRsperBatch,$fishRsperKgperBatch,$gravyRsperKgperBatch,$productRsperKgperBatch,$fishRsPerPouch,$gravyRsPerPouch,$productRsPerPouch,$recipeId);
	...
	if ($p["editId"]!="") {
		...
		$getHighLow = $ingredientRateMasterObj->displayHighLowRate($selIngredient);
		if($getHighLow!="")
		{
			foreach($getHighLow as $rate)
				{
					$ingHighPrice = $rate[0];
					$ingLowPrice = $rate[1];
				}
		...
		<TR>
			...
			<table>
				...
				<tr>
					...
					<td>
						<select name="materialType" id="materialType" disabled>
							<option value="">--Select--</option>
							<option value="1" <? if($materialType == 1){ ?> selected <? } ?>>RAW</option>
							<option value="2" <? if($materialType == 2){ ?> selected <? } ?>>CLEANED</option>
						</select>
					</td>
				</tr>
						...
						<tr>
							<td colspan="2"  height="10" ></td>
											...
											<tr>
												...
												<td colspan="2" align="center">
												<input type="submit" name="cmdCancel" class="button" value=" Cancel " onClick="return cancel('IngredientRateMaster.php');">&nbsp;&nbsp;
												<input type="submit" name="cmdSaveChange" class="button" value=" Save Changes " onClick="return validateIngredientRateMaster(document.frmIngredientRateMaster,<?=$mode;?>);">												</td>
												
												<?} else{?>

												<td  colspan="2" align="center">
												<input type="submit" name="cmdCancel" class="button" value=" Cancel " onClick="return cancel('IngredientRateMaster.php');">&nbsp;&nbsp;
												<input type="submit" name="cmdAdd" class="button" value=" Add " onClick="return validateIngredientRateMaster(document.frmIngredientRateMaster,<?=$mode;?>);">												</td>
...
<? if($maxpage>1){?>
	...
	<th class="listing-head" style="padding-left:10px; padding-right:10px;">Material<br>Type</th>
	...
	<th class="listing-head" nowrap style="padding-left:10px; padding-right:10px;">Final<br>Rate/Kg</th>
	<? if($edit==true){?>
	...
	<tr <?php if ($active==0) { ?> id="inactive" bgcolor="#afddf8" onMouseOver="ShowTip('<?=$disMsgInactive?>');" onMouseOut="UnTip();" <?php }?>>
		...
		<td class="listing-item" nowrap="nowrap" style="padding-left:10px; padding-right:10px;"><?if($materialType==1){?>RAW<?} else if($materialType==2){?>CLEANED<?}?></td>
		...
		<td class="listing-item" nowrap align="right" style="padding-left:10px; padding-right:10px;"><? if($ingHighPrice!=0){ echo $ingHighPrice; }?></td>
		<td class="listing-item" nowrap align="right" style="padding-left:10px; padding-right:10px;"><? if($ingLowPrice!=0){ echo $ingLowPrice; }?></td>
