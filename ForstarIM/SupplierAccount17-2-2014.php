<?php
	...
	if ($dateFrom!="" && $dateTill!="") {
		$landingCenterRecords	= $supplieraccountObj->fetchLandingCenterRecords($fromDate, $tillDate, $settlementDate, $dateSelectFrom);

		$supplierRecords	= $supplieraccountObj->fetchSupplierRecords($fromDate, $tillDate, $landingCenterId, $settlementDate, $dateSelectFrom);

		# Get Billing Comapany  Records
		$billingCompanyRecords = $supplieraccountObj->fetchBillingCompanyRecords($fromDate, $tillDate, $landingCenterId, $selectSupplier, $settlementDate, $dateSelectFrom);

		$RMChallanRecords	=  $supplieraccountObj->fetchChallanRecords($fromDate, $tillDate, $landingCenterId, $selectSupplier, $settlementDate, $dateSelectFrom, $billingCompany);

		#List All Fishes
		$fishMasterRecords	= $supplieraccountObj->fetchFishRecords($fromDate, $tillDate, $landingCenterId, $selectSupplier, $selChallanNo, $settlementDate, $dateSelectFrom, $billingCompany);

		if ($selFishId!="") {
			$processCodeRecords = $supplieraccountObj->getProcessCodeRecords($fromDate, $tillDate, $landingCenterId, $selectSupplier, $selChallanNo, $selFishId, $settlementDate, $dateSelectFrom, $billingCompany);	
		}
...
<? if($print==true){?>	<input type="button" name="Submit" value="Print" class="button" onClick="return printWindow('PrintSupplierAccount.php?supplyFrom=<?=$dateFrom?>&supplyTill=<?=$dateTill?>&landingCenter=<?=$landingCenterId?>&supplier=<?=$selectSupplier?>&selChallan=<?=$selChallanNo?>&fish=<?=$selFishId?>&processCode=<?=$processId?>&selSettlement=<?=$settlementDate?>&paymentMode=<?=$paymentMode?>&viewType=<?=$viewType?>&selOption=<?=$paidStatus?>&rmConfirmed=<?=$rmConfirmed?>&dateSelectFrom=<?=$dateSelectFrom?>&offset=<?=$offset?>&limit=<?=$limit?>&pageNo=<?=$pageNo?>&billingCompany=<?=$billingCompany?>',700,600);"><? }?></td>
                                        ...
                                        <td><table width="100" border="0">
                                          <tr>
                                            ...
                                            <td nowrap class="listing-item">Wt Challan Date</td>
											    ...
											    <td><select name="landingCenter" id="landingCenter" onchange="this.form.submit();">
                                                  <option value="">-- Select All --</option>
                                                  <?
		...
		?>
		  ...
		  <? }?>
					  ...
					  <tr><td><table width="200" border="0">
											  ...
											  <tr><td align="center"><input name="cmdSearch" type="submit" id="cmdSearch" value=" Search" class="button" onclick="return validateSupplierAccountSearch(document.frmSupplierAccount);"></td></tr>
											</table>
		...
		<? if( sizeof($catchEntryRecords)){ ?>
                       <tr>
                                <td colspan="24" align="center" bgcolor="#FFFFFF" class="listing-item">
											   ...
											   <tr>
											   <td align="right">
											   <table><tr><td align="center" bgcolor="#FFFFFF" class="listing-item">
											   <img src="images/x.gif" width="20" height="20">  <span style="vertical-align:top">- The entire transaction Rates are not defined.</span>
											   </td></tr></table>											   </td>
											   <td align="right">
											   <table><tr><td><a href="SettlementSummary.php?supplyFrom=<?=$dateFrom?>&supplyTill=<?=$dateTill?>&supplier=<?=$selectSupplier?>" class="link1">View Settlement Summary</a></td></tr></table>											   </td>
											   </tr>
											   <? if($maxpage>1){?>
											   <tr>
  ...
  <input type="hidden" name="hidRowCount" id="hidRowCount" value="<?=$i?>" >
	  ...
	  <? if($maxpage>1){?>
	  <tr bgcolor="#FFFFFF"><td colspan="24" style="padding-right:10px;"><div align="right">
				  <?php 				 			  
				 $nav  = '';
		for($page=1; $page<=$maxpage; $page++)
			{
