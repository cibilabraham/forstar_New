...
# MC Packing Conversion type, AC - Auto convert/ MC - Manually Convert
	...
	if ($p["cmdAddNew"]!="") {
		$addMode	=	true;		
		if ($p["mainId"]=="" && $dailyfrozenrepackingObj->checkBlankRecord()) {
			
			$mainId		=	$dailyfrozenrepackingObj->checkBlankRecord();
					
		} else {
			if ($p["mainId"]=="") {
				$tempMainTableRecIns=$dailyfrozenrepackingObj->addTempDataMainTable();
		...
		if ($dailyfrozenrepackingObj->checkBlankRecord() && $p["mainId"]=="") {
			$dailyFrozenPackingGradeRecDel = $dailyfrozenrepackingObj-> deleteFrozenRePackingGradeRec($mainId);
		}		
	}
...
#New Entry
	if ($p["cmdAdd"]!="") {
		...
		if( $fishId!="" && $processCode!="") {
			$updateFrozenRePackingRec =	$dailyfrozenrepackingObj->updateFrozenRePackingRec($mainId, $fishId, $processCode, $freezingStage, $eUCode, $brand, $frozenCode, $mCPacking, $repackReasonId, $numNewInnerPack, $numLabelCard, $numNewMC, $rePackEUCode, $rePackBrand, $rePackFrozenCode, $rePackMCPacking, $selectDate);
...
# Edit Re Packing
	...
	if( $p["editId"]!="" ){
		...
		$editId			=	$p["editId"];
		$editMode		=	true;
		$dailyFrozenRePackingRec		=	$dailyfrozenrepackingObj->find($editId);
...
if( $p["cmdSaveChange"]!="" ){?>
		...
		<?php $dailyFrozenRePackingMainId		=	$p["editId"];
			if ($dailyFrozenRePackingMainId!="") {
				...
				list($entryId)=$dailyfrozenrepackingObj->getEntryId($dailyFrozenRePackingMainId);						
				list($gradeEntryId)=$dailyfrozenrepackingObj->getGradeEntryId($entryId);
				$dailyfrozenrepackingObj->updateDailyFrozenPackingEntry($mCPacking,$repselFrozenCode,$entryId);
				...
				for ($j=1; $j<=$gradeRowCount; $j++) {
									...
									$dailyfrozenrepackingObj->updateDailyFrozenPackingGradeEdit($numMC,$numLS,$entryId,$gradeId);
									$dailyfrozenrepackingObj->updateDailyFrozenRepackingGrade($numMC,$numLS,$dailyFrozenrePackId,$gradeId);
									}
				}
				
				//$dailyFrozenPackingGradeRecDelMain = $dailyfrozenrepackingObj->deleteFrozenRePackingGradeRecMain($gradeEntryId);
				//$dailyFrozenPackingEntryRecDelMain = $dailyfrozenrepackingObj->deleteFrozenRePackingEntryRecMain($entryId);
				//$dailyFrozenRePackingRecDelMain	=$dailyfrozenrepackingObj->deleteDailyFrozenRePackingMainRecMain($dailyFrozenRePackingMainId);
				//list($glzentryId)=$dailyfrozenrepackingObj->getRepEntryId($dailyFrozenRePackingMainId);
				$dailyfrozenrepackingObj->updateDailyFrozenRepackingEntry($mCPacking,$repselFrozenCode,$dailyFrozenrePackId);
...
if( $p["cmdSaveChange1"]!="" ){
		...
		if( $fishId!="" && $processCode!="" && $mainId!="") {
			$dailyFrozenRePackingRecUptd =	$dailyfrozenrepackingObj->updateFrozenRePackingRec($mainId, $fishId, $processCode, $freezingStage, $eUCode, $brand, $frozenCode, $mCPacking, $repackReasonId, $numNewInnerPack, $numLabelCard, $numNewMC, $rePackEUCode, $rePackBrand, $rePackFrozenCode, $rePackMCPacking, $selectDate);
		}
		...
		$dailyFrozenRePackingRecUptd	=	false;*/
			...
			$selectDate		=	mysqlDateFormat($p["selDate"]);
						...
						$insertDailyRepacking=$dailyfrozenrepackingObj->insertDailyRepacking($selectDate,$processId,$freezingStageId,$frozenCodeId,$repmcPkId);
						if ($insertDailyRepacking)
						$dfpPOEntryId = $databaseConnect->getLastInsertedId();
						$dailyfrozenrepackingObj->addPhysicalStkdailyFrozenmain($selectDate, $userId,$dfpPOEntryId,$unit,$processorId);
						$dailymainId = $databaseConnect->getLastInsertedId();
						$dailyfrozenrepackingObj->adddailyfrozenEntries($dailymainId,$fish_id,$processId,$freezingStageId,$frozenCodeId,$repmcPkId);
						...
						for ($j=1; $j<=$gradeRowCount; $j++) {
									...
									if (($gradeId>0) && ($numMC>0 )) {
											...
											list($allocGrdTotal,$allocGrdLsTotal)=$dailyfrozenrepackingObj->getAllocQty($processId,$freezingStageId,$frozenCodeId,$MCPkgId,$fromDate,$tillDate,$gradeId);
											...
											if ($thaallocTotal!=0)
										{
												...
												$exisitingRecords = $dailyfrozenrepackingObj->checkRecordsExist($processId,$freezingStageId,$frozenCodeId,$MCPkgId,$fromDate,$tillDate,$gradeId);
													...
													foreach ($exisitingRecords as $er){
															...
															if ($n==1){
																$dailyFrozenPackingRecUp=$dailyfrozenrepackingObj->updateDailyFrozenPackingGrade($gradeUpid,$thaallocTotal,$thaallocLsTotal);
									...
									/*	else
										{
												...
												$exisitingRecords = $dailyfrozenrepackingObj->checkRecordsExist($processId,$freezingStageId,$frozenCodeId,$MCPkgId,$selectDate,$gradeId);
													if (sizeof($exisitingRecords)>0) {
	...
	if( $p["cmdDelete"]!=""){
		...
		for($i=1; $i<=$rowCount; $i++)		{
			...
			if ($dailyFrozenRePackingMainId!="") {
				...
				if($rmlotId=='0')
				{
					list($oldMainId)=$dailyfrozenrepackingObj->getOldMainId($dailyFrozenRePackingMainId);
					list($oldEntryId)=$dailyfrozenrepackingObj->getEntryId($oldMainId);
					$olddailygrd=$dailyfrozenrepackingObj->getGradeEntryId($oldEntryId[0]);
					list($oldNumPack)=$dailyfrozenrepackingObj->getMCNumPack($oldEntryId[1]);

					list($entryId)=$dailyfrozenrepackingObj->getEntryId($dailyFrozenRePackingMainId);
					$dailygrd=$dailyfrozenrepackingObj->getGradeEntryId($entryId[0]);
					list($numPack)=$dailyfrozenrepackingObj->getMCNumPack($entryId[1]);
					$numtotal='0';
					...
					{	
							...
							foreach($dailygrd as $delVal)
							{
								...
								//echo $delnumtotal.','.$numPack.'<br/>';
								 if( $delgrade_id==$grade_id)
								 {
									...
									// echo $oldtotalrepack.','.$delnumtotal.'<br/>';
									//$repckNew=$oldtotalrepack-$delnumtotal;

									$ls=$totalNumMCLS % $oldNumPack;
									$number=explode('.',($totalNumMCLS / $oldNumPack));
									$mc=$number[0];
									$repckNew='0';
									$updatedailygrd=$dailyfrozenrepackingObj->updateFrozenPackingGradeReEnter($id, $grade_id, $mc, $ls,$repckNew);
						...
						else
						{
							...
							foreach($dailygrd as $delVal)
							{
								...
								{
									$delnumtotal=(($delnumber_mc*$numPack)+$delnumber_loose_slab);
									$delnumtotal.','.$numPack.'<br/>';
									 if( $delgrade_id==$grade_id)
									 {
										$number=explode('.',($delnumtotal / $oldNumPack));
										$repacked=$number[0];
										$repckNew=$repacked-$repckQty;
										//echo $mc.','.$repacked.','.$repckQty.','.$repckNew.'<br/>';
								$updatedailygrd=$dailyfrozenrepackingObj->updateFrozenPackingGradeReEnter($id, $grade_id, $mc, $number_loose_slab,$repckNew);
									 }
								}
								else
								{
					...
					if($updatedailygrd)
					{
						$dailyFrozenPackingEntryRecDelMain = $dailyfrozenrepackingObj->deleteFrozenRePackingGradeWithEntryId($entryId[0]);
						$dailyfrozenrepackingObj->deleteFrozenRePackingEntryRecMain($entryId[0]);
						$dailyFrozenPackingGradeRecDelMain = 
						$dailyFrozenRePackingRecDelMain	=$dailyfrozenrepackingObj->deleteDailyFrozenRePackingMainRecMain($dailyFrozenRePackingMainId);
						$dailyFrozenPackingGradeRecDel = $dailyfrozenrepackingObj->deleteFrozenRePackingGradeRec($dailyFrozenrePackId);
						$dailyFrozenRePackingRecDel	=	$dailyfrozenrepackingObj->deleteDailyFrozenRePackingMainRec($dailyFrozenrePackId);
					}


					###commented on 10-11-2014
					/*list($entryId)=$dailyfrozenrepackingObj->getEntryId($dailyFrozenRePackingMainId);			
					list($gradeEntryId)=$dailyfrozenrepackingObj->getGradeEntryId($entryId);
					$dailyFrozenPackingGradeRecDelMain = $dailyfrozenrepackingObj->deleteFrozenRePackingGradeRecMain($gradeEntryId);
					$dailyFrozenPackingEntryRecDelMain = $dailyfrozenrepackingObj->deleteFrozenRePackingEntryRecMain($entryId);
				...
				else
				{
					...
					list($oldMainId)=$dailyfrozenrepackingObj->getOldMainIdRMLotID($dailyFrozenRePackingMainId);
					list($oldEntryId)=$dailyfrozenrepackingObj->getEntryIdRMLotID($oldMainId);
					$olddailygrd=$dailyfrozenrepackingObj->getGradeEntryIdRMLotID($oldEntryId[0]);
					list($oldNumPack)=$dailyfrozenrepackingObj->getMCNumPack($oldEntryId[1]);

					list($entryId)=$dailyfrozenrepackingObj->getEntryIdRMLotID($dailyFrozenRePackingMainId);
					$dailygrd=$dailyfrozenrepackingObj->getGradeEntryIdRMLotID($entryId[0]);
					list($numPack)=$dailyfrozenrepackingObj->getMCNumPack($entryId[1]);
					$numtotal='0';
					foreach($olddailygrd as $oldval)
					{	
							...
							foreach($dailygrd as $delVal)
							{
								...
								//echo $delnumtotal.','.$numPack.'<br/>';
								 if( $delgrade_id==$grade_id)
								 {
									...
									$mc=$number[0];
									$repckNew='0';
									$updatedailygrd=$dailyfrozenrepackingObj->updateFrozenPackingGradeReEnterRMLotID($id, $grade_id, $mc, $ls,$repckNew);
								...
								if($delrePack=='' || $delrePack=='0')
								{
									...
									$delnumtotal.','.$numPack.'<br/>';
									 if( $delgrade_id==$grade_id)
									 {
										...
										$repckNew=$repacked-$repckQty;
										//echo $mc.','.$repacked.','.$repckQty.','.$repckNew.'<br/>';
								$updatedailygrd=$dailyfrozenrepackingObj->updateFrozenPackingGradeReEnterRMLotID($id, $grade_id, $mc, $number_loose_slab,$repckNew);
									 }
								}
								else
								{
					...
					if($updatedailygrd)
					{
						$dailyFrozenPackingEntryRecDelMain = $dailyfrozenrepackingObj->deleteFrozenRePackingGradeRMLotIDWithEntryId($entryId[0]);
						$dailyfrozenrepackingObj->deleteFrozenRePackingEntryRecMainRMLotID($entryId[0]);
						$dailyFrozenPackingGradeRecDelMain = 
						$dailyFrozenRePackingRecDelMain	=$dailyfrozenrepackingObj->deleteDailyFrozenRePackingMainRecMainRMLotID($dailyFrozenRePackingMainId);
						$dailyFrozenPackingGradeRecDel = $dailyfrozenrepackingObj->deleteFrozenRePackingGradeRecRMLotID($dailyFrozenrePackId);
						$dailyFrozenRePackingRecDel	=	$dailyfrozenrepackingObj->deleteDailyFrozenRePackingMainRecRMLotID($dailyFrozenrePackId);
					}

					/*list($entryId)=$dailyfrozenrepackingObj->getEntryIdRMLotID($dailyFrozenRePackingMainId);		$dailyFrozenPackingEntryRecDelMain = $dailyfrozenrepackingObj->deleteFrozenRePackingGradeRMLotIDWithEntryId($entryId);	
					$dailyfrozenrepackingObj->deleteFrozenRePackingEntryRecMainRMLotID($entryId);
					$dailyFrozenRePackingRecDelMain	=$dailyfrozenrepackingObj->deleteDailyFrozenRePackingMainRecMainRMLotID($dailyFrozenRePackingMainId);
					$dailyFrozenPackingGradeRecDel = $dailyfrozenrepackingObj->deleteFrozenRePackingGradeRecRMLotID($dailyFrozenrePackId);
					$dailyFrozenRePackingRecDel	=	$dailyfrozenrepackingObj->deleteDailyFrozenRePackingMainRecRMLotID($dailyFrozenrePackId);*/
...
#Cancel 	
	if ($p["cmdCancel"]!="")
	{
		...
		if($p['editMode']==""){
			$dailyFrozenPackingGradeRecDel = $dailyfrozenrepackingObj-> deleteFrozenPackingGradeRec($entryId);
					
			$frozenPackingEntryRecDel	=	$dailyfrozenrepackingObj->deletePackingEntryRec($entryId);
			#Check Record Exists
			$exisitingRecords			=	$dailyfrozenrepackingObj->checkRecordsExist($mainId);
			if(sizeof($exisitingRecords)==0)
				{
					$dailyFrozenRePackingRecDel	=	$dailyfrozenrepackingObj->deleteDailyFrozenPackingMainRec($mainId);
				}
		}
		$mainId 		= "";
		$p["mainId"]	= "";
...
## -------------- Pagination Settings I ------------------
	...
	$dailyFrozenRePackingRecords		=	$dailyfrozenrepackingObj->getDFPForDateRange($fromdateSt, $tillDate,$offset, $limit);
	//$dailyFrozenRePackingRecordSize	=	sizeof($dailyFrozenRePackingRecords);
	$dailyFrozenRePackingRecordSize	=sizeof($dailyfrozenrepackingObj->getDFPReForDateRange($fromdateSt, $tillDate));
	$numrows=sizeof($dailyfrozenrepackingObj->getDFPReForDateRange($fromdateSt, $tillDate));
	...
	if ($editMode) {
		$heading	=	$label_editDailyFrozenRePacking;
	} else {
		$heading	=	$label_addDailyFrozenRePacking;
	}
					...
					<tr>
						<td   bgcolor="white"  style="<?=$leftBdr.$rightBdr.$bottomBdr.$topBdr ?>" >
							...
							<table cellpadding="0"  width="100%" cellspacing="0" border="0" align="center">
								<tr>
									<td width="1" background="images/heading_bg.gif" class="page_hint"></td>
									<td background="images/heading_bg.gif" class="pageName" >&nbsp;Daily Frozen Re-Packing </td>
...
<? //}?>
	...
	foreach($dailyFrozenRePackingRecords as $dfrp)
	{
		...
		//list($rePackedQty)=$dailyfrozenrepackingObj->getRepackGradeQty($selProcessCodeId,$selFreezingStageId,$selFrozenCodeId,$selMCPackingId,$fromDate);
		list($thawingTotal)=$dailyfrozenrepackingObj->getThaQty($selProcessCodeId,$selFreezingStageId,$selFrozenCodeId,$selMCPackingId,$fromdateSt,$tillDate);
		
		
		list($allocatedTotal)=$dailyfrozenrepackingObj->getAllocQty($selProcessCodeId,$selFreezingStageId,$selFrozenCodeId,$selMCPackingId,$fromdateSt,$tillDate);
