						...
						if ($selFish!="" && $selProcessCode!="" && $qelId)
						{
							...
							if ($gradeRowCount>0) 
							{
								...
								for ($g=1;$g<$gradeRowCount;$g++) 
								{
									...
									if(in_array($gradeId,$gradeArray))
									{
										$displayOrderId = $p["displayOrderId_".$g];
										$rate=$p["rate_".$g];
										if($frznPkgId)
										{
											$insGradeRec = $frznPkgRatingObj->addSelGradeRec($qelId,$frznPkgId, $gradeId, $displayOrderId,$rate,$userId,$processorId);
										}
									}
								}
							}
						...
						if ($selFish!="" && $selProcessCode!="" && $fznPkngRateId!="") {
							...
							if ($gradeRowCount>0) {
								...
								for ($g=1;$g<$gradeRowCount;$g++) {
									...
									//	if($frznPkgId)
										//{
											$insGradeRec = $frznPkgRatingObj->addSelGradeRec($fznPkngRateId,$frznPkgId, $gradeId, $displayOrderId,$rate,$userId,$processorId);
										//}
									}
								}
																		...
																		<!--  Dynamic Row Starts Here style="padding-left:5px;padding-right:5px;"-->
																			<tr id="catRow1">
																				<td style="padding:5 5 5 5px;">
																					<div id="filter" style="height:850px;"></div>
																					<div id="box">
																						<span id="boxtitle"></span>
																						  <iframe width="100%" height="300px" id="gradeExptIFrame" src="" style="border:none;" frameborder="0"></iframe>	
																							<!--<p align="center"> 
