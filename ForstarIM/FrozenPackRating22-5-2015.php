<?php
	...
	if ($p["cmdAdd"]!="" || $p["cmdSaveAndAddNew"]!="") {
		...
		//echo $qeName.$selProcessor.$effectiveDate.$rate;
			...
			if ($tableRowCount>0) {
				...
				for ($i=0; $i<$tableRowCount; $i++) 
				{
					...
					if ($status!='N') 
					{
						...
						if ($selFish!="" && $selProcessCode!="" && $selFreezingStage!="" && $selQuality!="" && $rateTag!="")
						{
							...
							if($hidTblRowCount>0)
							{
								for($j=0; $j<$hidTblRowCount; $j++)
								{
									...
									if ($fstatus!='N') 
									{
										...
										$frznPkngQELEntryRecIns = $frznPkgRatingObj->addFznPkgRateFrozen($qelId,$frznPkgId,$selFrozenCode);
									}
								}
							}
							//$processorId='';
							if ($rateTag!="") 
							{
								...
								for($j=0; $j<$rs; $j++)
								{
									...
									{
										$insGradeRec = $frznPkgRatingObj->addSelGradeRec($qelId,$frznPkgId,$grades,$userId);
										//$gradeIds[]=$grades;
						...
						if ($selFish!="" && $selProcessCode!="" && $selFreezingStage!="" && $selQuality!="" && $rateTag!="")
						{
							...
							if($hidTblRowCount>0)
							{
								for($j=0; $j<$hidTblRowCount; $j++)
								{
									...
									{
										$selFrozenCode = $p["selFrozenCode_".$i."_".$j];
										$frznPkngQELEntryRecIns = $frznPkgRatingObj->addFznPkgRateFrozen($fznPkngRateId,$frznPkgId,$selFrozenCode);
									}
								}
							}
						//$processorId='';
							...
							if ($rateTag!="") 
							{
								...
								for($j=0; $j<$rs; $j++)
								{
									...
									{
										//$gradeIds[]=$grades;
										$insGradeRec = $frznPkgRatingObj->addSelGradeRec($fznPkngRateId,$frznPkgId,$grades,$userId);
									}
									//printr($gradeIds); 
									//echo $grade[$i];
								}
																				...
																				<td style="padding:5 5 5 5px;">
																					<table  cellspacing="1" bgcolor="#999999" cellpadding="3" id="tblAddRawData">
																						...
																						<tr align="center" class="whiteRow" id="row_<?=$j?>">
																												...
																												<table id="tblfrozen_<?=$j?>">
																													...
																													?>
																														<tr id="rows_<?=$j?>_<?=$k?>">
																															...
																															<td><a href="###" onclick="setRowFrozenStatus('<?=$j?>','<?=$k?>');"><img title="Click here to remove this item" src="images/delIcon.gif" border="0" style="border:none;"></a>
																							...
																							<td  align="center" class="listing-item">
																							<a onclick="getGrade(document.getElementById('selProcessCode_<?=$j?>').value,document.getElementById('selFish_<?=$j?>').value,'<?=$j?>')" href="javascript:void(0)">Set</a>
																							</td>
																							<td align="center" class="listing-item">
