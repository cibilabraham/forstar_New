...
function calcProdnQtyLS()
	{
		var totalSlabs=0; var totalQty=0; var gradeId=""; var numLSAr = new Array();
		//alert("hii");
		var LSToMCConversionType   = document.getElementById("hidLS2MCType").value;
		var prodnRowCount 	= document.getElementById("hidProdnRowCountCvt").value;
		var gradeRowCount = document.getElementById("hidGradeRowCountCvt").value;
		...
		//alert(i);
			...
			if(gradeId=="")
			{
				gradeId=document.getElementById("gId_"+j).value;
			}
			else
			{
				gradeId+=','+document.getElementById("gId_"+j).value;
				...
				var nLS = document.getElementById("numLSCvt_"+j+"_"+i);
				var numLS = (nLS.value!="")?parseInt(nLS.value):0;
				// Convert LS to MC
				if (LSToMCConversionType=='AC')
				{				
					
					if (!isNaN(numLS)) document.getElementById("numLSCvt_"+j+"_"+i).value = numLS;
					if(numLSAr=="")
					{
						numLSAr=numLS;
					}
			...
			// Grade loop Ends here
			Slabs 	= totNumLS;
			totalSlabs += parseFloat(totNumLS);
			// total Qty	
			var totQty	= Slabs*filledWt;
			totalQty += parseFloat(totQty);
				
		} // Prodn Row count Ends here
		alert(gradeId);
		if(mcPackingIdCvt!="" && totalSlabs!="0" && totalQty!="0")
		{
			xajax_generateRMlotId(mcPackingIdCvt,totalSlabs,totalQty);
			return true;
		}
		else
		{
