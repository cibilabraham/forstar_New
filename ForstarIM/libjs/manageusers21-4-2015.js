	...
	var scount = 0; var j=0;
		...
		{	//alert("hii");
			...
			if(j==0)
			{
				...
				{
					alert("Are you sure that the user have permission for all company and unit");
					//return false;
				}
			}
...
function addNewRow(tableId,editUserDetail,company,unit,department,mode)
{
	...
	var companies	= "<select name='company_"+fldId+"' id='company_"+fldId+"' onchange=\" xajax_getAllUnits(document.getElementById('company_"+fldId+"').value,'"+fldId+"');\"><option value='0'>--SelectAll--</option>";
	<?php
	...
	var units	= "<select name='unit_"+fldId+"' id='unit_"+fldId+"'><option value='0'>--SelectAll--</option>";
	<?php
	...
	var departments	= "<select name='department_"+fldId+"' id='department_"+fldId+"'><option value='0'>--SelectAll--</option>";
	<?php
	...
	var role	= "<select name='role_"+fldId+"' id='role_"+fldId+"'><option value=''>--Select--</option>";
	<?php
	...
	?>	
	if (department=="<?=$roleIds?>")  var sel = "Selected";
		else var sel = "";

	role += "<option value=\"<?=$roleIds?>\" "+sel+"><?=$roleName?></option>";	
	<?php
			}
		}
