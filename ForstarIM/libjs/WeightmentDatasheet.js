...
function addNewRowProcurementAvailable()
{
	...
	cell10.className=	"fieldName"; cell10.align = "center";
	cell11.className=	"fieldName"; cell11.align = "center";
	cell12.className=	"fieldName"; cell12.align = "center";
	...
	var speciesdropDown = '<select name="product_species[]" id="product_species_'+rowcount+'" onchange="xajax_processCode(this.value,'+rowcount+',0);"><option value=""> -- Select --</option>';
	<?php 
		if(sizeof($speciesArray) > 0)
		{
			foreach($speciesArray as $species)
			...
			?>
				sel = '';
			 speciesdropDown+="<option value=\"<?=$speciesId?>\" "+sel+"><?=$speciesName?></option>";
			<?php
			}
		}
	?>
	...
	cell2.innerHTML	= "<select name='pondName[]' id='pondName_"+rowcount+"' onchange=\"xajax_weightmentSpecies(document.getElementById('rm_lot_id').value,document.getElementById('supplierName_"+rowcount+"').value,this.value,"+rowcount+");\" /><option value=''> --Select-- </option></select>";
	cell3.innerHTML	= '<select name="procurementCenter[]" id="procurementCenter_'+rowcount+'"  /><option value=""> --Select-- </option></select>';
																	
	cell4.innerHTML	= speciesdropDown;
	cell5.innerHTML	= '<select name="process_code[]" id="process_code_'+rowcount+'" /><option value=""> --Select-- </option></select>';
	cell6.innerHTML = qualityDropDown;
	cell7.innerHTML	= '<input type="text" name="count_code[]" id="count_code_'+rowcount+'" size="10"  />';
	cell8.innerHTML	= '<input type="text" name="weight[]" id="weight_'+rowcount+'" size="10"  onkeyup="calculateWeightAndPercent();" />';
	cell9.innerHTML	= '<input type="text" name="soft_precent[]" id="soft_precent_'+rowcount+'" size="10" onkeyup="calculateWeightAndPercent();"  />';
	cell10.innerHTML	= '<input type="text" name="soft_weight[]" id="soft_weight_'+rowcount+'" size="10" readonly />';
	cell11.innerHTML ='<a class="link1" onclick="getPhtCertificate('+rowcount+');" href="#">link tag</a>';
	cell12.innerHTML = imageButton+hiddenFields;
...
{
	var hidTableRowCounts=document.getElementById("hidTableRowCounts").value;
	var phtStat=0;
	for(i=0; i<hidTableRowCounts; i++)
	{
		if(document.getElementById('oldWeight_'+i))
		{
			//alert("hii");
			var weight=document.getElementById('weight_'+i).value;
			var oldWeight=document.getElementById('oldWeight_'+i).value;
			if(weight<=oldWeight)
			{
				return true;
			}
			else if(weight>oldWeight)
			{
				...
				if(phtTagData!="")
				{
					...
					for(j=0; j<len; j++)
					{
						...
						phtMonitoringEntryId=itemval[j].phtMonitoringEntryId;
						weightmentEntryId=itemval[j].weightmentEntryId;
						if(phtMonitoringEntryId!='' && weightmentEntryId!='')
						{
							phtStat=1;
						}
					}
					if(phtMonitoringEntryId!='' && weightmentEntryId!=''&& phtStat=='1')
					{
						alert("Please add pht cerificate for the increased supply quantity");
						return false;
					}
					else
					{
...
function getPhtCertificateEdit(i)
{	
	var supplierName=document.getElementById("supplierName_"+i).value;
	var pondName=document.getElementById("pondName_"+i).value;
	var product_species=document.getElementById("product_species_"+i).value;
	...
	{
		//alert("dialog");
		xajax_getPhtCertificateEdit(i,supplierName,pondName,product_species,weight,phtTagData);
		$( "#dialog" ).dialog({ width: 500, height:500, resizable: true, modal: true   });
	}
...
function changePhtTag(i)
{
	//alert("hii");
	var phtTagData=document.getElementById("phtTagData_"+i).value;
	var supplyQnty=document.getElementById("weight_"+i).value;
	//alert(phtTagData);
	if(phtTagData!='')
	{	var itemArray=[];
		var supplierName=document.getElementById("supplierName_"+i).value;
		var pondName=document.getElementById("pondName_"+i).value;
		var product_species=document.getElementById("product_species_"+i).value;
		var DatasheetDate=document.getElementById("data_sheet_date").value;
		var RmLotId=document.getElementById("rm_lot_id").value;
	//
		var newSupplyQnty="";
		var qntyStatus='';//if value=1 means balanceQnty for certificate=0; else value=positive of that difference value;
		var obj = JSON.parse(phtTagData);
		...
		{	if(newSupplyQnty=='')
			{
				var certificateNo=itemval[j].certificateNo;
				var availQnty=itemval[j].availableQnty;
				if(parseInt(supplyQnty)>parseInt(availQnty))
				{
					var diff=parseInt(supplyQnty)-parseInt(availQnty);
					var qntyStatus=1;
					var tagValue={"RowCnt":j,"certificateNo":certificateNo,"availableQnty":availQnty,"balanceQnty":diff,"qntyStatus":qntyStatus};
					itemArray.push(tagValue);
					var newSupplyQnty=diff;
				}
				else
				{
					var diff=parseInt(availQnty)-parseInt(supplyQnty);
					var qntyStatus=0;
					var tagValue={"RowCnt":j,"certificateNo":certificateNo,"availableQnty":availQnty,"balanceQnty":diff,"qntyStatus":qntyStatus};
					itemArray.push(tagValue);
					break;
			...
			else
			{
				var certificateNo=itemval[j].certificateNo;
				var availQnty=itemval[j].availableQnty;
				if(parseInt(newSupplyQnty)>parseInt(availQnty))
				{
					var diff=parseInt(newSupplyQnty)-parseInt(availQnty);
					var qntyStatus=1;
					var tagValue={"RowCnt":j,"certificateNo":certificateNo,"availableQnty":availQnty,"balanceQnty":diff,"qntyStatus":qntyStatus};
					itemArray.push(tagValue);
					var newSupplyQnty=diff;
				...
				{
					var diff=parseInt(availQnty)-parseInt(newSupplyQnty);
					var qntyStatus=0;
					var tagValue={"RowCnt":j,"certificateNo":certificateNo,"availableQnty":availQnty,"balanceQnty":diff,"qntyStatus":qntyStatus};
					itemArray.push(tagValue);
		...
		if(tagValue!='')
		{
			var CertificateSize=parseInt(j)+1;
			var tag={"Supplier":supplierName,"Pond":pondName,"Species":product_species,"SupplyQnty":supplyQnty,"DatasheetDate":DatasheetDate,"RmLotId":RmLotId,"CertificateSize":CertificateSize};
			
			var ModifiedJsonData ='{"tag":'+JSON.stringify(tag)+',"items": '+JSON.stringify(itemArray)+'  }';
			document.getElementById("phtTagData_"+i).value=ModifiedJsonData;
			//alert(ModifiedJsonData);
		}
...
function addCerificateEdit(tableId,editProcurmentVehicleId,VehicleId,VehicleNumber,mode,rowCnt)
{
	var fieldvalue=document.getElementById("certificateSize").value;
	var tbl		= document.getElementById(tableId);
	var lastRow	= tbl.rows.length;
	...
	var allCerificate=document.getElementById('certificateNo_0').innerHTML;
	var certificate	= "<select name='certificateNo_"+fieldvalue+"' id='certificateNo_"+fieldvalue+"' onchange=\"xajax_certificateNoEdit(this.value,'"+fieldvalue+"','"+rowCnt+"');\" >";
		certificate+=allCerificate;	
		certificate+= "</select>";	
	var ds = "N";	
	var imageButton = "<a href='###' onClick=\"setIssuanceCertificateStatus('"+fieldvalue+"');\" ><img title=\"Click here to remove this item\" SRC='images/delIcon.gif' BORDER='0' style='border:none;'></a>";
	var hiddenFields = "<input name='sstatus_"+fieldvalue+"' type='hidden' id='sstatus_"+fieldvalue+"' value=''><input name='sIsFromDB_"+fieldvalue+"' type='hidden' id='sIsFromDB_"+fieldvalue+"' value='"+ds+"'><input type='hidden' name='srmId_"+fieldvalue+"' id='srmId_"+fieldvalue+"' value=''><input id='phtMonitoringEntryId_"+fieldvalue+"' type='hidden' name='phtMonitoringEntryId_"+fieldvalue+"' value=''><input id='weightmentEntryId_"+fieldvalue+"' type='hidden' name='weightmentEntryId_"+fieldvalue+"' value=''>";
	cell1.innerHTML	= certificate;
	cell2.innerHTML = "<input id='availableQnty_"+fieldvalue+"' type='text' name='availableQnty_"+fieldvalue+"' value='' size='15' readonly style='text-align:right; border:none;'>";	
	cell3.innerHTML	= "<input id='balanceQnty_"+fieldvalue+"' type='text' name='balanceQnty_"+fieldvalue+"' value='' size='15'  style='text-align:right; border:none;'><input id='qntyStatus_"+fieldvalue+"' type='hidden' value='' tabindex='0' style='text-align:right; border:none;' size='15' name='qntyStatus_"+fieldvalue+"'>";
	//cell4.innerHTML = imageButton+hiddenFields;	
	cell4.innerHTML = hiddenFields;	
	fieldvalue		= parseInt(fieldvalue)+1;	
	document.getElementById("certificateSize").value = fieldvalue;	
//code end
}

function availableQntyEdit(rowCnt,certificateId,availableQnty,supplyRow)
{
	//alert(document.getElementById("certificateNo_"+rowCnt).value);
	var rc=parseInt(rowCnt)-1;
	if(document.getElementById("certificateNo_"+rc).disabled)
	{
		//alert("hii");
		document.getElementById("availableQnty_"+rowCnt).value=availableQnty;
		var diff=parseInt(document.getElementById("weight_"+supplyRow).value)-parseInt(document.getElementById("oldWeight_"+supplyRow).value);
		//alert(diff);
		if(parseInt(availableQnty)>parseInt(diff))
		{
			var balanceQnty=parseInt(availableQnty)-parseInt(diff);
			document.getElementById("balanceQnty_"+rowCnt).value=balanceQnty;
			document.getElementById("qntyStatus_"+rowCnt).value=0;
			jQuery("#addNew").hide();
		}
		else if(parseInt(diff)>parseInt(availableQnty))
		{	//alert("hii");
			var balanceQnty=parseInt(diff)-parseInt(availableQnty);
			document.getElementById("balanceQnty_"+rowCnt).value=balanceQnty;
			document.getElementById("qntyStatus_"+rowCnt).value=1;
		}
	}
	else
	{
		...
		document.getElementById("availableQnty_"+rowCnt).value=availableQnty;
		//var diff=parseInt(document.getElementById("weight_"+supplyRow).value)-parseInt(document.getElementById("oldWeight_"+supplyRow).value);
		var diff=document.getElementById("balanceQnty_"+rc).value;
		//alert(diff);
		if(parseInt(availableQnty)>parseInt(diff))
		{
			var balanceQnty=parseInt(availableQnty)-parseInt(diff);
...
function checkCertificateEdit(rowCnt)
{	var saveStatus='1';
	//document.getElementById("availableQnty_0").value
	var certificateSize=document.getElementById("certificateSize").value;
	//alert(certificateSize);
	...
	if(saveStatus=='1')
	{
		validateRepeatIssuanceEdit(rowCnt);
	}
}

function validateRepeatIssuanceEdit(rowCnt)
{
//alert('aaa');
	var result='';
	if (Array.indexOf != 'function') {  
	...
	{	var tag=[];
		...
		{	
			...
			if(sstatus!="N")
			{
				...
				var balanceQnty=document.getElementById("balanceQnty_"+i).value;
				var qntyStatus=document.getElementById("qntyStatus_"+i).value;
				var phtMonitoringEntryId=document.getElementById("phtMonitoringEntryId_"+i).value;
				var weightmentEntryId=document.getElementById("weightmentEntryId_"+i).value;
				//alert(certificateNo+"---"+availableQnty+"---"+balanceQnty+"---"+qntyStatus);
				var tagValue={"RowCnt":i,"certificateNo":certificateNo,"availableQnty":availableQnty,"balanceQnty":balanceQnty,"qntyStatus":qntyStatus,"phtMonitoringEntryId":phtMonitoringEntryId,"weightmentEntryId":weightmentEntryId};
				itemArray.push(tagValue);
			}
		}
