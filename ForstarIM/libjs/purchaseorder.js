...
function addNewPOItem(tableId, poEntryId, selFishId, selProcessCodeId, selEuCodeId, selBrandId, selGradeId, selFreezingStageId, selFrozenCodeId, selMCPackingId, numMC, pricePerKg, valueInUSD, valueInINR, cpyItem, mode)
{
	...
	if (mode==0) {
		...
		cell16.className = "listing-item"; cell15.align	= "center";cell15.noWrap = "true";
	}
	...
	if (cpyItem) {
		...
		if (fFieldId>=0) {
			selTypeId	 = document.getElementById('processType_'+fFieldId).value;
	...
	var processType= "<select name='processType_"+fieldId+"' id='processType_"+fieldId+"'  onchange=\"xajax_getfishCodes(document.getElementById('processType_"+fieldId+"').value, "+fieldId+",'');\"><option value=''>--Select--</option><option value='S'>Single</option><option value='C'>Combo</option>";
	processType += "</select>";

	var selectFish	= "<select name='selFish_"+fieldId+"' id='selFish_"+fieldId+"' onchange=\"xajax_getProcessCodes(document.getElementById('selFish_"+fieldId+"').value,document.getElementById('processType_"+fieldId+"').value, "+fieldId+", '');xajax_getFrznCodes("+fieldId+", document.getElementById('selFish_"+fieldId+"').value, '', '');\"><option value=''>--Select--</option>";
	/*<?php
	...
	var selectPC	= "<select name='selProcessCode_"+fieldId+"' id='selProcessCode_"+fieldId+"' onchange=\"xajax_getGradeRecs(document.getElementById('processType_"+fieldId+"').value,document.getElementById('selProcessCode_"+fieldId+"').value, "+fieldId+", ''); xajax_getFrznCodes("+fieldId+", document.getElementById('selFish_"+fieldId+"').value, document.getElementById('selProcessCode_"+fieldId+"').value, '');\"><option value=''>--Select--</option>";
	selectPC += "</select>";
	...
	cell2.innerHTML	= processType;
	cell3.innerHTML	= selectFish;
	cell4.innerHTML	= selectPC;
	...
	if (cpyItem && fFieldId>=0) {
		document.getElementById('processType_'+fieldId).value=selTypeId;
		xajax_getfishCodes(selTypeId,fieldId, selFishId)
		xajax_getProcessCodes(selFishId,selTypeId,fieldId, selProcessCodeId);
		xajax_getGradeRecs(selTypeId,selProcessCodeId, fieldId, selGradeId);
	...
	htmlBlock +="</select>";
		...
		for (var i=0; i<rowCount; i++) {
			...
			*/
			...
			htmlBlock += "<input type='hidden' name='hidPOItems_"+i+"_"+fldId+"' id='hidPOItems_"+i+"_"+fldId+"' size='6' value='"+ document.getElementById("poItems_"+i).value+"' />";
			htmlBlock += "</td>";
	...
	function chkMCQty()
	{
		...
		for (var i=0; i<rowCount; i++) {
			var rowStatus = document.getElementById("status_"+i).value;	
       			if (rowStatus!='N') {
					...
					var poItems 	= document.getElementById("poItems_"+i).value;
					var numMC		= document.getElementById("numMC_"+i).value;
					if (numMC!=0 && numMC!="" && !isNaN(numMC)) {							
						if (typeof(pcArr[poItems])!="undefined" && numMC!=0) {
							numMC = parseInt(numMC) + parseInt(pcArr[poItems]);
						}
						if (numMC!=0) pcArr[poItems] = parseInt(numMC);
					}
		...
		for (var i=0; i<splitTbleRowCount; i++) {
			var rowStatus = document.getElementById("spoStatus_"+i).value;	
				...
				var balanceMC = 0;
				for (var j=0; j<rowCount; j++) {
					var hidPOItems 	= document.getElementById("hidPOItems_"+j+"_"+i).value;
					var splitNumMC 	= document.getElementById("MCInInv_"+j+"_"+i).value;

					if (splitNumMC!=0 && splitNumMC!="" && !isNaN(splitNumMC)) {
						if (typeof(splitPCArr[hidPOItems])!="undefined" && splitNumMC!=0) {
							splitNumMC = parseInt(splitNumMC) + parseInt(splitPCArr[hidPOItems]);
						}
						if (splitNumMC!=0) splitPCArr[hidPOItems] = parseInt(splitNumMC);
					}
		...
		for (var l=0; l<sArr.length; l++) {
			...
			var rowStatus = document.getElementById("spoStatus_"+i).value;	
				...
				for (var j=0; j<rowCount; j++) {						
					...
					var mcVarientsCalc = parseFloat(pcArr[hidPOItems])+(parseFloat(pcArr[hidPOItems])*parseFloat(varients));
					var adjustMC	   = number_format(mcVarientsCalc,0,'','');	
					var calcAdjMC	   = parseInt(pcArr[hidPOItems])-adjustMC;
					balanceMC = parseInt(pcArr[hidPOItems])-parseInt(splitPCArr[hidPOItems]);
					//alert(balanceMC);
				//alert(parseInt(pcArr[hidPOItems]));
					
					if (l==0) 
						document.getElementById("MCInPO_"+j+"_"+i).value = parseInt(pcArr[hidPOItems]);
					else {
