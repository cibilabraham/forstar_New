...
{
	// var procurmentNo	=	form.procurmentNo.value;
	...
	if (procurmentNo=="") {
		alert("Please enter a procurmentNo No.");
	...
	if (supplierName=="") {
		alert("select supplierName.");
		form.supplierName.focus();
		return false;
	}
	
	if (pondName=="") {
		alert("select pond Name.");
		form.pondName.focus();
		return false;
	}
	if (entryDate=="") {
	...
	//var itemCount	=	document.getElementById("hidTableRowCount").value;
		...
		for (i=0; i<itemCount; i++)
		{
		   var status = document.getElementById("status_"+i).value;		    
	    	   if (status!='N') 
		    {
			var driver =document.getElementById("driverName_"+i);
			var vehicle =document.getElementById("vehicleNo_"+i);
