...
$.fn.editable = function(options){
	...
	options.toEditable = function(){
		...
		// Submit Event
		if(opts.submit){
			$('<button/>').appendTo($this).html(opts.submit).one('mouseup',function(){opts.toNonEditable($(this).parent(),true)});
			/*
			var submitBtn = $('<input type="button"/>');
			if (opts.submit) submitBtn.attr('value',opts.submit);
			submitBtn.appendTo($this)
						.html(opts.submit)
		...
		// Cancel Event
		if(opts.cancel)
			$('<button/>').appendTo($this).html(opts.cancel).one('mouseup',function(){opts.toNonEditable($(this).parent(),false)});
			/*
			var cancelBtn = $('<input type="button"/>');
			if (opts.cancel) cancelBtn.attr('value',opts.cancel);
...
$.editableFactory = {
	'text': {
		toEditable: function($this,options){
			var input = $('<input size="2" />');
			//$('<input/>').appendTo($this).val($this.data('editable.current'));
			if (options.size) input.attr('size',options.size);
			input.appendTo($this).val($this.data('editable.current'));
		},
