		...
		//if (!gradeAll && !selGroupEntry ) {
			...
			if (gArr.length>0) {
				...
				{	
					...
					/*//var jsonData='{"grade":'+JSON.stringify(gArrStr)+',"rate":'+JSON.stringify(exptRate)+'}';
					for(i=0; i<gArr.length; i++)
					{
						var grade={"grades":gArr[i]};
					}*/
					var jsonData='{"grade":'+JSON.stringify(gArr)+',"rate":'+JSON.stringify(exptRate)+'}';
					//var itemArray='[{"ratevalue":'+jsonData+'}]';
					console.log(itemArray);
						
					document.getElementById("rateTag_"+rowCnt).value=itemArray;
					xajax_displayFrozenRate(processcodeId,fishId,freezingStage,frozenCode,selQuality,rowId,jsonData);
				
				}
				else
				{	
					var rt = JSON.parse(rateTag);
					console.log(rt.ratevalue);
					var len=rt.length;
					console.log(len);
					//var itemval =obj.ratevalue ;
