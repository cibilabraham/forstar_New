		...
		//if (!gradeAll && !selGroupEntry ) {
			...
			if (gArr.length>0) 
			{
				if(rateTag!="")
				{
					...
					for(i=0; i<len; i++)
					{
						selgrades=rt[i].grade;
						selrate=rt[i].rate;
						var oldData='{"grade":'+JSON.stringify(selgrades)+',"rate":'+selrate+'}';
			...
			/*	if(rateTag!="")
				{
					...
					//console.log(itemArray);
					document.getElementById("rateTag_"+rowCnt).value=itemArray;
					//itemArray='[{"grade":["48","21"],"rate":45},{"grade":["72","24"],"rate":56},{"grade":["24","25"],"rate":78}]';
					xajax_displayFrozenRate(processcodeId,fishId,freezingStage,frozenCode,selQuality,rowId,itemArray);*/
