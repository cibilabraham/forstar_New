...
{	//alert("hii");
		...
		if (rowCount>0) {
			for (i=0; i<rowCount; i++) {
				...
				if (status!='N') {
					...
					var selFreezingStage = document.getElementById("selFreezingStage_"+i);
					var selQuality = document.getElementById("selQuality_"+i);
					...
					if (selFreezingStage.value=="") {
						alert("Please select a Freezing Stage.");
						selFreezingStage.focus();
						return false;
					}					
					...
					var hidTblRowCount = document.getElementById("hidTblRowCount_"+i);
					for(j=0; j<hidTblRowCount; j++)
					{
						var selFrozenCode = document.getElementById("selFrozenCode_"+i+"_"+j);
						var rateTag = document.getElementById("rateTag_"+i+"_"+j);

						if (selFrozenCode.value=="") {
						alert("Please select a FrozenCode");
						selFrozenCode.focus();
						return false;
						}	
	...
	//	var cell7	= row.insertCell(6);
		...
		var hiddenFields = "<input name='status_"+fieldId+"' type='hidden' id='status_"+fieldId+"' value=''><input name='IsFromDB_"+fieldId+"' type='hidden' id='IsFromDB_"+fieldId+"' value='"+ds+"'><input name='frozenPackId_"+fieldId+"' type='hidden' id='frozenPackId_"+fieldId+"' value='"+pcFromDB+"'>";
		
		var selFrozencode="<table cellspacing='1' cellpadding='3' ><tr><td ><table id='tblfrozen_"+fieldId+"' bgcolor='#999999' cellspacing='1' cellpadding='3'><tr><td bgcolor='#f2f2f2' class='listing-head'>Frozen Code</td><td bgcolor='#f2f2f2' class='listing-head'>Rate</td><td bgcolor='#f2f2f2'></td><tr id='rows_"+fieldId+"_0'><td bgcolor='#E8EDFF'>";
	...
	{	
		...
		var hiddenFields = "<input name='fstatus_"+rowCount+"_"+fld+"' type='hidden' id='fstatus_"+rowCount+"_"+fld+"' value=''><input name='IsFromDB_"+rowCount+"_"+fld+"' type='hidden' id='IsFromDB_"+rowCount+"_"+fld+"' value='"+ds+"'><input type='hidden' name='rateTag_"+rowCount+"_"+fld+"' id='rateTag_"+rowCount+"_"+fld+"' value=''><input type='hidden' name='frozenPackId_"+rowCount+"_"+fld+"' id='frozenPackId_"+rowCount+"_"+fld+"' value=''>";	
		var setGrade="<a href='javascript:void(0)' onclick=\"getGrade(document.getElementById('selProcessCode_"+rowCount+"').value,document.getElementById('selFish_"+rowCount+"').value,'"+rowCount+"','"+fld+"')\">Set Grade</a>";
		cell1.innerHTML	= selFrozencode;
		cell2.innerHTML	= setGrade;
	...
	function getGrade(processcodeId,fishId,rowId,fieldId)
	{
		var rateTag = document.getElementById('rateTag_'+rowId+'_'+fieldId).value;
		xajax_displayFrozenRate(processcodeId,fishId,rowId,fieldId,rateTag);
		$( "#dialog" ).dialog({ width: 650, resizable: true, modal: true  });
	...
	function chkGradeExist(rowId,fieldId)
	{
		var startDate		=	document.getElementById("start_date_"+rowId+"_"+fieldId).value;
		var endDate			=	document.getElementById("end_date_"+rowId+"_"+fieldId).value;
		if(startDate=="")
		{
			alert("Please enter from date");
			document.getElementById("gradeAll").checked=false;
			return false;
		}
		if(endDate=="")
		{
	...
	function addGrade(rowCnt,fieldId,processcodeId,fishId)
	{
		//alert("hii");
		...
		//var exptRate = document.getElementById("exptRate").value;
		var gradeAll = document.getElementById("gradeAll").checked;
		var rateTag =	document.getElementById("rateTag_"+rowCnt+"_"+fieldId).value;
		var startDate =	document.getElementById("start_date_"+rowCnt+"_"+fieldId).value;
		var endDate =	document.getElementById("end_date_"+rowCnt+"_"+fieldId).value;
		var editArrayCnt=document.getElementById("editArrayCnt").value;//row count of grades and rate used in edit
		var gradeChked = false; var gradeDischked=false;
		//alert(editArrayCnt);
		...
		for (i=0; i<rowCount; i++) {
			...
			{
				gradeChked = true;
				if(document.getElementById("gradeId_"+i).disabled==true)
				{
					gradeDischked=true;
				}
			}
			//var disChk = document.getElementById("gradeId_"+i).disabled;
			//if (disChk) gradeDischked = true;
		...
		if(!gradeAll && gradeChked && gradeDischked)
		{
			alert("Cannot save data already exist in db.");
			return false;
		}
		
		if (startDate=="") {
			alert("Please enter From Date.");
			document.getElementById("start_date_"+rowCnt+"_"+fieldId).focus();
			return false;
		}

		if (endDate=="") {
			alert("Please enter To Date.");
			document.getElementById("end_date_"+rowCnt+"_"+fieldId).focus();
			return false;
		}
		...
		if (gradeAll && rateTag=="") 
		{
			...
			var jsonData='{"GradeId":'+JSON.stringify(allGrades)+',"FromDate":'+JSON.stringify(startDate)+',"ToDate":'+JSON.stringify(endDate)+'}';
			itemArray.push(jsonData);
			itemArray='['+itemArray+']';
			document.getElementById("rateTag_"+rowCnt+"_"+fieldId).value=itemArray;
			xajax_displayFrozenRate(processcodeId,fishId,rowCnt,fieldId,itemArray);
		}
		...
		if (gradeAll && rateTag!="") 
		{
			if(rateTag!="")
			{
				...
				for(i=0; i<len; i++)
				{
					...
					oldStartDate=rt[i].FromDate;
					oldEndDate=rt[i].ToDate;
					//selRate=rt[i].Rate;
					//var oldData='{"GradeId":'+JSON.stringify(selGrades)+',"Rate":'+selRate+'}';
					var oldData ='{"GradeId":'+JSON.stringify(selGrades)+',"FromDate":'+JSON.stringify(oldStartDate)+',"ToDate":'+JSON.stringify(oldEndDate)+'}';
		...
		if (!gradeAll) 
		{
			...
			if (gArr.length>0) 
			{
				...
				//var jsonData='{"GradeId":'+JSON.stringify(gArr)+',"Rate":'+exptRate+'}';
				var jsonData ='{"GradeId":'+JSON.stringify(gArr)+',"FromDate":'+JSON.stringify(startDate)+',"ToDate":'+JSON.stringify(endDate)+'}';
				itemArray.push(jsonData);
				itemArray='['+itemArray+']';
				//alert(itemArray);
				document.getElementById("rateTag_"+rowCnt+"_"+fieldId).value=itemArray;
	...
	function editGrade(processcodeId,fishId,rowCnt,fieldId,grades)
	{
		var rateTag=document.getElementById("rateTag_"+rowCnt+"_"+fieldId).value;
		//alert(grades);
		//alert(processcodeId+','+fishId+','+freezingStage+','+frozenCode+','+selQuality+','+rowCnt+','+itemArray+','+grades);
		xajax_displayFrozenRate(processcodeId,fishId,rowCnt,fieldId,rateTag,grades);
	}
	...
	function getGrades(processcodeId,fishId,freezingStage,frozenCode,selQuality,rowCnt,fieldId)
	{
		var deleteStatus=0;
				...
				{	
					...
					{	
						...
						if(i==rowId)
						{
							...
							oldStartDate=rates[i].FromDate;
							oldEndDate=rates[i].ToDate;
							var oldData='{"GradeId":'+JSON.stringify(selGrades)+',"FromDate":'+JSON.stringify(oldStartDate)+',"ToDate":'+JSON.stringify(oldEndDate)+'}';
							//var oldData='{"GradeId":'+JSON.stringify(selGrades)+',"Rate":'+selRate+'}';
							itemArray.push(oldData);
						}
	...
	/*function setDate(id,field)
	{
		...
		setTimeout(
			...
			alert("dddd"+id+'--'+field);
				...
				(	
					{
						inputField  : "start_date_"+id+"_"+field,         // ID of the input field
						eventName	  : "click",	    // name of event
						button : "start_date_"+id+"_"+field, 
				...
				(	
					{
						inputField  : "end_date_"+id+"_"+field,         // ID of the input field
						eventName	  : "click",	    // name of event
						button : "end_date_"+id+"_"+field, 
						ifFormat    : "%d/%m/%Y",    // the date format
						singleClick : true,
						step : 1
					}
	...
	function startDate(id,field)
	{	
		//alert("hii");
		$( "#start_date_"+id+"_"+field).datepicker( {dateFormat:"dd/mm/yy",changeYear: true});
	}


	function endDate(id,field)
	{	//alert(id);
		$( "#end_date_"+id+"_"+field).datepicker( {dateFormat:"dd/mm/yy",changeYear: true});
		/*setTimeout(
			function(){
			Calendar.setup 
			(	
	...
	function displayGradeStatus(row, field)
	{
		//alert(row+','+field);
		var startDate		=	document.getElementById("start_date_"+row+"_"+field).value;
		var endDate			=	document.getElementById("end_date_"+row+"_"+field).value;
		var selFrozenCode	=	document.getElementById("selFrozenCode_"+row+"_"+field).value;
		if(startDate!="" && endDate!="")
		{
			var selFish  =	document.getElementById("selFish_"+row).value;
			var selProcessCode  =	document.getElementById("selProcessCode_"+row).value;
			if(selFish!="" && selProcessCode!="")
			{
				xajax_checkGradeStatus(selFish,selProcessCode,selFrozenCode,startDate,endDate);
			}
		}
	}
	...
	function assignGrade(grades)
	{
		//alert(grades);
		//alert(grades.length);
		var disableGradeAll=false;
		var gradeSize=document.getElementById("rowCount").value;
		//alert(grades+"--"+gradeSize);
		for(i=0; i<gradeSize; i++)
		{
			if(document.getElementById("gradeId_"+i).disabled==false)
			{
				var gradeId=document.getElementById("gradeId_"+i).value;
				if(grades.indexOf(gradeId)>-1) 
				{	
					disableGradeAll=true;
					document.getElementById("gradeId_"+i).disabled=true;
				}
				else
				{
		...
		if(disableGradeAll)
		{
			document.getElementById("gradeAll").disabled=true;
		}
	...
	function enableGrade()
	{
		...
		document.getElementById("gradeAll").disabled=false;
	}


	function checkDateStatus(i,rowId,fieldId)
	{
		var startDate		=	document.getElementById("start_date_"+rowId+"_"+fieldId).value;
		var endDate			=	document.getElementById("end_date_"+rowId+"_"+fieldId).value;
		if(startDate=="")
		{
			alert("Please enter from date");
			//document.getElementById("start_date_"+rowId+"_"+fieldId).focus();
			document.getElementById("gradeId_"+i).checked=false;
			return false;
		}
		if(endDate=="")
		{
			alert("Please enter to date");
			//document.getElementById("end_date_"+rowId+"_"+fieldId).focus();
			document.getElementById("gradeId_"+i).checked=false;
			return false;
		}
		if(startDate!="" && endDate!="")
