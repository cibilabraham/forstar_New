...
function getSupplierDetail(item,qty,requisitionId,row)
{
	//alert(item);
	var totalQty=document.getElementById('totalQty').value;
	var hidRowCount=document.getElementById('hidRowCount').value;
	
	xajax_getSupplierItemQuantity(item,totalQty,hidRowCount,qty,requisitionId,row);
...
function allotQuantity(rowCnt,requisitionId,supplierSize,item,qty)
{
	...
	else
	{
		...
		for(j=0; j<supplierSize; j++)
		{
			...
			{
				//var result=xajax_saveData(supplierStockId,item,supplierId,supplierStockQty,allotQuantity,requisitionId);
			}
		}
