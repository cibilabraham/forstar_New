...
//alert('11');
	...
	else
	{
		...
		var cell1Val = "<select  id='product_speciespro_"+fieldvalueId+"' name='product_speciespro_"+fieldvalueId+"' tabindex=1  onchange=\"xajax_produceCodepro(document.getElementById('product_speciespro_"+fieldvalueId+"').value,"+fieldvalueId+",''); \" ><option value=""> -- Select --</option>";
		<?php 	foreach($speciesRecs as $speciesval)
				{
											foreach($speciesval as $species)
											{
		...
		var processCde = '<select  id="process_codepro_'+fieldvalueId+'" name="process_codepro_'+fieldvalueId+'"><option value=""> -- Select --</option>';
	<?php 	foreach($processCodeRecs as $processCodeVal)
			{
										foreach($processCodeVal as $processCode)
										{
										//alert($sr[0]);
						$processCodeId		=	$processCode[1];
						$processCodeValue	=	stripSlash($processCode[2]);
						?>
						if(procesCode1=="<?=$processCodeId?>") var sel="Selected";
					  else var sel = "";
		...
		cell4.innerHTML	=processCde;
		//cell4.innerHTML	= '<input type="text" name="grade_count'+fieldId+'" id="grade_count'+fieldId+'" size="10" required />';
		cell5.innerHTML	= '<input type="text" name="count_codepro_'+fieldvalueId+'" id="count_codepro_'+fieldvalueId+'" size="10"   value="'+count_code+'"  />';
		cell6.innerHTML	= '<input type="text" name="weightpro_'+fieldvalueId+'" id="weightpro_'+fieldvalueId+'" size="10" onkeyup="checkValuepro();"  value="'+weight+'"  />';
		cell7.innerHTML	= '<input type="text" name="soft_precentpro_'+fieldvalueId+'" id="soft_precentpro_'+fieldvalueId+'"  size="10"  onkeyup="checksSoftValuepro();"  value="'+soft_precent+'"  />';
		cell8.innerHTML	= '<input type="text" name="soft_weightpro_'+fieldvalueId+'" id="soft_weightpro_'+fieldvalueId+'" size="10" value="'+soft_weight+'" />';
		var packageType			= "<select name='packageTypepro_"+fieldvalueId+"' Style='display:display;' id='packageTypepro_"+fieldvalueId+"' tabindex=1    ><option value=''> -- Select --</option>";
			//alert(packageType1);
			<?php
			if (sizeof($packageTypeRecs)>0) {	
		...
		?>	
		packageType += "</select>";
		cell9.innerHTML	= packageType;
		//cell8.innerHTML	= '<input type="text" name="pkg_typepro_'+fieldvalueId+'" id="pkg_typepro_'+fieldvalueId+'" size="10"  />';
		
		cell10.innerHTML	= '<input type="text" name="pkg_nospro_'+fieldvalueId+'" id="pkg_nospro_'+fieldvalueId+'" size="10" value="'+package_nos+'" />';
...
{	
	...
	{
		//alert("dialog");
		xajax_getPhtCertificate(i,supplierName,pondName,product_species,weight);
				...
				for(j=0; j<len; j++)
				{
					...
					{ 
						...
						else if(qntyStatus==0)
						{
							...
							/*var tavailQnty = balanceQnty;
							if (typeof(certifyArray[certificateNo])!="undefined" && tavailQnty!=0) {
								tavailQnty = parseInt(tavailQnty) + parseInt(certifyArray[certificateNo]);
							}
							if (tavailQnty!=0) certifyArray[certificateNo] = parseInt(tavailQnty);
							*/
						}

						/*var tsupplyQnty = supplyQnty;
						if (typeof(supplyArray[certificateNo])!="undefined" && tsupplyQnty!=0) {
							tsupplyQnty = parseInt(tsupplyQnty) + parseInt(supplyArray[certificateNo]);
						}
						if (tsupplyQnty!=0) supplyArray[certificateNo] = parseInt(tsupplyQnty);
						*/
						
					  //certifyArray[certificateNo]+=availQnty;
					 // supplyArray[certificateNo]+=supplyQnty;
					}
				}
			}
	}
...
/*function availableQnty_old(rowCnt,certificateId,availableQnty,supplyRow)
{
	var certificateNo=''; var availQnty=''; var certyQnty=''; var newCertificateQnty=''; var balanceQnty='';
	var hidTableCnt=document.getElementById("hidTableRowCounts").value;
	for(i=0; i<hidTableCnt; i++)
	{
		if(i==0)
		{
			...
			{	
				...
				for(j=0; j<len; j++)
				{
					...
					if(certificateNo==certificateId)
					{
					 newCertificateQnty	=parseInt(availableQnty)-(availQnty);
					}
				}
			}
			else
			{	if(supplyRow==i)
				{
					...
					//alert(hidTableCnt+'--'+1);
					blnQnty(rowCnt,availableQnty);
					newCertificateQnty	=availableQnty;
				}
			}
		}
		else
		{
			if(newCertificateQnty!='')
			{
				...
				{	
					...
					for(j=0; j<len; j++)
					{
						...
						if(certificateNo==certificateId)
						{
							newCertificateQnty	=parseInt(newCertificateQnty)-(availQnty);
							document.getElementById("availableQnty_"+rowCnt).value=newCertificateQnty;
							//alert(hidTableCnt+'--'+1);
							blnQnty(rowCnt,availableQnty);
						}
					}
				...
				for(j=0; j<len; j++)
				{
					...
					if(certificateNo==certificateId)
					{
						if(certyQnty=='')
						{
							if(qntyStatus=='1')
							{	alert("hii");
								document.getElementById("availableQnty_"+rowCnt).value=0;
								
							}
							else if(qntyStatus=='0')
							{
								//document.getElementById("availableQnty_"+rowCnt).value=balanceQnty;
								certyQnty=balanceQnty;
								//excecute next for loop
								//balabceqnty will be certyQnty
							}
						}
						else
						{
							//calculate with the new balanced Qnty;
						}
					}

				}
				//alert(itemval.length);
				//alert(obj.items[0].certificateNo);

			//	if(certificateNo==certificateId)
			//	{
			//		if(certyQnty=='')
			//		{   //alert(availableQnty+'-----'+availQnty);
			//			newCertificateQnty=availableQnty-availQnty;
			//			document.getElementById("availableQnty_"+rowCnt).value=newCertificateQnty;
			//		}
			//		else
			//		{
			//			newCertificateQnty=newCertificateQnty-availQnty;
			//			document.getElementById("availableQnty_"+rowCnt).value=newCertificateQnty;
			//		}
			//	}
			}
...
function blnQnty(rowCnt,availableQnty)
{	//alert("hii");
...
{ var saveStatus='1';
	...
	for(i=0; i<certificateSize; i++)
	{
		...
		if(status!='N')
		{
			...
			if((certificateNo!="" && availableQnty!="" && balanceQnty=="") || (certificateNo!="" && availableQnty!="" && balanceQnty=="NaN"))
			{
				alert("Cannot save data with non valid balance Qnty");
				saveStatus='0';
			}
		}
	}
...
//alert('aaa');
	...
	{	var tag=[];
		...
		//display fld values
		/*var fldVal=	document.getElementById("phtTagData_0").value;
		var chck=JSON.stringify(fldVal); 
		console.log(chck);*/
