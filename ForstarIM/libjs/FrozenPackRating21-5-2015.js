...
{	//alert("hii");
	...
	function addNewRawDataRow(tableId, sFishId, rateTag, selMode, sProcessCodeId, pcFromDB, userId, fznPkgQEListId, selQuickEntryList)
	{
		...
		var setRate="<a href='javascript:void(0)' onclick=\"getGrade(document.getElementById('selProcessCode_"+fieldId+"').value,document.getElementById('selFish_"+fieldId+"').value,document.getElementById('freezingStage').value,document.getElementById('frozenCode').value,document.getElementById('selQuality').value,'"+fieldId+"')\">Set</a>";
		var ds = "N";	
		//if( fieldId >= 1) 
	...
	function addGrade(rowCnt,processcodeId,fishId,freezingStage,frozenCode,selQuality)
	{
		//alert("hii");
	...
	function editGrade(processcodeId,fishId,freezingStage,frozenCode,selQuality,rowCnt,grades)
	{
		var rateTag=document.getElementById("rateTag_"+rowCnt).value;
		//alert(grades);
		//alert(processcodeId+','+fishId+','+freezingStage+','+frozenCode+','+selQuality+','+rowCnt+','+itemArray+','+grades);
		xajax_displayFrozenRate(processcodeId,fishId,freezingStage,frozenCode,selQuality,rowCnt,rateTag,grades);
	}
