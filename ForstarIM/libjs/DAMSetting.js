function validateDAMSetting(form)
	...
	function addNewItemRow(tableId, chkListName, chkPointEntryId)
	{
		...
		var selUnit 	= "<select name='selUnit_"+fieldId+"' id='selUnit_"+fieldId+"'><option>--Select--</option>";
		<?php if (sizeof($stockUnitRecs)>0)  {?>
			<?php foreach($stockUnitRecs as $stkUnit) {?>
				selUnit	+= "<option value='<?=$stkUnit[0]?>'><?=$stkUnit[1]?></option>";
			<?php }?>	
		<?php }?>
		selUnit		+= "</select>";
