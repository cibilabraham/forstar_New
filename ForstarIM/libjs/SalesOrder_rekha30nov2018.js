...
{		
	...
	if (!isNaN(calcTotalAmount) && invoiceType=='T') {
		...
		var totCentralTax = 0; 
		if (grandTotalAmount>0) totCentralTax = calcCentalTax(grandTotalAmount, totExAmt, totEduCessAmt, totSecEduCess, grTotCentralTaxAmt);
		/* 
		//22 JULY 11
		if ((discountPercent!="" || discountPercent!=0)) {
			var calcDiscountAmt = (grandTotalAmount*discountPercent)/100;			
...
{	
	...
	cell11.innerHTML = "<strong>Add"+"&nbsp;"+taxType+"</strong>(CGST&nbsp;"+taxPercent/2 +"% + SGST&nbsp;"+taxPercent/2+"%)&nbsp;"+taxPercent+"%";

	}else{
	cell11.innerHTML = "<strong>Add"+"&nbsp;"+taxType+"</strong>&nbsp;"+taxPercent+"%";
	}
	cell12.innerHTML = "<input name='taxAmount_"+tRowId+"' type='text' id='taxAmount_"+tRowId+"' size='8' readonly style='text-align:right;border:none;' value='"+taxAmt+"'>";
	cell13.innerHTML = "";
}
// adding Discount Row
...
function addNewSOItemRow(tableId, selStockId, unitPrice, qty, totalAmt, salesOrderEntryId, selMCPackId, numMCPack, numLoosePack, distMgnStateEntryId, taxPercent, pGrossWt, pMCPkgGrossWt, productCategoryComb, numPacks, mcPackageWt, mcCombination, pkgGroupComb, leftPkgRule, rightPkgRule, selDistributorId, stateId, productPriceRateListId, mode, distMgnRateListId, freePkts, basicRate, mrp, exDutyPercent, exDutyAmt, exDutyMasterId, exChapterSubhead, mcPkgWtId)
{
	...
	cell11.innerHTML = "<span name='chaptSubhead_"+fieldId+"' id='chaptSubhead_"+fieldId+"'>"+exChapterSubhead+"</span>";
	cell12.innerHTML = "<input name='totalAmount_"+fieldId+"' type='text' id='totalAmount_"+fieldId+"' size='8' readonly style='text-align:right;border:none;' value='"+totalAmt+"'>";
	...
	*/
	function showInvoiceType()
	{
		...
		if (invoiceType=='S') {
			...
			document.getElementById("numBoxRow").innerHTML = "<input type=\'text\' name=\"numBox\" id=\"numBox\" value=\"<?=$numBox?>\" size=\"3\" style=\"font-weight:bold;border:none;text-align:right;\" readonly=\"true\">";		
...
//enter refresh time in "minutes:seconds" Minutes should range from 0 to inifinity. Seconds should range from 0 to 59
	...
	*/
	...
	function calcCentalTax(totAmt, totExAmt, totEduCessAmt, totSecEduCess, grTotCentralTaxAmt)
	{		
