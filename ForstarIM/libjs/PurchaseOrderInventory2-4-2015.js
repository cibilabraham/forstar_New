...
function addNewStockItemRow(tableId, poItem, selStockId, qty, totalAmt, supplierRateListId, mode,selPlantId,notOver,description,unitprice,prindesc,stkQty)
{
	...
	var selectStock	= "<select name='selStock_"+fieldId+"' id='selStock_"+fieldId+"' onchange=\"xajax_getAllRecords('"+fieldId+"',"+supplierSelected+",document.getElementById('selStock_"+fieldId+"').value,document.getElementById('selPlant_"+fieldId+"').value,'<?=$poItem?>',document.getElementById('hidSupplierRateListId').value,'"+mode+"');  xajax_getStockBalanceQty('"+fieldId+"',document.getElementById('selStock_"+fieldId+"').value,document.getElementById('selSupplier').value);\" >";
	var sel;
