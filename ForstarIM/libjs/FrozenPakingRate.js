var prevId = "";
	...
	function addGrade(processcodeId, freezingStageId, qualityId, frozencodeId, rowId, rateListId, selRowId, frznPkgRateId, fishId, selGroupEntry, fishCategoryId)
	{
		...
		if (gradeAll && selGroupEntry) 
		{
			//alert("hii");
			var gArr = new Array();		
			var gpIdArr = new Array();		
			var j=0;
			for (i=0; i<rowCount; i++) 
			{
				var gradeId = document.getElementById("gradeId_"+i).value;
				var selgpId = document.getElementById("selgpId_"+i).value;
				if(selgpId!='')
				{
					gArr[j] = gradeId;
					gpIdArr[j] = selgpId;
			...
			if (gArr.length>0) {
				//alert("4");
				gArrStr = gArr.join(",");
				gpIdArrStr = gpIdArr.join(",");
				xajax_updateGrade(fishId, processcodeId, freezingStageId, qualityId, frozencodeId, rateListId, frznPkgRateId, exptRate, "A", gArrStr, '', preProcessorId,gpIdArrStr);
			}
		}
		...
		if ((gArr.length>0) || (gpIdArr.length>0)) {
				//alert("4");
				gArrStr = gArr.join(",");
				gpIdArrStr = gpIdArr.join(",");
				//alert(gArrStr+'----------'+gpIdArrStr);
				xajax_updateGrade(fishId, processcodeId, freezingStageId, qualityId, frozencodeId, rateListId, frznPkgRateId, exptRate, "E", gArrStr, '', preProcessorId,gpIdArrStr);
			}
		 }
		...
		{
			//alert("3");
			xajax_addGrade(fishId, processcodeId, freezingStageId, qualityId, frozencodeId, rateListId, frznPkgRateId, exptRate, '', '', '', preProcessorId);
		}*/
