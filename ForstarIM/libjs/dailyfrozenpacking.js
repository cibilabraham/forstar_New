...
function calcProdnQtyLS()
	{
		...
		var LSToMCConversionType   = document.getElementById("hidLS2MCType").value;
		//var prodnRowCount 	= document.getElementById("hidProdnRowCountCvt").value;
		var prodnRowCount=document.getElementById("hidTableRowCountCvt").value;
		var gradeRowCount = document.getElementById("hidGradeRowCountCvt").value;
		//alert("hii");
		var filledWt		= parseFloat(document.getElementById("filledWt").value);
		...
		//alert(i);
			...
			var companyId=document.getElementById("companyIdCvt_"+i).value;
			var unitId=document.getElementById("unitIdCvt_"+i).value;
		...
		var tbl		= document.getElementById(tableId);
			...
			*/
			...
			{	var cellhtml ="cell"+p;
				...
				window[cellhtml].innerHTML  = "<input name='gradeQty_"+l+"_"+fdId+"' type='text' id='gradeQty_"+l+"_"+fdId+"' size='5' style='text-align:right; border:none;' tabindex="+fdId+"  value='' onkeyup=\"return checkValid(document.getElementById('gradeQty_"+l+"_"+fdId+"').value,document.getElementById('rmlotId_CnvrtLs_"+fdId+"').value,"+l+","+fdId+");\"> <input name='hidgradeQty_"+l+"_"+fdId+"' type='hidden' id='hidgradeQty_"+l+"_"+fdId+"' value=''>";
			p++;
			}
			//var lst=	"cell"+p;
...
function checkValid(grdVal,rmlot,grdrw,fld)
{
	...
	if(grdVal!='' || grdVal!='0')
	{
		//alert(grdrw);
		//alert(fld); 
		var difference=''; var totalRMLS=0;
		...
		for(i=0; i<rmSz; i++)
		{
			...
			{	var grdRow=parseInt(grdrw)+1;
				...
				else
				{
						...
						var hidTableRowCountCvt=document.getElementById('hidTableRowCountCvt').value;
						for(l=1; l<=hidTableRowCountCvt; l++)
						{
							var rmlots=document.getElementById('rmLotStatusCvt_'+l).value;
							if(rmlot==rmlots)
							{
							//ls values
								//alert(grdVal+'--'+totalRMLS+'--'+difference);
							var hidgradeQtyOld=document.getElementById('hidgradeQty_'+grdrw+'_'+fld).value;
				
							if(hidgradeQtyOld > grdVal)
							{
								document.getElementById('numLSCvt_'+grdRow+'_'+l).value=document.getElementById('hidNumLSCvt_'+grdRow+'_'+l).value
								...
								{	
									...
									{
									//alert("hui");
										var nxtls=document.getElementById('hidNumLSCvt_'+grdRow+'_'+l).value;
										//alert(nxtls+'--'+difference);
										var diff2=parseInt(difference)-parseInt(nxtls);
										if(diff2==0)
										{
											document.getElementById('numLSCvt_'+grdRow+'_'+l).value=0;
											//var m=parseInt(l)+1;
											//document.getElementById('numLSCvt_'+grdRow+'_'+m).value=document.getElementById('hidNumLSCvt_'+grdRow+'_'+m).value
										}
										else if(diff2<0)
										{
											document.getElementById('numLSCvt_'+grdRow+'_'+l).value=parseInt(nxtls)-parseInt(difference);
										}
										else if(diff>0)
										{
											document.getElementById('numLSCvt_'+grdRow+'_'+l).value=0;
									...
									else if(difference=="" && difference!='0' )
									{
										...
										var hidnumLSCvt=document.getElementById('hidNumLSCvt_'+grdRow+'_'+l).value;
										if(hidnumLSCvt!='')
										{
										var diff=parseInt(grdVal)-parseInt(hidnumLSCvt);
										//alert(grdVal+'--'+hidnumLSCvt+'--diff'+diff);
										totalRMLS+=hidnumLSCvt;
										difference=diff;
											if(diff==0)
											{
												document.getElementById('numLSCvt_'+grdRow+'_'+l).value=0;
											...
											{
												document.getElementById('numLSCvt_'+grdRow+'_'+l).value=parseInt(hidnumLSCvt)-parseInt(grdVal);
											}
											else if(diff>0)
											{
												document.getElementById('numLSCvt_'+grdRow+'_'+l).value=0;
								...
								/*else if(diff>0)	
								{
									...
									var nxtls=document.getElementById('hidNumLSCvt_'+grdRow+'_'+l).value;
									var diff2=parseInt(diff)-parseInt(nxtls);
									if(diff2<=0)
									{
										document.getElementById('numLSCvt_'+grdRow+'_'+l).value=parseInt(nxtls)-parseInt(diff);
									}
								}*/
							}
							//alert(rmlots);
					...
					//alert(hidTableRowCountCvt);
				}
				
			}
		}
		//alert(rmSz);
		document.getElementById('hidgradeQty_'+grdrw+'_'+fld).value=grdVal;
	}
}
