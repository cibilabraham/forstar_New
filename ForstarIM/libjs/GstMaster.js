function validateGstMaster(form)
{	
		...
		if (rowCount>0) {
			for (i=0; i<rowCount; i++) {
				...
				if (status!='N') {
					...
					var gstPercent	    = document.getElementById("gstPercent_"+i);	
					var hidgstPercent	= document.getElementById("hidgstPercent_"+i);
					...
					if (!checkNumber(gstPercent.value)) {
						gstPercent.value = "";
						gstPercent.focus();
						return false;
					}
					...
					if (parseFloat(gstPercent.value)!=parseFloat(hidgstPercent.value)) {
...
// ------------------------------------------------------
	...
	{	
		var startDate 		= $("#startDate").val();
		var gstRateList  = $("#gstRateList").val();
		var cMsg = "The start date you have selected is "+startDate+". Do you wish to continue?";
		if (confirm(cMsg)) {
			if (gstRateList!=0 && startDate!="") {
				xajax_changeRLDate(gstRateList,startDate);
			}
		}
		return false;
	}
	...
	function deleteEDRateList()
	{
		...
		xajax_deleteEDRateList(gstRateList);
