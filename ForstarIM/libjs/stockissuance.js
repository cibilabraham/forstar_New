...
function getSupplierDetail(item,qty,requisitionId,row,company,unit)
{
	//alert(company+','+unit);
	var totalQty=document.getElementById('totalQty').value;
	var hidRowCount=document.getElementById('hidRowCount').value;
	
	xajax_getSupplierItemQuantity(item,totalQty,hidRowCount,qty,requisitionId,row,company,unit);
	$( "#dialog" ).dialog({width:500, resizable: true, modal: true});
}

function getSupplierDetailShow(item,qty,requisitionId,row)
{
	//alert(item);
	var totalQty=document.getElementById('totalQty').value;
	var hidRowCount=document.getElementById('hidRowCount').value;
	
	xajax_getSupplierItemQuantityShow(item,totalQty,hidRowCount,qty,requisitionId,row);
	$( "#dialog" ).dialog({width:500, resizable: true, modal: true});
}

function allotQuantity(rowCnt,requisitionId,supplierSize,item,qty,company,unit)
{
	var fieldvalue=0; var totalQty="0";
	for(i=0; i<supplierSize; i++)
	{
		var allotQuantity=document.getElementById("allotQuantity_"+i).value;
		if(allotQuantity!='')
		{
			totalQty=parseInt(totalQty)+parseInt(allotQuantity);
			fieldvalue=1;
		}
		
	}
	//alert(totalQty);
	if(i==supplierSize && fieldvalue=="0")
	{
		alert("Allot Quantity can't be null for all fields");
	}
	else if(totalQty>qty)
	{
		alert("Cannot save data.Alloted quantity is greater than required quantity");
	}
	else
	{
		//alert("data can save");
		...
		{
			var supplierId=document.getElementById("supplierId_"+j).value;
			var supplierStockQty=document.getElementById("supplierStockQty_"+j).value;
			var allotQuantity=document.getElementById("allotQuantity_"+j).value;
			var supplierStockId=document.getElementById("supplierStockId_"+j).value;
			if(allotQuantity!='0' &&  allotQuantity!='')
			{
				var result=xajax_saveData(supplierStockId,item,supplierId,supplierStockQty,allotQuantity,requisitionId,company,unit);
			}
		}
		if(i==supplierSize && result)
		{
			alert("Stock allocated successfully");
			$('#dialog').dialog('close');
			document.getElementById("frmStockIssuance").submit();
		}
		else
		{
			alert("Stock allocated failed");
		}
	}
}

function chkNumberStatus($row)
{
	var supplierStockQty=document.getElementById("supplierStockQty_"+$row).value;
	var allotQuantity=document.getElementById("allotQuantity_"+$row).value;
	if(parseInt(allotQuantity)>parseInt(supplierStockQty))
	{
		alert("Allote Quantity cannot be greater than Supplier Stock Quantity");
		document.getElementById("allotQuantity_"+$row).focus();
	}
}
