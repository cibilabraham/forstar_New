function validateAddDailyPreProcess(form)
{
	...
	if (confirmSave()) 
	{//alert("hii");
		jQuery('#rm_lot_id').attr("disabled", false); 
		jQuery('#selFish').attr("disabled", false); 
		jQuery('#processCode').attr("disabled", false); 
		return true;
	}
	 else 
	{
...
function nextProcess(e,form,name)
{
	...
	{	
		...
		var processSame = document.getElementById("processSame_"+rowId).value;
		//alert(processSame);
		var pcCount = document.getElementById("pcCount_"+selProcessId+'_'+processSame).value;
		//alert(pcCount);
		...
		//alert(avQty);
		var rowvalue=parseInt(rowId)+1;
		//alert(pcCount);
			for(i=rowvalue; i<=pcCount; i++) {
			//alert(i);
	...
	// Change Avaialble Qty
	function changeAvailableQty_old(rowId, selProcessId)
	{	
		//alert(rowId+"--->"+selProcessId);
