...
{	
	...
	if (pondName=="") {
		alert("Please enter the Pond Name");
		form.pondName.focus();
		return false;
	}
	if (count_code=="") {
...
// PC -> basket weight 
	...
	function saveRMInChallan(mode, saveType)
	{
		...
		var supplyChallanNoval 	= document.getElementById("supplyChallanNo").value;
		if(supplyChallanNoval=="")
		{
		...
		// var reason     = document.getElementById('reason'+qualityId).value;
				...
				if (nameOfquality!="" && nameOfquality!=0) {
				 
					var joinCntquantity    = total_new_entryVal+":"+nameOfquality+":"+quality_wt+":"+qualityPercent+":"+reason+":"+qualityId;
					alert(joinCntquantity);
					cntQuantityArr[i] = joinCntquantity;
					i++;
				}
		}
		if (entryId || catchEntryNewId) {
...
var tbl		= document.getElementById(tableId);
	...
	//cell6.className	= "listing-item"; cell6.align	= "center";
		...
		//alert("entered");
		//alert("<?=$vehileTypeId?>");
		var qualityName	= "<select name='qualityName_"+fldId+"' id='qualityName_"+fldId+"' onchange=\"xajax_getQualityDet(document.getElementById('qualityName_"+fldId+"').value,"+fldId+");\" ><option value='0'>--Select--</option>";
		if(qualitys!="")
		{
			for(i=0;i< len; i++)
			{
	...
	var billing = "<input name='billing_"+fldId+"' type='checkbox' id='billing_"+fldId+"' size='15' value='"+billingVal+"'>";
	var hiddenFields = "<input name='Status_"+fldId+"' type='hidden' id='Status_"+fldId+"' value=''><input name='IsFromDB_"+fldId+"' type='hidden' id='IsFromDB_"+fldId+"' value='"+ds+"'><input type='hidden' name='qualityId_"+fldId+"' id='qualityId_"+fldId+"' value='"+qualityid+"'><input type='hidden' name='quality_new_"+fldId+"' id='quality_new_"+fldId+"' value='"+quality_new+"'>";

	
	//cell1.innerHTML	= billing;
	cell1.innerHTML	=qualityName+billing;
	cell2.innerHTML	="<input name='qualityPercent_"+fldId+"' type='text' id='qualityPercent_"+fldId+"' size='5' value='0.00' />";	
	cell3.innerHTML	="<input name='qualityWeight_"+fldId+"' type='text' id='qualityWeight_"+fldId+"' size='5' onkeyup='return effectiveWtNew();' value=''  />";
	cell4.innerHTML	=" <input name='qualityReason_"+fldId+"' type='text' id='qualityReason_"+fldId+"' size='20' value='' autocomplete='off'/>";
