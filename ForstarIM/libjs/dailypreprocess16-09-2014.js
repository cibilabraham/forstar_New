function validateAddDailyPreProcess(form)
{
	...
	{
	//alert("hii");
		var rmLotId		 	 = form.rm_lot_id.value;
		if (rmLotId=="" && !validDPPEnabled) {
		alert("Please select a RM Lot Id.");
		form.rm_lot_id.focus();
		return false;
		}
	
		var fishLotId		 	 = form.selFishLotid.value;
		if (fishLotId=="" && !validDPPEnabled) {
		alert("Please select a fish.");
		form.selFishLotid.focus();
		return false;
		}
	}
	else
	...
	if (isQtyEntered==false) {
	 ...
	 {//alert("hii");
		if(document.getElementById("lotIdAvailable").checked==true)
		{
			...
			jQuery('#selFishLotid').attr("disabled", false); 
			jQuery('#processCodeLotid').attr("disabled", false); 
		}
		else
		{
