...
?>
	...
	?>
	  ...
	  <tr>                        
			...
			<? }?>	&nbsp;&nbsp;&nbsp;&nbsp;<? if ($searchMode==true){?>
                            <input type="button" name="Submit2" value=" View / Print" class="button" onclick="return printWindow('PrintdailystockreportSummary.php?stockFrom=<?=$sdateFrom?>&stockTo=<?=$selectDate?>&stocktype=<?=$stocktype?>&packType=<?=$packType?>&reportType=<?=$reportType?>',700,600);"
	...
	?>	
			...
			<td class="listing-item" nowrap width="80" style="padding-left:5px; padding-right:5px; border-right:1px solid #999999;"><?=$selMCPkgCode?>&nbsp;</td>
				...
				if ($reportType=="PRIR")
					...
					<td  class="listing-item" nowrap width="20" style="padding-left:5px; padding-right:5px; border-right:1px solid #999999;" align="right" valign="middle" ><?=($stkPrice!=0)?$stkPrice:"&nbsp;";?></td>
					<?php }
        			 if ($stocktype==2)
				{?>
...
<tr bgcolor="#FFFFFF">
			...
			<?php if ($packType=="MC")
					{?>
			<?=$totRSumMC;?>
			<?php }else if ($packType=="LS"){?><?=$totLSumLS;?> <?php }?>
			</td></tr>
