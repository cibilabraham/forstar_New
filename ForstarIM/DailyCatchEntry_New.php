<?php
	...
	*/
	...
	//die();
		...
		if($landingCenterId=="")
		{
			$landingCenter		= $p["landingCenterLot"];
		}
		else
		{
			$landingCenter	=$landingCenterId;
		}
		$fish			=	$p["fish"];
		$processCode		=	$p["processCode"];
		$supplyChallanNo	=	$p["supplyChallanNo"];
		...
		if($entryId!="" && $weighChallanNo!="") 
		{
			...
			else
			{
				...
				//rekha added code here dated on 11 june 2018
				//old
	...
	if ($p["cmdAddRawSelChallan"]!="" )	
	{
		...
		$rmLotIdStatus	=	$dailycatchentryObj->checkRmLotIdAvailable($lastId);
		//echo sizeof($rmLotIdStatus)>0;
		if(sizeof($rmLotIdStatus)>0)
		{	
			$rmLotIdAvailable=$rmLotIdStatus[0];
			$weightmentChallan=$rmLotIdStatus[1];
		}
		else
		{	
	...
	if (($p["editId"]!="" || $p["selRawMaterial"]!="" ) && $p["cmdCancel"]=="") {
		...
		$supplierIdVal=$payment;
		if($supplierIdVal=='0' || $supplierIdVal=='')$supplierIdVal=$recordMainSupply;	
		$subSupplierRecords	=	$subsupplierObj->filterSubSupplierRecords($supplierIdVal, $recordLanding);
		//printr($subSupplierRecords);	
...
//die();
	...
	else if($recordProcessCode=="") $processId	=	$p["processCode"];
	...
	*/	
	...
	if ($addMode || $editMode) {
		...
		//$billingCompanyRecords= $dailycatchentryObj->fetchAllActiveRecordsbillingCompany();
		//$allBillingCompanyRecords = $dailycatchentryObj->fetchAllActiveRecordsbillingCompany();

		# Get Default Billing Comapany Id
		//$defaultCompanyId=$billingCompanyObj->getDefaultBillingCompanyID();

		#List All Plants for a company
		list($companyNames,$unitRecords,$departmentRecords,$defaultCompany)= $manageusersObj->getUserReferenceSet($userId);
		if($billingCompany!="")
		{
			$plantRecords	=$unitRecords[$billingCompany];
		}
		else
		{
			$plantRecords	=$unitRecords[$defaultCompany];
		}
	...
	var alpha='<?=$alphaCode?>';
		...
		if(alpha=="")
		{
			xajax_getChallanDetails('<?=$recordUnit?>','<?=$billingCompany?>'); 
		}
		...
		xajax_filterSubSupplier('<?=$supplierIdVal?>','<?=$recordLanding?>', '');
		var delay=500;
		setTimeout(function(){
		getChellanDetail('<?=$mode?>');
							...
							<table width="50%" align="left">
										...
										<table cellpadding="0"  width="60%" cellspacing="0" border="0" align="left">
											...
											<tr>
												<td colspan="4" nowrap align="left" style="padding-left:10px; padding-right:10px;">
													...
													<input type="hidden" name="hidCatchEntryNewId" id="hidCatchEntryNewId" value="<?=$hidCatchEntryNewId?>" title="hide Entry Id not need">
												</td>
											</tr>    
											<tr>
																			...
																			<table align="center" cellpadding="0" cellspacing="0">
																				<tr>
																					...
																					<td nowrap>
																						...
																						?>
																						<select name="billingCompany" id="billingCompany" onchange="xajax_chkValidCNum(document.getElementById('billingCompany').value, document.getElementById('weighChallanNo').value, document.getElementById('selectDate').value, document.getElementById('entryId').value,document.getElementById('unit').value, '<?=$mode?>'); xajax_chkValidDate(document.getElementById('selectDate').value, document.getElementById('billingCompany').value, '<?=$mode?>');xajax_getChallanDetails(document.getElementById('unit').value,document.getElementById('billingCompany').value); xajax_getUnit(this.value,'','');xajax_getBillingCompanyRec(document.getElementById('billingCompany').value);">
																							<option value="">--Select--</option>
																							<?php
																							foreach ($companyNames as $bcr =>$bcrNm) {
																				...
																				<tr>
																					...
																					?>
																						<select name="unit" id="unit" tabindex="1" onkeypress="return focusNextBox(event,'document.frmDailyCatch','landingCenter');" onchange="xajax_getChallanDetails(document.getElementById('unit').value,document.getElementById('billingCompany').value); xajax_chkValidCNum(document.getElementById('billingCompany').value, document.getElementById('weighChallanNo').value, document.getElementById('selectDate').value, document.getElementById('entryId').value,document.getElementById('unit').value, '<?=$mode?>');xajax_getBillingCompanyRec(document.getElementById('billingCompany').value);">
																							<option value="">--Select--</option>
																							<?php 
																							foreach($plantRecords as $pr=>$prNm) {
																								$plantId		=	$pr;
																								$plantName		=	stripSlash($prNm);
																								$selected="";
																				...
																				<tr>
																					...
																					?>
																						<input name="supplyChallanNo" type="text" id="supplyChallanNo" size="6" value="<?=$recordChallanNo?>" tabindex="6" onkeypress="return focusNextBox(event,'document.frmDailyCatch','weighChallanNo');" onkeyup="xajax_chkChallanNo(this.value,document.getElementById('mainSupplier').value);" autocomplete="off" <?=$disableField?>/>
																					</td>
																				</tr>
																				<tr>
																					<td id="supChallan1" colspan="2" style="color:red; font-size:12px">
																					</td>
																				</tr>
																				<tr>
																					<td height="5">&nbsp;</td>
																		...
																		<td>
																			<table align="center" cellpadding="0" cellspacing="0">
																					...
																					<td nowrap>
																						...
																						<select name="billingCompanyLot" id="billingCompanyLot" onchange="xajax_getBillingCompanyRec(document.getElementById('billingCompanyLot').value); xajax_chkValidCNum(document.getElementById('billingCompanyLot').value, document.getElementById('weighChallanNo').value, document.getElementById('selectDate').value, document.getElementById('entryId').value, '<?=$mode?>'); xajax_chkValidDate(document.getElementById('selectDate').value, document.getElementById('billingCompanyLot').value, '<?=$mode?>');" disabled="disabled">
																							...
																							foreach ($companyNames as $bcr =>$bcrNm) {
																								...
																								$displayCName		= $bcrNm;
																								$selected = "";
																								if(($billingCompanyId==$billingCompany) ||  ($billingCompany=="" && $companyId==$defaultCompany) ) $selected = "selected";
																							?>
																							<option value="<?=$billingCompanyId?>" <?=$selected?>><?=$displayCName?></option>
																							<?	
																							}	
																					...
																					<td nowrap>
																						<select name="pondName" id="pondName" onchange="xajax_getCountCode(document.getElementById('pondName').value,document.getElementById('rm_lot_id').value,document.getElementById('payment').value,'');"  disabled="disabled">
																							...
																							foreach($pondRecs as $sr)
																							{
																								...
																								$sel="";
																								if($pondName==$pondNameId) echo $sel="Selected";
																							?>
																							<option value="<?=$pondNameId?>" <?=$sel?>><?=$pondNameVal?></option>
																							<? 
																							}
																							?>
																						</select>
																				...
																				<tr>
																					<td class="fieldName1" nowrap>Landing Center</td>
																					<td nowrap>
																						<select name="landingCenterLot" id="landingCenterLot"  disabled="disabled">
																							<option value="">-- Select --</option>
																							<?php 
																							foreach($landingCenterRecords as $lanCntr)
																							{
																								$landId		=	$lanCntr[0];
																								$landNameVal	=	stripSlash($lanCntr[1]);
																								$selected="";
																								if($recordLanding==$landId) echo $selected="Selected";
																							?>
																							<option value="<?=$landId?>" <?=$selected?>><?=$landNameVal?></option>
																							<? 
																							}
																							?>
																						</select>
																...
																<table align="center" width="96%" cellpadding="0" cellspacing="0">
																	<tr>
																		...
																		?>
																			...
																			<input name="weighChallanNo" type="text" id="weighChallanNo" size="8" value="<?=$recordWeighNo?>" tabindex="7" onkeypress="return focusNextBox(event,'document.frmDailyCatch','selectDate');" onchange="getChellanDetail(<?=$mode?>);" autocomplete="off"  />
																			<input name="hiddenWeighChallanNo" type="hidden" id="hiddenWeighChallanNo" size="2" value="" >
																		</td>
																	</tr>
																	<tr>
																		...
																		<td nowrap>	
																			...
																			?>
																			<input type="text" id="selectDate" name="selectDate" size="8" value="<?=$recordSelectDate?>" tabindex="8" onkeypress="return focusNextBox(event,'document.frmDailyCatch','selectTimeHour');" onchange="getChellanDetail(<?=$mode?>); xajax_chkValidDate(document.getElementById('selectDate').value, document.getElementById('billingCompany').value, '<?=$mode?>');" autocomplete="off" />
																		</td>
																	</tr>
																	<tr>
																...
																<tr>
																	...
																	if ($addMode==true) {
																		...
																		if ($processId) {					
																			...
																			//echo "r--".$recordProcessCode."pr--".$processId;
																	?>
																		...
																		<table border="0" cellpadding="0" cellspacing="0" align="center" width="100%">
																			<tr>
																				<td nowrap align="center" valign="top">
																					...
																					<iframe src="CatchEntryDeclaredItem.php?entryId=<?=$catchEntryNewId;?>&mainSupplier=<?=($supplierIdVal=="")?$recordMainSupply:$supplierIdVal?>&landingCenter=<?=($landingCenter=="")?$recordLanding:$landingCenter?>" width="457" frameborder="0" height="200" id="CatchEntryDeclaredItem"></iframe>
																				</td>
																			</tr>
																		</table>	
																	</td>
																					...
																					<table border="0">
																						...
																						<tr id="additonalWastage" width="500px">
																							<?php 
																								...
																								{
																								echo'<td colspan="3" align="right"><table width="93%">';								
																								$l=0;
																								...
																								<tr>
																									...
																									<td><input id="billing_<?php echo $l;?>" type="checkbox"  name="billing_<?php echo $l;?>" <?=$chkQuality?> value="<?=$billingId?>" onclick="checkStatus('<?php echo $l;?>')"></td>
																									<td class="listing-item" nowrap>
																									...
																									<td class="listing-item" nowrap>
																										<table width="100%">
																											<TR>
																												...
																												<input id="Status_<?php echo $l;?>" type="hidden" value="" name="Status_<?php echo $l;?>">
																											</TR>
																										</table>
																									</td>
																								</tr>
																										...
																										<td>
																											...
																											<tr id="Row_<?php echo $m;?>" class="whiteRow" align="center">
																												<td id="srNo_<?php echo $m;?>" class="listing-item" align="center">
																													<select id="qualityId_<?php echo $m;?>" onchange="xajax_getQualityDet(document.getElementById('qualityId_<?php echo $m;?>').value,<?php echo $m;?>);" name="qualityId_<?php echo $m;?>" style="width:96px">
																														<option value="">--select--</option>
																																<?php
																												...
																												<td class="listing-item" align="center">
																													<input id="qualityPercent_<?php echo $m;?>" type="text" value="<?php echo $qlty[2];?>" size="4" name="qualityPercent_<?php echo $m;?>">%
																												</td>
																												<td class="listing-item" align="center">
																													<input id="qualityWeight_<?php echo $m;?>" type="text" value="<?php echo $qlty[4];?>" onkeyup="return effectiveWtNew();" size="4" name="qualityWeight_<?php echo $m;?>">Kg
																												</td>
																												<td class="listing-item" align="center">
																													Reason<input id="qualityReason_<?php echo $m;?>" type="text" autocomplete="off" value="<?php echo $qlty[5];?>" size="18" name="qualityReason_<?php echo $m;?>">
																												</td>
								...
								<tr>
									 ...
									 <td align="center" nowrap colspan="2"><input type="submit" name="cmdCancel" class="button" value=" Cancel " onclick="return cancel('DailyCatchEntry_New.php');" />
										...
										<? if($add==true){?>
										<input type="button" name="cmdAddRaw" id="cmdAddRaw" class="button" value="Save & Add New Raw Material in Challan" onclick="return validateAddDailyCatchEntry(document.frmDailyCatch, '<?=$mode?>', 'RM');" tabindex="30" style="width:250px;">
										&nbsp;&nbsp;
									<? }?>
									 </td>
									 <?} else{?>
...
function assignSupplierDetails(supplierId,pondId,unitId,companyId,supplierChallanNo,makePayment,landingCenterId)
{
	...
	jQuery('#landingCenterLot').val(landingCenterId);
	xajax_getCountCode(pondId,rm_lot_id,supplierId,'');
	xajax_getChallanDetails(unitId,companyId); 
	xajax_chkValidCNum(companyId, document.getElementById('weighChallanNo').value, document.getElementById('selectDate').value, document.getElementById('entryId').value,unitId, '1');
	xajax_filterSubSupplier(supplierId,landingCenterId,'');
	xajax_allQualityhide();
...
function assignSupplierDetailsWeightment(weightmentId,supplierId,pondId,unitId,companyId,supplierChallanNo,makePayment,speciesid,processcodeid,countcode,landingCenter)
{
	if(makePayment == 1) {
		document.getElementById('make_payment').checked = true;
	}
	...
	{	
		...
		jQuery('#landingCenterLot').val(landingCenter);
		...
		xajax_getChallanDetails(unitId,companyId); 
		xajax_chkValidCNum(companyId,document.getElementById('weighChallanNo').value, document.getElementById('selectDate').value, document.getElementById('entryId').value,unitId, '1');
		xajax_filterSubSupplier(supplierId,landingCenter, '');
		xajax_allQualityhide();
		clearPopup();
		}, 1000);
...
?>
<script>
var rm_lot_id='<?php echo $rmLotIdAvailable?>';
var weightmentChallan='<?=$weightmentChallan?>';
document.getElementById('lotIdAvailable').checked = true;
jQuery('#autoUpdate').hide();
jQuery('#autoUpdate2').show();
//alert(rm_lot_id);
jQuery('#rm_lot_id').val(rm_lot_id);
jQuery('#weighChallanNo').val(weightmentChallan);
if($('#rm_lot_id').val().length!="")
{
	getSuppierDetails(rm_lot_id);
}
//alert($('#rm_lot_id').val().length);
</script>
<?
}
?>
...
if($editMode	==	true)
	...
	?>
		...
		if(lotIdAvailable == '1')
		{
			...
			jQuery('#count_code').attr("disabled", true); 
...
//alert(entryId);
setTimeout("updateTime()",300000);//5 minutes
//setTimeout("updateTime()",10000);//5 minutes

function updateTime()
{	//alert("hii");
	var mainId='<? echo $lastId; ?>';
	var entryId='<? echo $catchEntryNewId;?>';
	if(mainId!="" && entryId!="")
	{
		xajax_timeupdate(mainId,entryId);
		setTimeout("updateTime()",300000);
	}
}
