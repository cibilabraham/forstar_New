...
if( $p["cmdSaveChange"]!="" ){?>
		...
		<?php $dailyFrozenRePackingMainId		=	$p["editId"];
			if ($dailyFrozenRePackingMainId!="") {
				...
				$dailyfrozenrepackingObj->updateDailyFrozenPackingEntry($mCPacking,$entryId);
				...
				for ($j=1; $j<=$gradeRowCount; $j++) {
									...
									$dailyfrozenrepackingObj->updateDailyFrozenPackingGradeEdit($numMC,$numLS,$gradeEntryId);
									}
				}
	...
	if( $p["cmdDelete"]!=""){
		...
		for($i=1; $i<=$rowCount; $i++)		{
			...
			if ($dailyFrozenRePackingMainId!="") {
				...
				/*$dailyFrozenPackingGradeRecDel = $dailyfrozenrepackingObj-> deleteFrozenRePackingGradeRec($dailyFrozenRePackingMainId);
				$dailyFrozenRePackingRecDel	=	$dailyfrozenrepackingObj->deleteDailyFrozenRePackingMainRec($dailyFrozenRePackingMainId);
				list($prevId,$selectDate)=$dailyfrozenrepackingObj->getPrevId($dailyFrozenRePackingMainId);
				echo $prevId;
				list($pentryId,$fishId,$processId,$freezingStage,$frozenCode,$stkAllocateMCPkgId)=$dailyfrozenrepackingObj->getProduct($prevId);
				list($gradepId,$gradeId)=$dailyfrozenrepackingObj->getGradeProduct($pentryId);
				echo $gradepId;
				$dailyfrozenrepackingObj->addPhysicalStkdailyFrozenmain($selectDate,$userId,$dfpPOEntryId,$unit,$processorId);
						$dailymainId = $databaseConnect->getLastInsertedId();
						$dailyfrozenrepackingObj->adddailyfrozenEntries($dailymainId,$fishId,$processId,$freezingStage,$frozenCode,$stkAllocateMCPkgId);
						$dailyentrymainId = $databaseConnect->getLastInsertedId();
						$numMC=80;
						$numMC=90;
						echo "**********";
						$insertPOGradeRec = $dailyfrozenrepackingObj->adddailyFrozenGradeEntries($gradepId, $gradeId, $numMC, $numLS);
				echo "_____________";*/
									...
									<td colspan="2" >
										<table cellpadding="0"  width="100%" cellspacing="0" border="0" align="center">
											...
											<tr>
												...
												<input type="submit" name="cmdSaveChange" class="button" value=" Save Changes " onClick="return validateAddDailyFrozenRePacking(document.frmDailyFrozenRePacking);"></td>
...
<td nowrap style="padding-left:2px;padding-right:2px; <?=$leftBdr.$rightBdr.$bottomBdr?>" class="listing-head"  colspan="3" ><span class="listing-head" style="padding-left:2px;padding-right:2px; ">SET MC PKG</span></td>
...
&nbsp;				</td>
			...
			<tr bgcolor="White" id="allocateRow_<?=$i;?>" class="tr_clone">				
				...
				<td colspan="3" rowspan="2" nowrap class="listing-item" style="padding-left:2px;padding-right:2px; <?=$leftBdr.$rightBdr.$bottomBdr?><?//=$rightBdr.$bottomBdr?>"><span class="listing-head" style="padding-left:2px;padding-right:2px; ">
				...
				<tr>
						<td nowrap class="listing-item" title="Num of Loose Pack" align="center" width="50%" style="padding-left:2px;padding-right:2px;">
							<input name="numLS_<?=$j?>_<?=$i?>" type="text" id="numLS_<?=$j?>_<?=$i?>" size="4" style="text-align:right;" autocomplete="off" onkeyup="calcAllocateProdnQty();" onkeydown="return nTxtBoxAL(event,'document.frmDailyFrozenPacking',this);" value="<?=($availableLS!=0)?$availableLS:""?>"/>						</td>
					</tr>
					</table>				</td>
...
<? }?>
	...
	foreach($dailyFrozenRePackingRecords as $dfrp)
	{
		...
		?>
	<tr  bgcolor="WHITE"  >
		<td width="20"><input type="checkbox" name="delId_<?=$i;?>" id="delId_<?=$i;?>" value="<?=$dailyFrozenPackingMainId;?>"><?//=$dailyFrozenPackingMainId;?></td>
		...
		<tr  bgcolor="WHITE"  >
	  <td>&nbsp;</td>
	  <td colspan="3" nowrap class="listing-item" style="padding-left:10px;">&nbsp;</td>
