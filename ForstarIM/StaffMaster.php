<?php
	...
	if ($p["cmdAddStaff"]!="") {
		...
		$multiplyFactor = $p['multiplyFactor'];
		...
		if ($name!="" && $functions!="" && $cost!="") {
			...
			if(!$chkDuplicate)
			{
				...
				$staffRecIns	=	$staffMasterObj->addStaff($name,$functions,$cost,$allowance,$multiplyFactor,$effectiveDate,$actualCost,$type,$department,$userId);
			}
			if ($staffRecIns) {
	...
	if ($p["cmdSaveChange"]!="") 
	{
		...
		if ($staffId!="" && $name!="" && $functions!="" && $cost!="") 
		{
			...
			{
				$staffRecUptd		=	$staffMasterObj->updateStaff($staffId,$name,$functions,$cost,$allowance,$multiplyFactor,$effectiveDate,$actualCost,$type,$department);
			}
			else
			{
		...
		for ($i=1; $i<=$rowCount; $i++) 
		{
			...
			if ($staffId!="") 
			{
				...
				if($staffRcrd!="")
				{
					...
					//Checking whether Staff is Active or Not
					if($active == 1)
					{
							...
							<table cellpadding="0"  width="100%" cellspacing="0" border="0" align="center">
																	...
																	<tr>
																		...
																		<td colspan="2" >
																			<table cellpadding="0"  width="65%" cellspacing="0" border="0" align="center">
																				...
																				<tr>
																					<td class="fieldName" nowrap >Multiplication Factor</td>
																					<td><INPUT TYPE="text"  name="multiplyFactor" id="multiplyFactor" size="15" value="<?if($multiplyFactor!="")echo $multiplyFactor; else echo 1;?>" onkeyUp="totalCost();"	autocomplete="off" onkeypress="return isNumber (event);">
																					</td>
																				</tr>
																				<tr>
								...
								<tr>
									...
									?>
										...
										<? if($maxpage>1){?>
										<tr>
											<td align="right" style="padding-right:10px" colspan="8" class="navRow">
												<div align="right">
												<?php 				 			  
												$nav  = '';
												for($page=1; $page<=$maxpage; $page++)
