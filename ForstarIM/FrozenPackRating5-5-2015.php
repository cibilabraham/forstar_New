		...
		if ($frozenCode!="" && $qeName!="") {
			...
			if ($gradeRowCount>0) {
				...
				for ($g=1;$g<$gradeRowCount;$g++) {
					...
					$rate=$p["rate_".$g];
					if ($qelId) {
						$insGradeRec = $frznPkgRatingObj->addSelGradeRec($qelId, $gradeId, $displayOrderId,$rate,$userId);
					}
				}
			}
	...
	if ($p["editId"]!="" || $selQuickEntryList!="") {		
		...
		if (!$selQuickEntryList) 
		{
			...
			foreach ($getGdRecords as $gdr) {
				...
				$rate = $gdr[5];
				# Add Temp Rec
				$frznPkgRatingObj->addGradeRec('0', $iGradeId, $iDisOrderId, $userId,$rate);
			}
		}		
	}
		...
		if ($fznPkngRateId!="" && $frozenCode!="" && $qeName!="") {
			...
			if ($gradeRowCount>0) {
				...
				for ($g=1;$g<$gradeRowCount;$g++) {
					...
					$rate=$p["rate_".$g];
					if ($fznPkngRateId) {
						$insGradeRec = $frznPkgRatingObj->addSelGradeRec($fznPkngRateId, $gradeId, $displayOrderId,$rate,$userId);
					}
				}
