<?php
	...
	if ($p["cmdAdd"]!="" || $p["cmdSaveAndAddNew"]!="") {
		...
		if ($qeName!="" && $selProcessor!="" && $effectiveDate!="" && $rate!="") {
			$ratelistid=$frznPkgRatingObj->getRateListId($effectiveDate);
			if($ratelistid=="0" || $ratelistid=="")
			{
				$name=$qeName.$effectiveDate;
				$FrozenPackingRateListRecIns	=	$frozenPackingRateListObj->addFrozenPackingRateList($name, $effectiveDate,$userId);
				$ratelistId= $databaseConnect->getLastInsertedId();
			}
			$fznPkRateIns =	$frznPkgRatingObj->addFznPckRate($qeName,$selProcessor,$effectiveDate,$rate,$ratelistId,$userId);
									...
									if ($fstatus!='N') 
									{
										...
										if ($rateTag!="") 
										{
											...
											for($k=0; $k<$rs; $k++)
											{
												...
												{
													$insGradeRec = $frznPkgRatingObj->addSelGradeRec($qelId,$frznPkgId,$frznId,$grades,$rate,$userId);
												}
											}
										}
									}
	...
	if ($p["cmdSaveChange"]!="") {
		...
		if ($fznPkngRateId!="" && $selProcessor!="" && $qeName!="") {
			$updateFznPknEntryRec =	$frznPkgRatingObj->updateFznPkngRateRec($fznPkngRateId,$qeName,$selProcessor,$effectiveDate,$rate,$ratelistid,$userId);
				# Del Entry Recs
									...
									if ($fstatus!='N') 
									{
										...
										if ($rateTag!="") 
										{
											...
											for($k=0; $k<$rs; $k++)
											{
												...
												foreach($gradeTot->GradeId as $grades)
												{
													$insGradeRec = $frznPkgRatingObj->addSelGradeRec($fznPkngRateId,$frznPkgId,$frznId,$grades,$userId);
												}
											}
										}
									}
