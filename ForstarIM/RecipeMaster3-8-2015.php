		...
		if ($recipeCode!="" && $recipeName!="") {
			...
			for ($i=0; $i<$itemCount; $i++) {
				...
				if ($lastId!="" && $ingredientId!="" && $quantity!="") {
					$recipeItemsIns = $recipeMasterObj->addIngredientEntries($lastId, $ingredientId, $quantity, $fixedQtyChk, $fixedQty, $percentagePerBatch, $ratePerBatch, $ingGmsPerPouch, $percentageWtPerPouch, $ratePerPouch, $percentageCostPerPouch, $idealQty, $selIngType, $ratePerKg);
				}
			  }
		     }	
		}
			...
			for ($i=0; $i<$itemCount; $i++) {
			   $status = $p["status_".$i];
				...
				if ($recipeId!="" && $ingredientId!="" && $quantity!="") {
					$recipeItemsIns = $recipeMasterObj->addIngredientEntries($recipeId, $ingredientId, $quantity, $fixedQtyChk, $fixedQty, $percentagePerBatch, $ratePerBatch, $ingGmsPerPouch, $percentageWtPerPouch, $ratePerPouch, $percentageCostPerPouch, $idealQty, $selIngType, $ratePerKg);
				}
			  }
			}
		}
																										...
																										<TD nowrap style="padding-left:5px; padding-right:5px;" colspan="3">
																											<table cellspacing="1" cellpadding="3" id="tblAddIng" class="newspaperType">
																												...
																												foreach ($productRecs as $rec) {			
																														...
																														if ($selIngType=='ING') {
																															list($lastPrice,$declYield) = $recipeMasterObj->getIngredientRate($editIngredientId, $selRateList);
																														} else if ($selIngType=='SFP') {
																															list($lastPrice,$declYield) = $recipeMasterObj->getSemiFinishRate($editIngredientId);
																														} else {
																															$lastPrice	= 0;
																															$declYield	= 0;
																														}
					...
					<tr >
						<td colspan="3">
							<table cellpadding="0" cellspacing="0" align="center">
								<tr>
									<td>
										<? if($del==true){?><input type="submit" value=" Delete " class="button"  name="cmdDelete" onClick="return confirmDelete(this.form,'delId_',<?=$productMasterRecordSize;?>);" ><? }?>&nbsp;<? if($add==true){?><input type="submit" value=" Add New " name="cmdAddNew" class="button"><? }?>&nbsp;<? if($print==true){?><input type="button" value=" Print " name="btnPrint" class="button" onClick="javascript:printWindow('PrintRecipeMaster.php?selProductCategory=<?=$selProductCategoryId?>&selCategoryId=<?=$selCategoryId?>&selCusineId=<?=$selCusineId?>',700,600);"><?}?>
									</td>
								</tr>
							</table>			
						</td>
