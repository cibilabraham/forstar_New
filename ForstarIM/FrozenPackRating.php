<?php
	...
	if ($p["cmdAdd"]!="" || $p["cmdSaveAndAddNew"]!="") {
		...
		if ($qeName!="" && $selProcessor!="" && $effectiveDate!="" && $rate!="") {
			
			$fznPkRateIns =	$frznPkgRatingObj->addFznPckRate($qeName,$selProcessor,$effectiveDate,$rate,$userId);
			...
			if ($tableRowCount>0) {
				for ($i=0; $i<$tableRowCount; $i++) 
				{
					...
					if ($status!='N') 
					{
						...
						$selQuality = $p["selQuality_".$i];
						$hidTblRowCount = $p["hidTblRowCount_".$i];
						if ($selFish!="" && $selProcessCode!="" && $selFreezingStage!="" && $selQuality!="")
						{
							$frznPkngQELEntryRecIns = $frznPkgRatingObj->addFznPkgRateEntry($qelId, $selFish, $selProcessCode,$selFreezingStage,$selQuality);
							$frznPkgId = $databaseConnect->getLastInsertedId();
							if($hidTblRowCount>0)
							{
								for($j=0; $j<$hidTblRowCount; $j++)
								{
									$fstatus = $p["fstatus_".$i."_".$j];
									if ($fstatus!='N') 
									{
										$selFrozenCode = $p["selFrozenCode_".$i."_".$j];
										$rateTag = $p["rateTag_".$i."_".$j];
										$frznCodeIns = $frznPkgRatingObj->addFznPkgRateFrozen($qelId,$frznPkgId,$selFrozenCode);
										if ($frznCodeIns) $frznId = $databaseConnect->getLastInsertedId();
										if ($rateTag!="") 
										{
											...
											for($k=0; $k<$rs; $k++)
											{
												...
												{
													$fromDate=mysqlDateFormat($gradeTot->FromDate);
													$toDate=mysqlDateFormat($gradeTot->ToDate);
													$insGradeRec = $frznPkgRatingObj->addSelGradeRec($qelId,$frznPkgId,$frznId,$grades,$rate,$fromDate,$toDate,$userId);
												}
											}
										}
									}
	...
	if ($p["editId"]!="" || $selQuickEntryList!="") {		
		...
		$selProcessorId			= $fznPkQuickEntryListRec[2];
		$rate			= $fznPkQuickEntryListRec[3];
		$ratelistId			= $fznPkQuickEntryListRec[4];
		$effectiveDate			= dateFormat($fznPkQuickEntryListRec[5]);
	...
	if ($p["cmdSaveChange"]!="") {
		...
		$tableRowCount		= $p["hidTableRowCount"];
		//die();
		if ($fznPkngRateId!="" && $selProcessor!="" && $qeName!="") {
			$updateFznPknEntryRec =	$frznPkgRatingObj->updateFznPkngRateRec($fznPkngRateId,$qeName,$selProcessor,$effectiveDate,$rate,$userId);
				# Del Entry Recs
			...
			if ($tableRowCount>0) {
				for ($i=0; $i<$tableRowCount; $i++) {
					...
					if ($status!='N') {
						...
						if ($selFish!="" && $selProcessCode!="" && $selFreezingStage!="" && $selQuality!="")
						{
							$frznPkngQELEntryRecIns = $frznPkgRatingObj->addFznPkgRateEntry($fznPkngRateId, $selFish, $selProcessCode,$selFreezingStage,$selQuality);
							$frznPkgId = $databaseConnect->getLastInsertedId();
							if($hidTblRowCount>0)
							{
								for($j=0; $j<$hidTblRowCount; $j++)
								{
									...
									if ($fstatus!='N') 
									{
										$selFrozenCode = $p["selFrozenCode_".$i."_".$j];
										$rateTag = $p["rateTag_".$i."_".$j];
										$frznCodeIns = $frznPkgRatingObj->addFznPkgRateFrozen($fznPkngRateId,$frznPkgId,$selFrozenCode);
										if ($frznCodeIns) $frznId = $databaseConnect->getLastInsertedId();
										if ($rateTag!="") 
										{
											$rates=json_decode($rateTag);
											...
											for($k=0; $k<$rs; $k++)
											{
												...
												foreach($gradeTot->GradeId as $grades)
												{
													$fromDate=mysqlDateFormat($gradeTot->FromDate);
													$toDate=mysqlDateFormat($gradeTot->ToDate);
													$insGradeRec = $frznPkgRatingObj->addSelGradeRec($fznPkngRateId,$frznPkgId,$frznId,$grades,$rate,$fromDate,$toDate,$userId);
												}
											}
										}
									}
	...
	if ($p["cmdDelete"]!="") {
		...
		for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($fznPkngRateId!="") {
				...
				$delFrozenRecs = $frznPkgRatingObj->deleteFrozenPackRec($fznPkngRateId);
				# Del Entry Recs
				$delRawDataEntryRecs = $frznPkgRatingObj->delRateRawData($fznPkngRateId);
				# delete Main Rec
...
//$gradeRecs = $frznPkgRatingObj->getGradeRecs('36','10','77','01/05/2015','01/07/2016');	
?>
																				...
																				<td style="padding:5 5 5 5px;">
																					<table  cellspacing="1" bgcolor="#999999" cellpadding="3" id="tblAddRawData">
																						<tr bgcolor="#f2f2f2" align="center">
																							...
																							<td class="listing-head" style="padding-left:5px;padding-right:5px;" nowrap="true">&nbsp;</td>
																							<td>&nbsp;</td>			
																						</tr>	
																						<?php
																							$j=0;
																							...
																							foreach ($getRawDataRecs as $rdr) {			
																								...
																								//printr($rateDetails);
																								$frozenCodeDet=$frznPkgRatingObj->getFrozenCodeRecs($fznPkgQEListId,$pkgRateEntryId);
																								//$totalGrade=""; 
																						...
																						<tr align="center" class="whiteRow" id="row_<?=$j?>">
																							...
																							<td align="center" class="listing-item">
																								<select  id="selFreezingStage_<?=$j?>" name="selFreezingStage_<?=$j?>">
																								<!--<option value="">-- Select --</option>-->
																								<?php
																								if (sizeof($freezingStageRecords)>0) {	
																									foreach ($freezingStageRecords as $freezStage) {
																										$selected = ($freezingStageId==$freezStage[0])?"selected":"";
																								?>
																								<option value="<?=$freezStage[0]?>" <?=$selected?>><?=$freezStage[1]?></option>
																								<?php
																										}
																									}
																								?>
																							...
																							<td align="center" class="listing-item">
																								<select  id="selQuality_<?=$j?>" name="selQuality_<?=$j?>">
																								<!--<option value="">-- Select --</option>-->
																								<?php
																								if (sizeof($qualityMasterRecords)>0) {	
																									foreach ($qualityMasterRecords as $qualityMaster) {
																										$selected = ($qualityId==$qualityMaster[0])?"selected":"";
																								?>
																								<option value="<?=$qualityMaster[0]?>" <?=$selected?>><?=$qualityMaster[1]?></option>
																								<?php
																										}
																									}
																								?>
																							...
																							<td class="listing-item" align="center">
																								<table cellspacing="1" cellpadding="3" bgcolor="#999999">
																									<tbody>
																										<tr>
																											<td bgcolor="#E8EDFF">
																												<table id="tblfrozen_<?=$j?>" bgcolor="#999999" cellspacing="1" cellpadding="3">
																													<tbody>
																														<tr>
																															<td bgcolor="#f2f2f2" class="listing-head">Frozen Code</td>
																															<td bgcolor="#f2f2f2" class="listing-head">Rate</td>
																															<td bgcolor="#f2f2f2"></td>
																														</tr>
																													<? 
																													if(sizeof($frozenCodeDet)>0)
																													{
																														$k=0;
																														foreach($frozenCodeDet as $frznCode)
																														{
																															$packfrozenId=$frznCode[0];
																															$frozenCodeId=$frznCode[1];
																															$rateDetails=$frznPkgRatingObj->getSelGradeRecords($userId, $fznPkgQEListId,$pkgRateEntryId,$packfrozenId);
																															$tagRecords=""; $rateTag=""; $tagRec=""; $grade=""; $rateOld=""; $totalGrade="";
																															if(sizeof($rateDetails)>0)
																															{
																																$gradeAll=$rateDetails[0][0];
																																$fromDate=dateFormat($rateDetails[0][4]);
																																$toDate=dateFormat($rateDetails[0][5]);

																																//echo $gradeAll;
																																if($gradeAll=="0")
																																{
																																	$rateTag='[{"GradeId":["A"],"FromDate":"'.$fromDate.'","ToDate":"'.$toDate.'"}]';
																																	//echo $rateTag;
																																...
																																else
																																{
																																	...
																																	$gradeId=join(",",$grade);
																																	$gradeId='['.$gradeId.']';
																																	$totalGrade[]='{"GradeId":'.$gradeId.',"FromDate":"'.$fromDate.'","ToDate":"'.$toDate.'"}';
																																	//$totalGrade[]='{"GradeId":'.$gradeId.',"Rate":'.$rateOld.'}';
																															...
																															if(sizeof($totalGrade)>0 && $rateTag=="")
																															{
																													...
																													?>
																														<tr id="rows_<?=$j?>_<?=$k?>">
																															<td bgcolor="#E8EDFF">
																																<select name="selFrozenCode_<?=$j?>_<?=$k?>" id="selFrozenCode_<?=$j?>_<?=$k?>">
																																	<option value="">-- Select --</option>
																																	<? 
																																	if(sizeof($frozenPackingRecords)>0)
																																	{
																																		...
																																		{
																																			$selected = ($frozenCodeId==$frozenPacking[0])?"selected":"";
																																	?>
																																	<option value='<?=$frozenPacking[0]?>' <?=$selected?>><?=$frozenPacking[1]?></option>	
																																	<?
																																		}
																																	}
																																	?>
																															...
																															<td  align="center" class="listing-item" bgcolor="#E8EDFF" nowrap>
																																<a onclick="getGrade(document.getElementById('selProcessCode_<?=$j?>').value,document.getElementById('selFish_<?=$j?>').value,'<?=$j?>','<?=$k?>')" href="javascript:void(0)">Set Grade</a>
																															</td>		
																															<td bgcolor="#E8EDFF">
																																<a href="###" onclick="setRowFrozenStatus('<?=$j?>','<?=$k?>');"><img title="Click here to remove this item" src="images/delIcon.gif" border="0" style="border:none;"></a>
																																<input name="fstatus_<?=$j?>_<?=$k?>" type="hidden" id="fstatus_<?=$j?>_<?=$k?>" value="">
																																<input name="IsFromDB_<?=$j?>_<?=$k?>" type="hidden" id="IsFromDB_<?=$j?>_<?=$k?>" value="N">
																																<input id="rateTag_<?=$j?>_<?=$k?>" type="hidden" value='<?=$rateTag?>' name="rateTag_<?=$j?>_<?=$k?>">
																																<input type="hidden" name="frozenPackId_<?=$j?>_<?=$k?>" id="frozenPackId_<?=$j?>_<?=$k?>" value="">
																															</td>
																														</tr>
																													<?
																														$k++;
																										...
																										<tr><td class="listing-item" bgcolor="#E8EDFF"><input type="hidden" name="hidTblRowCount_<?=$j?>" id="hidTblRowCount_<?=$j?>" value="<?=$k?>"><a href="#" onclick="addrow('<?=$j?>');" style="color:red;font-size:10px">Add New</a></td></tr>
																									</tbody>
																								</table>
																							</td>
										...
										<td colspan="2" style="padding-left:10px;padding-right:10px;" align="center">
											...
											<? }?>
												...
												foreach ($fznPkngRateRecords as $fpqel) {
														...
														*/
														# ----------------------
														foreach ($getProcessCodeRecs as $pcr) 
														{
															$entryId=$pcr[4];
															$getFrozenCode=$frznPkgRatingObj->getFrozenData($frznCodeRate,$entryId);
														}
												?>
												...
												<tr <?=$displayRowStyle?> bgcolor="white">
													...
													<?php
															$numCol = 6;
															if (sizeof($getFrozenCode)>0) {
																$nextRec = 0;
																$pcName = "";
																foreach ($getFrozenCode as $cR) {
																	$frCode = $cR[1];
																	$nextRec++;
																	if($nextRec>1) echo "&nbsp;,&nbsp;"; echo $frCode;
																	if($nextRec%$numCol == 0) echo "<br/>";
																}
															}						
														?>
