<?php
	...
	if ($p['cmdUpload']!='') {
		...
		{
			$dt = Date('Ymd');
			$saveFileName = "ewaybill_".$salesOrderId."_".$dt.".pdf";
			//$saveFileName = "ewaybill_$dt_o_$salesOrderId.pdf";
			$folderName	= "Invoice_ewaybill";
			...
			if( $uploadCheck ) 
			$upload_Msg = "File $saveFileName has been uploaded."	;
			//header("Location:AssignStockCategories.php");
		}
		else {$errMsg = "Invalid file type. Please select a valid pdf file to upload." ;
		}
	...
	if ($p["cmdAdd"]!="") {
		...
		*/	
		...
		$GstActive 		= $p["hidGstActive"];
		$iGstActive 		= $p["hidiGstActive"];
		...
		/*rekha updated code GST/IGST */
		
		//$totgstAmt		= $p["totgstAmt"];
		
		$totcgstAmt = $p["totcgstAmt"]; 
		$totsgstAmt = $p["totsgstAmt"]; 
		$totgstAmt = $totcgstAmt + $totcgstAmt ;
		
		$totigstAmt		= $p["totigstAmt"];
	...
	if ($p["editId"]!="" || $g["editId"]) {
		...
		//echo($editId);
		/*
		# Chk already modified // rekha commented code here 
		...
		$trans_type=$salesOrderRec[57];
		$ewaybill_no=$salesOrderRec[58];
		$eway_file_name=$salesOrderRec[59];
		...
		//$invoiceNo 	= $salesOrderObj->getNextProformaInvoiceNo();
		//rekha modify here 
		
		$invoiceNo = $proformaInvoiceNo ;
	...
	if ($p["cmdSaveChange"]!="" || $p["cmdSaveAndConfirm"]) {
		...
		$totcgstAmt	= $p["totcgstAmt"];
		$totsgstAmt	= $p["totsgstAmt"];
		$selnatoftrans	= $p["selnatoftrans"];
		$ewaybillno	= $p["ewaybillno"];
		$file_eway	= $p["file_eway"];
...
<form name="frmSalesOrder" id="frmSalesOrder" action="SalesOrder.php" method="post" ENCTYPE="multipart/form-data">
									...
									<td colspan="2">
										<table cellpadding="0"  width="65%" cellspacing="0" border="0" align="center">
																		...
																		<TD valign="top">
																			<table>
																				...
																				<tr id="proformaInvNoRow">
																					<td class="fieldName" nowrap="true">*Proforma Invoice No.</td>
																					<td nowrap="true">
																		...
																		<td>
																			<table>
																				...
																				<tr>
																					...
																					<td nowrap>
																						<select id="unit" name="unit" onchange="xajax_getProformaInvoiceNo(document.getElementById('company').value,document.getElementById('unit').value);">" required>
																							<option value="">--select--</option>
																							<?php
																			...
																			<table>
																				<TR>
																					...
																					<td nowrap="true">
																						...
																						<!--<input type="text" name="gatePassNo" id="gatePassNo"/>
																						-->
															...
															<TD colspan="3">
																<table>
																	<tr>
																		...
																		<td>
																			<select name="selState" id="selState" onchange="xajax_calculate_so_bottom();xajax_getProductRecs(document.getElementById('selDistributor').value, document.getElementById('selState').value, '<?=$productPriceRateListId?>', '<?=$mode?>', document.getElementById('hidTableRowCount').value, document.getElementById('invoiceDate').value, '<?=$distMgnRateListId?>','', document.getElementById('billingType').value); xajax_getCityList(document.getElementById('selDistributor').value, document.getElementById('selState').value,'', document.getElementById('billingType').value); xajax_getAreaList(document.getElementById('selDistributor').value, document.getElementById('selState').value, '','', document.getElementById('billingType').value);">
																			<option value=""> -- select-- </option>
																			</select>
																		</td>
																		<td></td>					
											...
											<tr>
												...
												<input type="hidden" name="hiddGST_IGST" id="hiddGST_IGST" value="">
													<table  cellspacing="1" bgcolor="#999999" cellpadding="3" id="tblAddSOItem">
														<tr bgcolor="#f2f2f2" align="center">
															...
															<!--<div id="gst_igst" style="display:none;">-->
															...
															if($taxType='IGST'){
															?>															
															<td class="listing-head">IGST<br>(%)</td>
															<?php 
											...
											if($editMode){?>
												...
												<input type='hidden' name='taxType' id='taxType' value="<?=$edit_taxtype?>">
																				...
																				<TD valign="top">
																					...
																					<legend class="listing-item" style="line-height:normal;">Despatch Details</legend>
																						<table>
																							...
																							<tr>
																								<TD class="fieldName" nowrap="true">Nature Of Transaction:</TD>
																								<td class="listing-item" nowrap="true">
																									<select name="selnatoftrans" id="selnatoftrans" >
																										<option value="">-- Select --</option>
																										<option value="B2B" <?if($trans_type=='B2B') echo 'selected';?>>B2B</option>
																										<option value="B2C" <?if($trans_type=='B2C') echo 'selected';?>>B2C</option>
																									</select>																									
																							...
																							<tr>
																								<TD class="fieldName" nowrap="true">Eway Bill No:</TD>
																								<td class="listing-item" nowrap="true">
																			
																								<input type="text" name="ewaybillno" id="ewaybillno" size="20" value="<?=$ewaybill_no?>" autocomplete="off" />																								
																									
																								</td>
																							</tr>																							
																							
																							<tr>
																								<TD class="fieldName" nowrap="true">Upload Eway PDF:</TD>
																								<td class="listing-item" nowrap="true">
																									
																								<?if($eway_file_name!=""){
																									echo("<a href='Invoice_ewaybill/$eway_file_name' target='_blank'>".$eway_file_name."</a><br>"); 
																								}
																								
																								?>
																								
																									<input type="file" class="input-text" name="uploadFile" id="uploadFile" value="Browse" size="75"><input name="file_eway" id="file_eway" type="hidden" value="<?=$saveFileName?>">&nbsp;&nbsp;<input type="submit" class="button" name="cmdUpload" value="Upload" onclick="assignValue(this.form,'<?=$editId?>','editId');assignValue(this.form,'1','editSelectionChange');this.form.action='SalesOrder.php';">																								
																													
																									<?if($upload_Msg!="") {
																										echo "<br><span class='err1'><font color='00ff00'>".$upload_Msg."</font></span>";	
																									}
																									if($errMsg!="") {
																										echo "<br><span class='err1'>".$errMsg."</span>";	
																									}
							...
							<td colspan="2" style="padding-left:10px;padding-right:10px;">
								...
								<? if($maxpage>1){?>
								   ...
								   <? }?>
									<tr  bgcolor="#f2f2f2" align="center">
										...
										<td class="listing-head">Eway Bill No</td>
										<? if($edit==true){?>
										...
										*/
										...
										<tr <?=$listRowMouseOverStyle?>>
											...
											<td class="listing-item" nowrap style="padding-left:5px; padding-right:5px;" align="center"  onMouseover="ShowTip('<?=$gPassMsg?>');" onMouseout="UnTip();" id="gatePassCol_<?=$i?>" bgcolor="<?=$gPassBgColor?>">
												<?
												if($gPassStatus!='COMPLETED' && $gPassStatus!="" ){
													echo($gPassStatus."<br>");
												}
												?>
												<?=$gPassLink?>
											</td>
											<td class="listing-item" nowrap style="padding-left:5px; padding-top: 5px; padding-right:5px;" align="center">
											<!--<?//=$gPassLink?><br><br>-->
											<!-- rekha modified here -->
											<?
											if($tax_type_val=='GST' || $tax_type_val=='IGST'){
											?>
													<a href="javascript:printWindow('PrintSOGSTInvoice.php?selSOId=<?=$salesOrderId?>&priny',700,600)" class="link1" title="Click here to View Invoice">
														VIEW INVOICE
													</a>
												<?}	
												else{
											...
											<a href="###" onclick="displayPopUp_ewaybillno('<?=$eway_billno?>','<?=$eway_file_name?>');" class="link2"><img src='images/icon_viewbill.png' title='View Eway Bill No' width='20' height='20'></a>
											</td>		
											<? if($edit==true ){
												list($taxType, $billingForm, $billingStateId, $exBillingForm) 	= $salesOrderObj->getDistTaxType($distributorId, $stateId, 'N');
												
												if($taxType==$tax_type_val){
												?>
											<td class="listing-item" width="60" align="center" style="line-height:normal;font-size:8px;">
												<input type="submit" value=" Edit " name="cmdEdit" onClick="assignValue(this.form,<?=$salesOrderId;?>,'editId');assignValue(this.form,'1','editSelectionChange');this.form.action='SalesOrder.php';" >
												<?php
								...
								<tr>
									<td colspan="3" height="5" >
										...
										<input type="hidden" name="hidGstActive" id="hidGstActive" value="<?=$GstActive?>" readonly />
										<input type="hidden" name="hidiGstActive" id="hidiGstActive" value="<?=$iGstActive?>" readonly />
									</td>
								</tr>
							</table>						
						</td>
...
?>
	...
	<div id="dialog_billno" title="Eway Bill No" style="display:none" >
