...
*/
		...
		if ($purchaseOrderNo!="" ) {
			if ($selSupplierId!="") {
				$purchaseOrderRecIns = $ingredientPurchaseorderObj->addPurchaseOrder($purchaseOrderNo, $selSupplierId, $userId);
...
## ----------------- Pagination Settings I End ------------	
	...
	if($_REQUEST["action"]=="displayMsg")
	{
		...
		$checkGateNumberSettingsExist=$ingredientPurchaseorderObj->chkValidPOId($selDate);
		if (sizeof($checkGateNumberSettingsExist)>0){
		$alphaCode=$ingredientPurchaseorderObj->getAlphaCode($selDate);
		$alphaCodePrefix= $alphaCode[0];
		$numbergen=$checkGateNumberSettingsExist[0][0];
		$checkExist=$ingredientPurchaseorderObj->checkPOIDDisplayExist();
		if ($checkExist>0){
		$getFirstRecord=$ingredientPurchaseorderObj->getmaxPOId();
		$getFirstRec= $getFirstRecord[0];
		$getFirstRecEx=explode($alphaCodePrefix,$getFirstRec);
		$nextGatePassId=$getFirstRecEx[1]+1;
		$validendno=$ingredientPurchaseorderObj->getValidendnoPOId($selDate);
		...
		{
			$validPassNo=$ingredientPurchaseorderObj->getValidPOId($selDate);	
			$checkPassId=$ingredientPurchaseorderObj->chkValidPOId($selDate);
													...
													<TD>
														<table  cellspacing="1" bgcolor="#999999" cellpadding="3" id="tblAddPOIngItem">
															...
															foreach ($purchaseRecs as $pr) {
																...
																$unitPrice = $ingredientPurchaseorderObj->findIngredientRate($selSupplierId,$selIngId);
