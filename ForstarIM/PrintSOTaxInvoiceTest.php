<?php	
	...
	$poDate		= dateFormat($sORec[25]);
	$challanNo	= $sORec[26];
	$challanDate	= dateFormat($sORec[27]);

	$poNoDisplay	= $poNo;
	if ($sORec[25])	$poNoDisplay .= ",&nbsp;$poDate";

	$challanNoDisplay = $challanNo;
	if ($sORec[27])	$challanNoDisplay .= ",&nbsp;$challanDate";
	...
	if ($selSOId) {
		...
		$soItemRecs	= $salesOrderItemRecs;
		$limit		= 18;
		//$offset = (1-1)*$limit;
		$soRecSize = sizeof($salesOrderItemRecs);
		$tPage = ceil($soRecSize/$limit);
		//$allRecs = array();

		//$allRecs = array_splice($soItemRecs, $offset, $limit) ;
		//$allRecs = array();
		...
		for ($page=1;$page<=$tPage;$page++)
		{
			...
			//echo "<pre>";	 
			//echo "size=".sizeof($allRecs)."Page:$page";
			//print_r($allRecs);		
			//echo "<pre>";
		}
	}
	...
	# TopMain Header
	function topMainHeader($invoiceType, $print)
	{
		$topMainHeader = "<table cellpadding=\"0\" cellspacing=\"0\" width=\"100%\">
				<TR>
					...
					if ($invoiceType=='S') {
		$topMainHeader .= 	" <span style=\"font-size:16px;\">
							SAMPLE INVOICE
						</span>";
					} else {
		$topMainHeader .= "<span style=\"font-size:16px;\">TAX INVOICE</span>";
					} if ($invoiceType=='S') { 
		$topMainHeader .= "<br>	<span style=\"font-size:9px;\">(NOT FOR COMMERCIAL PURPOSE)</span>";
					}
					if($print==0){
		$topMainHeader .= "<div id=\"printMsg\" class=\"printSOMsg\">(ORIGINAL)</div>";
					 } else if ($print==1) {
		$topMainHeader .= "<div id=\"printMsg\" class=\"printSOMsg\">(DUPLICATE)</div>";
					} else  {
		$topMainHeader .= "<div id=\"printMsg\" class=\"printSOMsg\">(TRIPLICATE)</div>";
					}
		$topMainHeader .= "</td><td align=\"right\" valign=\"top\">
		...
		return $topMainHeader;
	}
	# top Second Header
	function topSubHeader($distributorName, $address, $cityName, $stateName, $pinCode, $telNo, $taxType, $cstNo, $tinNo, $pOGenerateId, $createdDate, $poNoDisplay, $challanNoDisplay, $transporterName)
	{
		$topSubHeader = "<table width='99%' cellpadding='0' cellspacing='0' class='print' align='center' style='border-bottom-width:0px'>
		<tr>
		...
		$topSubHeader .= 	$cityName.',&nbsp;'.$stateName;
		$topSubHeader .= 	" </td> </tr> ";		
						if ($pinCode!='') {
		$topSubHeader .=  	"<tr>
						<td class='listing-item' width='200' height='15' colspan='2' style='padding-left:10px;padding-right:10px;font-size:11px;'>
						...
						if ($telNo!='') {		
		$topSubHeader .=  	"<tr><td class='listing-item' width='200' height='20' colspan='2' style='padding-left:10px;padding-right:10px;font-size:11px;'>
							TEL - &nbsp;$telNo
						</td>
					</tr>";
						}
						if ($taxType=='CST') {					
		$topSubHeader .= 	"<tr><TD height='2'></TD></tr>
					<tr><TD colspan='2'>
						<table cellpadding='0' cellspacing='0'>
							<tr>
		...
		$topSubHeader .=		$pOGenerateId.',&nbsp;'.$createdDate;
		$topSubHeader .=	"	</td>
						</TR>
						</table></TD></tr>	
						<tr>			
	...
	# Second Page Header
	function secondPageMainHeader($invoiceType, $print)
	{
		$secondPageMainHeader = "<table cellpadding='0' cellspacing='0' width='100%'>
				<TR>
					<TD align='left' valign='top'><img src='images/ForstarLogo.png' alt=''/></TD>
					<td class='pageName' valign='bottom' align='center'>";
					 if ($invoiceType=='S') { 
		$secondPageMainHeader .= "<span style='font-size:16px;'>
							SAMPLE INVOICE
						</span>";
					 } else {
		$secondPageMainHeader .= "<span style='font-size:16px;'>
					 ...
					 if ($invoiceType=='S') {
		$secondPageMainHeader .= " <br>
						<span style='font-size:9px;'>
							(NOT FOR COMMERCIAL PURPOSE)
						</span>";
					  }
					  if($print==0){
		$secondPageMainHeader .= "<div id='printMsg' class='printSOMsg'>(ORIGINAL) - Cont.</div>";
		   			  } else if ($print==1) {
		$secondPageMainHeader .= "<div id='printMsg' class='printSOMsg'>(DUPLICATE) - Cont.</div>";
		 			  } else  {
		$secondPageMainHeader .=  "<div id='printMsg' class='printSOMsg'>(TRIPLICATE) - Cont.</div>";
  					  }
		$secondPageMainHeader .=  "</td>
		...
		return $secondPageMainHeader;
	}
	
	# Second Page Sub Header
	function secondPageSubHeader($distributorName, $cityName, $stateName, $pOGenerateId, $createdDate)
	{
		$secondPageSubHeader = "<table width='99%' cellpadding='0' cellspacing='0' class='print' align='center' style='border-bottom-width:0px'>
		<tr>
			<TD rowspan='6' width='400px;' style='border-bottom-width:0px'> 
				<table cellspacing='0' cellpadding='0' width='100%' class='tdBoarder'>
					...
					<tr>
						<td class='listing-item' width='200' height='15' colspan='2' style='padding-left:10px;padding-right:10px;font-size:11px;'>";
		$secondPageSubHeader .=		$cityName.',&nbsp;'.$stateName;
		$secondPageSubHeader .= "</td></tr></table></TD>
					...
					<TR>
						...
						<td class='listing-item-print' style='padding-left:3px;' nowrap='true' valign='middle'>";
		$secondPageSubHeader .= $pOGenerateId.',&nbsp;'.$createdDate;
		$secondPageSubHeader .= "</td></TR></table></TD></tr>
					</table>";
		return $secondPageSubHeader;
	}
?>
<html>
<head>
...
<table width="100%" align='center' border="0" cellpadding="0" cellspacing="0" bgcolor="#f2f2f2">
	...
	<tr bgcolor="White">
		<TD style="padding-left:5px; padding-right:5px;">
			<?=topMainHeader($invoiceType, $print);?>
		</TD>
	</tr>
	<!--<tr bgcolor="White">
		<TD style="padding-left:5px; padding-right:5px;">
 ...
 <tr bgcolor='white'>
  ...
  <tr bgcolor="white">
	<td align="LEFT" valign="top" width='100%'><?=topSubHeader($distributorName, $address, $cityName, $stateName, $pinCode, $telNo, $taxType, $cstNo, $tinNo, $pOGenerateId, $createdDate, $poNoDisplay, $challanNoDisplay, $transporterName);?>
...
<table width="99%" cellpadding="2" cellspacing="0" bgcolor="#CCCCCC"  class="print">
	...
	<th class="p-listing-head" style="padding-left:3px; padding-right:3px;" nowrap="true" width="12%">TOTAL<br/>(RS.)</th>
      ...
      <?php
		$numRows = 21; // Setting No.of rows 10/18
		$j = 0;
		$decreaseRow = 1;
		...
		foreach ($salesOrderItemRecs as $por) {
			...
			//echo "<pre>";
			//print_r($resultNumMCArr);
			//echo $resultNumMCArr[$j];
			//echo "</pre>";
	?>
      <tr bgcolor="#FFFFFF">
	...
	<td class="listing-item" align="right" style="padding-left:3px; padding-right:3px; font-size:8pt;"><?=number_format($quantity,0,'.','');?></td>
        <td class="listing-item" style="padding-left:3px; padding-right:3px; font-size:8pt;" align="right"><?=0?></td>
	...
	<td class="listing-item" style="padding-left:3px; padding-right:3px; font-size:8pt;" align="right"><?=$total?></td>
      </tr>
	  	<?php
		//echo "$i==$j.11hhhhhh<br>";
		if ($i%$numRows==0 && $salesOrderRecSize!=$numRows) {
			 $j++;
			
		?>
	<tr bgcolor="#FFFFFF">
        <td height="20" colspan="2" nowrap="nowrap" class="listing-head" align="right">Total:</td>
	...
	<TD colspan="17" align="center" >
		...
		<tr>
			...
			<table width="100%" cellpadding="0" cellspacing="0">
				<TR>
					<TD valign="top" height="5" width="400px"></td>
					<td rowspan="5" valign="bottom" style="line-height:100px;">
						<table width="100%" align="right" cellpadding="0" cellspacing="0" class="print" style="border-right-width:0px;" >
							<tr>
...
<!-- Ends Here -->	
	...
	<!-- Setting Page Break start Here-->
	  ...
	  <tr>
	   ...
	   <tr bgcolor="White">
		<TD colspan="17" align="center"><?=secondPageMainHeader($invoiceType, $print);?></TD>
	  </tr>
	<tr bgcolor="White"><TD height="5"></TD></tr>
	<tr>
	<td align="center" valign="top" width='100%' bgcolor="#FFFFFF">
	<table width='99%' bgcolor="#f2f2f2" id="tableIdD_<?=$print?>">
         <tr>
           <td class="listing-head" nowrap="nowrap" align='left' colspan='2' height="5"></td>
	 </tr>
	</table>
	...
	<tr bgcolor=white>
	<td align="LEFT" valign="top" width='100%'><?=secondPageSubHeader($distributorName, $cityName, $stateName, $pOGenerateId, $createdDate);?>	
	</td>
  </tr>
  <tr>
	<td colspan="17" align="center">
   ...
   ?>
	...
	?>
      <tr bgcolor="#FFFFFF">
        <td height="20" colspan="2" nowrap="nowrap" class="listing-head" align="right">
	...
	<td class="listing-item" style="padding-left:3px; padding-right:3px; font-size:8pt;" align="right"><strong><?// echo number_format($totalAmt,2);?></strong></td>
	<td class="listing-item" style="padding-left:3px; padding-right:3px; font-size:8px;" align="right"></td>
	...
	<?php
			# Roun of Calculation
			$roundFig = FloatVal(substr(number_format($grandTotalAmt,2,'.',''),-3));
			$roundVal = ($roundFig>=0.50)?(1-$roundFig):(-$roundFig);
			$roundVal = number_format($roundVal,2,'.','');
			
			$grandTotalAmt = round($grandTotalAmt+$roundVal);

		?>
	<tr>
		<td height='25' colspan="6" nowrap="nowrap"  align="right">
			<table cellspacing='0' cellpadding='0' width="100%" class="tdBoarder">
				<tr>
		...
		<td height='25' class="listing-head" style="padding-left:3px; padding-right:3px; font-size:8pt" align="right">Round</td>
		<td height='25' class="listing-item" style="padding-left:3px; padding-right:3px; font-size:8pt;" align="right"><strong><?=$roundVal?></strong></td>
	</tr>
	<tr bgcolor="#FFFFFF">
...
<!--tr bgcolor="White"><TD height="3"></TD></tr-->
 <tr bgcolor="White">
	<TD colspan="17" align="center" >
		<table width='99%' cellpadding='0' cellspacing='0' class="print" align="center" style="border-top-width:0px" id="tableIdH_<?=$print?>">
			<tr>	
				<td style="border-bottom-width:0px;border-top-width:0px">
					<table cellspacing='0' cellpadding='0' width="100%" class="tdBoarder" id="tableIdI_<?=$print?>">
						<tr><TD height="5"></TD></tr>
						<tr>
							<td align="left">
		<table width="85%" cellpadding="2" cellspacing="0" bgcolor="#CCCCCC"  class="print" style="border-left-width:0px;">			
		...
		<tr>
			...
			<tr>
				<TD style="line-height:11px;">
					<table cellpadding="0" cellspacing="0">
						<TR>
							<TD class="print-listing-head" style="padding-left:5px;padding-right:2px;font-size:7px;" nowrap="true" valign="middle">VAT TIN:</TD>
							<td class="listing-item" style="font-size:8px;" nowrap="true" valign="middle"><?=$vatTin?></td>
							<td class="print-listing-head" style="padding-left:5px;padding-right:5px;font-size:7px;" nowrap="true" valign="middle">CST TIN:</td>
							<td class="listing-item" style="font-size:8px;" nowrap="true" valign="middle"><?=$cstTin?></td>
						</TR>
					</table>
				</TD>
			</tr>
	  ...
	  <tr>
		...
		<table cellpadding="0" cellspacing="0" width="100%">
			<TR>
				...
				<td width="35%" align="center" class="listing-item">
					<!--(Page <?//$totalPage?> of <?//$totalPage?>)--><?//$totalPage?>
					<?
						if ($totalPage>1) echo "(Page $totalPage of $totalPage)";
					?>
				</td>
				<td width="35%" align="right" class="listing-item" style="font-size:8px;" nowrap="true">&nbsp;
					<!--This Invoice is issued subject to Mumbai Jurisdiction.-->
				</td>
			</TR>
		</table>
