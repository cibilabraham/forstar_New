<?php
					...
					for ($i=0; $i<$itemCount; $i++) {
						...
						if ($status!='N') {
							...
							if ($lastInsId && $selFish && $selProcessCode) {
								$purchaseOrderRawEntryRecIns = $purchaseorderObj->addPurchaseOrderRawEntries($lastInsId,$selFish, $selProcessCode, $selEuCode, $selBrand, $selGrade, $selFreezingStage, $selFrozenCode, $selMCPacking, $numMC, $pricePerKg, $valueInUSD, $valueInINR, $brandFrom, $wtType,$processType);	
							}
	...
	if ($p["editId"]!="" ) {
		...
		$company			= $purchaseOrderRec[24];	 
		$unit			= $purchaseOrderRec[25];	 
					...
					for ($i=0; $i<$itemCount; $i++) {
						...
						if ($status!='N') {
							...
							if ($poMainId && $selFish && $selProcessCode && $poEntryId=="") {
								$purchaseOrderRawEntryRecIns = $purchaseorderObj->addPurchaseOrderRawEntries($poMainId, $selFish, $selProcessCode, $selEuCode, $selBrand, $selGrade, $selFreezingStage, $selFrozenCode, $selMCPacking, $numMC, $pricePerKg, $valueInUSD, $valueInINR, $brandFrom,$wtType,$processType);	
							} else if ($poMainId && $selFish && $selProcessCode && $poEntryId!="") {
								$updatePORawEntry	= $purchaseorderObj->updatePORawEntries($poEntryId, $selFish, $selProcessCode, $selEuCode, $selBrand, $selGrade, $selFreezingStage, $selFrozenCode, $selMCPacking, $numMC, $pricePerKg, $valueInUSD, $valueInINR, $brandFrom,$wtType,$processType);
							}
						} // Status Check Ends here
			...
			if($poMainId!="") 
			{
				...
				if(sizeof($delPo)>0)
				{
					...
					foreach($delPo as $dp)
					{
						...
						if($allocateStatus==0)
						{
							$delAllocate=$purchaseorderObj->deleteRecordAllocate($allocateId);
						}
						else if($allocateStatus==1)
						{
							$delAllocateRMLot=$purchaseorderObj->deleteRecordAllocateRmLotId($allocateId);
						}
					}
					
					# Delete Sel chk list
					$delPOEntry=$purchaseorderObj->getPoEntryId($poMainId);
					$delPOEntryRMLot=$purchaseorderObj->getPoEntryIdRMLot($poMainId);
					$deleteSelChkList = $purchaseorderObj->delChkList($poMainId);
									...
									<td colspan="2" >
										<table cellpadding="0"  width="100%" cellspacing="0" border="0" align="center">
											...
											<tr>
												...
												<?} else{?>
												<td align="center">
													<input type="submit" name="cmdCancel" class="button" value=" Cancel " onClick="return cancel('PurchaseOrders.php');">&nbsp;&nbsp;<input type="submit" name="cmdAdd" id="cmdAdd" class="button" value=" Save &amp; Exit" onClick="return validatePurchaseOrder(document.frmPurchaseOrder, '');">
												</td>
																	...
																	<TR>
																		<TD valign="top">
																			<fieldset>
																				...
																				<?php }?>
																					<TR>
																						<td class="fieldName" nowrap="true">*Company.</td>
																						<td nowrap="true">
																							<select name="company" id="company" onchange="xajax_getUnits(this.value);" >
																								...
																								if (sizeof($companyRecords)>0) {	
																									foreach ($companyRecords as $cr=>$crs) {
																										...
																										if($company!="")
																										{
																											$sel  = ($company==$companyIds)?"Selected":"";
																										}
																										else
																										{
																					...
																					<TR>
																						...
																						<td nowrap="true">
																						<select name="unit" id="unit"   onchange="xajax_getPONumber('',document.getElementById('company').value,this.value)">
																							<option value="" >---Select--- </option>
																							<?php 
																							if($units>0)
																							{
																								...
																								{	
																									...
																									?>
																									 <option value="<?=$unitId;?>" <?=$sel?> ><?=$unitName;?></option>
																									<?php
																								}
																							}
																							?>
																					...
																					<TR>
																						...
																						<td nowrap="true">
																							<input name="poNo" id="poNo" type="text" size="12" value="<?=$poNo?>" onKeyUp="xajax_chkPONumberExist(document.getElementById('poNo').value,'<?=$mode?>','<?=$mainId?>');" autocomplete="off"  <?if($mode==0){?> readonly<? } ?>/>
																					...
																					<tr>
        																				<td class="fieldName" nowrap="nowrap" style="line-height:normal;">*Customer<br/>
																...
																<TD style="padding-left:10px;padding-right:10px;">
																	<table  cellspacing="1" bgcolor="#999999" cellpadding="3" id="tblPOItem">
																		<tr bgcolor="#f2f2f2" align="center">
																			...
																			<td class="listing-head">Process Type</td>
																		...
																		foreach ($poRawItemRecs as $rec) {
																			...
																			if($processType=="S")
																			{
																				$fishRecs		= $purchaseorderObj->getFishRecs();
																				$pcRecs		= $processcodeObj->getProcessCodeRecs($selFishId);
																				# Grade Recs
																				$gradeRecs	= $purchaseorderObj->getFrozenGradeRecs($selProcessCodeId);
																				$processTypes="Selected";
																				$processTypec="";
																			}
																			else if($processType=="C")
																			{	
																				$fishRecs		= $purchaseorderObj->getAssorted();
																				$pcRecs		= $purchaseorderObj->getSecondaryProcessCodeRecs();
																				$gradeRecs		= $purchaseorderObj->getSecondaryFrozenGradeRecs($selProcessCodeId);	
																				$processTypes="";
																				$processTypec="Selected";
																			}
																			# PC Recs
																			//$pcRecs		= $processcodeObj->getProcessCodeRecs($selFishId);
																		...
																		<tr align="center" class="whiteRow" id="row_<?=$k?>">
																			...
																			<td align="center" class="listing-item">
																				<select id="processType_<?=$k?>" onchange="xajax_getfishCodes(document.getElementById('processType_<?=$k?>').value, <?=$k?>,'');" name="processType_<?=$k?>">
																					<option value="">--Select--</option>
																					<option value="S" <?=$processTypes?>>Single</option>
																					<option value="C" <?=$processTypec?>>Combo</option>
																				</select>
																			</td>
																			<td align="center" class="listing-item">
																				<select id="selFish_<?=$k?>" name="selFish_<?=$k?>" onchange="xajax_getProcessCodes(document.getElementById('selFish_<?=$k?>').value, document.getElementById('processType_<?=$k?>').value ,'<?=$k?>', ''); xajax_getFrznCodes('<?=$k?>', document.getElementById('selFish_<?=$k?>').value, '', '');" >
																					<option value="">--Select--</option>
																					<?php
																					if (sizeof($fishRecs)>0) {	
																						foreach($fishRecs as $fr=>$frName) {
																							$fishId		= $fr;
																							$fishName	= stripSlash($frName);
																			...
																			<td align="center" class="listing-item">
																				<select id="selProcessCode_<?=$k?>" name="selProcessCode_<?=$k?>" onchange="xajax_getGradeRecs(document.getElementById('processType_<?=$k?>').value,document.getElementById('selProcessCode_<?=$k?>').value, '<?=$k?>', ''); xajax_getFrznCodes('<?=$k?>', document.getElementById('selFish_<?=$k?>').value, document.getElementById('selProcessCode_<?=$k?>').value, '');" >
																				<?php
																				if (!sizeof($pcRecs)) {
																				?>
																				<option value="">-- Select --</option>
																							...
																							<tr>
																								<td>
																									<table border="0">
																									  ...
																									  <tr>
																											<td nowrap="true" align="center" colspan="2" id="divNumExistTxt_<?=$l?>" style="line-height: normal; font-size: 10px; color: red;" class="listing-item"></td>
																									  </tr>
																								</tbody>
																							</table>
																							</td>
