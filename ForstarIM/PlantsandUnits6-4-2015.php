<?php
	...
	if ($p["cmdAddPlant"]!="") {
		...
		if ($no!="" && $name!="") {
			$plantRecIns	=	$plantandunitObj->addPlant($no, $name,$stdProduction,$basedOn);
			if ($plantRecIns) {
	...
	if ($p["editId"]!="") {
		...
		$stdProduction		=	stripSlash($plantRec[3]);
		$basedOn		=	stripSlash($plantRec[4]);
		if($basedOn!="") { 
	...
	if ($p["cmdSaveChange"]!="") {
		...
		$basedOn	=	addSlash($p["basedOn"]);
		if ($plantId!="" && $no!="" && $name!="") {
			$plantRecUptd	= $plantandunitObj->updatePlant($plantId, $no, $name,$stdProduction,$basedOn);
		}
