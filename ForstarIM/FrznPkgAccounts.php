<?php
	...
	{ 
		...
		for ($i=1; $i<=$rowCount; $i++) 
		{
			...
			for ($j=0; $j<sizeof($groupGradeArr); $j++) 
			{
				...
				if ($gEntryId!="" && $rate!="" && $rmlotid=="") 
				{			
					$fprRecUpdated =$frznPkgAccountsObj->updateDFPGradeRec($gEntryId, $settled, $rate, $gradeTotalRate,$rateListId);
				}
				else if ($gEntryId!="" && $rate!="" && $rmlotid!="") 
				{			
					$fprRecUpdated =$frznPkgAccountsObj->updateDFPGradeRMLotIdRec($gEntryId, $settled, $rate, $gradeTotalRate,$rateListId);
				}
	...
	if($dateFrom!='' &&  $dateTill!='')
	{
		...
		if($selProcessorId!='')
		{
			$rmlotRecords	=	$frznPkgAccountsObj->getRmLotRec($selProcessorId,$fromDate,$tillDate);
		}
			$selRMLotId=$p['rmlot'];
		
			$frnPkgRecs	=	$frznPkgAccountsObj->getDFPRecQryNew($fromDate, $tillDate, $selProcessorId,$offset, $limit,$selRMLotId);
			$frnPkgRecsSize		=	sizeof($frnPkgRecs);
			$frznPkgAllRecs	=	$frznPkgAccountsObj->getAllRecordsNew($fromDate, $tillDate, $selProcessorId,$selRMLotId);
						...
						<td colspan="3">
							<table cellpadding="0" cellspacing="0" align="center">
								...
								<tr>
									...
									{ 
									?>
										<input type="button" value=" Print " name="btnPrint" class="button" onclick="return printWindow('PrintFrznPkgAccounts.php?selProcessor=<?=$selProcessorId?>&fromDate=<?=$fromDate?>&tillDate=<?=$tillDate?>&offset=<?=$offset?>&limit=<?=$limit?>&selRMLotId=<?=$selRMLotId?>',700,600);" <? if(sizeof($frnPkgRecs)==0) echo "disabled";?>>
									<? 
									}
									?>
									</td>
									...
									<td>
										<table>
											<tr>
												...
												<td>
													<select name="selProcessor" id="selProcessor" onchange="submitForm('dateFrom','dateTo', frmFrznPkgAccounts);">
												...
												<td>
													<select name="rmlot" id="rmlot" onchange="submitForm('dateFrom','dateTo', frmFrznPkgAccounts);">
														<option value="">--Select--</option>
														<? 
														foreach($rmlotRecords as $rmlots) {
															$rmlotIds	= $rmlots[0];
															$rmlotNames	= stripSlash($rmlots[1]);
															$selected = ($selRMLotId==$rmlotIds)?"selected":"";
														?>
														<option value="<?=$rmlotIds?>" <?=$selected?>><?=$rmlotNames?></option>
														<? }?>
													</select>
												</td>
							...
							?>
								...
								<tr>
									<td class="listing-item" nowrap style="padding-left:10px; padding-right:10px;"><?=$rmlotNm?>
										<input type="hidden" name="rmlotid_<?=$i?>" id="rmlotid_<?=$i?>" size="5" value="<?=$rmlotid?>" />
									</td>
