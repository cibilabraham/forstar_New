<?php
	...
	//echo "<br>H=".$manageChallanObj->checkAllowedEntry('90000', '2009-04-11', 1);
	//echo "hh==".$manageChallanObj->chkAllowedSOEntry('2009-04-20'); // Sales Order
	...
	if ($p["cmdAdd"]!="") {	
		...
		{
			//die();
			//if ($alpha_code_prefix!="") $chkUnique = $manageChallanObj->checkAlphaCodeExist($alpha_code_prefix, ""); KD ON 26-9-19
			
			//if ($functionType!="" && $idDateFrom!="" && $idDateTo!="" && $startNo && $endNo && !$chkUnique) { KD ON 26-9-19
			if ($functionType!="" && $idDateFrom!="" && $idDateTo!="" && $startNo && $endNo) {
				$idRcIns = $manageChallanObj->addIdGenRec($functionType, $billingCompany, $idDateFrom, $idDateTo, $startNo, $endNo, $dEntryLimitDays, $soInvoiceType, $exporter,$unitid,$alpha_code_prefix,$auto_Generate);
			}
			if ($idRcIns) 
			{
				$addMode = false;
	...
	if ( $p["cmdDelete"]!="") {
		...
		for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($recId!="") {
				...
				$idGenRecDel = $manageChallanObj->deleteIdGenRec($recId);
			}
		}
		if ($idGenRecDel) {
	...
	*/
	...
	if ($p["cmdUpdateRec"]!="") {		
		...
		$manageChallanRecUptd = $manageChallanObj->updateChallanRecord($soDEntryLimitDays, $challanDEntryLimitDays);
	...
	# Get All Records
	$idGenRecords	= $manageChallanObj->fetchAllPagingRecords($offset, $limit, $filterFunctionType,$filterUnitName,$filterCompanyName);
	$idGenRecordsSize = sizeof($idGenRecords);

	## -------------- Pagination Settings II -------------------
	$numrows	=  sizeof($manageChallanObj->fetchAllRecords($filterFunctionType,$filterUnitName,$filterCompanyName));
	$maxpage	=  ceil($numrows/$limit);
	## ----------------- Pagination Settings II End ------------
//Rekha uncommented rec
...
<form name="frmManageChallan" action="ManageChallan.php" method="post">
			  ...
			  <table cellpadding="0"  width="100%" cellspacing="0" border="0" align="center">
                ...
                <tr> 
                  ...
                  <td colspan="2" > <table cellpadding="0"  width="90%" cellspacing="0" border="0" align="center">
                      ...
                      <tr> 
                        <? if($editMode){?>
                        <td align="center"> <input type="submit" name="cmdDelCancel" class="button" value=" Cancel " onClick="return cancel('ManageChallan.php');"> 
                        ...
                        <?} else{?>
                        <td  colspan="2" align="center"> <input type="submit" name="cmdAddCancel" class="button" value=" Cancel " onClick="return cancel('ManageChallan.php');"> 
			...
			<table width="200">
				...
				<tr id="billingComapanyRow">
					...
					<div style='display: flex;'> 				
						...
						<!--<select name="billingCompany" id="billingCompany" onchange="xajax_getAlphacode('1','');">-->
						<option value="">-- Select --</option>
						<?php
						...
						//	$cName			= $bcr[1];
							...
							//$defaultChk		= $bcrValue;
							$selected = "";
							//if (($billingCompanyId==$billingCompany) || ($billingCompany=="" && ($billingCompanyId==$defaultCompany))  ) $selected = "selected"; KD ON 26-9-19
							if (($billingCompanyId==$billingCompany)) $selected = "selected"; //KD ON 26-9-19
						?>
						...
						<div class="listing-item" style='padding:8px;' id='lbl_compcode'></div> 
					</div>
					</td>
				</tr>
				...
				<tr id="unitRowInv" >
					...
					<div class="listing-item" style='padding:8px;' id='lbl_unitcode'></div> 
					</div>
				</td>
			</tr>
				...
				<tr id="alphaCode" style="display: none;">
					<TD class="fieldName" nowrap="true">*Alpha Code Prefix:</TD>
					<td>
						<input type="text" id="alpha_code_prefix_new" name="alpha_code_prefix" value="0"/>
						<br/><span id="divPOIdExistTxt" class="listing-item" style="line-height:normal; font-size:10px; color:red;"></span>
...
<tr id="unitRow" style="display:none;">
		...
		<td class="listing-item" colspan="2">
			<table>
					...
					<TD>
                      ...
                      <tr> 
                        <? if($editMode){?>
                        <td align="center"> <input type="submit" name="cmdCancel" class="button" value=" Cancel " onClick="return cancel('ManageChallan.php');"> 
                          &nbsp;&nbsp; <input type="submit" name="cmdSaveChange" class="button" value=" Save Changes " onClick="return validateManageRMChallan(document.frmManageChallan);">                        </td>
                        <?} else{?>
                        <td  colspan="2" align="center"> <input type="submit" name="cmdCancel" class="button" value=" Cancel " onClick="return cancel('ManageChallan.php');"> 
                          &nbsp;&nbsp; <input type="submit" name="cmdAdd" class="button" value=" Add " onClick="return validateManageRMChallan(document.frmManageChallan);"></td>
                        <?}?>
                      </tr>
                      <tr> 
    ...
    <tr> 
	  ...
	  <div style="float:right;padding:10px;"><a href='ManageChallan_all.php' class="link1"><strong>View All Manage Challan</strong></a></div>
          ...
          <tr> 
            <td   bgcolor="white"> 
              ...
              <table cellpadding="0"  width="100%" cellspacing="0" border="0" align="center">
                <tr> 
                  <td width="1" background="images/heading_bg.gif" class="page_hint"></td>
                  <td background="images/heading_bg.gif" class="pageName" >&nbsp;Manage Challan </td>
                  <td background="images/heading_bg.gif" class="pageName" >
			<table cellpadding="0" cellspacing="0" align="right">	
				<tr>
					<td nowrap="true" class="listing-item">Function Name:</td>
					<td nowrap="true">
						<!--<select name="filterFunctionType" id="filterFunctionType" onchange="filterLoad(this);">-->
                  ...
                  <td colspan="3"> <table cellpadding="0" cellspacing="0" align="center">
                      <tr> 
                        <td><? if($del==true){?><input type="submit" value=" Delete " class="button"  name="cmdDelete" onClick="return confirmDelete(this.form,'delId_',<?=$idGenRecordsSize;?>);" > <? }?>
                          ...
                          &nbsp;<? if($print==true){?><input type="button" value=" Print " name="btnPrint" class="button" onClick="return printWindow('PrintManageChallan.php?filterFunctionType=<?=$filterFunctionType?>',700,600);"><? }?></td>
                      </tr>
                    </table></td>
                </tr>
                <tr> 
...
<? if($maxpage>1){?>
		...
		for ($page=1; $page<=$maxpage; $page++) {
			if ($page==$pageNo) {
      				...
      				$nav.= " <a href=\"ManageChallan.php?pageNo=$page&filterFunctionType=$filterFunctionType&filterUnitName=$filterUnitName&filterCompanyName=$filterCompanyName\" class=\"link1\">$page</a> ";
				//echo $nav;
   			}
		}
		if ($pageNo > 1) {
   			$page  = $pageNo - 1;
   			$prev  = " <a href=\"ManageChallan.php?pageNo=$page&filterFunctionType=$filterFunctionType&filterUnitName=$filterUnitName&filterCompanyName=$filterCompanyName\"  class=\"link1\"><<</a> ";
	 	} else {
		...
		if ($pageNo < $maxpage) {
   			$page = $pageNo + 1;
   			$next = " <a href=\"ManageChallan.php?pageNo=$page&filterFunctionType=$filterFunctionType&filterUnitName=$filterUnitName&filterCompanyName=$filterCompanyName\"  class=\"link1\">>></a> ";
	 	} else {
	...
	<? }?>
			...
			<?php
				//}
			?>-->
			<!-- <td class="listing-item" nowrap style="padding-left:10px; padding-right:10px;"><?=$alpha_code_prefix?></td> KD ON 26-9-19 -->
             <td class="listing-item" nowrap style="padding-left:10px; padding-right:10px;"><?=$billingCompanyRec[8]?></td>  <!-- KD ON 26-9-19 -->
			...
			<? if($edit==true){?>
                        	<td class="listing-item" align="center" width="40"><input type="submit" value=" Edit " name="cmdEdit" onClick="assignValue(this.form,<?=$numberGenId;?>,'editId'); assignValue(this.form,'1','editSelectionChange');this.form.action='ManageChallan.php';"></td>
			<? }?>
                      </tr>
                      	<?php
				}
