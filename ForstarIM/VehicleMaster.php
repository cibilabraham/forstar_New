<?php
	...
	if ($p["cmdAdd"]!="") {
		...
		//$registrationNumber		=	addSlash(trim($p["registrationNumber"]));
		...
		//$lastId = "";
		$chkUnique	=	$vehicleMasterObj->checkDuplicate($vehicleNumber);
		
		if ($vehicleNumber!="" && $vehicleType!="") {
		
			$vehicleMasterRecIns	=	$vehicleMasterObj->addVehicleMaster($vehicleNumber, $vehicleType,$description,$userId);
		...
		#Find the Last inserted Id From m_vehicle_master Table
			...
			#***************---------------------------------chemical and equipment insertion code starts----------------------------------------****************#
		...
		#***************---------------------------------chemical and equipment insertion code ends----------------------------------------****************#
		...
		}else if ($chkUnique) $err = " Failed to vehicle master. Please make sure the request number you have entered is not duplicate. ";
	}
	...
	if ($p["editId"]!="" ) {
		...
		//$registrationNumber			=    $vehicleMasterRec[3];
		$description			=    $vehicleMasterRec[4];
		#***************---------------------------------chemical and equipment fetch code starts----------------------------------------****************#
		...
		#***************---------------------------------chemical and equipment fetch code ends----------------------------------------****************#
	}
	...
	if ($p["cmdSaveChange"]!="") {
		...
		if ($vehicleMasterId!="" && $vehicleNumber!="" && $vehicleType!="" && $description!="") {
			$vehicleMasterRecUptd = $vehicleMasterObj->updateVehicleMaster($vehicleMasterId, $vehicleNumber, $vehicleType,$description);
		...
		//if ($vehicleMasterId!="" && $vehicleNumber!="" && $vehicleType!="" ) {
			...
			#***************---------------------------------chemical and equipment update query starts----------------------------------------****************#
					# ----------------------------Vehicle equipment
					/*for ($i=0; $i<$equipmentTableRowCount; $i++) {
						...
						if ($status=='N' && $equipmentId!="") {
							...
							$delHarvestingEquipmentRec = $vehicleMasterObj->delHarvestingEquipmentRec($equipmentId);
					...
					# ----------------------------Vehicle chemical
						/*for ($i=0; $i<$chemicalTableRowCount; $i++) {
							...
							if ($status=='N' && $chemicalId!="") {
								...
								//$driverMasterInUse = $driverMasterObj->testMethodRecInUse($vehicleTypeId);
								//if (!$driverMasterInUse)
								$delHarvestingChemicalRec = $vehicleMasterObj->delHarvestingChemicalRec($chemicalId);
	...
	if ($p["cmdDelete"]!="") {
		...
		for ($i=1; $i<=$rowCount; $i++) {
			...
			if ($vehicleMasterId!="") {
				...
				$vehicleCheckDel        =   $vehicleMasterObj->checkVehicleNumber($vehicleMasterId);
				
				//DIE();
				if(sizeof($vehicleCheckDel)>0)
				{
	...
	<td align="center">
		...
		<tr>
			<td colspan="2"  height="10" ></td>
											...
											<tr>
												<?php/*<td class="fieldName" nowrap >*Registration Number</td>
												<td><INPUT TYPE="text" NAME="registrationNumber" size="15" value="<?=$registrationNumber;?>"></td>
											</tr>*/?>
...
<? if($maxpage>1){?>
	...
	foreach($vehicleMasterRecords as $cr) {
		...
		//$registrationNumber       =	stripSlash($cr[3]);
		 ...
		 $active=$cr[5];
		$existingrecords=$cr[8];
		$procurementnumber=$cr[7];
	?>
	<tr <?php if ($active==0){?> bgcolor="#afddf8"  onMouseOver="ShowTip('<?=$disMsgInactive?>');" onMouseOut="UnTip();" <?php }?>>
		<td width="20"><input type="checkbox" name="delId_<?=$i;?>" id="delId_<?=$i;?>" value="<?=$vehicleMasterId;?>" ></td>
		<td class="listing-item" nowrap="nowrap" style="padding-left:10px; padding-right:10px;"><?=$vehicleNumber;?>&nbsp;
		<?php
		$vehicleSchedule	=	$vehicleMasterObj->getschedulehistory($vehicleMasterId);
		if(sizeof($vehicleSchedule)>0) { ?>
											<?php
												...
												$detailsvalue='<table width=100% border=1 cellspacing=0 cellpadding=2><tr bgcolor=#D9F3FF ><th  class=listing-head colspan=2>SCHEDULE DETAILS </th></tr><tr bgcolor=#D9F3FF ><th  class=listing-head>Procurement ID</th><th  class=listing-head>Schedule date</th></tr>';
												 ...
												 foreach($vehicleSchedule as $vehicleHistory )
												 {
												 $procurementId= $vehicleHistory[0];
													$schedule_date= dateformat($vehicleHistory[1]);
		...
		<td class="listing-item" width="60" align="center">
			<?php if ($active!=1){?><input type="submit" value=" Edit " name="cmdEdit" onClick="assignValue(this.form,<?=$vehicleMasterId;?>,'editId'); this.form.action='VehicleMaster.php';"  ><?php } ?>
		</td>
		<? }?>
