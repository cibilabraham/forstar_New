<?php
	...
	if ($p["cmdAddUser"]!="") {
		...
		if ($addUserName!="" && $addUserPassword!="") 
		{
			...
			for($i=0; $i<$hidUnitRowCount; $i++)
			{
				...
				if($Status!='N')
				{
					...
					$role		= $p["role_".$i];
					$userRecInsDetail	=	$manageusersObj->addUserDetail($lastId,$company,$unit,$department,$role);	
				}
			}
	...
	if ($p["cmdSaveChange"]!="") {
		...
		if ($upUserName!="" && $upUserPassword!=""  && $userEditId!="") {
			...
			for($i=0; $i<$hidUnitRowCount; $i++)
			{
				...
				if($status!='N')
				{
					...
					if($editUserDetail!='')
					{
						$userRecInsDetail	=	$manageusersObj->updateUserDetail($userEditId,$company,$unit,$department,$role,$editUserDetail);	
					}
					else if($editUserDetail=="")
					{
						$userRecInsDetail	=	$manageusersObj->addUserDetail($userEditId,$company,$unit,$department,$role);	
					}
				}
											...
											<tr>
												<td  height="10" colspan="2" class="fieldName" >
													<table width="200">
														...
														-->
														<tr>
															<td class="fieldName" nowrap>Company Name For Default View</td>
													...
													<table width="20%" cellspacing="1" bgcolor="#999999" cellpadding="3" id="tblUser" name="tblUser">
														...
														<tr id="Row_<?=$p?>" class="whiteRow" align="center">
															<td id="srNo_<?=$p?>" class="listing-item" align="center">
																<select id="company_<?=$p?>" onchange="xajax_getAllUnits(document.getElementById('company_<?=$p?>').value,'<?=$p?>')" name="company_<?=$p?>">
															...
															<td>
																<select id="unit_<?=$p?>"  name="unit_<?=$p?>">
																	<option value="0">--Select All--</option>
																	<?php
															...
															<td>
																<select id="department_<?=$p?>" name="department_<?=$p?>">
																	<option value="0">--Select All--</option>
																	<?php
															...
															<td>
																<select id="role_<?=$p?>"  name="role_<?=$p?>">
																	<option value="0">--Select All--</option>
																	<?php
																	if (sizeof($roleRecords)>0) {	
																		foreach($roleRecords as $rr) {
																			$roleIds		= $rr[0];
																			$roleName	= stripSlash($rr[1]);
																			$sel  = ($roleId==$roleIds)?"Selected":"";
																	?>	
																	<option value="<?=$roleIds?>" <?=$sel?>><?=$roleName?></option>
																	<?
																	} }
																	?>
																</select>
							...
							<td colspan="2" > 
								...
								?>
									...
									foreach($userRecords as $ur)
									{
										...
										$roles =	$manageusersObj->findDetails($ur[0]);
										
										$roleRec =	$manageroleObj->find($roles[5]);
									...
									<tr  bgcolor="WHITE" > 
										...
										<td class="listing-item" nowrap>&nbsp;<a class="link1" title="Click here to view details" href="javascript:printWindow('ViewUserDetails.php?userId=<?=$userId?>',700,600)">View Details</a></td>
