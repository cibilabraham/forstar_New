			...
			$selectDate		=	mysqlDateFormat($p["selDate"]);
						...
						if($hidrmLotID=='0')
						{
								...
								for ($j=1; $j<=$gradeRowCount; $j++) {
											...
											$bal1=$numMC1;
											//if ($numMC<$numMCG){
												/*if ($frozenCodeId!=$refrozenCodeId)
												{	echo $numMCG;
													echo "Total Slabs-$totalSlabsi<br>";
													echo "FROMIntialWt--$iwet<br>---intial-$filledWt<br>";
													echo "tOFinalWt--$fwet--<br>-Final-$ffilledWt<br>";
													echo "from numpacks---$numPacksRp<br>";
													echo "to numpacks----$numPacks<br>";
													$totMCb=floor($iwet/$fwet);
													echo "--Total MasterCart-$totMCb&&<br>";
													$totMCb=floor($totMCb*$numPacksRp/$numPacks);
													echo "--After conversion bal $totMCb";
													//$bal=$totMCb;
													//$totMCCon=Math.floor($totMCb*$numPacksRp/$numPacks);
													//$totLSb=($totMCb*$numPacksRp)%$numPacks;
													$totLSb=$iwet%$fwet;
													$bal=$totMCb;
													echo "LSB---$totLSb";
													//echo "Bal".$bal."pp";
											...
											//if ($numLS<$numLSG){
												//if ($frozenCodeId!=$refrozenCodeId)
												//{
													//$totLSb=Math.floor($iwet)%Math.floor($fwet);
													//$balLs=$totLSb;
												//}
												//else{
											//$balLs=$numLSG-$numLS;
											//$balLs=($balLs*$numPacksRp)/$numPacks;
											//}
											//}
														...
														$exisitingRecords = $dailyfrozenrepackingObj->checkRecordsExist($fish_id,$processId,$freezingStageId,$frozenCodeId,$MCPkgId,$fromDate,$tillDate,$gradeId);
															...
															foreach ($exisitingRecords as $er){
																###commented on 13-11-2014
																		...
																		$allocGrdTotal=0;
																		$allocGrdLsTotal=0;							echo $allocGrdLsTotal.','.$balLs.'<br/>';					
																		$gradeUpid=$er[2];
																		$mainId=$er[0];
																		$allocGrdTotal=$allocGrdTotal+$bal;
																		$allocGrdLsTotal=$allocGrdLsTotal+$balLs;
													...
													if ($thaallocTotal!=0)
												{
														...
														$exisitingRecords = $dailyfrozenrepackingObj->checkRecordsExistRMLotID($fish_id,$processId,$freezingStageId,$frozenCodeId,$MCPkgId,$fromDate,$tillDate,$gradeId,$hidrmLotID);
															...
															foreach ($exisitingRecords as $er){
																	...
																	if ($n==1){
																		//$dailyFrozenPackingRecUp=$dailyfrozenrepackingObj->updateDailyFrozenPackingGradeRMLot($gradeUpid,$thaallocTotal,$thaallocLsTotal,$repkdQty);
...
#Reglaze
	...
	<?php
		...
		$editMode		= true;
		/* Hide on 22 MAR 13
		$dailyFrozenPackingRec	= $frozenStockAllocationObj->find($editId, $frozenPackingEntryId);		
		$mainId		=	$dailyFrozenPackingRec[0];		
		$entryId 	= 	$dailyFrozenPackingRec[4];
		$selDate	=	dateFormat($dailyFrozenPackingRec[1]);
		$unit		=	$dailyFrozenPackingRec[2];
		if ($dailyFrozenPackingRec[3]!=0) $lotId = $dailyFrozenPackingRec[3];
		$editProcessorId		=	$dailyFrozenPackingRec[13];
		
		if ($p["editSelectionChange"]=='1'|| $p["unit"]=="") $unit = $dailyFrozenPackingRec[2];
		else $unit = $p["unit"];

		if ($p["editSelectionChange"]=='1'|| $p["fish"]=="") $fishId = $dailyFrozenPackingRec[5];
		else $fishId		=	$p["fish"];
		
		if ($p["editSelectionChange"]=='1'|| $p["processCode"]=="") $processId	=	$dailyFrozenPackingRec[6];
		else $processId	=	$p["processCode"];
		
		$freezingStage		=	$dailyFrozenPackingRec[7];
		$eUCode			=	$dailyFrozenPackingRec[8];
		$selBrd		  = $dailyFrozenPackingRec[9];
		$selBrdFrom	  = $dailyFrozenPackingRec[15]; // Company
		$brand	  = $selBrd."_".$selBrdFrom;

		$frozenCode		=	$dailyFrozenPackingRec[10];
		$mCPacking		=	$dailyFrozenPackingRec[11];
		$exportLotId		=	$dailyFrozenPackingRec[12];
		$selQuality		=	$dailyFrozenPackingRec[14];
		$customer		= 	$dailyFrozenPackingRec[16];
		# Get PC Recs
		$processCodeRecords	=	$processcodeObj->processCodeRecFilter($fishId);			
		#FrozenLot Id records for the selected date
		$frozenLotIdRecords = $frozenStockAllocationObj->fetchLotIdRecords($dailyFrozenPackingRec[1]);
		*/
		...
		if ($allocateMode) {
			/*
			$fpMCPkgRecs	= $frozenStockAllocationObj->getFPMCPkg($processId, $freezingStage, $frozenCode);
			...
			*/
			...
			$purchaseOrders = $purchaseorderObj->getPendingOrders($processId, $freezingStage, $frozenCode, $stkAllocateMCPkgId);
	...
	if ($p["cmdSaveChange_1"]!="") {
		...
		/* Comented on Dec 11 09 
		if ($mainId!="" && $entryId!="") {		
			$updateFrozenPackingEntryRec =	$frozenStockAllocationObj->updateDailyFrozenPackingEntry($fishId, $processCode, $freezingStage, $eUCode, $brand, $frozenCode, $mCPacking, $exportLotId, $entryId, $lotId, $selQuality, $allocateMode, $brandFrom, $customer);
			if (!$allocateMode) {
				$updatePackingMainRec =	$frozenStockAllocationObj->updatePackingMainRec($selectDate, $unit, $processorId, $mainId);	
			}			
		}
		*/
		...
		$dailyFrozenPackingRecUptd	=	false;
			...
			$selectDate		=	mysqlDateFormat($p["selDate"]);
						...
						if($hidrmLotID=='0')
						{
							$insertDailyThawing = $frozenStockAllocationObj->insertDailyThawing($selectDate,$processId,$freezingStageId,$frozenCodeId,$MCPkgId,$fishIdth);
							if ($insertDailyThawing)
							$dfpPOEntryId = $databaseConnect->getLastInsertedId();
						}
						else
						{
							$insertDailyThawing = $frozenStockAllocationObj->insertDailyThawingRMLot($selectDate,$processId,$freezingStageId,$frozenCodeId,$MCPkgId,$fishIdth,$hidrmLotID);
							if ($insertDailyThawing)
							$dfpPOEntryId = $databaseConnect->getLastInsertedId();
						}
						for ($j=1; $j<=$gradeRowCount; $j++) {
			   ...
			   if ($status!='N')
			   {
						...
						if ($POId>0 ) 
						{
							if($hidrmLotID=='0')
							{
								if ($POEntryId>0  ) {
									// Update record
									$updatePORec = $frozenStockAllocationObj->updateDFPPORecs($dFrznPkgEntryId, $POId, $totalSlabs, $totalQty, $POEntryId);
									$dfpPOEntryId = $POEntryId;
								} 
								else {
									$insertDFPPORecs =  $frozenStockAllocationObj->insertDFPPORecs($dFrznPkgEntryId, $POId, $totalSlabs, $totalQty, $processId, $freezingStageId, $frozenCodeId, $MCPkgId, $userId);
									// Last Entry Id
							...
							elseif($hidrmLotID!='0' || $hidrmLotID!='')
							{
								if ($POEntryId>0  ) {
									// Update record
									$updatePORec = $frozenStockAllocationObj->updateDFPPORMLotRecs($dFrznPkgEntryId, $POId, $totalSlabs, $totalQty, $POEntryId,$hidrmLotID);
									$dfpPOEntryId = $POEntryId;
								} 
								else {
									$insertDFPPORecs =  $frozenStockAllocationObj->insertDFPPORMLotRecs($dFrznPkgEntryId, $POId, $totalSlabs, $totalQty, $processId, $freezingStageId, $frozenCodeId, $MCPkgId, $userId,$hidrmLotID);
									// Last Entry Id
									if ($insertDFPPORecs) $dfpPOEntryId = $databaseConnect->getLastInsertedId();
								}
							}
				...
				if ($convert)
	{
		...
		//{
			//$flag1=1;		
				...
				<td>
				<table align="center">
				 <tr width=100%><td colspan=5 >&nbsp;<span style="color:red">--><?php //if ($norepack){
					
					//echo $flag1;
					//echo "It cannot be repacked.";//}?><!--Sorry Its already reglazed.But You can Repack</span></td>
				  </tr>
				</table>
				</td>
				</tr>-->
				...
				<?php /*if (trim($statusRpk)!="-1")
		{
				...
				<td>
				<table align="center">
				 <tr width=100%><td colspan=5 >&nbsp;<span style="color:red">Sorry Its already repacked</span></td>
				  </tr>
				</table>
				</td>
				</tr>-->
				...
				<?php if ($flag!=1)
		{
				...
				<td>
				<table align="center">
				 <tr width=100%><td colspan=5>&nbsp;</td><td>Reglazing <input type="radio" id="rpkrgz" name="optionrpkrgz"  value=3 onclick="return checkboxSel();" <?php if (($flag1==1) || ($block=="true")){?> disabled=true; <?php }?> />Repacking<input type="radio" id="rpkrgz" name="optionrpkrgz" onclick="return checkboxSel();" value=2 /></td></tr>
				  <tr width=100%><td colspan=5>&nbsp;&nbsp</td><td>&nbsp;&nbsp<input type=Submit value=Go name="cmdGo" class="button" onclick="return go(document.frmDailyFrozenPacking);"/>
				 <input type='hidden' name="hidoptrmLotID" id='hidoptrmLotID' value="<?=$hidrmLotID?>"/>
				  <input type=Submit name="cmdCancel" class="button" value=" Cancel " onClick=" this.form.action='FrozenStockAllocation.php?all=-1';">&nbsp;&nbsp;</td>
				  </tr>
				</table>
				</td>
				</tr>
...
<input type="hidden" name="cmdAddNew" value="1">
	...
	<tr><TD colspan="2">
		<table>
		<tr bgcolor="White"><TD class="listing-head" colspan="2">MC Packing = &nbsp;
...
<td nowrap style="padding-left:2px;padding-right:2px; <?=$leftBdr.$rightBdr.$bottomBdr?>" class="listing-head"  colspan="2" >
			...
			<tr bgcolor="White" id="allocateRow_<?=$i;?>" class="tr_clone">				
				...
				<?php /*foreach($purchaseOrders as $por) {
							$poId	= $por[0];								
							$poDate = $por[3];
					...
					list($availabledisMC, $availabledisLS) = $frozenStockAllocationObj->getAvailablePacksDis($processId, $freezingStage, $frozenCode, $sGradeId, $stkAllocateMCPkgId, $tillDate,$poId);
					$totAvailabledisMC += $availabledisMC;*/
					//echo "iii$j-----".$totAvailabledisMC."<br>";
					//if ($totAvailabledisMC!=0)
						//{
						//echo $poId;

						//if (!in_array($poId, $displayPoId)) {
					...
					?>
<!--<select name="POId_<?=$i?>11" id="POId_<?=$i?>11" style="width:120px;" >
					<option value="">--Select--</option>-->
					<?php
					//for($i=0;$i<count($displayPoId);$i++)
					//{?>
					<!--<option value="<?=$displayPoId[$i]?>" <?=$selected?>><?=$displayDate[$i]?></option>-->
					<?php // }?>
					<!--</select>-->
			...
			<tr bgcolor="White" id="allocateRow_<?=$i;?>" class="tr_clone">				
				...
				<table cellpadding="1" cellspacing="0" width="100%">
					<tr>
						...
						<!--<select name="mcPackingId_<?=$i?>" id="mcPackingId_<?=$i?>" onchange="xajax_assignMCPackChg(document.getElementById('mcPackingId_<?=$i?>').value,'<?=$i?>');"  >
                    <option value="0">--Select--</option>
                    <?php
						  foreach($mcpackingRecords as $mcp) {
							$mcPkgId		= $mcp[0];
...
<td nowrap style="padding-left:2px;padding-right:2px; <?=$leftBdr.$bottomBdr.$rightBdr ?>" class="listing-head"  colspan="1" >
				...
				<!--<td nowrap style="padding-left:2px;padding-right:2px; <?=$leftBdr.$rightBdr.$bottomBdr?>" class="listing-head"   >
				SET GLAZE %&nbsp;</td>-->
...
<!--<input type="text" name="selFrozenCode_<?=$k?>" value="<?=$dis[2];?>"  readonly  style="border:none;text-align:center" />-->
...
<!--<select id="selFrozenCode_<?=$k?>" name="selFrozenCode_<?=$k?>" onchange=" xajax_assignFrzPackChg(this.value);xajax_getFilledWt(document.getElementById('selFrozenCode_<?=$k?>').value, '<?=$k?>');" >
												<?php		
												if (sizeof($frozenCodeValues)>0) {	
													?>
													<option value="<?=$frozenCode?>" ><?=stripslashes($selFrozenCode1)?></option>
...
<!--<td nowrap style="padding-left:2px;padding-right:2px;<?=$rightBdr.$bottomBdr?>" >
...
<input name="cmdSaveAndAddNew" type="submit" class="button" id="cmdSaveAndAddNew" style="width:130px;" onclick="return validateAddDailyFrozenPacking(document.frmDailyFrozenPacking);" value="save &amp; Add New">
				...
				<table cellpadding="0" cellspacing="0" align="center">
					<tr>
						...
						<input type="submit" value=" Thawing " name="cmdThawe" onClick=" this.form.action='FrozenStockAllocation.php?all=1';return checkboxSel();" class="button">&nbsp;
												...
												<!--<input type="submit" value=" Repacking " name="cmdRepack" class="button" onClick=" this.form.action='FrozenStockAllocation.php?all=2';return checkboxSel();" >
											<input type="submit" value=" Reglazing  " name="cmdReglaze" class="button" onClick="this.form.action='FrozenStockAllocation.php?all=3';return checkboxSel();">-->
						<!--
						<? if($del==true){?>
						...
						<? if($print==true){?><input type="button" value=" Print " name="btnPrint" class="button" onClick="return printWindow('PrintFrozenStockAllocation.php?frozenPackingFrom=<?=$dateFrom?>&frozenPackingTill=<?=$dateTill?>&filterProcessCode=$filterProcessCode',700,600);"><? }?>
						-->	
						</td>
					</tr>
				</table>		
	...
	<input type=hidden name="hidstDate" id="hidstDate" value=<?=$dateFrom; ?> />
		<!--<INPUT type='checkbox' name='CheckAll' id='CheckAll' onClick="checkAll(this.form,'delGId_');" class="chkBox">-->
	</td>
...
<!--	<td class="listing-item" align="center" style="padding-left:2px;padding-right:2px;" nowrap>		
		<input type="submit" value=" Allocate " name="cmdAllocate" onClick="assignValue(this.form,<?=$dailyFrozenPackingMainId;?>,'editId'); assignValue(this.form,'1','editSelectionChange'); assignValue(this.form,'<?=$dailyFrozenPackingEntryId?>','editFrozenPackingEntryId'); assignValue(this.form,'<?=$dailyFrozenPackingMainId;?>','allocateId');  assignValue(this.form,'<?=$selEditCriteria?>','editCriteria'); this.form.action='FrozenStockAllocation.php?all=0';" <?=$allocateDisabled?>>
	</td>

	<td class="listing-item" align="center" style="padding-left:2px;padding-right:2px;" nowrap>		
		<input type="submit" value=" Thawing " name="cmdThawe" onClick="assignValue(this.form,<?=$dailyFrozenPackingMainId;?>,'editId'); assignValue(this.form,'1','editSelectionChange'); assignValue(this.form,'<?=$dailyFrozenPackingEntryId?>','editFrozenPackingEntryId'); assignValue(this.form,'<?=$dailyFrozenPackingMainId;?>','allocateId');  assignValue(this.form,'<?=$selEditCriteria?>','editCriteria'); this.form.action='FrozenStockAllocation.php?all=1';"  <?=$allocateDisabled?>></td>-->
	<?php
		 //}
	?>
	</tr>	
