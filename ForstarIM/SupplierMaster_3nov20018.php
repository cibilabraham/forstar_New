<?php
	...
	$selection 	="?pageNo=".$p["pageNo"]."&sectionFilter=".$p["sectionFilter"]."&srchName=".$p["srchName"];;
...
$combodisplay=3;
	...
	$supplierRecords	= $supplierMasterObj->fetchAllPagingRecords($offset, $limit, $sectionFilter,$srchName);
...
{
$supplierRecords	= $supplierMasterObj->fetchAllRecords($sectionFilter,$srchName);
...
//echo "$defaultFRN-$combodisplay";
	...
	$numrows	=  sizeof($supplierMasterObj->fetchAllRecords($sectionFilter,$srchName));
...
<? if($maxpage>1){?>
		...
		for ($page=1; $page<=$maxpage; $page++) {
			if ($page==$pageNo) {
      				...
      				$nav.= " <a href=\"SupplierMaster.php?pageNo=$page&sectionFilter=$sectionFilter&srchName=$srchName\" class=\"link1\">$page</a> ";
				//echo $nav;
   			}
		}
		if ($pageNo > 1) {
   			$page  = $pageNo - 1;
   			$prev  = " <a href=\"SupplierMaster.php?pageNo=$page&sectionFilter=$sectionFilter&srchName=$srchName\"  class=\"link1\"><<</a> ";
	 	} else {
		...
		if ($pageNo < $maxpage) {
   			$page = $pageNo + 1;
   			$next = " <a href=\"SupplierMaster.php?pageNo=$page&sectionFilter=$sectionFilter&srchName=$srchName\"  class=\"link1\">>></a> ";
	 	} else {
