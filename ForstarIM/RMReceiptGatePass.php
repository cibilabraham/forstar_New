<?php
		...
		{	
			...
			if($supplierSize>0 )
			{
				for($i=0; $i<$supplierSize; $i++) 
				{
					...
					$landing_center			=$p["landing_center_".$i];
					...
					$unit			=$p["unit_".$i];
						...
						{
							$supplierlist = $rmReceiptGatePassObj->addReceiptGatepassSupplier($lastId,$supplier_id,$pond_id,$challan_no,$challan_date,$Company_Name,$unit,$landing_center);
						}  # If 										
					...
					$chemicalSize=	$p['chemicalSize'];
						if ($supplierSize>0 )
						{
							for ($i=0; $i<$supplierSize; $i++)
							{
								...
								{
									$supplierlist = $rmReceiptGatePassObj->updateReceiptGatepassSupplier($receipt_id,$supplier_id,$pond_id,$challan_no,$challan_date,$Company_Name,$unit,$landing_center);
								}  # If 										
							} # Status check ends here
						}
	...
	if(isset($p['editId']))
	{
		...
		if(sizeof($editDetails) > 0)
		{
			...
			$selsupplier   = $editDetails[0]['supplier_id'];
			$material   = $editDetails[0]['material_type_id'];
			$selPondId=$editDetails[0]['pond_id'];
			$selLandId=$editDetails[0]['landingcenter_id'];
			$pondRecs = $rmReceiptGatePassObj->getfilterPondList($selsupplier);
			$landRecs = $rmReceiptGatePassObj->getLandingCenterSupplier($selsupplier);
		}
									...
									<td bgcolor="white">
										...
										<table width="100%" cellspacing="0" cellpadding="0" border="0" align="center">
														...
														<td colspan="2" >
																	...
																	<tr>
																		...
																		<td class="listing-item">
																			<input name="receiptGatePass" type="text" id="receiptGatePass" size="10" value="<?=$receiptGatePass;?>" tabindex="1"
																			<?php if($editMode){ ?> readonly="readonly" <? } else { ?> onkeyUp="checkReceiptGatePass(this.value);" <? } ?> tabindex="1" > <!--onchange="xajax_checkValidReceiptNumber(document.getElementById('receiptGatePass').value);"-->
																			<input type="hidden" id="selCompanyName" name="selCompanyName" value="<?=$selCompanyName?>">
																			<input type="hidden" id="unitId" name="unitId" value="<?=$unitId?>">
																			<input type="hidden" id="number_gen_id" name="number_gen_id" value="<?=$number_gen_id?>">
																		</td>
																		...
																		<table width="60%" align="left">
																			<tbody>
																				...
																				<tr>
																					...
																					<td height="10">
																						<input type="text" size="15" value="<?php echo $supplier_Challan_No;?>" id="supplier_Challan_No" name="supplier_Challan_No" onkeyup="chkChallanStat();" required>
																					</td>
																				</tr>
																				<tr>
																				...
																				<tr>				
																					...
																					<td height="5">
																						<select id="supplier" name="supplier" required onchange="xajax_getCenter(document.getElementById('supplier').value,document.getElementById('material').value); ">
																							<option value="">--select--</option>
											  											    <?php
																								if(sizeof($suppliers) > 0)
																								{
																									...
																									{
																										$sel = '';
																										if($selsupplier == $supplier[0]) $sel = 'selected';
																				...
																				<tr id="1" class="rawtype" style="display:none">	
																					...
																					<td height="5">
																						<select id="pond" name="pond" >
																							...
																							<? } ?>
																							<?php 	
																							foreach($pondRecs as $pondId => $pondName)
																							{
																								$sel  = ($selPondId==$pondId)?"Selected":"";
																							?>
																							<option value="<?=$pondId?>" <?=$sel?>><?=$pondName?></option>
																							<?
																							}
																							?>
																						</select>
																				...
																				<tr id="2" class="rawtype" style="display:none">				
																					...
																					<td height="5">
																						<select id="landingCenter" name="landingCenter" >
																							<? if($addMode){?><option value="">--select--</option><? } ?>
																							<?php 	
																							foreach($landRecs as $landId => $landName)
																							{
																								$selt  = ($selLandId==$landId)?"Selected":"";
																							?>
																							<option value="<?=$landId?>" <?=$selt?>><?=$landName?></option>";
																							<?
																							}
																							?>
																						</select>										      
											...
											<tr>
												...
												<input type="submit" value="<?php echo $buttonValue;?>" class="button" name="<?php echo $buttonName;?>" <? if($addMode){ ?>id="cmdAdd1" <? } ?> >
												</td>
											</tr>
	...
	<input type="hidden" name="addMode" id="addMode" value="<?php echo $addMode;?>" />
	<? if ($addMode!="") {?>
