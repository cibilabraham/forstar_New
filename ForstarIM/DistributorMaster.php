<?php
			...
			if ($distributorRecIns) {
				...
				for ($i=0; $i<$rowCount; $i++) {
					...
					if ($status!='N') {
						...
						$gistinNo		= addSlash(trim($p["gistinNo_".$i]));
	...
	if ($p["editId"]!="") {
		...
		/* rekha modify the code dated on 12 sept 2018 */
			$gistinNo = $getDistStateRecords[0];
		/* end code */
...
for ($i=1; $i<=$rowCount; $i++) {
	...
	$billFormArr 	= array("VAT"=>array("VN"=>"Normal", "ZP"=>"Zero Percent"), "CST"=>array(""=>"-- Select --", "FF"=>"Form F", "FC"=>"Form C", "FN"=>"Normal", "ZP"=>"Zero Percent"), "GST"=>array(""=>"-- Select --", "FF"=>"Form F", "FC"=>"Form C", "FN"=>"Normal", "ZP"=>"Zero Percent"), "IGST"=>array(""=>"-- Select --", "FF"=>"Form F", "FC"=>"Form C", "FN"=>"Normal", "ZP"=>"Zero Percent"));
...
<tr align="center" class="whiteRow" id="row_<?=$j?>">
	...
	<td nowrap="" align="center" class="listing-item">
		<table cellpadding='0' celspacing='0'>
			...
			<tr>
				<td nowrap="true" class="listing-head">*VAT/CST/GST/IGST/</td>
				<td align="left">
					<select onchange="displayBilling('<?=$j?>');" id="selTaxType_<?=$j?>" name="selTaxType_<?=$j?>">
						...
						<option value='GST' <?=($selTaxType=="GST")?"selected":"";?>>GST</option>
						<option value='IGST' <?=($selTaxType=="IGST")?"selected":"";?>>IGST</option>
					</select>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td nowrap="true" class="listing-head">GSTIN No</td>
				<td align="center">
					<input type="text" size="12" value="<?=$gistinNo?>" id="gistinNo_<?=$j?>" name="gistinNo_<?=$j?>" />
				</td>
			</tr>			
		</table>
				</fieldset>
