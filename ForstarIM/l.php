...
<form name="frmEntryGrossWt" action="catchentrygrosswt.php" method="post">
<table width="100%" border="0" cellpadding="0" cellspacing="0" height="100%">
  ...
  <!--tr>
	...
	<td>
	<input type="submit" name="cmdAddDaily" value="Add"  class="button" onclick=" return validateGrossEntry(document.frmEntryGrossWt);">
	</td>
  </tr-->
  <tr>
...
//				echo "nextControl=$nextControl<br>";
      ...
      <tr>
        ...
        <td class="listing-item" nowrap><input type="checkbox" name="delId_<?=$i;?>" id="delId_<?=$i;?>" value="<?=$hidId;?>" ><?=(($pageno-1)*15)+$i?></td>
        <td class="listing-item"><input type="hidden" name="grossId_<?=$i;?>" value="<?=$hidId?>" /><input type="text" name="grossWt_<?=$i;?>" id="grossWt_<?=$i;?>" value="<?=$gwt?>" size="3" style="text-align:right" tabindex="<?=$i;?>" onkeypress="return focusNext(event,'document.frmEntryGrossWt','<?=$nextControl?>');"/></td>
        <td class="listing-item"><input type="text" name="grossBasketWt_<?=$i;?>" id="grossBasketWt_<?=$i;?>" value="<?=$basketWt; ?>" size="2" style="text-align:right; width:25%;" /></td>
      </tr>
      <? 
	...
	<? }?>
	  ...
	  <input type="hidden" name="showAllValue" value="0" />
	  </td>
	  </tr>
	  <tr>
	    <td colspan="4" align="center" nowrap><input type="submit" value=" Delete " name="cmdDelete" class="button" onClick="return confirmDelete(this.form,'delId_',<?=$grossRecSize;?>);">&nbsp;<input type="submit" name="cmdSaveChange" class="button" value=" Save Changes " onclick="assignValue(this.form,'1','countSaved'); return validateGrossEntry(document.frmEntryGrossWt);" /><? if($p["cmdShowAll"]==""){ ?><input name="cmdShowAll" type="submit" class="button" id="cmdShowAll" value="Show All" onclick="return assignValue(this.form,'1','showAllValue');"/><? }?></td>
	    </tr>
	  <tr>
...
<? }?>
 ...
 var curr_basketWt	=	 parent.document.frmDailyCatch.dailyBasketWt.value;
	...
	/*var bQty	=	"grossBasketWt_";
	for (i=1; i<=15; i++)
	  {
	  	 document.frmEntryGrossWt.grossBasketWt_+i.value =  parent.document.frmDailyCatch.dailyBasketWt.value;
	  }*/
