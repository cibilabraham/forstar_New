<?php
	...
	if ($p["cmdAddProcessorsAccounts"]!="") {
		...
		for ($i=1; $i<=$rowCount; $i++) {		
			...
			if ($preProcessorQtyId!="" && $selectRate!="" && $actualAmount!="") {
	...
	if ($p["cmdSearch"]!="" || $updated==true || $pageNo!="" && ($processor!="" || $selProcessCode!="") ) {
		...
		# Fetch All Records
		$getAllRecords	= $processorsaccountsObj->getAllPreProcessorRecords($fromDate, $tillDate, $processor, $selProcessCode);	
...
?>
              ...
              <tr>
                <td   bgcolor="white"><!-- Form fields start -->
                    <table cellpadding="0"  width="100%" cellspacing="0" border="0" align="center">
                      ...
                      <tr>
                        <td width="1" ></td>
                        <td colspan="2"  align="center"><table cellpadding="0"  width="99%" cellspacing="0" border="0" align="center">
                            <tr>
                            ...
                            <tr>
                              ...
                              <td align="center" colspan="2">&nbsp;&nbsp;<? if($edit==true){?>
                                ...
                                &nbsp;&nbsp;<? if($print==true){?>
                                <input type="button" name="Submit" value="Print" class="button" onclick="return printWindow('PrintProcessorsAccounts.php?selProcessor=<?=$processor?>&fromDate=<?=$fromDate?>&tillDate=<?=$tillDate?>&offset=<?=$offset?>&limit=<?=$limit?>',700,600);" <? if(sizeof($dailyPreProcessRecords)==0) echo "disabled";?>><? }?></td>
                              <? } ?>
                            </tr>
				...
				<table width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#999999">
								...
								if( sizeof($dailyPreProcessRecords)){
								 ...
								 ?>
                                  <tr bgcolor="#f2f2f2">
                                    <td colspan="12" bgcolor="#FFFFFF" class="listing-item" align="center">
									<table width="100%">
									<tr>
										<td align="right">
										<table>
									...
									<img src="images/x.gif" width="20" height="20"> - The entire Process Rates are not defined.									</td></tr></table>									</td>
									<td align="right">
									<table>
									<tr><td class="listing-item"><a href="ProcessorSettlementSummary.php?supplyFrom=<?=$dateFrom?>&supplyTill=<?=$dateTo?>&selProcessor=<?=$processor?>" class="link1">View Settlement Summary</a>
									</td></tr></table></td>
									</tr>
									<? if($maxpage>1){?>
									<tr>
	...
	if ($pageNo < $maxpage)
	  ...
	  ?>
								  ...
								  <? if($err!="" ){?>
                                  <tr bgcolor="#f2f2f2">
                                    <td colspan="12" class="err1" align="center"><?=$err;?></td>
                                  </tr><? }?>
				...
				<input type="hidden" name="defaultYieldTolerance" id="defaultYieldTolerance" value="<?=$defaultYieldTolerance?>">
                                  <tr bgcolor="white">
                                    <td height="10" colspan="5" align="right" class="listing-head">Total:</td>
				    ...
				    <td height="10" colspan="3" align="right" class="listing-head"></td>
                                  ...
                                  <tr bgcolor="white">
                                    <td height="10" colspan="9" align="right" class="listing-head">Grand Total: </td>
