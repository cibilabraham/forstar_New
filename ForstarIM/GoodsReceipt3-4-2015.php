<?
	...
	if ($p["cmdAdd"]!="" ) {
		...
		if ($selPoId!="" && $storeEntry!="") {
			$goodsReceiptRecIns = $goodsreceiptObj->addGoodsReceipt($selPoId, $selDepartment, $challanNo, $billNo, $gateEntryNo, $storeEntry, $rejectedEntry, $userId, $grnRemarks);
	...
	$purchaseOrderRec	=	$purchaseOrderInventoryObj->find($selPoId);
													...
													?>
														<table width="300" cellspacing="1" bgcolor="#999999" cellpadding="3" id="tblAddItem">
															...
															foreach($purchaseRecs as $pr)	{			
																...
																?>
																<tr bgcolor="#FFFFFF">
											   						<td class="fieldName" nowrap style="padding-left:5px; padding-right:5px;"><?=$plantName;//=$plantUnitId;?>
																		<input type="hidden" value="<?=$plantUnitId?>" name="plantId_<?=$j?>" id="plantId_<?=$j?>">
																	</td>
