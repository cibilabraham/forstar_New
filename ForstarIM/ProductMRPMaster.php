<?php
	...
	$selection 	= "?pageNo=".$p["pageNo"]."&selProductCategory=".$p["selProductCategory"]."&selProductState=".$p["selProductState"]."&selProductGroup=".$p["selProductGroup"]."&srchName=".$p["srchName"];
	...
	#List all Records
	$productMRPMasterRecords = $productMRPMasterObj->fetchAllPagingRecords($offset, $limit, $selRateList, $selProductCategoryId, $selProductStateId, $selProductGroupId, $srchName);
	$productMRPMasterRecordSize    = sizeof($productMRPMasterRecords);

	## -------------- Pagination Settings II -------------------
	$fetchProductMRPMasterRecords = $productMRPMasterObj->fetchAllRecords($selRateList, $selProductCategoryId, $selProductStateId, $selProductGroupId, $srchName);	// fetch All Records
...
<? if($del==true){?><input type="submit" value=" Delete " class="button"  name="cmdDelete" onClick="return confirmDelete(this.form,'delId_',<?=$productMRPMasterRecordSize;?>);" ><? }?>&nbsp;<? if($add==true){?><input type="submit" value=" Add New " name="cmdAddNew" class="button"><? }?>&nbsp;<? if($print==true){?><input type="button" value=" Print " name="btnPrint" class="button" onClick="javascript:printWindow('PrintProductMRPMaster.php?selRateList=<?=$selRateList?>&selProductCategory=<?=$selProductCategoryId?>&selProductState=<?=$selProductStateId?>&selProductGroup=<?=$selProductGroupId?>',700,600);"><?}?></td>
	...
	<tr>
		...
		<div style='float:right;padding:2px;'><input type='text' name="srchName" value="<?=$srchName?>" size="30"> <input type="button" value="Search" onClick="if(this.form.srchName.value==''){ alert('Plaese Enter Serch Key');this.form.srchName.focus();return false; }else this.form.submit();" >&nbsp;<input type="reset" value="Cancel" onClick="this.form.srchName.value=''; this.form.submit();" ></div>
		</td>
	</tr>
	...
	<? if($maxpage>1){ ?>
		...
		for ($page=1; $page<=$maxpage; $page++) {
			if ($page==$pageNo) {
      				...
      				$nav.= " <a href=\"ProductMRPMaster.php?pageNo=$page&selProductCategory=$selProductCategoryId&selProductState=$selProductStateId&selProductGroup=$selProductGroupId&srchName=$srchName\" class=\"link1\">$page</a> ";				
   			}
		}
		if ($pageNo > 1) {
   			$page  = $pageNo - 1;
   			$prev  = " <a href=\"ProductMRPMaster.php?pageNo=$page&selProductCategory=$selProductCategoryId&selProductState=$selProductStateId&selProductGroup=$selProductGroupId&srchName=$srchName\"  class=\"link1\"><<</a> ";
	 	} else {
		...
		if ($pageNo < $maxpage) {
   			$page = $pageNo + 1;
   			$next = " <a href=\"ProductMRPMaster.php?pageNo=$page&selProductCategory=$selProductCategoryId&selProductState=$selProductStateId&selProductGroup=$selProductGroupId&srchName=$srchName\"  class=\"link1\">>></a> ";
	 	} else {
