<?php
	...
	if ($dateFrom!="" && $dateTill!="") {
		...
		if ($selectSupplier!="") {
			...
			if (sizeof($billingCompanyRecords)>0 && sizeof($billingCompanyRecords)==1) {
				...
				$billingCompany = $purchasestatementObj->getDefaultCompany();
	...
	/*rekha added */
			...
			$unitRecords = $purchasestatementObj->getOtherunit($selectSupplier, $fromDate, $tillDate, $acConfirmed, $billingCompany);
...
?>
	...
	<!-- rekha added code -->		
			<?php
			//if(is_object($unitRecords)){
			if (sizeof($unitRecords)>0 && ($companySpecific==true || $isAdmin)) {	
			?>			
			<tr>
				<TD class="fieldName" nowrap="true">Unit:</TD>
				<td>					
					<!--<select name="billingCompany" onchange="this.form.submit();">-->
					<select name="unit" onchange="getunits(this)">
					<option value="0">-- Select --</option>			
					<?
					foreach ($unitRecords as $usr) {
						$i++;
						$unitId	= $usr[0];
						$uName			= $usr[1];
						//$defaultunitChk		= $usr[2];
						$selected = "";
						if ($unitId==$unit) $selected = "selected";
					?>
					<option value="<?=$unitId?>" <?=$selected?>><?=$uName?></option>
					<?	
						}	
					?>	
