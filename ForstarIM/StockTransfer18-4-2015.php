<?
	...
	if ($p["cmdAdd"]!="" ) {
		...
		$unitTo=$p["toPlant"];
		$stockIdFrom=$p["item"];
		//list($stockId,$stockNameFrom)=$stockObj->findFromunit($stockIdFrom);
		//list($stockIdTo)=$stockObj->getStockId($stockNameFrom,$unitTo);
		$qty=$p["quantity"];
		$date=Date("Y-m-d");
		...
		$stockFromUnit=$stockObj->updateStockFromUnit($stockIdFrom,$qty,$unitFrom);
		$checkExist=$stockObj->checkStockExistUnit($stockIdFrom,$qty,$unitTo);
		if ($checkExist)
		{
		...
		$stockTransferInsertion=$stockObj->addStockTransfer($unitFrom,$unitTo,$stockIdFrom,$qty,$date);
		//$stockToUnit=$stockObj->updateStockToUnit($stockIdNameTo,$qty);
	...
	<table cellspacing="0"  align="center" cellpadding="0" width="70%" >
		...
		<tr>
			...
			<table width="200">
					...
					<!--<select name="fromPlant" id="fromPlant" onChange="this.form.submit()";>-->

					<select name="fromPlant" id="fromPlant" onChange="xajax_getStockqty(document.getElementById('item').value,document.getElementById('fromPlant').value)"; >
			...
			<? }?>
							...
							<tr>
							  ...
							  <!-- <input name="requestNo" type="text" id="requestNo" size="10" value="<?=$requestNo?>" tabindex="1" onchange="xajax_checkRequestNumberExist(document.getElementById('requestNo').value, '<?=$requestNo?>', <?=$mode?>);">-->
							   ...
							   <select name="item" id="item" onChange="xajax_getStockqty(document.getElementById('item').value,document.getElementById('fromPlant').value)";>
                                        <option value="">--select--</option>
										<?php
			...
			<? }?>
												  ...
												  <tr><td class="fieldName" align='right'>Enter&nbsp;Quantity</td><td><input type="text" name="quantity" id="quantity"></td></tr>
                                              </table></td>
...
<tr><TD nowrap style="padding-left:5px; padding-right:5px;">
	...
	<?} else{?>
		<td  colspan="2" align="center">
			<input type="submit" name="cmdCancel" class="button" value=" Cancel " onClick="return cancel('StockIssuance.php');">&nbsp;&nbsp;
												<input type="submit" name="cmdAdd" id="cmdAdd2" class="button" value=" Add "  onClick="return validateStockTransfer(document.frmStockTransfer);">&nbsp;&nbsp;												</td>
...
<? if($maxpage>1){?>
	...
	foreach ($stockTransferRecords as $str) {
		...
		$fromunit		=	$str[1];
		$plantRecfrom	=	$plantandunitObj->find($fromunit);
		//$plantId	=	$plantRec[0];
		//$no		=	$plantRec[1];
		$namefrom		=	stripSlash($plantRecfrom[2]);
		$tounit		=	$str[2];
		$plantRecto=$plantandunitObj->find($tounit);
		$nameto		=	stripSlash($plantRecto[2]);
		$item=$str[3];
		$itemNameRec=$stockObj->find($item);
		$itemName=$itemNameRec[2];
		$quantity=$str[4];
		$date=dateFormat($str[5]);
		//$department		=	$sir[2];
	...
	?>
	<tr  bgcolor="WHITE">
		<td width="20"><?=$date;?><!--<input type="checkbox" name="delId_<?=$i;?>" id="delId_<?=$i;?>" value="<?=$stockIssuanceId;?>" class="chkBox">--></td>
		<td class="listing-item" nowrap style="padding-left:10px; padding-right:10px;"><?=$itemName;?></td>
