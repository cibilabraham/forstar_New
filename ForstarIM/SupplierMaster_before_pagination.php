...
if ((!$addInv) && (!$addFRN) && (!$addRTE)){
	...
	if ($p["cmdAdd"]!="") {
		...
		if ($name!="" && $code!="") {
			$supplierRecIns	= $supplierMasterObj->addSupplier($code, $name, $address, $phone, $vatNo, $cstNo, $frozen, $inventory, $rte, $pinCode, $faxNo, $email, $panNo, $place, $landingCenter, $paymentBy, $currentUserId, $bankAcNo, $bankName, $supplierStatus);
	...
	if ($p["cmdSaveChange"]!="") {
		...
		if ($supplierId!="" && $name!="" && $code!="") {
			$supplierRecUptd = $supplierMasterObj->updateSupplier($supplierId, $code, $name, $address, $phone, $vatNo, $cstNo, $frozen, $inventory, $rte, $pinCode, $faxNo, $email, $panNo, $place, $landingCenter, $paymentBy, $bankAcNo, $bankName, $supplierStatus);
		}
...
//echo "The value of $edit";
	//if (($editFRN || $editInv || $editRTE ) || (!$editFRN && !$editInv && !$editRTE && $edit==true )) {
		...
		<td class="listing-item" align="center" style="padding-left:5px; padding-right:5px;">
			<input type="submit" value=" Edit " name="cmdEdit" onClick="assignValue(this.form,<?=$supplierId;?>,'editId');assignValue(this.form,'1','editSelectionChange');this.form.action='SupplierMaster.php';">
		</td>
