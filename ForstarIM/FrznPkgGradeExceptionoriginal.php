<?php
	...
	$rateExist = $frznPkgRateObj->chkRateExist($fishId, $processCodeId, $freezingStageId, $qualityId, $frozenCodeId, $rateListId);
	...
	if ($frznPkgRateId) {
		...
		$frznGradeRecs = $frznPkgRateObj->getFrznPkgGrade($frznPkgRateId);

		# Sel Grade recs
		$gradeArr = $frznPkgRateObj->getSelGrade($frznPkgRateId);	
		$gradeAllChk = "";
		//if (in_array(0, $gradeArr)) $gradeAllChk = "disabled";
	}
	
	# Grade Recs
	$gradeRecs = $frznPkgRateObj->getGrades($processCodeId, $freezingStageId, $qualityId, $frozenCodeId);
	...
	if ($p["cmdDelete"]!="") {
		...
		for ($i=1; $i<=$tblRowCount; $i++) {
			...
			if ($delId!="") {
				$delSelGradeCombination = $frznPkgRateObj->deleteFrznPkgRateGrade($groupEntryId);				
			...
			<table>
				<TR><TD valign="top">
					...
					<td nowrap style="padding-left:10px;">
						...
						<input type="button" name="cmdAddExptRate" id="cmdAddExptRate" value="Save" onclick="addGrade('<?=$processCodeId?>', '<?=$freezingStageId?>', '<?=$qualityId?>', '<?=$frozenCodeId?>', '<?=$rowId?>', '<?=$rateListId?>', '<?=$selRowId?>', '<?=$frznPkgRateId?>', '<?=$fishId?>', '<?=$selGroupEntry?>', '<?=$fishCategoryId?>');" />
					</td>
				</TR>
			</table>
			</TD></TR>
...
<!-- New Listing ends here -->
						...
						foreach ($frznGradeRecs as $fgr) {
							...
							$groupedGradeId = $fgr->gid;
							$editVal = $groupedGradeId.":".$gRate.":".$fgr->processorid;
							$groupEntryId = $fgr->entryid;
						...
						?>
							<TR>
								...
								<td>
									<input type="submit" name="cmdEdit" value="Edit" onClick="assignValue(this.form,'<?=$editVal;?>','editId');assignValue(this.form,'<?=$groupEntryId;?>','hidGroupEntryId');this.form.action='FrznPkgGradeException.php';" />
								</td>
							</TR>
						<?php
							 }
